...
.toast--error {
    border-left-color: var(--danger-color);
            ...
            <style>${Q}</style>
            ${J()}
        `}}if(!customElements.get("app-notifier"))customElements.define("app-notifier",U);var M=`:host {
    display: inline-block;
    width: 100%;
    font-family: var(--font-sans);
    position: relative;
...
.select-option.selected:hover {
    background: rgba(var(--accent-rgb), 0.15);
        ...
        `}}class m{static STORAGE_KEY="lastboard_theme";static CLASS_NAME="dark-mode";static init(){let o=localStorage.getItem(this.STORAGE_KEY);if(o==="dark")this.enableDark();else if(o==="light")this.enableLight();else if(window.matchMedia&&window.matchMedia("(prefers-color-scheme: dark)").matches)this.enableDark();else this.enableLight()}static enableDark(){document.documentElement.classList.add(this.CLASS_NAME),localStorage.setItem(this.STORAGE_KEY,"dark"),this.setCookie(this.STORAGE_KEY,"dark",365)}static enableLight(){document.documentElement.classList.remove(this.CLASS_NAME),localStorage.setItem(this.STORAGE_KEY,"light"),this.setCookie(this.STORAGE_KEY,"light",365)}static toggle(){if(document.documentElement.classList.contains(this.CLASS_NAME))this.enableLight();else this.enableDark()}static isDark(){return document.documentElement.classList.contains(this.CLASS_NAME)}static setCookie(o,r,a){let d="";if(a){let f=new Date;f.setTime(f.getTime()+a*24*60*60*1000),d="; expires="+f.toUTCString()}document.cookie=o+"="+(r||"")+d+"; path=/"}static async sync(){try{let o=await fetch("/api/me");if(o.ok){let r=await o.json();if(r.theme==="dark")this.enableDark();else if(r.theme==="light")this.enableLight()}}catch(o){}}}Z(async()=>{let o=1,r=3,a="/images/default-avatar.svg",d=document.querySelector(".setup-container"),f=document.getElementById("setupForm"),c=document.getElementById("nextBtn"),b=document.getElementById("backBtn"),w=document.getElementById("feedback"),u=document.getElementById("language"),z=document.getElementById("username"),C=document.getElementById("password"),F=document.getElementById("confirmPassword"),l={light:document.getElementById("themeLight"),dark:document.getElementById("themeDark")},E=s.getAvailableLocales(),v=navigator.language.split("-")[0];if(!E.find((t)=>t.code===v))v="en";if(u){let t=u;t.options=E.map((n)=>({value:n.code,label:`${n.flag} ${n.name}`})),t.value=v,s.setLanguage(v),u.addEventListener("change",(n)=>{let i=n.target.value;s.setLanguage(i),y()})}if(m.init(),m.isDark())l.dark?.classList.add("selected"),l.light?.classList.remove("selected");else l.light?.classList.add("selected"),l.dark?.classList.remove("selected");function A(t){if(Object.entries(l).forEach(([n,i])=>{if(i)n===t?i.classList.add("selected"):i.classList.remove("selected")}),t==="dark")m.enableDark();else m.enableLight()}Object.entries(l).forEach(([t,n])=>{if(n)n.addEventListener("click",()=>A(t))});let S=["https://cdn.jsdelivr.net/gh/homarr-labs/dashboard-icons/png/proxmox.png","https://cdn.jsdelivr.net/gh/homarr-labs/dashboard-icons/png/pi-hole.png","https://cdn.jsdelivr.net/gh/homarr-labs/dashboard-icons/png/home-assistant.png","https://cdn.jsdelivr.net/gh/homarr-labs/dashboard-icons/png/docker.png"],$=document.getElementById("avatarInput"),j=document.getElementById("avatarTrigger"),K=document.getElementById("avatarPreview"),P=document.getElementById("avatarUrl");if(j&&$)j.addEventListener("click",()=>$.click()),$.addEventListener("change",(t)=>{let n=t.target.files?.[0];if(n){let i=new FileReader;i.onload=(g)=>{let h=g.target?.result;if(h)K.setAttribute("src",h),P.value=h,a=h},i.readAsDataURL(n)}});let y=()=>{let t=(i,g)=>{let h=document.getElementById(i);if(h)h.textContent=s.t(g)};t("pageSubtitle","setup.subtitle");let n=(i,g)=>{let h=document.querySelector(`label[for="${i}"]`);if(h)h.textContent=s.t(g)};if(n("language","settings.language"),n("username","setup.root_user"),n("password","settings.new_password"),n("confirmPassword","settings.confirm_password"),t("lblInterfaceTheme","setup.interface_theme"),t("lblThemeLight","settings.light"),t("lblThemeDark","settings.dark"),t("summaryKeyLang","settings.language"),t("summaryKeyTheme","settings.theme"),t("summaryKeyAdmin","setup.summary_admin"),o===3)c.textContent=s.t("setup.create_admin");else c.textContent=s.t("general.next");if(b)b.textContent=s.t("general.back");t("changeImageText","action.change_image"),t("readyMsg","setup.ready_msg")};s.subscribe(y),y();function Y(){let t=document.getElementById("progressFill");if(t)t.style.width=`${(o-1)/(r-1)*100}%`;if(document.querySelectorAll(".step-dot").forEach((i)=>{let g=parseInt(i.getAttribute("data-step")||"0");if(i.classList.remove("active","completed"),g===o)i.classList.add("active");if(g<o)i.classList.add("completed")}),document.querySelectorAll(".wizard-step").forEach((i)=>i.classList.remove("active")),document.getElementById(`step${o}`)?.classList.add("active"),b)b.style.display=o===1?"none":"block";b.textContent=s.t("general.back");let n=s.t("general.next");if(c.textContent=o===r?s.t("setup.create_admin"):n,o===3)try{let i=s.getLocale();if(i)document.getElementById("summaryLang").textContent=`${i.flag} ${i.name}`;let g=s.t("settings.system");if(l.light?.classList.contains("selected"))g=s.t("settings.light");if(l.dark?.classList.contains("selected"))g=s.t("settings.dark");document.getElementById("summaryTheme").textContent=g,document.getElementById("summaryUser").textContent=z.value||"admin"}catch(i){console.error("Error updating summary:",i)}w.style.display="none",c.removeAttribute("disabled")}function W(){if(w.style.display="none",o===1)return!0;if(o===2){let t=z.value.trim(),n=C.value,i=F.value;if(t.length<2)return x(s.t("setup.error_username"),"error"),!1;if(n.length<4)return x(s.t("setup.error_password"),"error"),!1;if(n!==i)return x(s.t("notifier.password_mismatch")||"Passwords do not match","error"),!1;return!0}return!0}function oo(t){w.style.display="block",w.textContent=t}c.addEventListener("click",async()=>{if(!W())return;if(o<r)o++,Y();else await V()}),b.addEventListener("click",()=>{if(o>1)o--,Y()}),document.addEventListener("keydown",(t)=>{if(t.key==="Enter")t.preventDefault(),c.click()});function x(t,n="success"){if(window.notifier)window.notifier.show(t,n);else console.log(`[${n}] ${t}`)}async function V(){c.textContent=s.t("setup.creating"),c.setAttribute("disabled","true");let t="dark";if(l.light?.classList.contains("selected"))t="light";if(l.dark?.classList.contains("selected"))t="dark";let n={username:z.value.trim(),password:C.value,language:u.value,theme:t,avatar_url:a};try{let i=await fetch("/api/setup",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)});if(i.ok)localStorage.setItem("lastboard_lang",n.language),c.textContent=s.t("setup.welcome_admin"),c.style.backgroundColor="var(--accent-alt, #00f5a0)",x(s.t("setup.welcome_admin"),"success"),setTimeout(()=>window.location.href="/",1000);else{let g=(await i.text()).trim();x(`${s.t("setup.failed")}: ${s.t(g)||g}`,"error"),c.removeAttribute("disabled"),c.textContent=s.t("setup.create_admin")}}catch(i){console.error(i),x(s.t("setup.error_connection"),"error"),c.removeAttribute("disabled")}}requestAnimationFrame(()=>d.style.opacity="1")});
