...
.avatar__initials {
    user-select: none;
}`;class vr extends HTMLElement{static get observedAttributes(){return["src","alt","initials"]}constructor(){super();this.attachShadow({mode:"open"})}connectedCallback(){this.render()}attributeChangedCallback(o,r,a){if(r!==a)this.render()}render(){let o=this.getAttribute("src"),r=this.getAttribute("initials")||"??",a=this.getAttribute("alt")||"User Avatar";this.shadowRoot.innerHTML=`
            <style>${hr}</style>
            ${br({src:o||void 0,initials:r,alt:a})}
        `}}if(!customElements.get("app-avatar"))customElements.define("app-avatar",vr);_();oo();Ko();_();var fr=(o)=>`
...
@keyframes blink {
    ...
    100% {
        ...
        `}}if(!customElements.get("app-topbar"))customElements.define("app-topbar",Hr);_();var Br=({title:o,dropdownOpen:r})=>`
    <header class="header">
...
(max-width: 1024px) {
...
}`;class Qr extends HTMLElement{dropdownOpen;constructor(){super();this.attachShadow({mode:"open"}),this.dropdownOpen=!1}connectedCallback(){this.render(),this.setupListeners()}toggleDropdown(o){this.dropdownOpen=typeof o==="boolean"?o:!this.dropdownOpen,this.render()}setupListeners(){this.shadowRoot.addEventListener("click",(o)=>{let r=o.target,a=r.closest(".header__btn-changelog");if(a)a.style.display="none";if(r.closest("#profile-trigger"))o.stopPropagation(),this.toggleDropdown()}),window.addEventListener("click",()=>{if(this.dropdownOpen)this.toggleDropdown(!1)})}render(){let o=this.getAttribute("title")||"Task";this.shadowRoot.innerHTML=`
            <style>${Jr}</style>
            ${Br({title:o,dropdownOpen:this.dropdownOpen})}
        `}}if(!customElements.get("app-header"))customElements.define("app-header",Qr);_();var ao=(o)=>String(o).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;"),Oa=(o)=>{if(!o)return"#";let r=o.trim().toLowerCase();if(r.startsWith("http://")||r.startsWith("https://")||r==="#")return ao(o);return"#"},Kr=({bookmarks:o,isEditing:r,isSearching:a,isTouchDevice:t,maxCols:i=12})=>{let e=Array.isArray(o)?o:[],s=(l)=>e.filter((c)=>c.parent_id===l);return`
    ${(t?e:a?e:e.filter((l)=>!l.parent_id)).map((l)=>{let c={};try{c=typeof l.content==="string"?JSON.parse(l.content):l.content}catch(k){console.error("Failed to parse content for item",l.id,k)}let p=l.type==="section",x={x:l.x||1,y:l.y||1},h=Math.min(l.w||1,i),u=l.h||1;if(p){let k=s(l.id),G=(c.title||"").trim();return`
            <fieldset class="bookmark-grid__section"
               ...
               `:""}
            </fieldset>`}return Ar(l,c,r,x,h,u,t)}).join("")}

    <div id="ghost-element" class="bookmark-grid__ghost" style="display: none;"></div>
    `};function Ar(o,r,a,t,i,e=0,s=!1){let d=e||o.h||1;if(o.type==="widget"){let h=r.widgetId,k={clock:"widget-clock",notepad:"widget-notepad",telemetry:"widget-telemetry"}[h]||"div",m=(r.text||"").replace(/"/g,"&quot;");return`
    ...
    `}let l=r.icon||"",p=l.startsWith("http")||l.startsWith("/")?`<img src="${ao(l)}" alt="${ao(r.label)}" class="bookmark-grid__icon-img" draggable="false" />`:l?ao(l):'<svg class="bookmark-grid__icon-svg" viewBox="0 0 24 24"><path d="M10 6L8.59 7.41 13.17 12l-4.58 4.59L10 18l6-6z"/></svg>';return`
        <a ${a?'role="button"':`href="${Oa(r.url)}" target="_blank"`} class="bookmark-grid__card"
           draggable="${a}"
           data-id="${o.id}"
           data-type="${o.type}"
           data-orig-x="${o.x}" data-orig-y="${o.y}"
...
.toast--error {
    border-left-color: var(--danger-color);
            ...
            <style>${xa}</style>
            ${ga()}
        `}}if(!customElements.get("app-notifier"))customElements.define("app-notifier",ba);_();var ha=()=>`
...
}`;class ma extends HTMLElement{icons=[];filteredIcons=[];selectedIcon="";searchQuery="";isLoading=!0;debounceTimer=null;inputElement=null;constructor(){super();this.attachShadow({mode:"open"})}async connectedCallback(){if(this.shadowRoot.childElementCount>0)return;if(this.renderBase(),this.setupListeners(),this.icons.length===0)await this.loadIcons()}async loadIcons(){try{this.icons=await Do.loadIcons(),this.filteredIcons=this.icons.slice(0,50),this.isLoading=!1,this.updateGrid()}catch(o){console.error("[IconPicker] Failed to load icons",o),this.isLoading=!1,this.updateGrid()}}setupListeners(){let o=this.shadowRoot;this.inputElement=o.getElementById("icon-search"),this.inputElement?.addEventListener("input",(a)=>{let t=a.target;if(this.searchQuery=t.value,this.debounceTimer)window.clearTimeout(this.debounceTimer);this.debounceTimer=window.setTimeout(()=>{this.performSearch()},100)}),o.getElementById("grid-container")?.addEventListener("click",(a)=>{let i=a.target.closest(".icon-picker__item");if(i&&i.dataset.icon)this.selectedIcon=i.dataset.icon,this.updateSelected(),this.updateGrid(),this.dispatchEvent(new CustomEvent("icon-selected",{detail:{iconName:this.selectedIcon},bubbles:!0,composed:!0}))})}performSearch(){this.filteredIcons=Do.searchIcons(this.searchQuery,50),this.updateGrid()}getSelectedIcon(){return this.selectedIcon}setSelectedIcon(o){this.selectedIcon=o,this.updateSelected(),this.updateGrid()}renderBase(){this.shadowRoot.innerHTML=`
            <style>${_a}</style>
            ${ha()}
        `}updateGrid(){let o=this.shadowRoot.getElementById("grid-container");if(o){if(!this.searchQuery.trim()&&!this.isLoading){o.style.display="none",o.innerHTML="";return}o.style.display="grid",o.innerHTML=fa(this.filteredIcons,this.selectedIcon,this.isLoading)}}updateSelected(){let o=this.shadowRoot.getElementById("selected-container");if(o)o.innerHTML=va(this.selectedIcon)}}if(!customElements.get("icon-picker"))customElements.define("icon-picker",ma);_();var ya=({isOpen:o,isEditMode:r})=>`
    <dialog id="modal">
        <div class="modal-header">
...
.visibility-row .checkbox-group {
    ...
    margin-bottom: 0;
        ...
        `,this.dialog=this.shadowRoot.querySelector("dialog"),this.shadowRoot.querySelectorAll(".card").forEach((o)=>{o.addEventListener("click",()=>{let r=o.getAttribute("data-id"),a=Po.find((t)=>t.id===r);if(a)this.selectWidget(a)})})}}customElements.define("add-widget-modal",$a);H();_();var za=`:host {
    display: contents;
}
...
.modal-actions {
    ...
    justify-content: flex-end;
        ...
        `,this.setupDynamicListeners()}}if(!customElements.get("confirmation-modal"))customElements.define("confirmation-modal",La);_();async function qa(o){try{await n.ensureInitialized(),await o()}catch(r){console.error("[Bootstrap] Critical failure:",r),document.body.innerHTML=`
            <div style="
                display: flex; 
                flex-direction: column; 
                align-items: center; 
        ...
        `}}qo();var q=document.getElementById("main-topbar"),Ha=document.getElementById("right-drawer"),Ho=document.getElementById("dashboard-root"),eo,Bo,Jo;qa(async()=>{Q.init(),await w.fetchUser();let o=w.getUser();if(o){if(o.language)await n.setLanguage(o.language);if(o.preferences&&o.preferences.grid_columns)document.documentElement.style.setProperty("--user-preferred-columns",o.preferences.grid_columns.toString());if(o.theme==="dark")Q.enableDark();else if(o.theme==="light")Q.enableLight();else if(window.matchMedia&&window.matchMedia("(prefers-color-scheme: dark)").matches)Q.enableDark();else Q.enableLight();if(o.id)g.setUserId(o.id)}if(q){if(q.setState({user:o}),o&&o.project_name)q.setAttribute("title",o.project_name);w.subscribe((a)=>{if(a){if(q.setState({user:a}),a.project_name)q.setAttribute("title",a.project_name)}})}await g.fetchItems(),vt(),co.start(),eo=document.createElement("add-bookmark-modal"),document.body.appendChild(eo),Bo=document.createElement("add-widget-modal"),document.body.appendChild(Bo),Jo=document.createElement("confirmation-modal"),document.body.appendChild(Jo);let r=document.createElement("widget-config-modal");document.body.appendChild(r)});no.on(io.SHOW_CONFIRMATION,async(o)=>{let{title:r,message:a,onConfirm:t}=o.detail;if(Jo){if(await Jo.confirm(r,a)&&t)t()}});no.on(io.SHOW_WIDGET_CONFIG,(o)=>{let{item:r,type:a}=o.detail;if(a==="bookmark"){if(eo)eo.openForEdit(r)}else{let t=document.querySelector("widget-config-modal");if(t)t.open(r)}});no.on(io.NOTIFY,(o)=>{let r=document.querySelector("app-notifier");if(r)r.show(o.detail.message,o.detail.type)});if(q)q.addEventListener("drawer-toggle",(o)=>{if(o.detail.action==="open")Ha.open(),q.setState({drawerOpen:!0});else Ha.close(),q.setState({drawerOpen:!1})}),q.addEventListener("edit-mode-change",(o)=>{let r=o.detail.active;if(Ho.classList.toggle("edit-mode",r),!r)window.location.reload()}),q.addEventListener("search-input",(o)=>{let r=o.detail.query;g.setSearchQuery(r)}),q.addEventListener("add-item",async(o)=>{let r=o.detail.action;if(r==="add-bookmark"){if(eo)eo.open()}else if(r==="add-widget"){if(Bo)Bo.open()}else if(r==="add-section"){let t=g.getState().items||[],{collisionService:i}=await Promise.resolve().then(() => (V(),ro)),e=i.findFirstAvailableSlot(1,1,t),s={type:"section",x:e.x,y:e.y,w:1,h:1,content:JSON.stringify({title:""})},d=await g.addItem(s);if(d)no.emit(io.SHOW_WIDGET_CONFIG,{item:d,type:"section"})}});window.addEventListener("widget-selected",async(o)=>{let a=o.detail,i=g.getState().items||[],{collisionService:e}=await Promise.resolve().then(() => (V(),ro)),s=e.findFirstAvailableSlot(a.defaultW,a.defaultH,i),d={type:"widget",x:s.x,y:s.y,w:a.defaultW,h:a.defaultH,content:JSON.stringify({widgetId:a.id,text:a.id==="notepad"?"":void 0})};await g.addItem(d)});window.addEventListener("drawer-close",()=>{if(q)q.setState({drawerOpen:!1})});window.addEventListener("click",(o)=>{let r=o.target,a=document.getElementById("add-menu"),t=document.getElementById("add-toggle");if(a&&a.classList.contains("active")&&!a.contains(r)&&t&&!t.contains(r))a.classList.remove("active")});function vt(){if(!Ho)return;Ho.innerHTML="";let o=document.createElement("bookmark-grid");Ho.appendChild(o)}
