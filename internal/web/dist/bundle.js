...
.paper {
    ...
    height: 100%;
            ...
            <style>${qo}</style>
            ${Bo()}
        `}}if(!customElements.get("app-paper"))customElements.define("app-paper",Ho);var Go=`:host {
...
.avatar__initials {
    user-select: none;
}`;class Fo extends HTMLElement{static get observedAttributes(){return["src","alt","initials"]}constructor(){super();this.attachShadow({mode:"open"})}connectedCallback(){this.render()}attributeChangedCallback(o,t,r){if(t!==r)this.render()}render(){let o=this.getAttribute("src"),t=this.getAttribute("initials")||"??",r=this.getAttribute("alt")||"User Avatar";this.shadowRoot.innerHTML=`
            <style>${Eo}</style>
            ${Co({src:o||void 0,initials:t,alt:r})}
        `}}if(!customElements.get("app-avatar"))customElements.define("app-avatar",Fo);f();J();no();f();var Qo=(o)=>`
...
`,Vt=(o,t)=>{if(!o)return"";if(!t)return"";if(t.is_docker)return`
    ...
    `},co=(o,t,r)=>{return`
        ...
        `:""}
    `},Yo=(o,t,r,i,n)=>{let e=r?.toLowerCase()==="admin"||r?.toLowerCase()==="administrator";return`
...
.select-option.selected:hover {
    background: rgba(var(--accent-rgb), 0.15);
        ...
        `,this.shadowRoot.getElementById("trigger")?.addEventListener("click",(r)=>{r.stopPropagation(),this.toggle()})}}if(!customElements.get("app-select"))customElements.define("app-select",Uo);var Oo=`:host {
    display: block;
    color: var(--text-main);
    max-width: 900px;
    margin: 0 auto;
...
}`;class Xo extends HTMLElement{prefs;users=[];getCsrfToken(){let o=document.cookie.split("; ").find((t)=>t.startsWith("csrf_token="));return o?decodeURIComponent(o.split("=")[1]):""}static get observedAttributes(){return["section"]}constructor(){super();this.attachShadow({mode:"open"}),this.prefs={accent_color:"#0078D4",language:"en",widget_min_width:140,beta_updates:!1}}unsubscribe=null;_renderPending=!1;connectedCallback(){this.unsubscribe=k.subscribe((t)=>{if(t&&t.preferences)this.prefs={...t.preferences,project_name:t.project_name||t.preferences.project_name||"Lastboard"},this.render()}),this.fetchPrefs();let o=this.getAttribute("section");if(o==="users")this.fetchUsers();this.render()}disconnectedCallback(){if(this.unsubscribe)this.unsubscribe(),this.unsubscribe=null}async fetchPrefs(){let o=k.getUser();if(o&&o.preferences)this.prefs={...o.preferences,project_name:o.project_name||o.preferences.project_name||"Lastboard"},this.render()}async fetchUsers(){try{this.users=await B.getUsers(),this.render()}catch(o){console.error("Failed to fetch users",o)}}attributeChangedCallback(o,t,r){if(o==="section"&&t!==r){if(r==="users")this.fetchUsers();this.render()}}async savePrefs(o){let t={...this.prefs};if(this.prefs={...this.prefs,...o},o.accent_color)this.applyAccent(o.accent_color);if(o.language)a.setLanguage(o.language);if(o.theme)if(o.theme==="dark")q.enableDark();else q.enableLight();try{await k.updatePreferences(o)}catch{if(this.prefs=t,o.language&&t.language)a.setLanguage(t.language)}this.render()}applyAccent(o){let t=o,r={blue:"#228be6",indigo:"#4c6ef5",cyan:"#15aabf",teal:"#12b886",orange:"#fd7e14",red:"#fa5252",grape:"#be4bdb"};if(!o.startsWith("#"))t=r[o]||"#0078D4";document.documentElement.style.setProperty("--accent",t);let i=parseInt(t.slice(1,3),16),n=parseInt(t.slice(3,5),16),e=parseInt(t.slice(5,7),16);if(!isNaN(i)&&!isNaN(n)&&!isNaN(e))document.documentElement.style.setProperty("--accent-rgb",`${i}, ${n}, ${e}`)}async handleBetaToggle(o){this.savePrefs({beta_updates:o}),this.render()}updateDensity(o){let t=this.shadowRoot.getElementById("val-widget_min_width");if(t)t.textContent=o+"px";document.documentElement.style.setProperty("--widget-min-size",o+"px")}commitDensity(o){let t=parseInt(o);k.updatePreferences({widget_min_width:t})}updateColumns(o){let t=this.shadowRoot.getElementById("val-grid_columns");if(t)t.textContent=o;document.documentElement.style.setProperty("--user-preferred-columns",o)}commitColumns(o){let t=parseInt(o);k.updatePreferences({grid_columns:t})}async updateUsername(o){let t=k.getUser();if(!t)return;if(await k.updateProfile({username:o,avatar_url:this.prefs.avatar_url||t.avatar_url}),window.notifier)window.notifier.show(a.t("notifier.username_updated"))}async updatePassword(){let o=this.shadowRoot.getElementById("current-password")?.value,t=this.shadowRoot.getElementById("new-password")?.value,r=this.shadowRoot.getElementById("confirm-password")?.value;if(!o){if(window.notifier)window.notifier.show(a.t("notifier.password_required"),"error");return}if(t!==r){if(window.notifier)window.notifier.show(a.t("notifier.password_mismatch"),"error");return}if(!t){if(window.notifier)window.notifier.show(a.t("notifier.password_empty"),"error");return}try{let i=await k.changePassword({current_password:o,new_password:t});if(this.shadowRoot.getElementById("current-password").value="",this.shadowRoot.getElementById("new-password").value="",this.shadowRoot.getElementById("confirm-password").value="",window.notifier)window.notifier.show(a.t("notifier.password_changed_relogin"));setTimeout(()=>{localStorage.removeItem("lastboard_user_cache"),document.body.style.opacity="0",window.location.href="/login"},2000)}catch(i){if(window.notifier)window.notifier.show(a.t("notifier.password_incorrect"),"error")}}async handleAvatarChange(o){let t=o.target.files?.[0];if(!t)return;if(!t.type.startsWith("image/")){if(window.notifier)window.notifier.show(a.t("notifier.invalid_image_type"),"error");return}try{let r=await this.resizeImage(t,256,256),i=k.getUser();if(!i)return;if(this.prefs.avatar_url=r,await k.updateProfile({username:i.username,avatar_url:r}),this.render(),window.notifier)window.notifier.show(a.t("notifier.avatar_updated"))}catch(r){if(console.error("Resize failed",r),window.notifier)window.notifier.show(a.t("notifier.avatar_error"),"error")}}resizeImage(o,t,r){return new Promise((i,n)=>{let e=new FileReader;e.onload=(s)=>{let d=new Image;d.onload=()=>{let l=document.createElement("canvas"),c=d.width,p=d.height;if(c>p){if(c>t)p=Math.round(p*t/c),c=t}else if(p>r)c=Math.round(c*r/p),p=r;l.width=c,l.height=p;let h=l.getContext("2d");if(!h){n(Error("Canvas context not available"));return}h.drawImage(d,0,0,c,p),i(l.toDataURL("image/jpeg",0.8))},d.onerror=(l)=>n(l),d.src=s.target?.result},e.onerror=(s)=>n(s),e.readAsDataURL(o)})}openAddUserModal(){let o=this.shadowRoot.getElementById("add-user-modal");if(o){o.showModal();let t=this.shadowRoot.getElementById("new-user-role");if(t)t.options=[{value:"user",label:a.t("settings.role_user")},{value:"admin",label:a.t("settings.role_admin")}]}}async createUser(){let o=this.shadowRoot.getElementById("new-user-username").value,t=this.shadowRoot.getElementById("new-user-password").value,r=this.shadowRoot.getElementById("new-user-role").value;if(!o||!t){if(window.notifier)window.notifier.show(a.t("notifier.fields_required"),"error");return}try{if(await B.createUser({username:o,password:t,role:r}),window.notifier)window.notifier.show(a.t("notifier.user_created"));let i=this.shadowRoot.getElementById("add-user-modal");if(i)i.close();this.fetchUsers()}catch(i){if(window.notifier)window.notifier.show(a.t("notifier.user_create_error"),"error")}}openEditUserModal(o,t,r){let i=this.shadowRoot.getElementById("edit-user-modal");if(i){this.shadowRoot.getElementById("edit-user-id").value=o.toString(),this.shadowRoot.getElementById("edit-user-username").value=t,this.shadowRoot.getElementById("edit-user-password").value="",i.showModal();let n=this.shadowRoot.getElementById("edit-user-role");if(n)n.options=[{value:"user",label:a.t("settings.role_user")},{value:"admin",label:a.t("settings.role_admin")}],n.value=r}}async updateAdminUser(){let o=parseInt(this.shadowRoot.getElementById("edit-user-id").value),t=this.shadowRoot.getElementById("edit-user-username").value,r=this.shadowRoot.getElementById("edit-user-password").value,i=this.shadowRoot.getElementById("edit-user-role").value;if(!t){if(window.notifier)window.notifier.show(a.t("notifier.username_required"),"error");return}try{if(await B.adminUpdateUser({id:o,username:t,role:i,password:r||void 0}),window.notifier)window.notifier.show(a.t("notifier.user_updated"));let n=this.shadowRoot.getElementById("edit-user-modal");if(n)n.close();this.fetchUsers()}catch(n){if(window.notifier)window.notifier.show(a.t("notifier.user_update_error"),"error")}}async deleteUser(o){let t=document.querySelector("confirmation-modal");if(t){if(!await t.confirm(a.t("general.delete"),a.t("notifier.user_delete_confirm")))return}try{let r=[...this.users];if(this.users=this.users.filter((i)=>i.id!==o),this.render(),await B.deleteUser(o),window.notifier)window.notifier.show(a.t("notifier.user_deleted"));this.fetchUsers()}catch(r){this.fetchUsers();let i=a.t("notifier.user_delete_error");if(r.message&&r.message.includes("error.cannot_delete_superadmin"))i=a.t("notifier.user_delete_superadmin");if(window.notifier)window.notifier.show(i,"error")}}getContent(o){let t=k.getUser()||{username:"Guest",initials:"??",role:"View Only",avatar_url:"",accent_color:"#0078D4",language:"en",preferences:{}};switch(o){case"account":return Qo(t);case"theme":{let r={blue:"#228be6",indigo:"#4c6ef5",cyan:"#15aabf",teal:"#12b886",orange:"#fd7e14",red:"#fa5252",grape:"#be4bdb"},i=Object.keys(r);return Jo(this.prefs,r,i)}case"personalization":return Zo(this.prefs);case"advanced":return To();case"users":return Ko(this.users);case"about":return Yo(this.version,this.updateInfo,t.role||"user",t.preferences?.beta_updates||!1,this.isCheckingUpdates);default:return`<div class="bento-card"><h3>${o}</h3><p class="settings-content__text-dim">${a.t("settings.default_module_desc")}</p></div>`}}version="v1.2.5";updateInfo=null;checkUpdatesPromise=null;isCheckingUpdates=!1;async handleManualCheckUpdates(){if(this.isCheckingUpdates)return;this.isCheckingUpdates=!0,this.updateUpdateUI();try{if(await this.checkForUpdates(),this.updateInfo&&!this.updateInfo.available){if(window.notifier)window.notifier.show(a.t("settings.up_to_date"))}}finally{this.isCheckingUpdates=!1,this.updateUpdateUI()}}async checkForUpdates(){if(this.checkUpdatesPromise)return this.checkUpdatesPromise;return this.checkUpdatesPromise=(async()=>{try{let{updateService:o}=await Promise.resolve().then(() => (po(),lo)),t=this.prefs.beta_updates||!1;if(this.updateInfo=await o.check(t),this.updateInfo)this.version=this.updateInfo.current_version,this.updateUpdateUI()}catch(o){console.error("Check update failed",o)}finally{this.checkUpdatesPromise=null}})(),this.checkUpdatesPromise}async performUpdate(o){let t=document.querySelector("confirmation-modal");if(t){if(!await t.confirm(a.t("settings.update_available"),a.t("notifier.update_start_confirm")))return}let r=this.shadowRoot.getElementById("btn-update-now"),i=this.shadowRoot.getElementById("update-status");if(r)r.loading=!0;if(i){i.style.display="block";let n=i.querySelector("p");if(n)n.textContent=a.t("notifier.update_downloading")}try{let n=await fetch("/api/system/update/perform",{method:"POST",headers:{"Content-Type":"application/json","X-CSRF-Token":this.getCsrfToken()},body:JSON.stringify({asset_url:o})});if(n.ok){if(i){let e=i.querySelector("p");if(e)e.textContent=a.t("notifier.update_verified")}setTimeout(()=>{window.location.reload()},5000)}else{let e=await n.text();if(i){i.style.background="rgba(var(--danger-rgb), 0.1)",i.style.borderColor="rgba(var(--danger-rgb), 0.3)";let s=i.querySelector("p");if(s)s.style.color="var(--danger-color)",s.textContent=a.t("notifier.update_failed")+": "+e}if(r)r.loading=!1}}catch(n){if(console.error("Update failed",n),i){i.style.background="rgba(var(--danger-rgb), 0.1)",i.style.borderColor="rgba(var(--danger-rgb), 0.3)";let e=i.querySelector("p");if(e)e.style.color="var(--danger-color)",e.textContent=a.t("notifier.update_error")}if(r)r.loading=!1}}downloadBackup(){window.location.href="/api/system/backup"}async restoreBackup(o){if(!o)return;let t=document.querySelector("confirmation-modal");if(t){if(!await t.confirm(a.t("general.restore"),a.t("notifier.restore_confirm")))return}let r=new FormData;r.append("backup_file",o);try{if((await fetch("/api/system/restore",{method:"POST",headers:{"X-CSRF-Token":this.getCsrfToken()},body:r})).ok){if(window.notifier)window.notifier.show(a.t("notifier.restore_success"));setTimeout(()=>window.location.reload(),2000)}else if(window.notifier)window.notifier.show(a.t("notifier.restore_failed"),"error")}catch(i){if(console.error("Restore error",i),window.notifier)window.notifier.show(a.t("notifier.restore_failed"),"error")}}openResetModal(){let o=this.shadowRoot.getElementById("reset-confirm-modal"),t=this.shadowRoot.getElementById("reset-confirm-input");if(o){if(t)t.value="";o.showModal()}}async executeFactoryReset(){let o=this.shadowRoot.getElementById("reset-confirm-input");if(!o||o.value.trim()!=="delete"){if(window.notifier)window.notifier.show(a.t("notifier.reset_confirm_text"),"error");o.focus();return}let t=this.shadowRoot.getElementById("btn-reset-confirm");try{if(t)t.disabled=!0,t.textContent=a.t("settings.restoring")||"Restoring...";if((await fetch("/api/system/reset",{method:"POST",headers:{"X-CSRF-Token":this.getCsrfToken()}})).ok){let i=document.createElement("div");Object.assign(i.style,{position:"fixed",top:"0",left:"0",width:"100%",height:"100%",backgroundColor:"rgba(0,0,0,0.9)",zIndex:"9999",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",color:"white",fontFamily:"var(--font-main, sans-serif)"}),i.innerHTML=`
...
(max-width: 1024px) {
    ...
    .header__profile-container {
        ...
        `}}if(!customElements.get("app-header"))customElements.define("app-header",tt);f();var U=(o)=>String(o).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;"),St=(o)=>{if(!o)return"#";let t=o.trim().toLowerCase();if(t.startsWith("http://")||t.startsWith("https://")||t==="#")return U(o);return"#"},rt=({bookmarks:o,isEditing:t,isLoading:r,isSearching:i,isTouchDevice:n,maxCols:e=12})=>{let s=Array.isArray(o)?o:[];if(r&&s.length===0)return tr(e);let d=(c)=>s.filter((p)=>p.parent_id===c);return`
    ${(n?s:i?s:s.filter((c)=>!c.parent_id)).map((c)=>{let p={};try{p=typeof c.content==="string"?JSON.parse(c.content):c.content}catch(y){console.error("Failed to parse content for item",c.id,y)}let h=c.type==="section",w={x:c.x||1,y:c.y||1},u=Math.min(c.w||1,e),_=c.h||1;if(h){let y=d(c.id),m=(p.title||"").trim();return`
            <fieldset class="bookmark-grid__section"
               data-id="${c.id}"
               data-orig-x="${c.x}" data-orig-y="${c.y}"
               draggable="${t}"
               style="--x: ${w.x}; --y: ${w.y}; --w: ${u}; --h: ${_};">
               ${m?`<legend class="section-title">${U(m)}</legend>`:""}
               ...
               `:""}
            </fieldset>`}return it(c,p,t,w,u,_,n)}).join("")}

    <div id="ghost-element" class="bookmark-grid__ghost" style="display: none;"></div>
    `};function it(o,t,r,i,n,e=0,s=!1){let d=e||o.h||1;if(o.type==="widget"){let y=t.widgetId,x={clock:"widget-clock",notepad:"widget-notepad",telemetry:"widget-telemetry",weather:"widget-weather",markdown:"widget-markdown"}[y]||"div",A=(t.text||"").replace(/"/g,"&quot;");return`
        <div class="bookmark-grid__card"
            ...
            ${r?`
            <div class="bookmark-actions">
                 ${["clock","telemetry","weather"].includes(y)?`<button class="action-btn btn-edit" title="${a.t("general.edit")}">✎</button>`:""}
                 <button class="action-btn btn-delete" title="${a.t("general.delete")}">\uD83D\uDDD1</button>
            </div>
            ${["clock","telemetry"].includes(y)?"":'<div class="resize-handle"></div>'}
            `:""}
        </div>
    `}let l=t.icon||"",p=l.startsWith("http")||l.startsWith("/")?`<img src="${U(l)}" alt="${U(t.label)}" class="bookmark-grid__icon-img" draggable="false" />`:l?U(l):'<svg class="bookmark-grid__icon-svg" viewBox="0 0 24 24"><path d="M10 6L8.59 7.41 13.17 12l-4.58 4.59L10 18l6-6z"/></svg>',h=r?'role="button"':`href="${St(t.url)}" target="_blank"`,w=!s?`--x: ${i.x}; --y: ${i.y}; --w: ${n}; --h: ${d};`:"",u=t.borderColor?`border-color: ${t.borderColor} !important;`:"",_=t.labelPos==="top"?"label-top":t.labelPos==="section"?"label-section":t.labelPos==="off"?"label-off":"";return`
        <a ${h} class="bookmark-grid__card ${_}"
           draggable="${r}"
           data-id="${o.id}"
           data-type="${o.type}"
           data-orig-x="${o.x}" data-orig-y="${o.y}"
           style="${w} ${u}">
            ...
            <span class="bookmark-grid__label">${U(t.label)||"Bookmark"}</span>
    ...
    `}function or(o,t,r=!1,i=12,n=!1){return o.map((e)=>{let s={};try{s=typeof e.content==="string"?JSON.parse(e.content):e.content}catch(p){console.error("Failed to parse content for item (nested)",e.id,p)}let d={x:e.x||1,y:e.y||1},l=e.h||1,c=Math.min(e.w||1,i);return it(e,s,t,d,c,l,n)}).join("")}function tr(o){return`
        <style>
            .skeleton {
                background: var(--surface);
                pointer-events: none;
...
`;class nt extends HTMLElement{_itemId=0;_content="";_isDirty=!1;_isLocked=!0;_isSaving=!1;_lastLockTime=0;_editor=null;_syncStatus=null;_dock=null;_editorContainer=null;_modal=null;_imgResizer=null;_activeImage=null;_pendingAction=null;static get observedAttributes(){return["item-id"]}constructor(){super();this.attachShadow({mode:"open"})}connectedCallback(){this.render(),this.loadContent(),window.addEventListener("beforeunload",this.handleBeforeUnload.bind(this)),a.subscribe(()=>{if(this._editor)this._editor.dataset.placeholder=a.t("widget.notepad.placeholder")})}disconnectedCallback(){window.removeEventListener("beforeunload",this.handleBeforeUnload.bind(this))}attributeChangedCallback(o,t,r){if(o==="item-id")this._itemId=parseInt(r),this.loadContent()}handleBeforeUnload(o){if(this._isDirty)o.preventDefault(),o.returnValue=""}render(){if(!this.shadowRoot)return;let o=this._isLocked?"locked":"unlocked",t=this._isLocked?"locked":"",r=this._isLocked?"hidden":"";this.shadowRoot.innerHTML=`
            ...
            <div class="glass-dock ${r}" id="glass-dock">
                <div class="dock-group">
                    <button class="dock-btn" data-fmt="undo" title="${a.t("widget.notepad.tool.undo")}">${v.undo}</button>
                    <button class="dock-btn" data-fmt="redo" title="${a.t("widget.notepad.tool.redo")}">${v.redo}</button>
                </div>

                <div class="dock-group">
                    <button class="dock-btn" data-fmt="formatBlock:H1" title="${a.t("widget.notepad.tool.h1")}" style="width:30px;">${v.h1}</button>
                    <button class="dock-btn" data-fmt="formatBlock:H2" title="${a.t("widget.notepad.tool.h2")}" style="width:30px;">${v.h2}</button>
                    <button class="dock-btn" data-fmt="formatBlock:H3" title="${a.t("widget.notepad.tool.h3")}" style="width:30px;">${v.h3}</button>
                </div>

                <div class="dock-group">
                    <button class="dock-btn" data-fmt="bold" title="${a.t("widget.notepad.tool.bold")}">${v.bold}</button>
                    <button class="dock-btn" data-fmt="italic" title="${a.t("widget.notepad.tool.italic")}">${v.italic}</button>
                    <button class="dock-btn" data-fmt="underline" title="${a.t("widget.notepad.tool.underline")}">${v.underline}</button>
                    <button class="dock-btn" data-fmt="strikeThrough" title="${a.t("widget.notepad.tool.strike")}">${v.strike}</button>
                ...
                <div class="dock-group">
                    <button class="dock-btn" data-fmt="justifyLeft" title="${a.t("widget.notepad.tool.left")}">${v.alignLeft}</button>
                    <button class="dock-btn" data-fmt="justifyCenter" title="${a.t("widget.notepad.tool.center")}">${v.alignCenter}</button>
                    <button class="dock-btn" data-fmt="justifyRight" title="${a.t("widget.notepad.tool.right")}">${v.alignRight}</button>
                    <button class="dock-btn" data-fmt="justifyFull" title="${a.t("widget.notepad.tool.justify")}">${v.alignJustify}</button>
                </div>

                <div class="dock-group">
                     <button class="dock-btn" id="btn-link" title="${a.t("widget.notepad.tool.link")}">${v.link}</button>
                     <button class="dock-btn" id="btn-image" title="${a.t("widget.notepad.tool.image")}">${v.image}</button>
                </div>

                <div class="dock-group">
                    <button class="dock-btn" data-fmt="insertUnorderedList" title="${a.t("widget.notepad.tool.bullet")}">${v.list}</button>
                    <button class="dock-btn" data-fmt="insertOrderedList" title="${a.t("widget.notepad.tool.number")}">${v.ordered}</button>
                    <button class="dock-btn" data-fmt="insertChecklist" title="${a.t("widget.notepad.tool.check")}">${v.check}</button>
                </div>

                <div class="dock-group">
                    <button class="dock-btn" data-fmt="formatBlock:BLOCKQUOTE" title="${a.t("widget.notepad.tool.quote")}">${v.quote}</button>
                     <button class="dock-btn" data-fmt="formatBlock:PRE" title="${a.t("widget.notepad.tool.code")}">${v.code}</button>
                    <button class="dock-btn" data-fmt="removeFormat" title="${a.t("widget.notepad.tool.clear")}">${v.eraser_clean}</button>
                </div>
            </div>
        ...
        `,this.cpuBar=this.shadowRoot.querySelector(".cpu-bar"),this.ramBar=this.shadowRoot.querySelector(".ram-bar"),this.tempBar=this.shadowRoot.querySelector(".temp-bar"),this.cpuText=this.shadowRoot.querySelector(".cpu-text"),this.ramText=this.shadowRoot.querySelector(".ram-text"),this.tempText=this.shadowRoot.querySelector(".temp-text")}}customElements.define("widget-telemetry",et);f();M();var st=new Map,lt=900000,dt={widgetId:"weather",latitude:0,longitude:0,city:"",unit:"celsius",showForecast:!1,forecastDays:5};function O(o){if(o===0)return`<svg viewBox="0 0 64 64" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round">
        ...
        `}forecastView(o,t,r){let i=a.getLocale().code,n=Math.min(this._config.forecastDays||5,t.time.length-1),e="";for(let s=1;s<=n;s++){let d=new Date(t.time[s]+"T00:00:00"),l=d.toLocaleDateString(i,{weekday:"short"}),c=d.toLocaleDateString(i,{weekday:"long",day:"numeric",month:"long"}),p=xo(t.weathercode[s]),h=E(t.temperature_2m_max[s],r),w=E(t.temperature_2m_min[s],r),u=t.sunrise?.[s]?.split("T")[1]||"",_=t.sunset?.[s]?.split("T")[1]||"",y=t.wind_speed_10m_max?.[s]!=null?`${Math.round(t.wind_speed_10m_max[s])} km/h`:"";e+=`<div class="fc-day">
                <span class="fc-name">${l}</span>
                <span class="fc-icon">${O(t.weathercode[s])}</span>
                <span class="fc-temp">${h}</span>
                <div class="fc-tooltip">
                    <div class="tt-date">${c}</div>
                    <div class="tt-desc">${p}</div>
                    <div class="tt-row"><span class="tt-label">${a.t("widget.weather.max_min")}</span><span>${h} / ${w}</span></div>
                    ${u?`<div class="tt-row"><span class="tt-label">${a.t("widget.weather.sunrise")}</span><span>${u}</span></div>`:""}
                    ${_?`<div class="tt-row"><span class="tt-label">${a.t("widget.weather.sunset")}</span><span>${_}</span></div>`:""}
                    ${y?`<div class="tt-row"><span class="tt-label">${a.t("widget.weather.wind")}</span><span>${y}</span></div>`:""}
                </div>
            </div>`}return`
            <div class="fc-header">
                <div class="fc-h-icon">${O(o.weathercode)}</div>
                <div class="fc-h-info">
        ...
        `}render(){if(!this.shadowRoot)return;this.shadowRoot.innerHTML=`<style>${ar}</style><div class="weather-body"></div>`,this.updateDisplay()}}var ar=`
:host {
    display: flex;
    flex-direction: column;
    align-items: center;
...
}`;class yt extends HTMLElement{icons=[];filteredIcons=[];selectedIcon="";searchQuery="";isLoading=!0;debounceTimer=null;inputElement=null;constructor(){super();this.attachShadow({mode:"open"})}async connectedCallback(){if(this.shadowRoot.childElementCount>0)return;if(this.renderBase(),this.setupListeners(),this.icons.length===0)await this.loadIcons()}async loadIcons(){try{this.icons=await ho.loadIcons(),this.filteredIcons=this.icons.slice(0,50),this.isLoading=!1,this.updateGrid()}catch(o){console.error("[IconPicker] Failed to load icons",o),this.isLoading=!1,this.updateGrid()}}setupListeners(){let o=this.shadowRoot;this.inputElement=o.getElementById("icon-search"),this.inputElement?.addEventListener("input",(r)=>{let i=r.target;if(this.searchQuery=i.value,this.debounceTimer)window.clearTimeout(this.debounceTimer);this.debounceTimer=window.setTimeout(()=>{this.performSearch()},100)}),o.getElementById("grid-container")?.addEventListener("click",(r)=>{let n=r.target.closest(".icon-picker__item");if(n&&n.dataset.icon)this.selectedIcon=n.dataset.icon,this.updateGrid(),this.dispatchEvent(new CustomEvent("icon-selected",{detail:{iconName:this.selectedIcon},bubbles:!0,composed:!0}))})}performSearch(){this.filteredIcons=ho.searchIcons(this.searchQuery,50),this.updateGrid()}getSelectedIcon(){return this.selectedIcon}setSelectedIcon(o){this.selectedIcon=o,this.updateGrid()}renderBase(){this.shadowRoot.innerHTML=`
            <style>${_t}</style>
            ${ft()}
        `}updateGrid(){let o=this.shadowRoot.getElementById("grid-container");if(o){if(o.style.display="grid",!this.searchQuery.trim()&&!this.isLoading){o.innerHTML='<div class="icon-picker__empty">Start typing to search icons...</div>';return}o.innerHTML=ut(this.filteredIcons,this.selectedIcon,this.isLoading)}}}if(!customElements.get("icon-picker"))customElements.define("icon-picker",yt);f();var dr=(o)=>`
    <div class="color-picker-row">
        <label for="bookmark-borderColor">${a.t("bookmark.border_color")||"Border Color"}</label>
        <div class="color-input-wrapper">
            <input type="color" id="bookmark-borderColor" name="borderColor" value="${o}" />
        </div>
    </div>
`,wt=({isOpen:o,isEditMode:t,title:r,url:i,protocol:n,selectedIcon:e,color:s,statusPosition:d,checkStatus:l,labelPosition:c,visibleTouch:p})=>`
    <dialog id="modal">
        <div class="modal-header">
        ...
        <div class="tab-container">
            <form class="modal-form" id="bookmark-form">
                    ...
                    <div class="form-row" style="display: flex; gap: 12px; align-items: flex-end;">
                        ...
                        <div class="form-group" style="flex: 1;">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" id="bookmark-label" name="label" placeholder="${a.t("bookmark.placeholder_label")}" value="${r}" required />
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="input-group">
                            ...
                            <div class="icon-dropdown">
                                <button type="button" class="icon-dropdown-btn" id="status-pos-btn" title="Status Position" style="position: relative;">
                                    ${l?`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="4" y="4" width="16" height="16" rx="2" stroke-opacity="0.5" />
                                ...
                                <input type="hidden" name="statusPos" id="bookmark-statusPos" value="${l?d:"off"}">
                            </div>
                    ...
                    <div class="form-group" style="margin-top: 12px;">
                        <div class="input-group">
                            <div class="icon-dropdown">
                                ...
                                <input type="hidden" name="visibleTouch" id="bookmark-visibleTouch" value="${p?"on":"off"}">
                            </div>
                ...
                <div class="form-actions">
                    <button type="submit" class="btn btn-primary">${a.t("general.save")}</button>
                </div>
            </form>
        </div>
        ...
        <div id="menu-overlay" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; pointer-events: none; z-index: 9999;">
            ...
            <div class="dropdown-menu" id="status-pos-menu">
                <div class="dropdown-item ${!l?"selected":""}" data-status-pos="off" title="Off">
            ...
            <div class="dropdown-menu" id="touch-pos-menu">
                <div class="dropdown-item ${p?"selected":""}" data-touch="on" title="Visible">
                ...
                <div class="dropdown-item ${!p?"selected":""}" data-touch="off" title="Hidden">
...
.btn:active {
    transform: translateY(1px);
}`;class Mt extends HTMLElement{dialog=null;isEditMode=!1;currentItemId=null;bookmarkTitle="";url="";protocol="http://";selectedIconName=null;color="#333333";statusPosition="top-right";checkStatus=!1;labelPosition="bottom";visibleTouch=!0;_unsubscribeI18n;clickHandler=null;submitHandler=null;escapeHandler=null;inputHandler=null;constructor(){super();this.attachShadow({mode:"open"}),this.setupHandlers()}setupHandlers(){this.clickHandler=async(o)=>{let t=o.target;if(t.closest("#modal-close")){o.preventDefault(),this.close();return}if(t.classList.contains("tab-btn")){o.preventDefault();let i=t.dataset.tab;this.switchTab(i)}if(t.closest("#icon-trigger-btn")){o.preventDefault();let i=this.shadowRoot.getElementById("icon-modal-component");if(i){let n=await i.open();if(n)this.selectedIconName=n,this.updateIconPreview()}}if(t.closest("#label-pos-btn"))o.preventDefault(),o.stopPropagation(),this.toggleDropdown("label-pos-menu",t.closest("#label-pos-btn"));else if(t.closest("#status-pos-btn"))o.preventDefault(),o.stopPropagation(),this.toggleDropdown("status-pos-menu",t.closest("#status-pos-btn"));else if(t.closest("#touch-pos-btn"))o.preventDefault(),o.stopPropagation(),this.toggleDropdown("touch-pos-menu",t.closest("#touch-pos-btn"));else if(t.closest("#protocol-btn"))o.preventDefault(),o.stopPropagation(),this.toggleDropdown("protocol-menu",t.closest("#protocol-btn"));else this.closeAllDropdowns();let r=t.closest(".dropdown-item");if(r){o.preventDefault(),o.stopPropagation();let i=r;if(i.dataset.pos)this.labelPosition=i.dataset.pos,this.updateTriggers();if(i.dataset.statusPos){let n=i.dataset.statusPos;if(n==="off")this.checkStatus=!1;else this.checkStatus=!0,this.statusPosition=n;this.updateTriggers()}if(i.dataset.touch)this.visibleTouch=i.dataset.touch==="on",this.updateTriggers();if(i.dataset.protocol)this.protocol=i.dataset.protocol,this.updateTriggers()}},this.inputHandler=(o)=>{let t=o.target;if(t.id==="bookmark-label")this.bookmarkTitle=t.value;if(t.id==="bookmark-url")this.url=t.value;if(t.id==="bookmark-borderColor")this.color=t.value},this.submitHandler=async(o)=>{o.preventDefault();let t=this.selectedIconName?`https://cdn.jsdelivr.net/gh/homarr-labs/dashboard-icons/png/${this.selectedIconName}.png`:"",r=JSON.stringify({label:this.bookmarkTitle,url:this.protocol+this.url.replace(/^https?:\/\//,""),description:"",icon:t,iconName:this.selectedIconName,statusCheck:this.checkStatus,statusPos:this.checkStatus?this.statusPosition:"off",visibleTouch:this.visibleTouch,openInNewTab:!0,labelPos:this.labelPosition,borderColor:this.color});try{if(this.isEditMode&&this.currentItemId){if(await g.updateItem({id:this.currentItemId,content:r}),window.notifier)window.notifier.show(a.t("notifier.bookmark_updated"))}else{let{collisionService:i}=await Promise.resolve().then(() => (Z(),Y)),n=g.getState(),e=Array.isArray(n.items)?n.items:[],s=i.findFirstAvailableSlot(1,1,e);if(await g.addItem({type:"bookmark",x:s.x,y:s.y,w:1,h:1,content:r}),window.notifier)window.notifier.show(a.t("notifier.bookmark_added"))}this.close()}catch(i){if(console.error("[Modal] Error:",i),window.notifier)window.notifier.show(a.t("notifier.bookmark_error"),"error")}},this.escapeHandler=(o)=>{if(o.key==="Escape"){let t=this.shadowRoot.querySelector("icon-selection-modal");if(this.dialog?.open)this.close()}}}updateTriggers(){let o=this.shadowRoot,t=o.getElementById("label-pos-btn");if(t){t.innerHTML=`
            ...
            `;let e=o.getElementById("bookmark-labelPos");if(e)e.value=this.labelPosition;o.querySelectorAll("#label-pos-menu .dropdown-item").forEach((s)=>{s.classList.toggle("selected",s.dataset.pos===this.labelPosition)})}let r=o.getElementById("status-pos-btn");if(r){r.innerHTML=`
        ...
        `,this.dialog=this.shadowRoot.querySelector("dialog"),this.shadowRoot.querySelectorAll(".card").forEach((o)=>{o.addEventListener("click",()=>{let t=o.getAttribute("data-id"),r=vo.find((i)=>i.id===t);if(r)this.selectWidget(r)})})}}customElements.define("add-widget-modal",Lt);M();f();var At=`:host {
    display: contents;
}
...
@keyframes fadeContent {
    ...
    to {
        ...
        transform: translateY(0);
...
}`;var fo=new Map;async function Bt(o,t){try{let r=await fetch(`https://timeapi.io/api/timezone/coordinate?latitude=${o}&longitude=${t}`);if(!r.ok)throw Error("TimeAPI failed");return(await r.json()).timeZone||"local"}catch(r){return console.error("[Timezone] TimeAPI error:",r),"local"}}async function gr(o){if(!o||o.trim()==="")return"local";let t=o.toLowerCase().trim();if(fo.has(t))return fo.get(t);try{let r=`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(o)}&limit=1`,i=await fetch(r,{headers:{"User-Agent":"Lastboard/1.0"}});if(!i.ok)throw Error("Geocoding failed");let n=await i.json();if(n.length===0)return"local";let{lat:e,lon:s}=n[0],d=await Bt(parseFloat(e),parseFloat(s));return fo.set(t,d),d}catch(r){return console.error("[Timezone] Error resolving city:",o,r),"local"}}class qt extends HTMLElement{dialog=null;currentItem=null;_clockSelectedLat=null;_clockSelectedLon=null;constructor(){super();this.attachShadow({mode:"open"})}connectedCallback(){this.render(),this.setupEventListeners()}open(o){if(this.currentItem=o,this.render(),this.setupEventListeners(),this.dialog)this.dialog.showModal(),this.switchTab("general")}close(){if(this.dialog)this.dialog.close();this.currentItem=null}setupEventListeners(){if(!this.shadowRoot)return;if(this.shadowRoot.querySelectorAll(".tab-btn").forEach((o)=>{o.addEventListener("click",(t)=>{t.preventDefault();let r=t.currentTarget.dataset.tab;if(r)this.switchTab(r)})}),this.shadowRoot.getElementById("close-btn")?.addEventListener("click",()=>this.close()),this.shadowRoot.getElementById("save-btn")?.addEventListener("click",()=>this.save()),this.currentItem){let t=(typeof this.currentItem.content==="string"?JSON.parse(this.currentItem.content):this.currentItem.content).widgetId;if(t==="clock"){let r=this.shadowRoot.getElementById("clock-search-btn");if(r)r.addEventListener("click",()=>this.searchClockCity())}else if(t==="weather"){let r=this.shadowRoot.getElementById("weather-search-btn");if(r)r.addEventListener("click",()=>this.searchWeatherCity());let i=this.shadowRoot.getElementById("weather-forecast"),n=this.shadowRoot.getElementById("weather-days-row");if(i&&n)i.addEventListener("change",()=>{n.style.display=i.checked?"":"none"});let e=this.shadowRoot.getElementById("weather-days"),s=this.shadowRoot.getElementById("weather-days-dec"),d=this.shadowRoot.getElementById("weather-days-inc");if(e&&s&&d)s.addEventListener("click",()=>{let l=parseInt(e.value)||1;if(l>1)e.value=String(l-1)}),d.addEventListener("click",()=>{let l=parseInt(e.value)||1;if(l<6)e.value=String(l+1)})}}}switchTab(o){if(!this.shadowRoot)return;this.shadowRoot.querySelectorAll(".tab-btn").forEach((t)=>{t.classList.toggle("active",t.dataset.tab===o)}),this.shadowRoot.querySelectorAll(".tab-content").forEach((t)=>{t.classList.toggle("active",t.id===`tab-${o}`)})}async save(){if(!this.currentItem)return;let o=typeof this.currentItem.content==="string"?JSON.parse(this.currentItem.content):this.currentItem.content,t=o.widgetId,r={...o};if(t==="clock"){let i=this.shadowRoot?.getElementById("clock-city"),n=this.shadowRoot?.getElementById("clock-12h"),e=this.shadowRoot?.getElementById("clock-date"),s=this.shadowRoot?.getElementById("save-btn"),d=i?.value||"";if(s)s.disabled=!0,s.textContent=a.t("general.loading");let l="local";if(this._clockSelectedLat!=null&&this._clockSelectedLon!=null)l=await Bt(this._clockSelectedLat,this._clockSelectedLon);else if(d)l=await gr(d);if(r.city=d,r.timezone=l,r.hour12=n?.checked||!1,r.showDate=e?.checked||!1,this._clockSelectedLat=null,this._clockSelectedLon=null,s)s.disabled=!1,s.textContent=a.t("general.save")}else if(t==="weather"){let i=this.shadowRoot?.getElementById("weather-city"),n=this.shadowRoot?.getElementById("weather-lat"),e=this.shadowRoot?.getElementById("weather-lon"),s=this.shadowRoot?.getElementById("weather-fahrenheit"),d=this.shadowRoot?.getElementById("weather-forecast"),l=this.shadowRoot?.getElementById("weather-days");r.city=i?.value||"",r.latitude=parseFloat(n?.value)||0,r.longitude=parseFloat(e?.value)||0,r.unit=s?.checked?"fahrenheit":"celsius",r.showForecast=d?.checked||!1,r.forecastDays=parseInt(l?.value)||5}else if(t==="telemetry"){let i=this.shadowRoot?.getElementById("telemetry-interval");r.interval=i?parseInt(i.value):1000}if(this.currentItem.type==="section"){let i=this.shadowRoot?.getElementById("section-title");r.title=i?i.value:"",delete r.name}await g.updateItem({id:this.currentItem.id,content:JSON.stringify(r)}),this.close()}render(){if(!this.shadowRoot)return;let o={},t="";if(this.currentItem)o=typeof this.currentItem.content==="string"?JSON.parse(this.currentItem.content):this.currentItem.content,t=o.widgetId;let r=(d)=>String(d).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;"),i=()=>{if(!this.currentItem)return{general:"",customization:""};if(t==="clock"){let d=o.city||"",l=o.hour12||!1,c=o.showDate!==!1,p=`
                    <div class="field-group">
                        <label>${a.t("widget.clock.city")}</label>
                        <div class="input-row">
                            <input type="text" id="clock-city" value="${r(d)}" placeholder="${a.t("widget.clock.city_placeholder")}"/>
                ...
                `;return{general:p,customization:h}}else if(t==="weather"){let d=o.city||"",l=o.latitude||"",c=o.longitude||"",p=o.unit==="fahrenheit",h=o.showForecast||!1,w=o.forecastDays||5,u=`
                    <div class="field-group">
                        <label>${a.t("widget.weather.city")}</label>
                        <div class="input-row">
                            <input type="text" id="weather-city" value="${r(d)}" placeholder="${a.t("widget.weather.city_placeholder")}"/>
                ...
                `,_=`
                    <div class="field-group check-row">
                        <input type="checkbox" id="weather-fahrenheit" ${p?"checked":""} />
                        <label for="weather-fahrenheit">${a.t("widget.weather.use_fahrenheit")}</label>
                    </div>

                    <div class="field-group check-row">
                        <input type="checkbox" id="weather-forecast" ${h?"checked":""} />
                        <label for="weather-forecast">${a.t("widget.weather.show_forecast")}</label>
                    </div>

                    <div class="field-group row-aligned" id="weather-days-row" ${!h?'style="display:none;"':""}>
                ...
                `;return{general:u,customization:_}}else if(t==="telemetry"){let d=o.interval||1000,l=`
                ...
                `}}else if(this.currentItem.type==="section"){let d=o.title||"",l=`
                    <div class="field-group">
                        <label>${a.t("bookmark.label")}</label>
                        <div class="input-row">
                            <input type="text" id="section-title" value="${r(d)}" placeholder="${a.t("section.placeholder_title")}" />
                        </div>
                        <small>${a.t("section.leave_empty")}</small>
                    </div>
                `,c=`
                    <p style="color: var(--text-dim); font-size: 13px;">${a.t("general.no_options")}</p>
                `;return{general:l,customization:c}}return{general:`<p>${a.t("widget.config.no_config")}</p>`,customization:""}},n=()=>{if(this.currentItem?.type==="section")return a.t("section.edit_title");return a.t("widget.config.title")},e=i();this.shadowRoot.innerHTML=`
            <style>${At}</style>
            <dialog id="modal">
                <div class="modal-header">
                    <div class="integrated-tabs">
...
.modal-actions {
    ...
    justify-content: flex-end;
        ...
        `}}oo();var j=document.getElementById("main-topbar"),Ft=document.getElementById("right-drawer"),to=document.getElementById("dashboard-root"),N,ro,io;Et(async()=>{q.init();let[o,t]=await Promise.all([k.fetchUser(),g.fetchItems()]),r=k.getUser();if(r){if(r.language)await a.setLanguage(r.language);if(r.preferences&&r.preferences.grid_columns)document.documentElement.style.setProperty("--user-preferred-columns",r.preferences.grid_columns.toString());if(r.theme==="dark")q.enableDark();else if(r.theme==="light")q.enableLight();else if(window.matchMedia&&window.matchMedia("(prefers-color-scheme: dark)").matches)q.enableDark();else q.enableLight();if(r.id)g.setUserId(r.id)}if(j){if(j.setState({user:r}),r&&r.project_name)j.setAttribute("title",r.project_name);k.subscribe((n)=>{if(n){if(j.setState({user:n}),n.project_name)j.setAttribute("title",n.project_name)}})}br(),W.start(),N=document.createElement("add-bookmark-modal"),document.body.appendChild(N),ro=document.createElement("add-widget-modal"),document.body.appendChild(ro),io=document.createElement("confirmation-modal"),document.body.appendChild(io);let i=document.createElement("widget-config-modal");document.body.appendChild(i)});V.on(X.SHOW_CONFIRMATION,async(o)=>{let{title:t,message:r,onConfirm:i}=o.detail;if(io){if(await io.confirm(t,r)&&i)i()}});V.on(X.SHOW_WIDGET_CONFIG,(o)=>{let{item:t,type:r}=o.detail;if(r==="bookmark"){if(N)N.openForEdit(t)}else{let i=document.querySelector("widget-config-modal");if(i)i.open(t)}});V.on(X.NOTIFY,(o)=>{let t=document.querySelector("app-notifier");if(t)t.show(o.detail.message,o.detail.type)});if(j)j.addEventListener("drawer-toggle",(o)=>{if(o.detail.action==="open")Ft.open(),j.setState({drawerOpen:!0});else Ft.close(),j.setState({drawerOpen:!1})}),j.addEventListener("edit-mode-change",(o)=>{let t=o.detail.active;if(to.classList.toggle("edit-mode",t),!t)window.location.reload()}),j.addEventListener("search-input",(o)=>{let t=o.detail.query;g.setSearchQuery(t)}),j.addEventListener("add-item",async(o)=>{let t=o.detail.action;if(t==="add-bookmark"){if(N)N.open()}else if(t==="add-widget"){if(ro)ro.open()}else if(t==="add-section"){let i=g.getState().items||[],{collisionService:n}=await Promise.resolve().then(() => (Z(),Y)),e=n.findFirstAvailableSlot(1,1,i),s={type:"section",x:e.x,y:e.y,w:1,h:1,content:JSON.stringify({title:""})},d=await g.addItem(s);if(d)V.emit(X.SHOW_WIDGET_CONFIG,{item:d,type:"section"})}});window.addEventListener("widget-selected",async(o)=>{let r=o.detail,n=g.getState().items||[],{collisionService:e}=await Promise.resolve().then(() => (Z(),Y)),s=e.findFirstAvailableSlot(r.defaultW,r.defaultH,n),d={type:"widget",x:s.x,y:s.y,w:r.defaultW,h:r.defaultH,content:JSON.stringify({widgetId:r.id,text:r.id==="notepad"||r.id==="markdown"?"":void 0})};await g.addItem(d)});window.addEventListener("drawer-close",()=>{if(j)j.setState({drawerOpen:!1})});window.addEventListener("click",(o)=>{let t=o.target,r=document.getElementById("add-menu"),i=document.getElementById("add-toggle");if(r&&r.classList.contains("active")&&!r.contains(t)&&i&&!i.contains(t))r.classList.remove("active")});function br(){if(!to)return;to.innerHTML="";let o=document.createElement("bookmark-grid");to.appendChild(o)}
