...
.avatar__initials {
    user-select: none;
        ...
        `}}if(!customElements.get("app-avatar"))customElements.define("app-avatar",vr);_();oo();Ro();_();var fr=(o)=>`
...
(max-width: 1024px) {
    ...
    .header__profile-container {
        ...
        `}}if(!customElements.get("app-header"))customElements.define("app-header",Qr);_();var ao=(o)=>String(o).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;"),Oa=(o)=>{if(!o)return"#";let r=o.trim().toLowerCase();if(r.startsWith("http://")||r.startsWith("https://")||r==="#")return ao(o);return"#"},Rr=({bookmarks:o,isEditing:r,isSearching:a,isTouchDevice:t,maxCols:i=12})=>{let e=Array.isArray(o)?o:[],s=(l)=>e.filter((c)=>c.parent_id===l);return`
    ${(t?e:a?e:e.filter((l)=>!l.parent_id)).map((l)=>{let c={};try{c=typeof l.content==="string"?JSON.parse(l.content):l.content}catch(w){console.error("Failed to parse content for item",l.id,w)}let p=l.type==="section",x={x:l.x||1,y:l.y||1},h=Math.min(l.w||1,i),u=l.h||1;if(p){let w=s(l.id),G=(c.title||"").trim();return`
            <fieldset class="bookmark-grid__section"
               ...
               `:""}
            </fieldset>`}return Kr(l,c,r,x,h,u,t)}).join("")}

    <div id="ghost-element" class="bookmark-grid__ghost" style="display: none;"></div>
    `};function Kr(o,r,a,t,i,e=0,s=!1){let d=e||o.h||1;if(o.type==="widget"){let h=r.widgetId,w={clock:"widget-clock",notepad:"widget-notepad",telemetry:"widget-telemetry"}[h]||"div",m=(r.text||"").replace(/"/g,"&quot;");return`
