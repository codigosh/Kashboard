...
(max-width: 1024px) {
    ...
    ${(t?e:a?e:e.filter((p)=>!p.parent_id)).map((p)=>{let c={};try{c=typeof p.content==="string"?JSON.parse(p.content):p.content}catch(k){console.error("Failed to parse content for item",p.id,k)}let l=p.type==="section",x={x:p.x||1,y:p.y||1},v=Math.min(p.w||1,i),u=p.h||1;if(l){let k=s(p.id),G=(c.title||"").trim();return`
            <fieldset class="bookmark-grid__section"
               data-id="${p.id}"
               data-orig-x="${p.x}" data-orig-y="${p.y}"
               draggable="${r}"
               style="--x: ${x.x}; --y: ${x.y}; --w: ${v}; --h: ${u};">
               ...
               `:""}
            </fieldset>`}return Kr(p,c,r,x,v,u,t)}).join("")}

    <div id="ghost-element" class="bookmark-grid__ghost" style="display: none;"></div>
    `};function Kr(o,r,a,t,i,e=0,s=!1){let d=e||o.h||1;if(o.type==="widget"){let v=r.widgetId,k={clock:"widget-clock",notepad:"widget-notepad",telemetry:"widget-telemetry"}[v]||"div",m=(r.text||"").replace(/"/g,"&quot;");return`
        <div class="bookmark-grid__card"
            ...
            ${a?`
            <div class="bookmark-actions">
                 ${["clock","telemetry"].includes(v)?`<button class="action-btn btn-edit" title="${n.t("general.edit")}">✎</button>`:""}
                 <button class="action-btn btn-delete" title="${n.t("general.delete")}">\uD83D\uDDD1</button>
            </div>
            ${["clock","telemetry"].includes(v)?"":'<div class="resize-handle"></div>'}
            `:""}
        </div>
    `}let p=r.icon||"",l=p.startsWith("http")||p.startsWith("/")?`<img src="${ao(p)}" alt="${ao(r.label)}" class="bookmark-grid__icon-img" draggable="false" />`:p?ao(p):'<svg class="bookmark-grid__icon-svg" viewBox="0 0 24 24"><path d="M10 6L8.59 7.41 13.17 12l-4.58 4.59L10 18l6-6z"/></svg>';return`
