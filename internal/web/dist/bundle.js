...
(max-width: 1024px) {
    ...
    .header__profile-container {
        ...
        `}}if(!customElements.get("app-header"))customElements.define("app-header",Rr);_();var ao=(o)=>String(o).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;"),Ya=(o)=>{if(!o)return"#";let r=o.trim().toLowerCase();if(r.startsWith("http://")||r.startsWith("https://")||r==="#")return ao(o);return"#"},Kr=({bookmarks:o,isEditing:r,isSearching:a,isTouchDevice:t,maxCols:i=12})=>{let e=Array.isArray(o)?o:[],s=(p)=>e.filter((c)=>c.parent_id===p);return`
    ${(t?e:a?e:e.filter((p)=>!p.parent_id)).map((p)=>{let c={};try{c=typeof p.content==="string"?JSON.parse(p.content):p.content}catch($){console.error("Failed to parse content for item",p.id,$)}let l=p.type==="section",x={x:p.x||1,y:p.y||1},u=Math.min(p.w||1,i),h=p.h||1;if(l){let $=s(p.id),G=(c.title||"").trim();return`
            <fieldset class="bookmark-grid__section"
               ...
               `:""}
            </fieldset>`}return Qr(p,c,r,x,u,h)}).join("")}

    <div id="ghost-element" class="bookmark-grid__ghost" style="display: none;"></div>
    `};function Qr(o,r,a,t,i,e=0){let s=e||o.h||1;if(o.type==="widget"){let x=r.widgetId,h={clock:"widget-clock",notepad:"widget-notepad",telemetry:"widget-telemetry"}[x]||"div",G=(r.text||"").replace(/"/g,"&quot;");return`
    ...
    `}let d=r.icon||"",c=d.startsWith("http")||d.startsWith("/")?`<img src="${ao(d)}" alt="${ao(r.label)}" class="bookmark-grid__icon-img" draggable="false" />`:d?ao(d):'<svg class="bookmark-grid__icon-svg" viewBox="0 0 24 24"><path d="M10 6L8.59 7.41 13.17 12l-4.58 4.59L10 18l6-6z"/></svg>';return`
        <a ${a?'role="button"':`href="${Ya(r.url)}" target="_blank"`} class="bookmark-grid__card"
           draggable="${a}"
           data-id="${o.id}"
           data-type="${o.type}"
           data-orig-x="${o.x}" data-orig-y="${o.y}"
