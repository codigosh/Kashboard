...
.avatar__initials {
    user-select: none;
        ...
        `}}if(!customElements.get("app-avatar"))customElements.define("app-avatar",Co);f();U();xo();f();var Po=(o)=>`
...
`,bt=(o,r)=>{if(!o)return"";if(!r)return"";if(r.is_docker)return`
    ...
    `},uo=(o,r,t)=>{return`
        ...
        `:""}
    `},rr=(o,r,t,a,e)=>{let n=t?.toLowerCase()==="admin"||t?.toLowerCase()==="administrator";return`
...
.select-option.selected:hover {
    background: rgba(var(--accent-rgb), 0.15);
        ...
        `,this.shadowRoot.getElementById("trigger")?.addEventListener("click",(t)=>{t.stopPropagation(),this.toggle()})}}if(!customElements.get("app-select"))customElements.define("app-select",tr);var _o=`:host {
    display: inline-block;
    position: relative;
    width: 100%;
    height: 100%;
...
@media (max-width: 900px) {
    ...
    .settings-content__personalization-grid {
        grid-template-columns: 1fr;
...
}`;class nr extends HTMLElement{prefs;users=[];getCsrfToken(){let o=document.cookie.split("; ").find((r)=>r.startsWith("csrf_token="));return o?decodeURIComponent(o.split("=")[1]):""}static get observedAttributes(){return["section"]}constructor(){super();this.attachShadow({mode:"open"}),this.prefs={accent_color:"#0078D4",language:"en",widget_min_width:140,beta_updates:!1}}unsubscribe=null;_renderPending=!1;connectedCallback(){this.unsubscribe=k.subscribe((r)=>{if(r&&r.preferences)this.prefs={...r.preferences,project_name:r.project_name||r.preferences.project_name||"Lastboard"},this.render()}),this.fetchPrefs();let o=this.getAttribute("section");if(o==="users")this.fetchUsers();this.render()}disconnectedCallback(){if(this.unsubscribe)this.unsubscribe(),this.unsubscribe=null}async fetchPrefs(){let o=k.getUser();if(o&&o.preferences)this.prefs={...o.preferences,project_name:o.project_name||o.preferences.project_name||"Lastboard"},this.render()}async fetchUsers(){try{this.users=await M.getUsers(),this.render()}catch(o){console.error("Failed to fetch users",o)}}attributeChangedCallback(o,r,t){if(o==="section"&&r!==t){if(t==="users")this.fetchUsers();this.render()}}async savePrefs(o){let r={...this.prefs};if(this.prefs={...this.prefs,...o},o.accent_color)this.applyAccent(o.accent_color);if(o.language)i.setLanguage(o.language);if(o.theme)if(o.theme==="dark")F.enableDark();else F.enableLight();try{await k.updatePreferences(o)}catch{if(this.prefs=r,o.language&&r.language)i.setLanguage(r.language)}this.render()}applyAccent(o){let r=o,t={blue:"#228be6",indigo:"#4c6ef5",cyan:"#15aabf",teal:"#12b886",orange:"#fd7e14",red:"#fa5252",grape:"#be4bdb"};if(!o.startsWith("#"))r=t[o]||"#0078D4";document.documentElement.style.setProperty("--accent",r);let a=parseInt(r.slice(1,3),16),e=parseInt(r.slice(3,5),16),n=parseInt(r.slice(5,7),16);if(!isNaN(a)&&!isNaN(e)&&!isNaN(n))document.documentElement.style.setProperty("--accent-rgb",`${a}, ${e}, ${n}`)}async handleBetaToggle(o){this.savePrefs({beta_updates:o}),this.render()}updateDensity(o){let r=this.shadowRoot.getElementById("val-widget_min_width");if(r)r.textContent=o+"px";document.documentElement.style.setProperty("--widget-min-size",o+"px")}commitDensity(o){let r=parseInt(o);k.updatePreferences({widget_min_width:r})}updateColumns(o){let r=this.shadowRoot.getElementById("val-grid_columns");if(r)r.textContent=o;document.documentElement.style.setProperty("--user-preferred-columns",o)}commitColumns(o){let r=parseInt(o);k.updatePreferences({grid_columns:r})}async updateUsername(o){let r=k.getUser();if(!r)return;if(await k.updateProfile({username:o,avatar_url:this.prefs.avatar_url||r.avatar_url}),window.notifier)window.notifier.show(i.t("notifier.username_updated"))}async updatePassword(){let o=this.shadowRoot.getElementById("current-password")?.value,r=this.shadowRoot.getElementById("new-password")?.value,t=this.shadowRoot.getElementById("confirm-password")?.value;if(!o){if(window.notifier)window.notifier.show(i.t("notifier.password_required"),"error");return}if(r!==t){if(window.notifier)window.notifier.show(i.t("notifier.password_mismatch"),"error");return}if(!r){if(window.notifier)window.notifier.show(i.t("notifier.password_empty"),"error");return}try{let a=await k.changePassword({current_password:o,new_password:r});if(this.shadowRoot.getElementById("current-password").value="",this.shadowRoot.getElementById("new-password").value="",this.shadowRoot.getElementById("confirm-password").value="",window.notifier)window.notifier.show(i.t("notifier.password_changed_relogin"));setTimeout(()=>{localStorage.removeItem("lastboard_user_cache"),document.body.style.opacity="0",window.location.href="/login"},2000)}catch(a){if(window.notifier)window.notifier.show(i.t("notifier.password_incorrect"),"error")}}async handleAvatarChange(o){let r=o.target.files?.[0];if(!r)return;if(!r.type.startsWith("image/")){if(window.notifier)window.notifier.show(i.t("notifier.invalid_image_type"),"error");return}try{let t=await this.resizeImage(r,256,256),a=k.getUser();if(!a)return;if(this.prefs.avatar_url=t,await k.updateProfile({username:a.username,avatar_url:t}),this.render(),window.notifier)window.notifier.show(i.t("notifier.avatar_updated"))}catch(t){if(console.error("Resize failed",t),window.notifier)window.notifier.show(i.t("notifier.avatar_error"),"error")}}resizeImage(o,r,t){return new Promise((a,e)=>{let n=new FileReader;n.onload=(s)=>{let p=new Image;p.onload=()=>{let d=document.createElement("canvas"),l=p.width,c=p.height;if(l>c){if(l>r)c=Math.round(c*r/l),l=r}else if(c>t)l=Math.round(l*t/c),c=t;d.width=l,d.height=c;let x=d.getContext("2d");if(!x){e(Error("Canvas context not available"));return}x.drawImage(p,0,0,l,c),a(d.toDataURL("image/jpeg",0.8))},p.onerror=(d)=>e(d),p.src=s.target?.result},n.onerror=(s)=>e(s),n.readAsDataURL(o)})}openAddUserModal(){let o=this.shadowRoot.getElementById("add-user-modal");if(o){o.showModal();let r=this.shadowRoot.getElementById("new-user-role");if(r)r.options=[{value:"user",label:i.t("settings.role_user")},{value:"admin",label:i.t("settings.role_admin")}]}}async createUser(){let o=this.shadowRoot.getElementById("new-user-username").value,r=this.shadowRoot.getElementById("new-user-password").value,t=this.shadowRoot.getElementById("new-user-role").value;if(!o||!r){if(window.notifier)window.notifier.show(i.t("notifier.fields_required"),"error");return}try{if(await M.createUser({username:o,password:r,role:t}),window.notifier)window.notifier.show(i.t("notifier.user_created"));let a=this.shadowRoot.getElementById("add-user-modal");if(a)a.close();this.fetchUsers()}catch(a){if(window.notifier)window.notifier.show(i.t("notifier.user_create_error"),"error")}}openEditUserModal(o,r,t){let a=this.shadowRoot.getElementById("edit-user-modal");if(a){this.shadowRoot.getElementById("edit-user-id").value=o.toString(),this.shadowRoot.getElementById("edit-user-username").value=r,this.shadowRoot.getElementById("edit-user-password").value="",a.showModal();let e=this.shadowRoot.getElementById("edit-user-role");if(e)e.options=[{value:"user",label:i.t("settings.role_user")},{value:"admin",label:i.t("settings.role_admin")}],e.value=t}}async updateAdminUser(){let o=parseInt(this.shadowRoot.getElementById("edit-user-id").value),r=this.shadowRoot.getElementById("edit-user-username").value,t=this.shadowRoot.getElementById("edit-user-password").value,a=this.shadowRoot.getElementById("edit-user-role").value;if(!r){if(window.notifier)window.notifier.show(i.t("notifier.username_required"),"error");return}try{if(await M.adminUpdateUser({id:o,username:r,role:a,password:t||void 0}),window.notifier)window.notifier.show(i.t("notifier.user_updated"));let e=this.shadowRoot.getElementById("edit-user-modal");if(e)e.close();this.fetchUsers()}catch(e){if(window.notifier)window.notifier.show(i.t("notifier.user_update_error"),"error")}}async deleteUser(o){let r=document.querySelector("confirmation-modal");if(r){if(!await r.confirm(i.t("general.delete"),i.t("notifier.user_delete_confirm")))return}let t=document.querySelector("password-confirm-modal");if(t){let a=await t.prompt(i.t("general.delete"),i.t("notifier.user_delete_confirm"));if(!a)return;try{let e=[...this.users];if(this.users=this.users.filter((n)=>n.id!==o),this.render(),await M.deleteUser(o,a),window.notifier)window.notifier.show(i.t("notifier.user_deleted"));this.fetchUsers()}catch(e){this.fetchUsers();let n=i.t("notifier.user_delete_error");if(e.message&&e.message.includes("error.cannot_delete_superadmin"))n=i.t("notifier.user_delete_superadmin");else if(e.status===401)n=i.t("notifier.password_incorrect");if(window.notifier)window.notifier.show(n,"error")}}}getContent(o){let r=k.getUser()||{username:"Guest",initials:"??",role:i.t("role.view_only"),avatar_url:"",accent_color:"#0078D4",language:"en",preferences:{}};switch(o){case"account":return Po(r);case"theme":{let t={blue:"#228be6",indigo:"#4c6ef5",cyan:"#15aabf",teal:"#12b886",orange:"#fd7e14",red:"#fa5252",grape:"#be4bdb"},a=Object.keys(t);return Wo(this.prefs,t,a)}case"personalization":return Io(this.prefs);case"advanced":return or();case"users":return So(this.users);case"about":return rr(this.version,this.updateInfo,r.role||"user",r.preferences?.beta_updates||!1,this.isCheckingUpdates);default:return`<div class="bento-card"><h3>${o}</h3><p class="settings-content__text-dim">${i.t("settings.default_module_desc")}</p></div>`}}version="v1.3.2-Beta.06";updateInfo=null;checkUpdatesPromise=null;isCheckingUpdates=!1;async handleManualCheckUpdates(){if(this.isCheckingUpdates)return;this.isCheckingUpdates=!0,this.updateUpdateUI();try{if(await this.checkForUpdates(),this.updateInfo&&!this.updateInfo.available){if(window.notifier)window.notifier.show(i.t("settings.up_to_date"))}}finally{this.isCheckingUpdates=!1,this.updateUpdateUI()}}async checkForUpdates(){if(this.checkUpdatesPromise)return this.checkUpdatesPromise;return this.checkUpdatesPromise=(async()=>{try{let{updateService:o}=await Promise.resolve().then(() => (vo(),fo)),r=this.prefs.beta_updates||!1;if(this.updateInfo=await o.check(r),this.updateInfo)this.version=this.updateInfo.current_version,this.updateUpdateUI()}catch(o){console.error("Check update failed",o)}finally{this.checkUpdatesPromise=null}})(),this.checkUpdatesPromise}async performUpdate(o){let r=document.querySelector("confirmation-modal");if(r){if(!await r.confirm(i.t("settings.update_available"),i.t("notifier.update_start_confirm")))return}let t=this.shadowRoot.getElementById("btn-update-now"),a=this.shadowRoot.getElementById("update-status");if(t)t.loading=!0;if(a){a.style.display="block";let e=a.querySelector("p");if(e)e.textContent=i.t("notifier.update_downloading")}try{let e=await fetch("/api/system/update/perform",{method:"POST",headers:{"Content-Type":"application/json","X-CSRF-Token":this.getCsrfToken()},body:JSON.stringify({asset_url:o})});if(e.ok){if(a){let n=a.querySelector("p");if(n)n.textContent=i.t("notifier.update_verified")}setTimeout(()=>{window.location.reload()},5000)}else{let n=await e.text();if(a){a.style.background="rgba(var(--danger-rgb), 0.1)",a.style.borderColor="rgba(var(--danger-rgb), 0.3)";let s=a.querySelector("p");if(s)s.style.color="var(--danger-color)",s.textContent=i.t("notifier.update_failed")+": "+n}if(t)t.loading=!1}}catch(e){if(console.error("Update failed",e),a){a.style.background="rgba(var(--danger-rgb), 0.1)",a.style.borderColor="rgba(var(--danger-rgb), 0.3)";let n=a.querySelector("p");if(n)n.style.color="var(--danger-color)",n.textContent=i.t("notifier.update_error")}if(t)t.loading=!1}}downloadBackup(){window.location.href="/api/system/backup"}async restoreBackup(o){if(!o)return;let r=document.querySelector("confirmation-modal");if(r){if(!await r.confirm(i.t("general.restore"),i.t("notifier.restore_confirm")))return}let t=new FormData;t.append("backup_file",o);try{if((await fetch("/api/system/restore",{method:"POST",headers:{"X-CSRF-Token":this.getCsrfToken()},body:t})).ok){if(window.notifier)window.notifier.show(i.t("notifier.restore_success"));setTimeout(()=>window.location.reload(),2000)}else if(window.notifier)window.notifier.show(i.t("notifier.restore_failed"),"error")}catch(a){if(console.error("Restore error",a),window.notifier)window.notifier.show(i.t("notifier.restore_failed"),"error")}}async executeFactoryReset(){let o=document.querySelector("password-confirm-modal");if(!o)return;let r=await o.prompt(i.t("settings.factory_reset"),i.t("settings.confirm_reset_msg"));if(!r)return;let t=this.shadowRoot.getElementById("btn-reset-confirm");try{if(t)t.disabled=!0,t.textContent=i.t("settings.restoring")||"Restoring...";if((await fetch("/api/system/reset",{method:"POST",headers:{"Content-Type":"application/json","X-CSRF-Token":this.getCsrfToken()},body:JSON.stringify({password:r})})).ok){let e=document.createElement("div");Object.assign(e.style,{position:"fixed",top:"0",left:"0",width:"100%",height:"100%",backgroundColor:"rgba(0,0,0,0.9)",zIndex:"9999",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",color:"white",fontFamily:"var(--font-main, sans-serif)"}),e.innerHTML=`
                ...
                `,document.body.appendChild(e);let n=0,s=async()=>{n++;try{if((await fetch("/",{method:"HEAD",cache:"no-store"})).ok){window.location.href="/setup";return}}catch(p){}if(n<60)setTimeout(s,1000);else window.location.href="/setup"};setTimeout(s,2000)}else if(window.notifier)window.notifier.show(i.t("notifier.reset_failed"),"error")}catch(a){if(console.error("Reset error",a),window.notifier)window.notifier.show(i.t("notifier.reset_error"),"error")}}render(){if(this._renderPending)return;this._renderPending=!0,requestAnimationFrame(()=>{this._renderPending=!1,this._doRender()})}_doRender(){if(this.shadowRoot.innerHTML=`
            <style>${ir}</style>
            <div class="fade-in">
                ${this.getContent(this.getAttribute("section")||"account")}
            </div>
        `,this.shadowRoot.querySelectorAll(".settings-content__checkbox").forEach((o)=>{o.addEventListener("click",()=>o.classList.toggle("settings-content__checkbox--checked"))}),this.initSelects(),this.getAttribute("section")==="about")this.updateBetaBadgeVisuals()}initSelects(){let o=this.shadowRoot.getElementById("language-select");if(o){let e=i.getAvailableLocales();o.options=e.map((n)=>({value:n.code,label:`${n.flag} ${n.name}`})),o.addEventListener("change",(n)=>{this.savePrefs({language:n.detail})})}let r=[{value:"user",label:i.t("settings.role_user")},{value:"admin",label:i.t("settings.role_admin")}],t=this.shadowRoot.getElementById("new-user-role");if(t)t.options=r;let a=this.shadowRoot.getElementById("edit-user-role");if(a)a.options=r}updateBetaBadgeVisuals(){setTimeout(()=>{let o=this.shadowRoot.getElementById("beta-updates-toggle-badge");if(o)o.checked=this.prefs.beta_updates||!1},0)}updateUpdateUI(){let o=this.shadowRoot.getElementById("update-zone-wrapper");if(o){let r=k.getUser(),t=r?.role?.toLowerCase()==="admin"||r?.role?.toLowerCase()==="administrator";o.innerHTML=uo(t,this.updateInfo,this.isCheckingUpdates)}}}if(!customElements.get("settings-content"))customElements.define("settings-content",nr);U();f();var er=({user:o,isOpen:r,selectedSection:t,updateAvailable:a})=>`
        ...
        <div class="right-drawer__header">
            ...
            <div class="right-drawer__user-info">
                ...
                ${o.role?.toLowerCase()==="admin"||o.role?.toLowerCase()==="administrator"?`<span class="right-drawer__role">
                    ${(()=>{let e=(o.role||"").toLowerCase();if(o.is_superadmin)return i.t("settings.role_super_admin");if(e==="admin"||e==="administrator")return i.t("settings.role_admin");if(e==="user")return i.t("settings.role_user");return o.role||i.t("settings.default_role")})()}
                </span>`:""}
            </div>
        </div>
        ...
        <div class="top-bar__actions">
            ...
            <div id="drawer-toggle" class="top-bar__toggle" style="position: relative;" title="${i.t("topbar.drawer_tooltip")}">
                ...
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor">
                    ...
                    <rect class="top-bar__indicator ${e?"top-bar__indicator--active":""}" 
...
@keyframes blink {
    ...
    100% {
        ...
        `}}if(!customElements.get("app-topbar"))customElements.define("app-topbar",lr);f();var gr=()=>`
    <div class="dot"></div>
...
(max-width: 1024px) {
    ...
    .header__profile-container {
        ...
        `}}if(!customElements.get("app-header"))customElements.define("app-header",vr);f();var D=(o)=>String(o).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;"),wt=(o)=>{if(!o)return"#";let r=o.trim().toLowerCase();if(r.startsWith("http://")||r.startsWith("https://")||r==="#")return D(o);return"#"},ur=({bookmarks:o,isEditing:r,isLoading:t,isSearching:a,isTouchDevice:e,maxCols:n=12})=>{let s=Array.isArray(o)?o:[];if(t&&s.length===0)return zt(n);let p=(l)=>s.filter((c)=>c.parent_id===l);return`
    ${(e?s:a?s:s.filter((l)=>!l.parent_id)).map((l)=>{let c={};try{c=typeof l.content==="string"?JSON.parse(l.content):l.content}catch(m){console.error("Failed to parse content for item",l.id,m)}let x=l.type==="section",u={x:l.x||1,y:l.y||1},v=Math.min(l.w||1,n),y=l.h||1;if(x){let m=p(l.id),w=(c.title||"").trim(),b=c.borderColor&&!r?`border-color: ${c.borderColor} !important;`:"",A=!r?`border-width: ${c.borderWidth||1}px !important;`:"";return`
            <fieldset class="bookmark-grid__section"
               data-id="${l.id}"
               data-orig-x="${l.x}" data-orig-y="${l.y}"
               draggable="${r}"
               style="--x: ${u.x}; --y: ${u.y}; --w: ${v}; --h: ${y}; ${b} ${A}">
               ...
               `:""}
            </fieldset>`}return mr(l,c,r,u,v,y,e)}).join("")}

    <div id="ghost-element" class="bookmark-grid__ghost" style="display: none;"></div>
    `};function mr(o,r,t,a,e,n=0,s=!1){let p=n||o.h||1;if(o.type==="widget"){let m=r.widgetId,b={clock:"widget-clock",notepad:"widget-notepad",telemetry:"widget-telemetry",weather:"widget-weather",markdown:"widget-markdown"}[m]||"div",H=(r.text||"").replace(/"/g,"&quot;"),h=r.borderColor&&!t?`border-color: ${r.borderColor} !important;`:"",Y=!t?`border-width: ${r.borderWidth||1}px !important;`:"";return`
        <div class="bookmark-grid__card"
            draggable="${t}"
            data-id="${o.id}"
            data-type="${o.type}"
            data-orig-x="${o.x}" data-orig-y="${o.y}"
            ${!s?`style="--x: ${a.x}; --y: ${a.y}; --w: ${e}; --h: ${p}; ${h} ${Y}"`:""}>
    ...
    `}function kt(o,r,t=!1,a=12,e=!1){return o.map((n)=>{let s={};try{s=typeof n.content==="string"?JSON.parse(n.content):n.content}catch(c){console.error("Failed to parse content for item (nested)",n.id,c)}let p={x:n.x||1,y:n.y||1},d=n.h||1,l=Math.min(n.w||1,a);return mr(n,s,r,p,l,d,e)}).join("")}function zt(o){return`
        <style>
            .skeleton {
                background: var(--surface);
                pointer-events: none;
...
`;class yr extends HTMLElement{_itemId=0;_content="";_isDirty=!1;_isLocked=!0;_isSaving=!1;_lastLockTime=0;_editor=null;_syncStatus=null;_dock=null;_editorContainer=null;_modal=null;_imgResizer=null;_activeImage=null;_pendingAction=null;_boundBeforeUnload;_unsubscribeI18n;static get observedAttributes(){return["item-id"]}constructor(){super();this.attachShadow({mode:"open"}),this._boundBeforeUnload=this.handleBeforeUnload.bind(this)}connectedCallback(){this.render(),this.loadContent(),window.addEventListener("beforeunload",this._boundBeforeUnload),this._unsubscribeI18n=i.subscribe(()=>{if(this._editor)this._editor.dataset.placeholder=i.t("widget.notepad.placeholder")})}disconnectedCallback(){if(window.removeEventListener("beforeunload",this._boundBeforeUnload),this._unsubscribeI18n)this._unsubscribeI18n()}attributeChangedCallback(o,r,t){if(o==="item-id")this._itemId=parseInt(t),this.loadContent()}handleBeforeUnload(o){if(this._isDirty)o.preventDefault(),o.returnValue=""}render(){if(!this.shadowRoot)return;let o=this._isLocked?"locked":"unlocked",r=this._isLocked?"locked":"",t=this._isLocked?"hidden":"";this.shadowRoot.innerHTML=`
            <style>
                ${$t}
        ...
        `,this.cpuBar=this.shadowRoot.querySelector(".cpu-bar"),this.ramBar=this.shadowRoot.querySelector(".ram-bar"),this.tempBar=this.shadowRoot.querySelector(".temp-bar"),this.cpuText=this.shadowRoot.querySelector(".cpu-text"),this.ramText=this.shadowRoot.querySelector(".ram-text"),this.tempText=this.shadowRoot.querySelector(".temp-text")}}customElements.define("widget-telemetry",wr);f();j();var kr=new Map,$r=900000,zr={widgetId:"weather",latitude:0,longitude:0,city:"",unit:"celsius",showForecast:!1,forecastDays:5};function C(o){if(o===0)return`<svg viewBox="0 0 64 64" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round">
    ...
    </svg>`}function yo(o){let r={0:"clear_sky",1:"mainly_clear",2:"partly_cloudy",3:"overcast",45:"fog",48:"fog",51:"light_drizzle",53:"drizzle",55:"heavy_drizzle",56:"freezing_drizzle",57:"freezing_drizzle",61:"light_rain",63:"rain",65:"heavy_rain",66:"freezing_rain",67:"freezing_rain",71:"light_snow",73:"snow",75:"heavy_snow",77:"snow_grains",80:"light_showers",81:"showers",82:"heavy_showers",85:"snow_showers",86:"heavy_snow_showers",95:"thunderstorm",96:"thunderstorm_hail",99:"thunderstorm_hail"};return i.t(`widget.weather.${r[o]||"unknown"}`)}function Z(o,r){if(r==="fahrenheit")return`${Math.round(o*9/5+32)}°F`;return`${Math.round(o)}°C`}async function jt(o,r){let t=`${o.toFixed(2)},${r.toFixed(2)}`,a=kr.get(t);if(a&&Date.now()-a.timestamp<$r)return a.data;let e=`https://api.open-meteo.com/v1/forecast?latitude=${o}&longitude=${r}&current_weather=true&daily=weathercode,temperature_2m_max,temperature_2m_min,sunrise,sunset,wind_speed_10m_max&timezone=auto&forecast_days=7`,n=await fetch(e);if(!n.ok)throw Error("Weather API error");let s=await n.json();return kr.set(t,{data:s,timestamp:Date.now()}),s}class jr extends HTMLElement{_config={...zr};_itemId=0;_unsubscribe=null;_unsubscribeI18n;_weatherData=null;_refreshTimer=null;_loading=!1;_error=!1;constructor(){super();this.attachShadow({mode:"open"})}static get observedAttributes(){return["item-id"]}attributeChangedCallback(o,r,t){if(o==="item-id")this._itemId=parseInt(t)}connectedCallback(){this.render(),this.loadWeather(),this._refreshTimer=setInterval(()=>this.loadWeather(),$r),this._unsubscribe=g.subscribe((o)=>{if(!this._itemId)return;let r=o.items.find((t)=>t.id===this._itemId);if(!r?.content)return;try{let t=typeof r.content==="string"?JSON.parse(r.content):r.content;if(t.widgetId!=="weather")return;let a=this._config.latitude,e=this._config.longitude;if(this._config={...zr,...t},a!==this._config.latitude||e!==this._config.longitude)this.loadWeather();else this.updateDisplay()}catch(t){}}),this._unsubscribeI18n=i.subscribe(()=>this.updateDisplay())}disconnectedCallback(){if(this._unsubscribe)this._unsubscribe();if(this._unsubscribeI18n)this._unsubscribeI18n();if(this._refreshTimer)clearInterval(this._refreshTimer)}async loadWeather(){if(!this._config.latitude&&!this._config.longitude){this._loading=!1,this._error=!1,this.updateDisplay();return}this._loading=!0,this._error=!1,this.updateDisplay();try{this._weatherData=await jt(this._config.latitude,this._config.longitude),this._loading=!1}catch(o){this._loading=!1,this._error=!0}this.updateDisplay()}updateDisplay(){let o=this.shadowRoot?.querySelector(".weather-body");if(!o){this.render();return}if(!this._config.latitude&&!this._config.longitude){o.innerHTML=this.emptyState(C(0),i.t("widget.weather.no_location"));return}if(this._loading){o.innerHTML=this.emptyState(C(3),i.t("general.loading"),!0);return}if(this._error||!this._weatherData){o.innerHTML=this.emptyState(C(-1),i.t("general.error"));return}let r=this._weatherData.current_weather,t=this._weatherData.daily,a=this._config.unit||"celsius";if(this._config.showForecast&&t)o.innerHTML=this.forecastView(r,t,a);else o.innerHTML=this.simpleView(r,t,a)}emptyState(o,r,t=!1){return`<div class="state"><div class="state-icon${t?" pulse":""}">${o}</div><span class="state-text">${r}</span></div>`}simpleView(o,r,t){return`
            <div class="icon-main">${C(o.weathercode)}</div>
            <div class="temp-main">${Z(o.temperature,t)}</div>
            <div class="desc">${yo(o.weathercode)}</div>
        ...
        `}forecastView(o,r,t){let a=i.getLocale().code,e=Math.min(this._config.forecastDays||5,r.time.length-1),n="";for(let s=1;s<=e;s++){let p=new Date(r.time[s]+"T00:00:00"),d=p.toLocaleDateString(a,{weekday:"short"}),l=p.toLocaleDateString(a,{weekday:"long",day:"numeric",month:"long"}),c=yo(r.weathercode[s]),x=Z(r.temperature_2m_max[s],t),u=Z(r.temperature_2m_min[s],t),v=r.sunrise?.[s]?.split("T")[1]||"",y=r.sunset?.[s]?.split("T")[1]||"",m=r.wind_speed_10m_max?.[s]!=null?`${Math.round(r.wind_speed_10m_max[s])} km/h`:"";n+=`<div class="fc-day">
                ...
                <div class="fc-tooltip">
                    ...
                    ${v?`<div class="tt-row"><span class="tt-label">${i.t("widget.weather.sunrise")}</span><span>${v}</span></div>`:""}
            ...
            <div class="fc-header">
                ...
                <div class="fc-h-info">
                    ...
                    <div class="fc-h-desc">${yo(o.weathercode)}</div>
                </div>
...
.status-indicator.status-pos-bottom-left {
    ...
    top: auto;
    right: auto;
}`;class Ar extends HTMLElement{bookmarks=[];allItems=[];isEditing=!1;isLoading=!0;searchQuery="";_unsubscribe;_unsubscribeI18n;_resizeObserver;_resizeDebounce=null;dragTargetId=null;ghostEl=null;dragOffsetX=0;dragOffsetY=0;isResizing=!1;resizeTargetId=null;initialResizeX=0;initialResizeY=0;initialResizeW=0;initialResizeH=0;currentColWidth=0;currentGridCols=12;isTouchDevice=!1;constructor(){super();this.attachShadow({mode:"open"})}_widgetModal;_boundActionClick=this.handleActionClick.bind(this);_boundMouseMove=this.handleWindowMouseMove.bind(this);_boundMouseUp=this.handleWindowMouseUp.bind(this);_boundUpdateTouchState=()=>{};applyFilters(){let o=this.isTouchDevice,r=!!this.searchQuery;if(r&&!o)this.classList.add("search-active");else this.classList.remove("search-active");if(o||r)this.bookmarks=this.allItems.filter((t)=>{let a=t.content;if(typeof t.content==="string")try{a=JSON.parse(t.content)}catch{return!1}if(o&&t.type!=="bookmark")return!1;if(o&&a.visibleTouch===!1)return!1;if(r)return(a.label||"").toLowerCase().includes(this.searchQuery);return!0});else this.bookmarks=this.allItems}connectedCallback(){let o=()=>{let r=window.innerWidth<768,t=window.matchMedia("(pointer: coarse)").matches||"ontouchstart"in window||navigator.maxTouchPoints>0;return r&&t};if(this._boundUpdateTouchState=()=>{let r=o();if(this.isTouchDevice!==r){if(this.isTouchDevice=r,r)this.classList.add("touch-mode");else this.classList.remove("touch-mode");this.applyFilters(),this.render()}},window.addEventListener("resize",this._boundUpdateTouchState),window.addEventListener("orientationchange",this._boundUpdateTouchState),this.isTouchDevice=o(),this.isTouchDevice)this.classList.add("touch-mode");this.applyFilters(),this.render(),this.updateGridMetrics(),this._resizeObserver=new ResizeObserver(()=>{if(this._resizeDebounce)return;this._resizeDebounce=setTimeout(()=>{this._resizeDebounce=null,this.updateGridMetrics();let r=this.getBoundingClientRect();g.setGridMetrics(r.width,this.currentGridCols),this.applyFilters(),this.render()},16)}),this._resizeObserver.observe(this),this._unsubscribe=g.subscribe((r)=>{let t=!1;if(this.isEditing!==r.isEditing)this.isEditing=r.isEditing,t=!0;if(this.isLoading!==r.loading)this.isLoading=r.loading,t=!0;if(this.searchQuery!==r.searchQuery)this.searchQuery=r.searchQuery,t=!0;let a=Array.isArray(r.items)?r.items:[];if(JSON.stringify(this.allItems)!==JSON.stringify(a)||t)this.allItems=a,this.applyFilters(),t=!0;if(t)this.render()}),this.setupDragListeners(),this.setupResizeListeners(),this.setupActionListeners(),I.start(),this._unsubscribeI18n=i.subscribe(()=>this.render()),Promise.resolve().then(() => (U(),ro)).then(({userStore:r})=>{this._unsubscribeUser=r.subscribe(()=>{requestAnimationFrame(()=>{this.updateGridMetrics(),this.render()})})})}_unsubscribeUser=null;setupActionListeners(){let o=this.shadowRoot;o.removeEventListener("click",this._boundActionClick),o.addEventListener("click",this._boundActionClick)}async handleActionClick(o){if(!this.isEditing)return;let r=o.target;if(!r)return;let t=r.closest(".btn-delete"),a=r.closest(".btn-edit");if(t||a)o.preventDefault(),o.stopPropagation();else if(r.closest("a"))o.preventDefault();if(t){let e=t.closest(".bookmark-grid__card, .bookmark-grid__group, .bookmark-grid__section");if(!e)return;let n=parseInt(e.dataset.id||"0"),s=this.allItems.find((c)=>c.id===n);if(!s)return;let p=s.type==="section"?i.t("type.section"):s.type==="widget"?i.t("type.widget"):i.t("type.bookmark"),{eventBus:d,EVENTS:l}=await Promise.resolve().then(() => (to(),wo));d.emit(l.SHOW_CONFIRMATION,{title:`${i.t("general.delete")} ${p}`,message:i.t("bookmark.delete_confirm_message"),onConfirm:()=>{g.deleteItem(n)}});return}if(a){let e=a.closest(".bookmark-grid__card, .bookmark-grid__group, .bookmark-grid__section");if(!e)return;let n=parseInt(e.dataset.id||"0"),s=this.allItems.find((p)=>p.id===n);if(s){let{eventBus:p,EVENTS:d}=await Promise.resolve().then(() => (to(),wo));if(s.type==="widget")p.emit(d.SHOW_WIDGET_CONFIG,{item:s,type:"widget"});else if(s.type==="section")p.emit(d.SHOW_WIDGET_CONFIG,{item:s,type:"section"});else p.emit(d.SHOW_WIDGET_CONFIG,{item:s,type:"bookmark"})}return}}disconnectedCallback(){if(this._unsubscribe)this._unsubscribe();if(this._unsubscribeI18n)this._unsubscribeI18n();if(this._unsubscribeUser)this._unsubscribeUser();if(this._resizeObserver)this._resizeObserver.disconnect();if(this._resizeDebounce)clearTimeout(this._resizeDebounce);if(this._updateGhostTimeout)clearTimeout(this._updateGhostTimeout);window.removeEventListener("mousemove",this._boundMouseMove),window.removeEventListener("mouseup",this._boundMouseUp),window.removeEventListener("resize",this._boundUpdateTouchState),window.removeEventListener("orientationchange",this._boundUpdateTouchState),I.stop()}updateGridMetrics(){let o=this.getBoundingClientRect(),r=getComputedStyle(this),t=r.getPropertyValue("--grid-cols").trim(),a=parseInt(t)||12,e=r.columnGap||r.gap||"16px",n=parseFloat(e)||16;if(o.width<=0){this.currentGridCols=a,this.currentColWidth=100,this.style.setProperty("--current-grid-cols",a.toString()),this.style.setProperty("--row-height","100px");return}let s=(a-1)*n,p=(o.width-s)/a;if(p<10)p=10;this.currentGridCols=a,this.currentColWidth=p,this.style.setProperty("--current-grid-cols",String(a)),this.style.setProperty("--row-height",`${p}px`),this.ensureGridBuffer()}ensureGridBuffer(){if(this.isTouchDevice){this.style.minHeight="";return}let o=0;this.bookmarks.forEach((s)=>{let p=(s.y||1)+(s.h||1);if(p>o)o=p});let t=o+1,a=this.currentColWidth||100,e=16,n=t*a+(t-1)*e;this.style.minHeight=`${n}px`}setupResizeListeners(){window.addEventListener("mousemove",this._boundMouseMove),window.addEventListener("mouseup",this._boundMouseUp),this.shadowRoot.addEventListener("mousedown",(o)=>{let r=o;if(!this.isEditing)return;let t=r.target;if(t.classList.contains("resize-handle")){r.preventDefault(),r.stopPropagation();let a=t.closest(".bookmark-grid__card, .bookmark-grid__section");if(!a||!a.dataset.id)return;let e=parseInt(a.dataset.id),n=this.bookmarks.find((v)=>v.id===e);if(!n)return;this.isResizing=!0,this.resizeTargetId=e,this.initialResizeX=r.clientX,this.initialResizeY=r.clientY,this.initialResizeW=n.w,this.initialResizeH=n.h;let s=this,p=s.getBoundingClientRect(),l=getComputedStyle(s).getPropertyValue("--current-grid-cols").trim(),c=l?parseInt(l,10):9,x=isNaN(c)?9:c,u=16;this.currentColWidth=(p.width-(x-1)*u)/x,this.updateGhost({x:n.x,y:n.y,w:n.w,h:n.h},!0)}})}_updateGhostTimeout=null;handleWindowMouseMove(o){if(!this.isResizing||!this.resizeTargetId)return;if(this._updateGhostTimeout)return;this._updateGhostTimeout=setTimeout(()=>{this._updateGhostTimeout=null,this.processResizeMove(o)},16)}processResizeMove(o){if(!this.isResizing||!this.resizeTargetId)return;let r=o.clientX-this.initialResizeX,t=o.clientY-this.initialResizeY,a=Math.round(r/(this.currentColWidth+16)),e=Math.round(t/(this.currentColWidth+16)),n=this.initialResizeW+a,s=this.initialResizeH+e,p=this.bookmarks.find((d)=>d.id===this.resizeTargetId);if(p){let d=this.applyResizeConstraints(n,s,p),l={x:p.x,y:p.y,w:d.w,h:d.h,id:p.id,parent_id:p.parent_id},c=J.calculateDropValidity(l,this.bookmarks,this.currentGridCols);this.updateGhost(l,c.valid)}}applyResizeConstraints(o,r,t){let a=1,e=1;if(t.type==="widget"){let n=t.content;if(typeof t.content==="string")try{n=JSON.parse(t.content)}catch{return{w:Math.max(1,Math.min(12,o)),h:Math.max(1,Math.min(12,r))}}let s=(n.widgetId||"").toLowerCase();if(s==="notepad")a=2,e=2;else if(s==="clock")return{w:2,h:1};else if(s==="telemetry")return{w:2,h:1};let p=Math.max(a,Math.min(12,o)),d=Math.max(e,Math.min(12,r));return{w:p,h:d}}else if(t.type==="section"){let n=Math.max(1,Math.min(12,o)),s=Math.max(1,Math.min(12,r));return{w:n,h:s}}else{let n=Math.max(1,Math.min(2,o)),s=Math.max(1,Math.min(2,r));return{w:n,h:s}}}async handleWindowMouseUp(o){if(!this.isResizing||!this.resizeTargetId)return;if(this._updateGhostTimeout)clearTimeout(this._updateGhostTimeout),this._updateGhostTimeout=null;let r=o.clientX-this.initialResizeX,t=o.clientY-this.initialResizeY,a=Math.round(r/(this.currentColWidth+16)),e=Math.round(t/(this.currentColWidth+16)),n=this.initialResizeW+a,s=this.initialResizeH+e,p=this.bookmarks.find((x)=>x.id===this.resizeTargetId);if(!p)return;let d=this.applyResizeConstraints(n,s,p);n=d.w,s=d.h;let l={x:p.x,y:p.y,w:n,h:s,id:p.id,parent_id:p.parent_id};if(J.calculateDropValidity(l,this.bookmarks,this.currentGridCols).valid&&(p.w!==n||p.h!==s)){if(p.type==="section"){let x=n*s,u=this.bookmarks.filter((v)=>v.parent_id===p.id);if(u.sort((v,y)=>v.y-y.y||v.x-y.x),u.length>x){let v=u.slice(x),y=[...this.bookmarks],m=v.map((w)=>{let b=J.findFirstAvailableSlot(w.w,w.h,y,this.currentGridCols);return y.push({...w,x:b.x,y:b.y,parent_id:void 0}),{id:w.id,x:b.x,y:b.y,parent_id:void 0}});await Promise.all(m.map((w)=>g.updateItem(w)))}}await g.resizeItem(p.id,n,s)}if(this.isResizing=!1,this.resizeTargetId=null,this.ghostEl)this.ghostEl.style.display="none"}setupDragListeners(){let o=this.shadowRoot,r=this;o.addEventListener("dragstart",(a)=>{let e=a;if(!this.isEditing){e.preventDefault();return}let n=e.target.closest('[draggable="true"]');if(n&&n.dataset.id){this.dragTargetId=parseInt(n.dataset.id),e.dataTransfer.effectAllowed="move",n.style.opacity="0.5";let s=n.getBoundingClientRect();if(this.dragOffsetX=e.clientX-s.left,this.dragOffsetY=e.clientY-s.top,e.dataTransfer)e.dataTransfer.setDragImage(n,this.dragOffsetX,this.dragOffsetY)}}),o.addEventListener("dragend",(a)=>{let e=a.target.closest('[draggable="true"]');if(e)e.style.opacity="1";if(this.dragTargetId=null,this.ghostEl)this.ghostEl.style.display="none";this.shadowRoot.querySelectorAll(".bookmark-grid__section").forEach((s)=>s.classList.remove("drop-target"))});let t=null;r.addEventListener("dragover",(a)=>{let e=a;if(!this.isEditing||!this.dragTargetId)return;if(e.preventDefault(),e.dataTransfer.dropEffect="move",t)return;t=setTimeout(()=>{t=null,this.processDragOver(e,r)},16)}),r.addEventListener("drop",async(a)=>{let e=a;if(t)clearTimeout(t),t=null;await this.handleDrop(e,r)})}processDragOver(o,r){let e=document.querySelector(".main-content");if(e){let z=e.getBoundingClientRect(),W=o.clientY;if(W>z.bottom-100)e.scrollBy(0,15);else if(W<z.top+100)e.scrollBy(0,-15)}let n=r.getBoundingClientRect(),s=this.currentGridCols||12,p=n.width,d=16,l=(p-(s-1)*d)/s,c=this.bookmarks.find((z)=>z.id===this.dragTargetId);if(!c)return;let x=o.clientX-this.dragOffsetX,u=o.clientY-this.dragOffsetY,v=x-n.left,y=u-n.top,m=Math.floor(v/(l+d)),w=Math.floor(y/(l+d)),b=Math.max(1,Math.min(s-c.w+1,m+1)),A=Math.max(1,w+1),H={x:b,y:A,w:c.w,h:c.h,id:c.id,parent_id:c.parent_id},h=J.calculateDropValidity(H,this.bookmarks,s),B=!this.bookmarks.some((z)=>{if(z.id===c.id)return!1;if(z.parent_id!==c.parent_id&&!h.targetGroup)return!1;let W=z.x||1,qo=z.y||1,it=z.w||1,nt=z.h||1;if(h.targetGroup&&z.id===h.targetGroup.id)return!1;return b<W+it&&b+c.w>W&&A<qo+nt&&A+c.h>qo})&&h.valid;if(this.updateGhost(H,B),this.shadowRoot.querySelectorAll(".bookmark-grid__section").forEach((z)=>z.classList.remove("drop-target")),h.targetGroup&&B){let z=this.shadowRoot.querySelector(`.bookmark-grid__section[data-id="${h.targetGroup.id}"]`);if(z)z.classList.add("drop-target")}}async handleDrop(o,r){if(!this.isEditing||!this.dragTargetId)return;o.preventDefault();let t=this.bookmarks.find((h)=>h.id===this.dragTargetId);if(!t)return;let a=r.getBoundingClientRect(),e=this.currentGridCols||12,n=a.width,s=16,p=(n-(e-1)*s)/e,d=o.clientX-this.dragOffsetX,l=o.clientY-this.dragOffsetY,c=d-a.left,x=l-a.top,u=Math.floor(c/(p+s)),v=Math.floor(x/(p+s)),y=Math.max(1,Math.min(e-t.w+1,u+1)),m=Math.max(1,v+1),w={x:y,y:m,w:t.w,h:t.h,id:t.id,parent_id:t.parent_id},b=J.calculateDropValidity(w,this.bookmarks,e);if(this.bookmarks.some((h)=>{if(h.id===t.id)return!1;if(h.parent_id!==t.parent_id&&!b.targetGroup)return!1;let Y=h.x||1,B=h.y||1,Lo=h.w||1,z=h.h||1;if(b.targetGroup&&h.id===b.targetGroup.id)return!1;return y<Y+Lo&&y+t.w>Y&&m<B+z&&m+t.h>B})&&!b.targetGroup){if(this.ghostEl)this.ghostEl.style.display="none";this.shadowRoot.querySelectorAll(".drop-target").forEach((h)=>h.classList.remove("drop-target"));return}if(b.valid){let h={id:t.id,x:b.x,y:b.y};if(b.targetGroup)h.parent_id=b.targetGroup.id,h.x=b.x-b.targetGroup.x+1,h.y=b.y-b.targetGroup.y+1;else h.parent_id=void 0;await g.updateItem(h)}if(this.ghostEl)this.ghostEl.style.display="none";this.shadowRoot.querySelectorAll(".bookmark-grid__section").forEach((h)=>h.classList.remove("drop-target"))}updateGhost(o,r){if(!this.ghostEl)this.ghostEl=this.shadowRoot.getElementById("ghost-element");if(!this.ghostEl)return;if(this.ghostEl.style.display="block",this.ghostEl.style.setProperty("--ghost-x",String(o.x)),this.ghostEl.style.setProperty("--ghost-y",String(o.y)),this.ghostEl.style.setProperty("--ghost-w",String(o.w)),this.ghostEl.style.setProperty("--ghost-h",String(o.h)),r)this.ghostEl.classList.remove("invalid");else this.ghostEl.classList.add("invalid")}render(){if(this.isEditing)this.classList.add("edit-mode");else this.classList.remove("edit-mode");this.shadowRoot.innerHTML=`
            <style>${Lr}</style>
        ...
        `,this.setupActionListeners(),this.ghostEl=this.shadowRoot.getElementById("ghost-element")}}if(!customElements.get("bookmark-grid"))customElements.define("bookmark-grid",Ar);var Hr=()=>`
    <div class="toast-container"></div>
`;var Mr=`.toast-container {
    position: fixed;
    bottom: 20px;
    right: 20px;
    z-index: 1000;
...
`,Gr=(o,r,t)=>{if(t)return`<div class="icon-picker__loading">${i.t("general.loading")}</div>`;if(o.length===0)return`<div class="icon-picker__empty">${i.t("general.no_icons")}</div>`;return o.map((a)=>`
        <div class="icon-picker__item ${r===a.name?"icon-picker__item--selected":""}"
             data-icon="${a.name}"
             title="${a.name}">
            <img src="${a.url}" alt="${a.name}" loading="lazy" />
        </div>
    `).join("")};class Jr{icons=[];loaded=!1;loading=!1;BASE_URL="https://cdn.jsdelivr.net/gh/homarr-labs/dashboard-icons";FALLBACK_URL="https://raw.githubusercontent.com/homarr-labs/dashboard-icons/main";async loadIcons(){if(this.loaded)return this.icons;if(this.loading)return await new Promise((o)=>setTimeout(o,100)),this.loadIcons();this.loading=!0;try{let o=await fetch(`${this.BASE_URL}/tree.json`);if(!o.ok)o=await fetch(`${this.FALLBACK_URL}/tree.json`);if(!o.ok)throw Error("Failed to fetch icon list");let r=await o.json(),t=Array.isArray(r)?r:r.png||[];if(Array.isArray(t)&&t.length>0)this.icons=t.filter((a)=>a.endsWith(".png")||a.endsWith(".svg")).map((a)=>{let e=a.replace(/\.(png|svg)$/,"");return{name:e,url:`${this.BASE_URL}/png/${e}.png`}}).sort((a,e)=>a.name.localeCompare(e.name));else console.warn("[IconService] Unexpected tree.json structure, using fallback",r),this.icons=this.getFallbackIcons();return this.loaded=!0,this.loading=!1,this.icons}catch(o){return console.error("[IconService] Failed to load icons:",o),this.loading=!1,this.icons=this.getFallbackIcons(),this.loaded=!0,this.icons}}getFallbackIcons(){return["github","gitlab","docker","proxmox","truenas","plex","jellyfin","nextcloud","cloudflare","nginx","traefik","portainer","grafana","prometheus","influxdb","pihole","adguard","homeassistant","esphome","frigate","unraid","synology","opnsense","pfsense","wireguard","openvpn","bitwarden","vaultwarden","sonarr","radarr","lidarr","bazarr","prowlarr","overseerr","tautulli","transmission","qbittorrent","deluge","sabnzbd","nzbget","calibre","paperless","photoprism","immich","mealie","freshrss","miniflux","wallabag","linkding","shiori","firefox","chrome","vscode","code-server","jupyter","portainer"].map((r)=>({name:r,url:`${this.BASE_URL}/png/${r}.png`}))}searchIcons(o,r=50){if(!o.trim())return this.icons.slice(0,r);let t=o.toLowerCase().trim();return this.icons.filter((a)=>a.name.toLowerCase().includes(t)).slice(0,r)}getIconUrl(o){return`${this.BASE_URL}/png/${o}.png`}}var ko=new Jr;var Rr=`:host {
    display: block;
}
...
@keyframes pulse {
...
}`;class Zr extends HTMLElement{icons=[];filteredIcons=[];selectedIcon="";searchQuery="";isLoading=!0;debounceTimer=null;inputElement=null;constructor(){super();this.attachShadow({mode:"open"})}async connectedCallback(){if(this.shadowRoot.childElementCount>0)return;if(this.renderBase(),this.setupListeners(),this.icons.length===0)await this.loadIcons()}async loadIcons(){try{this.icons=await ko.loadIcons(),this.filteredIcons=this.icons.slice(0,50),this.isLoading=!1,this.updateGrid()}catch(o){console.error("[IconPicker] Failed to load icons",o),this.isLoading=!1,this.updateGrid()}}setupListeners(){let o=this.shadowRoot;this.inputElement=o.getElementById("icon-search"),this.inputElement?.addEventListener("input",(t)=>{let a=t.target;if(this.searchQuery=a.value,this.debounceTimer)window.clearTimeout(this.debounceTimer);this.debounceTimer=window.setTimeout(()=>{this.performSearch()},100)}),o.getElementById("grid-container")?.addEventListener("click",(t)=>{let e=t.target.closest(".icon-picker__item");if(e&&e.dataset.icon)this.selectedIcon=e.dataset.icon,this.updateGrid(),this.dispatchEvent(new CustomEvent("icon-selected",{detail:{iconName:this.selectedIcon},bubbles:!0,composed:!0}))})}performSearch(){this.filteredIcons=ko.searchIcons(this.searchQuery,50),this.updateGrid()}getSelectedIcon(){return this.selectedIcon}setSelectedIcon(o){this.selectedIcon=o,this.updateGrid()}renderBase(){this.shadowRoot.innerHTML=`
            <style>${Rr}</style>
            ${Qr()}
        `}updateGrid(){let o=this.shadowRoot.getElementById("grid-container");if(o){if(o.style.display="grid",!this.searchQuery.trim()&&!this.isLoading){o.innerHTML=`<div class="icon-picker__empty">${i.t("general.search_icons")}</div>`;return}o.innerHTML=Gr(this.filteredIcons,this.selectedIcon,this.isLoading)}}disconnectedCallback(){if(this.debounceTimer)window.clearTimeout(this.debounceTimer)}}if(!customElements.get("icon-picker"))customElements.define("icon-picker",Zr);f();var Yr=["#3b82f6","#10b981","#f59e0b","#ef4444","#8b5cf6","#ec4899","#64748b","#f8fafc","#1e293b","#333333"],Mt=(o,r)=>{let t=!Yr.includes(o)&&o!=="";return`
    ...
    `},Br=({isOpen:o,isEditMode:r,title:t,url:a,protocol:e,selectedIcon:n,color:s,statusPosition:p,checkStatus:d,labelPosition:l,visibleTouch:c,borderWidth:x})=>`
    <dialog id="modal">
        <div class="modal-header">
                    ...
                    <div class="form-row" style="display: flex; gap: 12px; align-items: flex-end;">
                        ...
                        <div class="form-group" style="width: 52px;">
                            <button type="button" class="independent-icon-btn" id="icon-trigger-btn" title="${i.t("bookmark.icon")}">
                                ${n?`<img src="https://cdn.jsdelivr.net/gh/homarr-labs/dashboard-icons/png/${n}.png" alt="icon" style="width: 36px; height: 36px; object-fit: contain;">`:'<svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect><circle cx="8.5" cy="8.5" r="1.5"></circle><polyline points="21 15 16 10 5 21"></polyline></svg>'}
                            </button>
                        </div>
                        ...
                        <div class="input-group">
                            ...
                            <div class="icon-dropdown protocol-dropdown">
                                <button type="button" class="icon-dropdown-btn" id="protocol-btn" title="${i.t("bookmark.protocol")}" style="position: relative; justify-content: space-between; padding: 0 8px;">
                                    <span id="protocol-text" style="font-size: 13px; font-weight: 500;">${e}</span>
                                    <svg viewBox="0 0 24 24" width="10" height="10" fill="currentColor" style="opacity: 0.7;">
                                        <path d="M7 10l5 5 5-5z" />
                                    </svg>
                                </button>
                                <input type="hidden" name="protocol" id="bookmark-protocol" value="${e}">
                            </div>
        ...
        <div id="menu-overlay" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; pointer-events: none; z-index: 9999;">
            ...
            <div class="dropdown-menu" id="protocol-menu" style="width: 85px;">
                <div class="dropdown-item ${e==="http://"?"selected":""}" data-protocol="http://" title="${i.t("bookmark.protocol")} HTTP">
                    <span style="font-size: 13px;">http://</span>
                </div>
                <div class="dropdown-item ${e==="https://"?"selected":""}" data-protocol="https://" title="${i.t("bookmark.protocol")} HTTPS">
...
<dialog id="icon-modal">
    <div class="modal-header">
        <h3 class="modal-title">${i.t("general.select_icon")}</h3>
        <button class="modal-close" id="close-btn" aria-label="${i.t("general.close")}">
...
.premium-swatch-picker {
    ...
    z-index: 10;
    /* Ensure it's on top */
}`;var Gt=["#3b82f6","#10b981","#f59e0b","#ef4444","#8b5cf6","#ec4899","#64748b","#f8fafc","#1e293b","#333333"];class Nr extends HTMLElement{dialog=null;isEditMode=!1;currentItemId=null;bookmarkTitle="";url="";protocol="http://";selectedIconName=null;color="#333333";statusPosition="top-right";checkStatus=!1;labelPosition="bottom";visibleTouch=!0;borderWidth=1;_unsubscribeI18n;clickHandler=null;submitHandler=null;escapeHandler=null;inputHandler=null;constructor(){super();this.attachShadow({mode:"open"}),this.setupHandlers()}setupHandlers(){this.clickHandler=async(o)=>{let r=o.target;if(r.closest("#modal-close")){o.preventDefault(),this.close();return}if(r.classList.contains("tab-btn")){o.preventDefault();let n=r.dataset.tab;this.switchTab(n)}if(r.closest("#icon-trigger-btn")){o.preventDefault();let n=this.shadowRoot.getElementById("icon-modal-component");if(n){let s=await n.open();if(s)this.selectedIconName=s,this.updateIconPreview()}}if(r.closest("#label-pos-btn"))o.preventDefault(),o.stopPropagation(),this.toggleDropdown("label-pos-menu",r.closest("#label-pos-btn"));else if(r.closest("#status-pos-btn"))o.preventDefault(),o.stopPropagation(),this.toggleDropdown("status-pos-menu",r.closest("#status-pos-btn"));else if(r.closest("#touch-pos-btn"))o.preventDefault(),o.stopPropagation(),this.toggleDropdown("touch-pos-menu",r.closest("#touch-pos-btn"));else if(r.closest("#protocol-btn"))o.preventDefault(),o.stopPropagation(),this.toggleDropdown("protocol-menu",r.closest("#protocol-btn"));else this.closeAllDropdowns();let t=r.closest(".premium-color-swatch");if(t&&!t.classList.contains("premium-color-swatch--custom")){o.preventDefault(),this.color=t.dataset.color,this.updateTriggers();return}let a=r.closest(".border-width-btn");if(a){o.preventDefault(),this.borderWidth=parseInt(a.dataset.width),this.updateTriggers();return}let e=r.closest(".dropdown-item");if(e){o.preventDefault(),o.stopPropagation();let n=e;if(n.dataset.pos)this.labelPosition=n.dataset.pos,this.updateTriggers();if(n.dataset.statusPos){let s=n.dataset.statusPos;if(s==="off")this.checkStatus=!1;else this.checkStatus=!0,this.statusPosition=s;this.updateTriggers()}if(n.dataset.touch)this.visibleTouch=n.dataset.touch==="on",this.updateTriggers();if(n.dataset.protocol)this.protocol=n.dataset.protocol,this.updateTriggers()}},this.inputHandler=(o)=>{let r=o.target;if(r.id==="bookmark-label")this.bookmarkTitle=r.value;if(r.id==="bookmark-url")this.url=r.value;if(r.id==="bookmark-borderColor")this.color=r.value,this.updateTriggers()},this.submitHandler=async(o)=>{o.preventDefault();let r=this.selectedIconName?`https://cdn.jsdelivr.net/gh/homarr-labs/dashboard-icons/png/${this.selectedIconName}.png`:"",t=JSON.stringify({label:this.bookmarkTitle,url:this.protocol+this.url.replace(/^https?:\/\//,""),description:"",icon:r,iconName:this.selectedIconName,statusCheck:this.checkStatus,statusPos:this.checkStatus?this.statusPosition:"off",visibleTouch:this.visibleTouch,openInNewTab:!0,labelPos:this.labelPosition,borderColor:this.color,borderWidth:this.borderWidth});try{if(this.isEditMode&&this.currentItemId){if(await g.updateItem({id:this.currentItemId,content:t}),window.notifier)window.notifier.show(i.t("notifier.bookmark_updated"))}else{let{collisionService:a}=await Promise.resolve().then(() => (O(),T)),e=g.getState(),n=Array.isArray(e.items)?e.items:[],s=a.findFirstAvailableSlot(1,1,n);if(await g.addItem({type:"bookmark",x:s.x,y:s.y,w:1,h:1,content:t}),window.notifier)window.notifier.show(i.t("notifier.bookmark_added"))}this.close()}catch(a){if(console.error("[Modal] Error:",a),window.notifier)window.notifier.show(i.t("notifier.bookmark_error"),"error")}},this.escapeHandler=(o)=>{if(o.key==="Escape"){let r=this.shadowRoot.querySelector("icon-selection-modal");if(this.dialog?.open)this.close()}}}updateTriggers(){let o=this.shadowRoot,r=o.getElementById("label-pos-btn");if(r){r.innerHTML=`
            ...
            `;let n=o.getElementById("bookmark-labelPos");if(n)n.value=this.labelPosition;o.querySelectorAll("#label-pos-menu .dropdown-item").forEach((s)=>{s.classList.toggle("selected",s.dataset.pos===this.labelPosition)})}let t=o.getElementById("status-pos-btn");if(t){t.innerHTML=`
        ...
        `,this.dialog=this.shadowRoot.querySelector("dialog"),this.shadowRoot.querySelectorAll(".card").forEach((o)=>{o.addEventListener("click",()=>{let r=o.getAttribute("data-id"),t=zo.find((a)=>a.id===r);if(t)this.selectWidget(t)})})}}customElements.define("add-widget-modal",Er);j();f();var Vr=`:host {
    display: contents;
}
...
.border-width-btn span {
    ...
    font-weight: 500;
                ...
                `,u="";return{general:x,customization:""+p()}}else if(r==="weather"){let d=o.city||"",l=o.latitude||"",c=o.longitude||"",x=o.unit==="fahrenheit",u=o.showForecast||!1,v=o.forecastDays||5,y=`
                    <div class="field-group">
                    ...
                    <div class="field-group">
                        ...
                        <div class="input-row">
                            <input type="number" id="weather-lat" value="${l}" placeholder="${i.t("general.latitude")}" step="0.0001" />
                            <input type="number" id="weather-lon" value="${c}" placeholder="${i.t("general.longitude")}" step="0.0001" />
                        </div>
                    </div>
                ...
                `,m="";return{general:y,customization:""+p()}}else if(r==="telemetry"){let d=o.interval||1000,l=`
                    <p style="color: var(--text-dim); font-size: 13px; margin-bottom: 16px;">${i.t("widget.telemetry.description")}</p>
                `,c=`
                ...
                `;return{general:l,customization:c+p()}}else if(r==="notepad"){let d=o.text||"",l=`
                    <div style="text-align: center; padding: 32px 20px; color: var(--text-dim);">
                        <p>${i.t("widget.config.check_personalize")}</p>
                    </div>
                ...
                `,c="";return{general:l,customization:""+p()}}return{general:`<p>${i.t("widget.config.no_config")}</p>`,customization:""}},e=()=>{if(this.currentItem?.type==="section")return i.t("section.edit_title");return i.t("widget.config.title")},n=a();this.shadowRoot.innerHTML=`
            <style>${Vr}</style>
            <dialog id="modal">
                <div class="modal-header">
                    <div class="integrated-tabs">
        ...
        `,this.dialog=this.shadowRoot.querySelector("dialog");let s=this.shadowRoot.getElementById("telemetry-interval");if(s)s.options=[{value:"1000",label:"1s"},{value:"2000",label:"2s"},{value:"5000",label:"5s"},{value:"10000",label:"10s"}]}async searchClockCity(){let o=this.shadowRoot?.getElementById("clock-city"),r=this.shadowRoot?.getElementById("clock-results-container"),t=this.shadowRoot?.getElementById("clock-results");if(!o||!r||!t)return;let a=o.value.trim();if(!a)return;t.innerHTML=`<div class="search-loading">${i.t("general.loading")}</div>`,r.style.display="";try{let n=await(await fetch(`https://geocoding-api.open-meteo.com/v1/search?name=${encodeURIComponent(a)}&count=5&language=${i.getLocale().code}`)).json();if(!n.results||n.results.length===0){t.innerHTML=`<div class="search-empty">${i.t("general.no_icons")}</div>`;return}t.innerHTML=n.results.map((s)=>`
    ...
    80% {
...
}`;class ot extends HTMLElement{dialog=null;titleText=i.t("general.confirm");messageText=i.t("general.please_enter_password");resolvePromise=null;_unsubscribeI18n;constructor(){super();this.attachShadow({mode:"open"})}connectedCallback(){this.render(),this._unsubscribeI18n=i.subscribe(()=>{if(this.dialog?.open)this.render()})}disconnectedCallback(){if(this._unsubscribeI18n)this._unsubscribeI18n()}setupDynamicListeners(){let o=this.shadowRoot.getElementById("modal-close"),r=this.shadowRoot.getElementById("btn-confirm"),t=this.shadowRoot.getElementById("password-confirm"),a=this.shadowRoot.querySelector(".password-input-group");this.dialog=this.shadowRoot.getElementById("modal");let e=(s)=>{if(this.resolvePromise)this.resolvePromise(s),this.resolvePromise=null;if(this.dialog?.close(),t)t.value=""},n=()=>{let s=t?.value||"";if(s.trim()===""){if(window.notifier)window.notifier.show(i.t("notifier.password_required"),"error");if(a)a.classList.remove("shake"),a.offsetWidth,a.classList.add("shake");t?.focus();return}e(s)};if(o)o.onclick=()=>e(null);if(r)r.onclick=()=>n();if(t)t.onkeydown=(s)=>{if(s.key==="Enter")s.preventDefault(),n()},t.oninput=()=>{if(a)a.classList.remove("shake")};if(this.dialog)this.dialog.onclick=(s)=>{if(s.target===this.dialog)e(null)}}async prompt(o,r){this.titleText=o,this.messageText=r,this.render(),this.dialog?.showModal();let t=this.shadowRoot.getElementById("password-confirm");if(t)t.value="",setTimeout(()=>t.focus(),50);return new Promise((a)=>{this.resolvePromise=a})}render(){this.shadowRoot.innerHTML=`
            <style>${Sr}</style>
        ...
        `,this._editor=this.shadowRoot.querySelector(".editor"),this._preview=this.shadowRoot.querySelector(".preview"),this._headerActions=this.shadowRoot.querySelector(".header-actions"),this._lockBtn=this.shadowRoot.querySelector(".lock-btn"),this._togglePreviewBtn=this.shadowRoot.querySelector(".toggle-preview"),this.bindEvents()}bindEvents(){this._editor?.addEventListener("input",()=>this.handleInput()),this._editor?.addEventListener("keydown",(o)=>this.handleKeydown(o)),this._headerActions?.addEventListener("click",(o)=>{let r=o.target.closest(".action-btn");if(!r)return;if(r.classList.contains("toggle-preview"))this.togglePreview();else if(r.classList.contains("lock-btn"))this.toggleLock()})}handleInput(){if(this._isLocked)return;if(this._isDirty=!0,this._isPreviewMode)this.updatePreview()}handleKeydown(o){if(this._isLocked)return;if(o.key==="Tab")o.preventDefault(),this.insertText("  ");if((o.ctrlKey||o.metaKey)&&o.key==="s")o.preventDefault(),this.save()}insertText(o){if(!this._editor)return;let r=this._editor.selectionStart,t=this._editor.selectionEnd,a=this._editor.value;this._editor.value=a.substring(0,r)+o+a.substring(t),this._editor.selectionStart=this._editor.selectionEnd=r+o.length,this.handleInput()}togglePreview(){if(this._isPreviewMode=!this._isPreviewMode,this._togglePreviewBtn)this._togglePreviewBtn.classList.toggle("active",this._isPreviewMode);this.applyViewState()}updatePreview(){if(this._editor&&this._preview)try{this._preview.innerHTML=jo.parse(this._editor.value)}catch(o){console.error("Markdown parsing failed",o),this._preview.innerHTML=`<div class="error">${i.t("widget.markdown.error.preview")}</div>`}}applyViewState(){if(this._isPreviewMode){if(this.updatePreview(),this._preview)this._preview.classList.add("visible"),this._preview.style.display="block";if(this._editor)this._editor.style.display="none",this._editor.style.visibility="hidden"}else{if(this._preview)this._preview.classList.remove("visible"),this._preview.style.display="none";if(this._editor)this._editor.style.display="block",this._editor.style.visibility="visible"}}toggleLock(){if(this._isLocked=!this._isLocked,this._editor)this._editor.disabled=this._isLocked;if(this._lockBtn)if(this._lockBtn.innerHTML=this._isLocked?S.lock:S.unlock,this._lockBtn.title=this._isLocked?i.t("widget.markdown.tool.unlock"):i.t("widget.markdown.tool.lock"),this._isLocked)this._lockBtn.classList.add("locked"),this._lockBtn.classList.remove("unlocked");else this._lockBtn.classList.remove("locked"),this._lockBtn.classList.add("unlocked");if(this._isLocked){if(this._isPreviewMode=!0,this._togglePreviewBtn)this._togglePreviewBtn.style.display="none"}else{if(this._isPreviewMode=!1,this._togglePreviewBtn)this._togglePreviewBtn.style.display="flex",this._togglePreviewBtn.classList.remove("active");this._editor?.focus()}if(this.applyViewState(),this._isLocked&&this._isDirty)this.save()}async loadContent(){if(!this._itemId)return;let r=g.getState().items.find((t)=>t.id===this._itemId);if(r){let t=R.getMarkdownText(r.content);if(this._content=t,this._editor)this._editor.value=t;if(this._isLocked)this.updatePreview()}}async save(){if(!this._itemId||!this._editor)return;this._isSaving=!0;let o=this._editor.value;this._content=o;try{let t=g.getState().items.find((a)=>a.id===this._itemId);if(t){let a=R.setMarkdownText(t.content,o);await g.updateItem({...t,content:a}),this._isDirty=!1}}catch(r){console.error(r)}finally{this._isSaving=!1}}}customElements.define("widget-markdown",rt);f();async function tt(o){try{await i.ensureInitialized(),await o()}catch(r){console.error("[Bootstrap] Critical failure:",r),document.body.innerHTML=`
            <div style="
                ...
                text-align: center;
            ">
                <h1 style="color: #fa5252;">${i.t("error.system_error")}</h1>
                <p>${i.t("error.init_failed")}</p>
                <p style="opacity: 0.7; font-size: 0.9em;">${i.t("error.check_console")}</p>
                <button onclick="window.location.reload()" style="
                    ...
                    cursor: pointer;
                    border-radius: 4px;
                ">${i.t("general.reload")}</button>
            </div>
        `}}to();var L=document.getElementById("main-topbar"),at=document.getElementById("right-drawer"),ao=document.getElementById("dashboard-root"),P,io,no,eo;tt(async()=>{F.init();let[o,r]=await Promise.all([k.fetchUser(),g.fetchItems()]),t=k.getUser();if(t){if(t.language)await i.setLanguage(t.language);if(t.preferences&&t.preferences.grid_columns)document.documentElement.style.setProperty("--user-preferred-columns",t.preferences.grid_columns.toString());if(t.theme==="dark")F.enableDark();else if(t.theme==="light")F.enableLight();else if(window.matchMedia&&window.matchMedia("(prefers-color-scheme: dark)").matches)F.enableDark();else F.enableLight();if(t.id)g.setUserId(t.id)}if(L){if(L.setState({user:t}),t&&t.project_name)L.setAttribute("title",t.project_name);k.subscribe((e)=>{if(e){if(L.setState({user:e}),e.project_name)L.setAttribute("title",e.project_name)}})}Bt(),I.start(),P=document.createElement("add-bookmark-modal"),document.body.appendChild(P),io=document.createElement("add-widget-modal"),document.body.appendChild(io),no=document.createElement("confirmation-modal"),document.body.appendChild(no),eo=document.createElement("password-confirm-modal"),document.body.appendChild(eo);let a=document.createElement("widget-config-modal");document.body.appendChild(a)});E.on(N.SHOW_CONFIRMATION,async(o)=>{let{title:r,message:t,onConfirm:a}=o.detail;if(no){if(await no.confirm(r,t)&&a)a()}});E.on(N.SHOW_PASSWORD_CONFIRM,async(o)=>{let{title:r,message:t,onConfirm:a}=o.detail;if(eo){let e=await eo.prompt(r,t);if(e!==null&&a)a(e)}});E.on(N.SHOW_WIDGET_CONFIG,(o)=>{let{item:r,type:t}=o.detail;if(t==="bookmark"){if(P)P.openForEdit(r)}else{let a=document.querySelector("widget-config-modal");if(a)a.open(r)}});E.on(N.NOTIFY,(o)=>{let r=document.querySelector("app-notifier");if(r)r.show(o.detail.message,o.detail.type)});if(L)L.addEventListener("drawer-toggle",(o)=>{if(o.detail.action==="open")at.open(),L.setState({drawerOpen:!0});else at.close(),L.setState({drawerOpen:!1})}),L.addEventListener("edit-mode-change",(o)=>{let r=o.detail.active;if(ao.classList.toggle("edit-mode",r),!r)window.location.reload()}),L.addEventListener("search-input",(o)=>{let r=o.detail.query;g.setSearchQuery(r)}),L.addEventListener("add-item",async(o)=>{let r=o.detail.action;if(r==="add-bookmark"){if(P)P.open()}else if(r==="add-widget"){if(io)io.open()}else if(r==="add-section"){let a=g.getState().items||[],{collisionService:e}=await Promise.resolve().then(() => (O(),T)),n=e.findFirstAvailableSlot(1,1,a),s={type:"section",x:n.x,y:n.y,w:1,h:1,content:JSON.stringify({title:""})},p=await g.addItem(s);if(p)E.emit(N.SHOW_WIDGET_CONFIG,{item:p,type:"section"})}});window.addEventListener("widget-selected",async(o)=>{let t=o.detail,e=g.getState().items||[],{collisionService:n}=await Promise.resolve().then(() => (O(),T)),s=n.findFirstAvailableSlot(t.defaultW,t.defaultH,e),p={type:"widget",x:s.x,y:s.y,w:t.defaultW,h:t.defaultH,content:JSON.stringify({widgetId:t.id,text:t.id==="notepad"||t.id==="markdown"?"":void 0})};await g.addItem(p)});window.addEventListener("drawer-close",()=>{if(L)L.setState({drawerOpen:!1})});window.addEventListener("click",(o)=>{let r=o.target,t=document.getElementById("add-menu"),a=document.getElementById("add-toggle");if(t&&t.classList.contains("active")&&!t.contains(r)&&a&&!a.contains(r))t.classList.remove("active")});function Bt(){if(!ao)return;ao.innerHTML="";let o=document.createElement("bookmark-grid");ao.appendChild(o)}
