...
}`;class Lr extends HTMLElement{dropdownOpen;constructor(){super();this.attachShadow({mode:"open"}),this.dropdownOpen=!1}connectedCallback(){this.render(),this.setupListeners()}toggleDropdown(o){this.dropdownOpen=typeof o==="boolean"?o:!this.dropdownOpen,this.render()}setupListeners(){this.shadowRoot.addEventListener("click",(o)=>{let r=o.target,a=r.closest(".header__btn-changelog");if(a)a.style.display="none";if(r.closest("#profile-trigger"))o.stopPropagation(),this.toggleDropdown()}),window.addEventListener("click",()=>{if(this.dropdownOpen)this.toggleDropdown(!1)})}render(){let o=this.getAttribute("title")||"Task";this.shadowRoot.innerHTML=`
            <style>${Jr}</style>
            ${Rr({title:o,dropdownOpen:this.dropdownOpen})}
        `}}if(!customElements.get("app-header"))customElements.define("app-header",Lr);_();var ao=(o)=>String(o).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;"),Oa=(o)=>{if(!o)return"#";let r=o.trim().toLowerCase();if(r.startsWith("http://")||r.startsWith("https://")||r==="#")return ao(o);return"#"},Mr=({bookmarks:o,isEditing:r,isSearching:a,isTouchDevice:t,maxCols:i=12})=>{let e=Array.isArray(o)?o:[],s=(p)=>e.filter((c)=>c.parent_id===p);return`
    ${(t?e.filter((p)=>p.type!=="section"):a?e:e.filter((p)=>!p.parent_id)).map((p)=>{let c={};try{c=typeof p.content==="string"?JSON.parse(p.content):p.content}catch(m){console.error("Failed to parse content for item",p.id,m)}let l=p.type==="section",g={x:p.x||1,y:p.y||1},u=Math.min(p.w||1,i),h=p.h||1;if(l){let m=s(p.id),G=(c.title||"").trim();return`
            <fieldset class="bookmark-grid__section"
               ...
               `:""}
            </fieldset>`}return Qr(p,c,r,g,u,h)}).join("")}
...
.visibility-row .checkbox-group {
    ...
    margin-bottom: 0;
        ...
        `,this.dialog=this.shadowRoot.querySelector("dialog"),this.shadowRoot.querySelectorAll(".card").forEach((o)=>{o.addEventListener("click",()=>{let r=o.getAttribute("data-id"),a=Po.find((t)=>t.id===r);if(a)this.selectWidget(a)})})}}customElements.define("add-widget-modal",$a);K();_();var za=`:host {
    display: contents;
}
...
.modal-actions {
    ...
    justify-content: flex-end;
        ...
        `}}Bo();var B=document.getElementById("main-topbar"),Ka=document.getElementById("right-drawer"),Ko=document.getElementById("dashboard-root"),eo,Ro,Jo;Ba(async()=>{L.init(),await $.fetchUser();let o=$.getUser();if(o){if(o.language)await n.setLanguage(o.language);if(o.theme==="dark")L.enableDark();else if(o.theme==="light")L.enableLight();else if(window.matchMedia&&window.matchMedia("(prefers-color-scheme: dark)").matches)L.enableDark();else L.enableLight();if(o.id)x.setUserId(o.id)}if(B){if(B.setState({user:o}),o&&o.project_name)B.setAttribute("title",o.project_name);$.subscribe((a)=>{if(a){if(B.setState({user:a}),a.project_name)B.setAttribute("title",a.project_name)}})}await x.fetchItems(),ht(),co.start(),eo=document.createElement("add-bookmark-modal"),document.body.appendChild(eo),Ro=document.createElement("add-widget-modal"),document.body.appendChild(Ro),Jo=document.createElement("confirmation-modal"),document.body.appendChild(Jo);let r=document.createElement("widget-config-modal");document.body.appendChild(r)});no.on(io.SHOW_CONFIRMATION,async(o)=>{let{title:r,message:a,onConfirm:t}=o.detail;if(Jo){if(await Jo.confirm(r,a)&&t)t()}});no.on(io.SHOW_WIDGET_CONFIG,(o)=>{let{item:r,type:a}=o.detail;if(a==="bookmark"){if(eo)eo.openForEdit(r)}else{let t=document.querySelector("widget-config-modal");if(t)t.open(r)}});no.on(io.NOTIFY,(o)=>{let r=document.querySelector("app-notifier");if(r)r.show(o.detail.message,o.detail.type)});if(B)B.addEventListener("drawer-toggle",(o)=>{if(o.detail.action==="open")Ka.open(),B.setState({drawerOpen:!0});else Ka.close(),B.setState({drawerOpen:!1})}),B.addEventListener("edit-mode-change",(o)=>{let r=o.detail.active;if(Ko.classList.toggle("edit-mode",r),!r)window.location.reload()}),B.addEventListener("search-input",(o)=>{let r=o.detail.query;x.setSearchQuery(r)}),B.addEventListener("add-item",async(o)=>{let r=o.detail.action;if(r==="add-bookmark"){if(eo)eo.open()}else if(r==="add-widget"){if(Ro)Ro.open()}else if(r==="add-section"){let t=x.getState().items||[],{collisionService:i}=await Promise.resolve().then(() => (C(),ro)),e=i.findFirstAvailableSlot(1,1,t),s={type:"section",x:e.x,y:e.y,w:1,h:1,content:JSON.stringify({title:""})},d=await x.addItem(s);if(d)no.emit(io.SHOW_WIDGET_CONFIG,{item:d,type:"section"})}});window.addEventListener("widget-selected",async(o)=>{let a=o.detail,i=x.getState().items||[],{collisionService:e}=await Promise.resolve().then(() => (C(),ro)),s=e.findFirstAvailableSlot(a.defaultW,a.defaultH,i),d={type:"widget",x:s.x,y:s.y,w:a.defaultW,h:a.defaultH,content:JSON.stringify({widgetId:a.id,text:a.id==="notepad"?"":void 0})};await x.addItem(d)});window.addEventListener("drawer-close",()=>{if(B)B.setState({drawerOpen:!1})});window.addEventListener("click",(o)=>{let r=o.target,a=document.getElementById("add-menu"),t=document.getElementById("add-toggle");if(a&&a.classList.contains("active")&&!a.contains(r)&&t&&!t.contains(r))a.classList.remove("active")});function ht(){if(!Ko)return;Ko.innerHTML="";let o=document.createElement("bookmark-grid");Ko.appendChild(o)}
