...
.avatar__initials {
    user-select: none;
        ...
        `}}if(!customElements.get("app-avatar"))customElements.define("app-avatar",Zo);m();B();no();m();var Ko=(o)=>`
...
`,Sr=(o,r)=>{if(!o)return"";if(!r)return"";if(r.is_docker)return`
...
@keyframes blink {
    ...
    100% {
        ...
        `}}if(!customElements.get("app-topbar"))customElements.define("app-topbar",or);m();var rr=()=>`
    <div class="dot"></div>
...
}`;j();class ir extends HTMLElement{unsubscribe=null;constructor(){super();this.attachShadow({mode:"open"})}connectedCallback(){this.render(),this.unsubscribe=g.subscribe((o)=>{if(o.isOffline)this.classList.add("visible");else this.classList.remove("visible")})}disconnectedCallback(){if(this.unsubscribe)this.unsubscribe()}render(){this.shadowRoot.innerHTML=`
            <style>${tr}</style>
            ${rr()}
        `}}if(!customElements.get("offline-badge"))customElements.define("offline-badge",ir);m();var ar=({title:o,dropdownOpen:r})=>`
    <header class="header">
...
(max-width: 1024px) {
    ...
    .header__profile-container {
        ...
        `}}if(!customElements.get("app-header"))customElements.define("app-header",er);m();var X=(o)=>String(o).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;"),st=(o)=>{if(!o)return"#";let r=o.trim().toLowerCase();if(r.startsWith("http://")||r.startsWith("https://")||r==="#")return X(o);return"#"},sr=({bookmarks:o,isEditing:r,isLoading:t,isSearching:i,isTouchDevice:n,maxCols:e=12})=>{let s=Array.isArray(o)?o:[];if(t&&s.length===0)return dt(e);let p=(c)=>s.filter((l)=>l.parent_id===c);return`
    ${(n?s:i?s:s.filter((c)=>!c.parent_id)).map((c)=>{let l={};try{l=typeof c.content==="string"?JSON.parse(c.content):c.content}catch(v){console.error("Failed to parse content for item",c.id,v)}let x=c.type==="section",u={x:c.x||1,y:c.y||1},f=Math.min(c.w||1,e),y=c.h||1;if(x){let v=p(c.id),w=(l.title||"").trim(),b=l.borderColor&&!r?`border-color: ${l.borderColor} !important;`:"",M=!r?`border-width: ${l.borderWidth||1}px !important;`:"";return`
            <fieldset class="bookmark-grid__section"
               data-id="${c.id}"
               data-orig-x="${c.x}" data-orig-y="${c.y}"
               draggable="${r}"
               style="--x: ${u.x}; --y: ${u.y}; --w: ${f}; --h: ${y}; ${b} ${M}">
    ...
    `}function pt(o,r,t=!1,i=12,n=!1){return o.map((e)=>{let s={};try{s=typeof e.content==="string"?JSON.parse(e.content):e.content}catch(l){console.error("Failed to parse content for item (nested)",e.id,l)}let p={x:e.x||1,y:e.y||1},d=e.h||1,c=Math.min(e.w||1,i);return pr(e,s,r,p,c,d,n)}).join("")}function dt(o){return`
        <style>
            .skeleton {
                background: var(--surface);
                pointer-events: none;
...
`;class lr extends HTMLElement{_itemId=0;_content="";_isDirty=!1;_isLocked=!0;_isSaving=!1;_lastLockTime=0;_editor=null;_syncStatus=null;_dock=null;_editorContainer=null;_modal=null;_imgResizer=null;_activeImage=null;_pendingAction=null;static get observedAttributes(){return["item-id"]}constructor(){super();this.attachShadow({mode:"open"})}connectedCallback(){this.render(),this.loadContent(),window.addEventListener("beforeunload",this.handleBeforeUnload.bind(this)),a.subscribe(()=>{if(this._editor)this._editor.dataset.placeholder=a.t("widget.notepad.placeholder")})}disconnectedCallback(){window.removeEventListener("beforeunload",this.handleBeforeUnload.bind(this))}attributeChangedCallback(o,r,t){if(o==="item-id")this._itemId=parseInt(t),this.loadContent()}handleBeforeUnload(o){if(this._isDirty)o.preventDefault(),o.returnValue=""}render(){if(!this.shadowRoot)return;let o=this._isLocked?"locked":"unlocked",r=this._isLocked?"locked":"",t=this._isLocked?"hidden":"";this.shadowRoot.innerHTML=`
        ...
        `,this.cpuBar=this.shadowRoot.querySelector(".cpu-bar"),this.ramBar=this.shadowRoot.querySelector(".ram-bar"),this.tempBar=this.shadowRoot.querySelector(".temp-bar"),this.cpuText=this.shadowRoot.querySelector(".cpu-text"),this.ramText=this.shadowRoot.querySelector(".ram-text"),this.tempText=this.shadowRoot.querySelector(".temp-text")}}customElements.define("widget-telemetry",cr);m();j();var gr=new Map,br=900000,xr={widgetId:"weather",latitude:0,longitude:0,city:"",unit:"celsius",showForecast:!1,forecastDays:5};function N(o){if(o===0)return`<svg viewBox="0 0 64 64" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round">
    ...
    </svg>`}function bo(o){let r={0:"clear_sky",1:"mainly_clear",2:"partly_cloudy",3:"overcast",45:"fog",48:"fog",51:"light_drizzle",53:"drizzle",55:"heavy_drizzle",56:"freezing_drizzle",57:"freezing_drizzle",61:"light_rain",63:"rain",65:"heavy_rain",66:"freezing_rain",67:"freezing_rain",71:"light_snow",73:"snow",75:"heavy_snow",77:"snow_grains",80:"light_showers",81:"showers",82:"heavy_showers",85:"snow_showers",86:"heavy_snow_showers",95:"thunderstorm",96:"thunderstorm_hail",99:"thunderstorm_hail"};return a.t(`widget.weather.${r[o]||"unknown"}`)}function J(o,r){if(r==="fahrenheit")return`${Math.round(o*9/5+32)}°F`;return`${Math.round(o)}°C`}async function ct(o,r){let t=`${o.toFixed(2)},${r.toFixed(2)}`,i=gr.get(t);if(i&&Date.now()-i.timestamp<br)return i.data;let n=`https://api.open-meteo.com/v1/forecast?latitude=${o}&longitude=${r}&current_weather=true&daily=weathercode,temperature_2m_max,temperature_2m_min,sunrise,sunset,wind_speed_10m_max&timezone=auto&forecast_days=7`,e=await fetch(n);if(!e.ok)throw Error("Weather API error");let s=await e.json();return gr.set(t,{data:s,timestamp:Date.now()}),s}class hr extends HTMLElement{_config={...xr};_itemId=0;_unsubscribe=null;_unsubscribeI18n;_weatherData=null;_refreshTimer=null;_loading=!1;_error=!1;constructor(){super();this.attachShadow({mode:"open"})}static get observedAttributes(){return["item-id"]}attributeChangedCallback(o,r,t){if(o==="item-id")this._itemId=parseInt(t)}connectedCallback(){this.render(),this.loadWeather(),this._refreshTimer=setInterval(()=>this.loadWeather(),br),this._unsubscribe=g.subscribe((o)=>{if(!this._itemId)return;let r=o.items.find((t)=>t.id===this._itemId);if(!r?.content)return;try{let t=typeof r.content==="string"?JSON.parse(r.content):r.content;if(t.widgetId!=="weather")return;let i=this._config.latitude,n=this._config.longitude;if(this._config={...xr,...t},i!==this._config.latitude||n!==this._config.longitude)this.loadWeather();else this.updateDisplay()}catch(t){}}),this._unsubscribeI18n=a.subscribe(()=>this.updateDisplay())}disconnectedCallback(){if(this._unsubscribe)this._unsubscribe();if(this._unsubscribeI18n)this._unsubscribeI18n();if(this._refreshTimer)clearInterval(this._refreshTimer)}async loadWeather(){if(!this._config.latitude&&!this._config.longitude){this._loading=!1,this._error=!1,this.updateDisplay();return}this._loading=!0,this._error=!1,this.updateDisplay();try{this._weatherData=await ct(this._config.latitude,this._config.longitude),this._loading=!1}catch(o){this._loading=!1,this._error=!0}this.updateDisplay()}updateDisplay(){let o=this.shadowRoot?.querySelector(".weather-body");if(!o){this.render();return}if(!this._config.latitude&&!this._config.longitude){o.innerHTML=this.emptyState(N(0),a.t("widget.weather.no_location"));return}if(this._loading){o.innerHTML=this.emptyState(N(3),a.t("general.loading"),!0);return}if(this._error||!this._weatherData){o.innerHTML=this.emptyState(N(-1),a.t("general.error"));return}let r=this._weatherData.current_weather,t=this._weatherData.daily,i=this._config.unit||"celsius";if(this._config.showForecast&&t)o.innerHTML=this.forecastView(r,t,i);else o.innerHTML=this.simpleView(r,t,i)}emptyState(o,r,t=!1){return`<div class="state"><div class="state-icon${t?" pulse":""}">${o}</div><span class="state-text">${r}</span></div>`}simpleView(o,r,t){return`
        ...
        `}render(){if(!this.shadowRoot)return;this.shadowRoot.innerHTML=`<style>${gt}</style><div class="weather-body"></div>`,this.updateDisplay()}}var gt=`
:host {
    display: flex;
    flex-direction: column;
    align-items: center;
...
`;customElements.define("widget-weather",hr);var fr=`:host {
    /* Fixed Grid System */
...
@keyframes pulse {
    ...
    `},Mr=({isOpen:o,isEditMode:r,title:t,url:i,protocol:n,selectedIcon:e,color:s,statusPosition:p,checkStatus:d,labelPosition:c,visibleTouch:l,borderWidth:x})=>`
    <dialog id="modal">
        <div class="modal-header">
...
.premium-swatch-picker {
    ...
    /* Ensure it's on top */
        ...
        `,this.dialog=this.shadowRoot.querySelector("dialog"),this.shadowRoot.querySelectorAll(".card").forEach((o)=>{o.addEventListener("click",()=>{let r=o.getAttribute("data-id"),t=vo.find((i)=>i.id===r);if(t)this.selectWidget(t)})})}}customElements.define("add-widget-modal",Fr);j();m();var Qr=`:host {
    display: contents;
}
...
.border-width-btn span {
    ...
    font-weight: 500;
                ...
                `,u="";return{general:x,customization:""+p()}}else if(r==="weather"){let d=o.city||"",c=o.latitude||"",l=o.longitude||"",x=o.unit==="fahrenheit",u=o.showForecast||!1,f=o.forecastDays||5,y=`
                    <div class="field-group">
                ...
                `;return{general:y,customization:v+p()}}else if(r==="telemetry"){let d=o.interval||1000,c=`
...
.modal-actions {
    ...
    justify-content: flex-end;
        ...
        `}}oo();var L=document.getElementById("main-topbar"),Tr=document.getElementById("right-drawer"),ro=document.getElementById("dashboard-root"),C,to,io;Ur(async()=>{G.init();let[o,r]=await Promise.all([k.fetchUser(),g.fetchItems()]),t=k.getUser();if(t){if(t.language)await a.setLanguage(t.language);if(t.preferences&&t.preferences.grid_columns)document.documentElement.style.setProperty("--user-preferred-columns",t.preferences.grid_columns.toString());if(t.theme==="dark")G.enableDark();else if(t.theme==="light")G.enableLight();else if(window.matchMedia&&window.matchMedia("(prefers-color-scheme: dark)").matches)G.enableDark();else G.enableLight();if(t.id)g.setUserId(t.id)}if(L){if(L.setState({user:t}),t&&t.project_name)L.setAttribute("title",t.project_name);k.subscribe((n)=>{if(n){if(L.setState({user:n}),n.project_name)L.setAttribute("title",n.project_name)}})}kt(),W.start(),C=document.createElement("add-bookmark-modal"),document.body.appendChild(C),to=document.createElement("add-widget-modal"),document.body.appendChild(to),io=document.createElement("confirmation-modal"),document.body.appendChild(io);let i=document.createElement("widget-config-modal");document.body.appendChild(i)});D.on(V.SHOW_CONFIRMATION,async(o)=>{let{title:r,message:t,onConfirm:i}=o.detail;if(io){if(await io.confirm(r,t)&&i)i()}});D.on(V.SHOW_WIDGET_CONFIG,(o)=>{let{item:r,type:t}=o.detail;if(t==="bookmark"){if(C)C.openForEdit(r)}else{let i=document.querySelector("widget-config-modal");if(i)i.open(r)}});D.on(V.NOTIFY,(o)=>{let r=document.querySelector("app-notifier");if(r)r.show(o.detail.message,o.detail.type)});if(L)L.addEventListener("drawer-toggle",(o)=>{if(o.detail.action==="open")Tr.open(),L.setState({drawerOpen:!0});else Tr.close(),L.setState({drawerOpen:!1})}),L.addEventListener("edit-mode-change",(o)=>{let r=o.detail.active;if(ro.classList.toggle("edit-mode",r),!r)window.location.reload()}),L.addEventListener("search-input",(o)=>{let r=o.detail.query;g.setSearchQuery(r)}),L.addEventListener("add-item",async(o)=>{let r=o.detail.action;if(r==="add-bookmark"){if(C)C.open()}else if(r==="add-widget"){if(to)to.open()}else if(r==="add-section"){let i=g.getState().items||[],{collisionService:n}=await Promise.resolve().then(() => (Y(),O)),e=n.findFirstAvailableSlot(1,1,i),s={type:"section",x:e.x,y:e.y,w:1,h:1,content:JSON.stringify({title:""})},p=await g.addItem(s);if(p)D.emit(V.SHOW_WIDGET_CONFIG,{item:p,type:"section"})}});window.addEventListener("widget-selected",async(o)=>{let t=o.detail,n=g.getState().items||[],{collisionService:e}=await Promise.resolve().then(() => (Y(),O)),s=e.findFirstAvailableSlot(t.defaultW,t.defaultH,n),p={type:"widget",x:s.x,y:s.y,w:t.defaultW,h:t.defaultH,content:JSON.stringify({widgetId:t.id,text:t.id==="notepad"||t.id==="markdown"?"":void 0})};await g.addItem(p)});window.addEventListener("drawer-close",()=>{if(L)L.setState({drawerOpen:!1})});window.addEventListener("click",(o)=>{let r=o.target,t=document.getElementById("add-menu"),i=document.getElementById("add-toggle");if(t&&t.classList.contains("active")&&!t.contains(r)&&i&&!i.contains(r))t.classList.remove("active")});function kt(){if(!ro)return;ro.innerHTML="";let o=document.createElement("bookmark-grid");ro.appendChild(o)}
