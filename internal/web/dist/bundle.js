...
.paper {
    ...
    height: 100%;
}`;class qo extends HTMLElement{constructor(){super();this.attachShadow({mode:"open"})}connectedCallback(){this.render()}render(){this.shadowRoot.innerHTML=`
            <style>${Ho}</style>
            ${Bo()}
        `}}if(!customElements.get("app-paper"))customElements.define("app-paper",qo);var Co=`:host {
...
(max-width: 1024px) {
    ...
    .header__profile-container {
        ...
        `}}if(!customElements.get("app-header"))customElements.define("app-header",tt);f();var U=(o)=>String(o).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;"),or=(o)=>{if(!o)return"#";let t=o.trim().toLowerCase();if(t.startsWith("http://")||t.startsWith("https://")||t==="#")return U(o);return"#"},rt=({bookmarks:o,isEditing:t,isLoading:r,isSearching:i,isTouchDevice:n,maxCols:e=12})=>{let s=Array.isArray(o)?o:[];if(r&&s.length===0)return rr(e);let d=(c)=>s.filter((l)=>l.parent_id===c);return`
    ...
    `}let p=t.icon||"",l=p.startsWith("http")||p.startsWith("/")?`<img src="${U(p)}" alt="${U(t.label)}" class="bookmark-grid__icon-img" draggable="false" />`:p?U(p):'<svg class="bookmark-grid__icon-svg" viewBox="0 0 24 24"><path d="M10 6L8.59 7.41 13.17 12l-4.58 4.59L10 18l6-6z"/></svg>',h=r?'role="button"':`href="${or(t.url)}" target="_blank"`,w=!s?`--x: ${i.x}; --y: ${i.y}; --w: ${n}; --h: ${d};`:"",u=t.borderColor?`border-color: ${t.borderColor} !important;`:"",_=t.labelPos==="top"?"label-top":t.labelPos==="section"?"label-section":t.labelPos==="off"?"label-off":"";return`
    ...
    `}function tr(o,t,r=!1,i=12,n=!1){return o.map((e)=>{let s={};try{s=typeof e.content==="string"?JSON.parse(e.content):e.content}catch(l){console.error("Failed to parse content for item (nested)",e.id,l)}let d={x:e.x||1,y:e.y||1},p=e.h||1,c=Math.min(e.w||1,i);return it(e,s,t,d,c,p,n)}).join("")}function rr(o){return`
        <style>
            .skeleton {
                background: var(--surface);
                pointer-events: none;
...
`,kt=({isOpen:o,isEditMode:t,title:r,url:i,protocol:n,selectedIcon:e,color:s,statusPosition:d,checkStatus:p,labelPosition:c,visibleTouch:l})=>`
    <dialog id="modal">
        <div class="modal-header">
...
.btn:active {
    transform: translateY(1px);
            ...
            <style>${Mt}</style>
            ${kt({isOpen:!0,isEditMode:this.isEditMode,title:this.bookmarkTitle,url:this.url,protocol:this.protocol,selectedIcon:this.selectedIconName,color:this.color,statusPosition:this.statusPosition,checkStatus:this.checkStatus,labelPosition:this.labelPosition,visibleTouch:this.visibleTouch})}
        ...
        `,this.dialog=this.shadowRoot.querySelector("dialog"),this.shadowRoot.querySelectorAll(".card").forEach((o)=>{o.addEventListener("click",()=>{let t=o.getAttribute("data-id"),r=vo.find((i)=>i.id===t);if(r)this.selectWidget(r)})})}}customElements.define("add-widget-modal",At);M();f();var Bt=`:host {
    display: contents;
}
