...
(max-width: 1024px) {
    ...
    .header__profile-container {
        ...
        `}}if(!customElements.get("app-header"))customElements.define("app-header",vr);f();var D=(o)=>String(o).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;"),wt=(o)=>{if(!o)return"#";let r=o.trim().toLowerCase();if(r.startsWith("http://")||r.startsWith("https://")||r==="#")return D(o);return"#"},ur=({bookmarks:o,isEditing:r,isLoading:t,isSearching:a,isTouchDevice:n,maxCols:e=12})=>{let s=Array.isArray(o)?o:[];if(t&&s.length===0)return zt(e);let p=(c)=>s.filter((l)=>l.parent_id===c);return`
    ${(n?s:a?s:s.filter((c)=>!c.parent_id)).map((c)=>{let l={};try{l=typeof c.content==="string"?JSON.parse(c.content):c.content}catch(_){console.error("Failed to parse content for item",c.id,_)}let x=c.type==="section",u={x:c.x||1,y:c.y||1},v=Math.min(c.w||1,e),y=c.h||1;if(x){let _=p(c.id),w=(l.title||"").trim(),b=l.borderColor&&!r?`border-color: ${l.borderColor} !important;`:"",H=!r?`border-width: ${l.borderWidth||1}px !important;`:"";return`
    ...
    `};function _r(o,r,t,a,n,e=0,s=!1){let p=e||o.h||1;if(o.type==="widget"){let _=r.widgetId,b={clock:"widget-clock",notepad:"widget-notepad",telemetry:"widget-telemetry",weather:"widget-weather",markdown:"widget-markdown"}[_]||"div",A=(r.text||"").replace(/"/g,"&quot;"),h=r.borderColor&&!t?`border-color: ${r.borderColor} !important;`:"",B=!t?`border-width: ${r.borderWidth||1}px !important;`:"";return`
        <div class="bookmark-grid__card"
            draggable="${t}"
            data-id="${o.id}"
            data-type="${o.type}"
            data-orig-x="${o.x}" data-orig-y="${o.y}"
            ${!s?`style="--x: ${a.x}; --y: ${a.y}; --w: ${n}; --h: ${p}; ${h} ${B}"`:""}>
    ...
    `}function kt(o,r,t=!1,a=12,n=!1){return o.map((e)=>{let s={};try{s=typeof e.content==="string"?JSON.parse(e.content):e.content}catch(l){console.error("Failed to parse content for item (nested)",e.id,l)}let p={x:e.x||1,y:e.y||1},d=e.h||1,c=Math.min(e.w||1,a);return _r(e,s,r,p,c,d,n)}).join("")}function zt(o){return`
        <style>
            .skeleton {
                background: var(--surface);
                pointer-events: none;
...
`;class yr extends HTMLElement{_itemId=0;_content="";_isDirty=!1;_isLocked=!0;_isSaving=!1;_lastLockTime=0;_editor=null;_syncStatus=null;_dock=null;_editorContainer=null;_modal=null;_imgResizer=null;_activeImage=null;_pendingAction=null;_boundBeforeUnload;_unsubscribeI18n;static get observedAttributes(){return["item-id"]}constructor(){super();this.attachShadow({mode:"open"}),this._boundBeforeUnload=this.handleBeforeUnload.bind(this)}connectedCallback(){this.render(),this.loadContent(),window.addEventListener("beforeunload",this._boundBeforeUnload),this._unsubscribeI18n=i.subscribe(()=>{if(this._editor)this._editor.dataset.placeholder=i.t("widget.notepad.placeholder")})}disconnectedCallback(){if(window.removeEventListener("beforeunload",this._boundBeforeUnload),this._unsubscribeI18n)this._unsubscribeI18n()}attributeChangedCallback(o,r,t){if(o==="item-id")this._itemId=parseInt(t),this.loadContent()}handleBeforeUnload(o){if(this._isDirty)o.preventDefault(),o.returnValue=""}render(){if(!this.shadowRoot)return;let o=this._isLocked?"locked":"unlocked",r=this._isLocked?"locked":"",t=this._isLocked?"hidden":"";this.shadowRoot.innerHTML=`
            ...
            <div class="glass-dock ${t}" id="glass-dock">
                <div class="dock-group">
                    <button class="dock-btn" data-fmt="undo" title="${i.t("widget.notepad.tool.undo")}">${m.undo}</button>
                    <button class="dock-btn" data-fmt="redo" title="${i.t("widget.notepad.tool.redo")}">${m.redo}</button>
                </div>

                <div class="dock-group">
                    <button class="dock-btn" data-fmt="formatBlock:H1" title="${i.t("widget.notepad.tool.h1")}" style="width:30px;">${m.h1}</button>
                    <button class="dock-btn" data-fmt="formatBlock:H2" title="${i.t("widget.notepad.tool.h2")}" style="width:30px;">${m.h2}</button>
                    <button class="dock-btn" data-fmt="formatBlock:H3" title="${i.t("widget.notepad.tool.h3")}" style="width:30px;">${m.h3}</button>
                </div>

                <div class="dock-group">
                    <button class="dock-btn" data-fmt="bold" title="${i.t("widget.notepad.tool.bold")}">${m.bold}</button>
                    <button class="dock-btn" data-fmt="italic" title="${i.t("widget.notepad.tool.italic")}">${m.italic}</button>
                    <button class="dock-btn" data-fmt="underline" title="${i.t("widget.notepad.tool.underline")}">${m.underline}</button>
                    <button class="dock-btn" data-fmt="strikeThrough" title="${i.t("widget.notepad.tool.strike")}">${m.strike}</button>
                ...
                <div class="dock-group">
                    <button class="dock-btn" data-fmt="justifyLeft" title="${i.t("widget.notepad.tool.left")}">${m.alignLeft}</button>
                    <button class="dock-btn" data-fmt="justifyCenter" title="${i.t("widget.notepad.tool.center")}">${m.alignCenter}</button>
                    <button class="dock-btn" data-fmt="justifyRight" title="${i.t("widget.notepad.tool.right")}">${m.alignRight}</button>
                    <button class="dock-btn" data-fmt="justifyFull" title="${i.t("widget.notepad.tool.justify")}">${m.alignJustify}</button>
                </div>

                <div class="dock-group">
                     <button class="dock-btn" id="btn-link" title="${i.t("widget.notepad.tool.link")}">${m.link}</button>
                     <button class="dock-btn" id="btn-image" title="${i.t("widget.notepad.tool.image")}">${m.image}</button>
                </div>

                <div class="dock-group">
                    <button class="dock-btn" data-fmt="insertUnorderedList" title="${i.t("widget.notepad.tool.bullet")}">${m.list}</button>
                    <button class="dock-btn" data-fmt="insertOrderedList" title="${i.t("widget.notepad.tool.number")}">${m.ordered}</button>
                    <button class="dock-btn" data-fmt="insertChecklist" title="${i.t("widget.notepad.tool.check")}">${m.check}</button>
                </div>

                <div class="dock-group">
                    <button class="dock-btn" data-fmt="formatBlock:BLOCKQUOTE" title="${i.t("widget.notepad.tool.quote")}">${m.quote}</button>
                     <button class="dock-btn" data-fmt="formatBlock:PRE" title="${i.t("widget.notepad.tool.code")}">${m.code}</button>
                    <button class="dock-btn" data-fmt="removeFormat" title="${i.t("widget.notepad.tool.clear")}">${m.eraser_clean}</button>
                </div>
            </div>
        ...
        `}forecastView(o,r,t){let a=i.getLocale().code,n=Math.min(this._config.forecastDays||5,r.time.length-1),e="";for(let s=1;s<=n;s++){let p=new Date(r.time[s]+"T00:00:00"),d=p.toLocaleDateString(a,{weekday:"short"}),c=p.toLocaleDateString(a,{weekday:"long",day:"numeric",month:"long"}),l=wo(r.weathercode[s]),x=Z(r.temperature_2m_max[s],t),u=Z(r.temperature_2m_min[s],t),v=r.sunrise?.[s]?.split("T")[1]||"",y=r.sunset?.[s]?.split("T")[1]||"",_=r.wind_speed_10m_max?.[s]!=null?`${Math.round(r.wind_speed_10m_max[s])} km/h`:"";e+=`<div class="fc-day">
                ...
                <div class="fc-tooltip">
                    ...
                    ${_?`<div class="tt-row"><span class="tt-label">${i.t("widget.weather.wind")}</span><span>${_}</span></div>`:""}
                </div>
            </div>`}return`
            <div class="fc-header">
                ...
                <div class="fc-h-info">
                    ...
                    <div class="fc-h-desc">${wo(o.weathercode)}</div>
                </div>
...
`,Gr=(o,r,t)=>{if(t)return`<div class="icon-picker__loading">${i.t("general.loading")}</div>`;if(o.length===0)return`<div class="icon-picker__empty">${i.t("general.no_icons")}</div>`;let a=o.reduce((e,s)=>{if(!e[s.provider])e[s.provider]=[];return e[s.provider].push(s),e},{}),n="";for(let[e,s]of Object.entries(a))n+=`
...
}`;class Zr extends HTMLElement{icons=[];filteredIcons=[];selectedIcon="";searchQuery="";isLoading=!0;debounceTimer=null;inputElement=null;previewElement=null;uploadBtn=null;fileInput=null;customUrl=null;constructor(){super();this.attachShadow({mode:"open"})}async connectedCallback(){if(this.shadowRoot.childElementCount>0)return;if(this.renderBase(),this.setupListeners(),this.icons.length===0)await this.loadIcons()}async loadIcons(){try{this.icons=await ao.loadIcons(),this.filteredIcons=ao.searchIcons("",12),this.isLoading=!1,this.updateGrid()}catch(o){console.error("[IconPicker] Failed to load icons",o),this.isLoading=!1,this.updateGrid()}}setupListeners(){let o=this.shadowRoot;this.inputElement=o.getElementById("icon-search"),this.previewElement=o.getElementById("icon-preview"),this.uploadBtn=o.getElementById("icon-upload-btn"),this.fileInput=o.getElementById("icon-upload-input"),this.inputElement?.addEventListener("input",(t)=>{let a=t.target;if(this.searchQuery=a.value,this.searchQuery.startsWith("http://")||this.searchQuery.startsWith("https://")||this.searchQuery.startsWith("data:image/")){if(this.customUrl=this.searchQuery,this.previewElement)this.previewElement.innerHTML=`<img src="${this.customUrl}" alt="preview" />`;this.filteredIcons=[],this.updateGrid()}else{if(this.customUrl=null,this.previewElement)this.previewElement.innerHTML='<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="color: var(--text-dim);"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect><circle cx="8.5" cy="8.5" r="1.5"></circle><polyline points="21 15 16 10 5 21"></polyline></svg>';if(this.debounceTimer)window.clearTimeout(this.debounceTimer);this.debounceTimer=window.setTimeout(()=>{this.performSearch()},100)}}),this.uploadBtn?.addEventListener("click",()=>{this.fileInput?.click()}),this.fileInput?.addEventListener("change",(t)=>{let a=t.target;if(a.files&&a.files.length>0){let n=a.files[0],e=new FileReader;e.onload=(s)=>{if(s.target&&typeof s.target.result==="string"){let p=s.target.result;this.selectedIcon=p,this.dispatchEvent(new CustomEvent("icon-selected",{detail:{iconName:p},bubbles:!0,composed:!0}))}},e.readAsDataURL(n)}}),o.getElementById("grid-container")?.addEventListener("click",(t)=>{let n=t.target.closest(".icon-picker__item");if(n&&n.dataset.icon)this.selectedIcon=n.dataset.icon,this.updateGrid(),this.dispatchEvent(new CustomEvent("icon-selected",{detail:{iconName:this.selectedIcon},bubbles:!0,composed:!0}));else if(n&&n.dataset.iconurl)this.selectedIcon=n.dataset.iconurl,this.updateGrid(),this.dispatchEvent(new CustomEvent("icon-selected",{detail:{iconName:n.dataset.iconurl},bubbles:!0,composed:!0}))})}performSearch(){this.filteredIcons=ao.searchIcons(this.searchQuery,12),this.updateGrid()}getSelectedIcon(){return this.selectedIcon}setSelectedIcon(o){this.selectedIcon=o,this.updateGrid()}renderBase(){this.shadowRoot.innerHTML=`
            <style>${Rr}</style>
            ${Qr()}
        `}updateGrid(){let o=this.shadowRoot.getElementById("grid-container");if(o){if(o.style.display="block",!this.searchQuery.trim()&&!this.isLoading){o.innerHTML=`<div class="icon-picker__empty">${i.t("general.search_icons")}</div>`;return}if(this.customUrl){o.innerHTML=`
                    <div class="icon-picker__group">
                        <div class="icon-picker__group-header">${i.t("general.custom_url")}</div>
                        <div class="icon-picker__group-grid" style="grid-template-columns: repeat(4, 1fr);">
                            <div class="icon-picker__item" style="grid-column: span 4; display: flex; gap: 12px; padding: 12px;" data-icon="${this.customUrl}">
                                ...
                                <span style="font-size: 13px; color: var(--text-main); word-break: break-all;">${i.t("general.click_to_use_url")}</span>
                            </div>
                        </div>
                    </div>
    ...
    <dialog id="modal">
        ...
        <div id="menu-overlay" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; pointer-events: none; z-index: 9999;">
            ...
            <div class="dropdown-menu" id="label-pos-menu">
                <div class="dropdown-item ${c==="bottom"?"selected":""}" data-pos="bottom" title="${i.t("bookmark.label_bottom")}">
                ...
                <div class="dropdown-item ${c==="top"?"selected":""}" data-pos="top" title="${i.t("bookmark.label_top")}">
                ...
                <div class="dropdown-item ${c==="section"?"selected":""}" data-pos="section" title="${i.t("bookmark.label_section")}">
                ...
                <div class="dropdown-item ${c==="off"?"selected":""}" data-pos="off" title="${i.t("bookmark.label_hidden")}">
            ...
            <div class="dropdown-menu" id="status-pos-menu">
                ...
                <div class="dropdown-item ${d&&p==="top-left"?"selected":""}" data-status-pos="top-left" title="${i.t("bookmark.pos_top_left")}">
                ...
                <div class="dropdown-item ${d&&p==="top-right"?"selected":""}" data-status-pos="top-right" title="${i.t("bookmark.pos_top_right")}">
                ...
                <div class="dropdown-item ${d&&p==="bottom-left"?"selected":""}" data-status-pos="bottom-left" title="${i.t("bookmark.pos_bottom_left")}">
                ...
                <div class="dropdown-item ${d&&p==="bottom-right"?"selected":""}" data-status-pos="bottom-right" title="${i.t("bookmark.pos_bottom_right")}">
            ...
            <div class="dropdown-menu" id="touch-pos-menu">
                <div class="dropdown-item ${l?"selected":""}" data-touch="on" title="${i.t("bookmark.visible")}">
                ...
                <div class="dropdown-item ${!l?"selected":""}" data-touch="off" title="${i.t("bookmark.hidden")}">
...
.border-width-btn span {
    ...
    font-weight: 500;
                ...
                `};if(!this.currentItem)return{general:"",customization:""};if(r==="clock"){let d=o.city||"",c=o.hour12||!1,l=o.showDate!==!1,x=`
                    ...
                    <div class="field-group check-row">
                        <input type="checkbox" id="clock-seconds" ${o.showSeconds!==!1?"checked":""} />
                        <label for="clock-seconds">${i.t("widget.clock.show_seconds")}</label>
                    </div>
                `,u="";return{general:x,customization:""+p()}}else if(r==="weather"){let d=o.city||"",c=o.latitude||"",l=o.longitude||"",x=o.unit==="fahrenheit",u=o.showForecast||!1,v=o.forecastDays||5,y=`
                    ...
                    <div class="field-group">
                        ...
                        <div class="input-row">
                            <input type="number" id="weather-lat" value="${c}" placeholder="${i.t("general.latitude")}" step="0.0001" />
                            <input type="number" id="weather-lon" value="${l}" placeholder="${i.t("general.longitude")}" step="0.0001" />
                        </div>
                    </div>
                ...
                `,_="";return{general:y,customization:""+p()}}else if(r==="telemetry"){let d=o.interval||1000,c=`
                ...
                `;return{general:c,customization:l+p()}}else if(r==="notepad"){let d=o.text||"",c=`
...
@keyframes shake {
    ...
    80% {
        ...
        `}}to();var L=document.getElementById("main-topbar"),at=document.getElementById("right-drawer"),io=document.getElementById("dashboard-root"),P,no,eo,so;tt(async()=>{M.init();let[o,r]=await Promise.all([k.fetchUser(),g.fetchItems()]),t=k.getUser();if(t){if(t.language)await i.setLanguage(t.language);if(t.preferences&&t.preferences.grid_columns)document.documentElement.style.setProperty("--user-preferred-columns",t.preferences.grid_columns.toString());if(t.theme==="dark")M.enableDark();else if(t.theme==="light")M.enableLight();else if(window.matchMedia&&window.matchMedia("(prefers-color-scheme: dark)").matches)M.enableDark();else M.enableLight();if(t.id)g.setUserId(t.id)}if(L){if(L.setState({user:t}),t&&t.project_name)L.setAttribute("title",t.project_name);k.subscribe((n)=>{if(n){if(L.setState({user:n}),n.project_name)L.setAttribute("title",n.project_name)}})}Yt(),I.start(),P=document.createElement("add-bookmark-modal"),document.body.appendChild(P),no=document.createElement("add-widget-modal"),document.body.appendChild(no),eo=document.createElement("confirmation-modal"),document.body.appendChild(eo),so=document.createElement("password-confirm-modal"),document.body.appendChild(so);let a=document.createElement("widget-config-modal");document.body.appendChild(a)});E.on(N.SHOW_CONFIRMATION,async(o)=>{let{title:r,message:t,onConfirm:a}=o.detail;if(eo){if(await eo.confirm(r,t)&&a)a()}});E.on(N.SHOW_PASSWORD_CONFIRM,async(o)=>{let{title:r,message:t,onConfirm:a}=o.detail;if(so){let n=await so.prompt(r,t);if(n!==null&&a)a(n)}});E.on(N.SHOW_WIDGET_CONFIG,(o)=>{let{item:r,type:t}=o.detail;if(t==="bookmark"){if(P)P.openForEdit(r)}else{let a=document.querySelector("widget-config-modal");if(a)a.open(r)}});E.on(N.NOTIFY,(o)=>{let r=document.querySelector("app-notifier");if(r)r.show(o.detail.message,o.detail.type)});if(L)L.addEventListener("drawer-toggle",(o)=>{if(o.detail.action==="open")at.open(),L.setState({drawerOpen:!0});else at.close(),L.setState({drawerOpen:!1})}),L.addEventListener("edit-mode-change",(o)=>{let r=o.detail.active;if(io.classList.toggle("edit-mode",r),!r)window.location.reload()}),L.addEventListener("search-input",(o)=>{let r=o.detail.query;g.setSearchQuery(r)}),L.addEventListener("add-item",async(o)=>{let r=o.detail.action;if(r==="add-bookmark"){if(P)P.open()}else if(r==="add-widget"){if(no)no.open()}else if(r==="add-section"){let a=g.getState().items||[],{collisionService:n}=await Promise.resolve().then(() => (U(),T)),e=n.findFirstAvailableSlot(1,1,a),s={type:"section",x:e.x,y:e.y,w:1,h:1,content:JSON.stringify({title:""})},p=await g.addItem(s);if(p)E.emit(N.SHOW_WIDGET_CONFIG,{item:p,type:"section"})}});window.addEventListener("widget-selected",async(o)=>{let t=o.detail,n=g.getState().items||[],{collisionService:e}=await Promise.resolve().then(() => (U(),T)),s=e.findFirstAvailableSlot(t.defaultW,t.defaultH,n),p={type:"widget",x:s.x,y:s.y,w:t.defaultW,h:t.defaultH,content:JSON.stringify({widgetId:t.id,text:t.id==="notepad"||t.id==="markdown"?"":void 0})};await g.addItem(p)});window.addEventListener("drawer-close",()=>{if(L)L.setState({drawerOpen:!1})});window.addEventListener("click",(o)=>{let r=o.target,t=document.getElementById("add-menu"),a=document.getElementById("add-toggle");if(t&&t.classList.contains("active")&&!t.contains(r)&&a&&!a.contains(r))t.classList.remove("active")});function Yt(){if(!io)return;io.innerHTML="";let o=document.createElement("bookmark-grid");io.appendChild(o)}
