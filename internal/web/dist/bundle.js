...
.paper {
    ...
    height: 100%;
}`;class qr extends HTMLElement{constructor(){super();this.attachShadow({mode:"open"})}connectedCallback(){this.render()}render(){this.shadowRoot.innerHTML=`
            <style>${Fr}</style>
            ${Hr()}
        `}}if(!customElements.get("app-paper"))customElements.define("app-paper",qr);var Br=`:host {
...
.avatar__initials {
    user-select: none;
}`;class Zr extends HTMLElement{static get observedAttributes(){return["src","alt","initials"]}constructor(){super();this.attachShadow({mode:"open"})}connectedCallback(){this.render()}attributeChangedCallback(o,r,i){if(r!==i)this.render()}render(){let o=this.getAttribute("src"),r=this.getAttribute("initials")||"??",i=this.getAttribute("alt")||"User Avatar";this.shadowRoot.innerHTML=`
            <style>${Qr}</style>
            ${Jr({src:o||void 0,initials:r,alt:i})}
        `}}if(!customElements.get("app-avatar"))customElements.define("app-avatar",Zr);u();co();Wo();u();var Yr=(o)=>`
...
`,Lr=(o,r,i)=>{let t=i?.toLowerCase()==="admin"||i?.toLowerCase()==="administrator";return`
    <div class="bento-grid" style="grid-template-columns: 1fr;">
...
.select-option.selected:hover {
    background: rgba(var(--accent-rgb), 0.15);
        ...
        `,this.shadowRoot.getElementById("trigger")?.addEventListener("click",(i)=>{i.stopPropagation(),this.toggle()})}}if(!customElements.get("app-select"))customElements.define("app-select",Rr);var Nr=`:host {
    display: block;
    color: var(--text-main);
    max-width: 900px;
    margin: 0 auto;
...
(max-width: 1024px) {
...
}`;class Sr extends HTMLElement{dropdownOpen;constructor(){super();this.attachShadow({mode:"open"}),this.dropdownOpen=!1}connectedCallback(){this.render(),this.setupListeners()}toggleDropdown(o){this.dropdownOpen=typeof o==="boolean"?o:!this.dropdownOpen,this.render()}setupListeners(){this.shadowRoot.addEventListener("click",(o)=>{let r=o.target,i=r.closest(".header__btn-changelog");if(i)i.style.display="none";if(r.closest("#profile-trigger"))o.stopPropagation(),this.toggleDropdown()}),window.addEventListener("click",()=>{if(this.dropdownOpen)this.toggleDropdown(!1)})}render(){let o=this.getAttribute("title")||"Task";this.shadowRoot.innerHTML=`
            <style>${Ir}</style>
            ${Pr({title:o,dropdownOpen:this.dropdownOpen})}
        `}}if(!customElements.get("app-header"))customElements.define("app-header",Sr);u();var lo=(o)=>String(o).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;"),ft=(o)=>{if(!o)return"#";let r=o.trim().toLowerCase();if(r.startsWith("http://")||r.startsWith("https://")||r==="#")return lo(o);return"#"},oi=({bookmarks:o,isEditing:r,isSearching:i,isTouchDevice:t,maxCols:a=12})=>{let n=Array.isArray(o)?o:[],d=(p)=>n.filter((g)=>g.parent_id===p);return`
    ${(t?n:i?n:n.filter((p)=>!p.parent_id)).map((p)=>{let g={};try{g=typeof p.content==="string"?JSON.parse(p.content):p.content}catch(y){console.error("Failed to parse content for item",p.id,y)}let l=p.type==="section",b={x:p.x||1,y:p.y||1},_=Math.min(p.w||1,a),h=p.h||1;if(l){let y=d(p.id),H=(g.title||"").trim();return`
            <fieldset class="bookmark-grid__section"
               data-id="${p.id}"
               data-orig-x="${p.x}" data-orig-y="${p.y}"
               draggable="${r}"
               style="--x: ${b.x}; --y: ${b.y}; --w: ${_}; --h: ${h};">
               ${H?`<legend class="section-title">${lo(H)}</legend>`:""}
               ...
               `:""}
            </fieldset>`}return ri(p,g,r,b,_,h,t)}).join("")}

    <div id="ghost-element" class="bookmark-grid__ghost" style="display: none;"></div>
    `};function ri(o,r,i,t,a,n=0,d=!1){let c=n||o.h||1;if(o.type==="widget"){let _=r.widgetId,y={clock:"widget-clock",notepad:"widget-notepad",telemetry:"widget-telemetry"}[_]||"div",w=(r.text||"").replace(/"/g,"&quot;");return`
        <div class="bookmark-grid__card"
            ...
            ${i?`
            <div class="bookmark-actions">
                 ${["clock","telemetry"].includes(_)?`<button class="action-btn btn-edit" title="${s.t("general.edit")}">✎</button>`:""}
                 <button class="action-btn btn-delete" title="${s.t("general.delete")}">\uD83D\uDDD1</button>
            </div>
            ${["clock","telemetry"].includes(_)?"":'<div class="resize-handle"></div>'}
            `:""}
        </div>
    `}let p=r.icon||"",l=p.startsWith("http")||p.startsWith("/")?`<img src="${lo(p)}" alt="${lo(r.label)}" class="bookmark-grid__icon-img" draggable="false" />`:p?lo(p):'<svg class="bookmark-grid__icon-svg" viewBox="0 0 24 24"><path d="M10 6L8.59 7.41 13.17 12l-4.58 4.59L10 18l6-6z"/></svg>';return`
        <a ${i?'role="button"':`href="${ft(r.url)}" target="_blank"`} class="bookmark-grid__card"
           draggable="${i}"
           data-id="${o.id}"
           data-type="${o.type}"
           data-orig-x="${o.x}" data-orig-y="${o.y}"
           ${!d?`style="--x: ${t.x}; --y: ${t.y}; --w: ${a}; --h: ${c};"`:""}>
            ...
            <span class="bookmark-grid__label">${lo(r.label)||"Bookmark"}</span>
    ...
    `}function et(o,r,i=!1,t=12,a=!1){return o.map((n)=>{let d={};try{d=typeof n.content==="string"?JSON.parse(n.content):n.content}catch(l){console.error("Failed to parse content for item (nested)",n.id,l)}let c={x:n.x||1,y:n.y||1},p=n.h||1,g=Math.min(n.w||1,t);return ri(n,d,r,c,g,p,a)}).join("")}Z();S();u();u();Z();class ii extends HTMLElement{timer;timeEl=null;dateEl=null;_unsubscribe=null;_unsubscribeI18n;constructor(){super();this.attachShadow({mode:"open"})}isEditing=!1;configMode=!1;_config={timezone:"local",hour12:!1,showDate:!0};_itemId=0;static get observedAttributes(){return["item-id","content","mode"]}attributeChangedCallback(o,r,i){if(o==="item-id")this._itemId=parseInt(i);if(o==="content")try{let t=typeof i==="string"?JSON.parse(i):i;if(t&&typeof t==="object")this._config={...this._config,...t},this.updateTime()}catch(t){}}connectedCallback(){this.render(),this.updateTime(),this.timer=setInterval(()=>{this.updateTime()},1000),this._unsubscribe=x.subscribe((o)=>{if(this.isEditing!==o.isEditing)this.isEditing=o.isEditing,this.render();if(this._itemId){let r=o.items.find((i)=>i.id===this._itemId);if(r&&r.content)try{let i=typeof r.content==="string"?JSON.parse(r.content):r.content;if(i.widgetId==="clock")this._config={...this._config,...i},this.updateTime()}catch(i){}}}),this._unsubscribeI18n=s.subscribe(()=>{this.updateTime()})}disconnectedCallback(){if(this._unsubscribe)this._unsubscribe();if(this._unsubscribeI18n)this._unsubscribeI18n();if(this.timer)clearInterval(this.timer)}updateTime(){if(!this.timeEl||!this.dateEl)return;let o=new Date,r={hour:"2-digit",minute:"2-digit",second:"2-digit",hour12:this._config.hour12},i=void 0;if(this._config.timezone&&this._config.timezone!=="local")i=this._config.timezone;let t=s.getLocale().code;try{let d=new Intl.DateTimeFormat(t,{...r,timeZone:i}).formatToParts(o).map((c)=>{if(c.type==="dayPeriod")return`<span class="ampm">${c.value.toUpperCase().replace(/\./g,"").trim()}</span>`;return c.value}).join("");this.timeEl.innerHTML=d}catch(a){this.timeEl.textContent=o.toLocaleTimeString()}if(this._config.showDate){let a={weekday:"long",day:"numeric",month:"long",timeZone:i};this.dateEl.textContent=o.toLocaleDateString(t,a),this.dateEl.style.display="block"}else this.dateEl.style.display="none"}render(){if(!this.shadowRoot)return;this.shadowRoot.innerHTML=`
            ...
            <div class="time">--:--:--</div>
            <div class="date">-- --</div>
        `,this.timeEl=this.shadowRoot.querySelector(".time"),this.dateEl=this.shadowRoot.querySelector(".date")}}customElements.define("widget-clock",ii);var Fo=globalThis,Eo=Fo.ShadowRoot&&(Fo.ShadyCSS===void 0||Fo.ShadyCSS.nativeShadow)&&"adoptedStyleSheets"in Document.prototype&&"replace"in CSSStyleSheet.prototype,Po=Symbol(),ti=new WeakMap;class Io{constructor(o,r,i){if(this._$cssResult$=!0,i!==Po)throw Error("CSSResult is not constructable. Use `unsafeCSS` or `css` instead.");this.cssText=o,this._strings=r}get styleSheet(){let o=this._styleSheet,r=this._strings;if(Eo&&o===void 0){let i=r!==void 0&&r.length===1;if(i)o=ti.get(r);if(o===void 0){if((this._styleSheet=o=new CSSStyleSheet).replaceSync(this.cssText),i)ti.set(r,o)}}return o}toString(){return this.cssText}}var _t=(o)=>{if(o._$cssResult$===!0)return o.cssText;else if(typeof o==="number")return o;else throw Error(`Value passed to 'css' function must be a 'css' function result: ${o}. Use 'unsafeCSS' to pass non-literal values, but take care to ensure page security.`)},vt=(o)=>new Io(typeof o==="string"?o:String(o),void 0,Po),qo=(o,...r)=>{let i=o.length===1?o[0]:r.reduce((t,a,n)=>t+_t(a)+o[n+1],o[0]);return new Io(i,o,Po)},ai=(o,r)=>{if(Eo)o.adoptedStyleSheets=r.map((i)=>i instanceof CSSStyleSheet?i:i.styleSheet);else for(let i of r){let t=document.createElement("style"),a=Fo.litNonce;if(a!==void 0)t.setAttribute("nonce",a);t.textContent=i.cssText,o.appendChild(t)}},kt=(o)=>{let r="";for(let i of o.cssRules)r+=i.cssText;return vt(r)},So=Eo?(o)=>o:(o)=>o instanceof CSSStyleSheet?kt(o):o;var{is:yt,defineProperty:ut,getOwnPropertyDescriptor:si,getOwnPropertyNames:wt,getOwnPropertySymbols:mt,getPrototypeOf:ni}=Object,$t=!1,q=globalThis;if($t)q.customElements??=customElements;var K=!0,Y,di=q.trustedTypes,zt=di?di.emptyScript:"",pi=K?q.reactiveElementPolyfillSupportDevMode:q.reactiveElementPolyfillSupport;if(K)q.litIssuedWarnings??=new Set,Y=(o,r)=>{if(r+=` See https://lit.dev/msg/${o} for more information.`,!q.litIssuedWarnings.has(r)&&!q.litIssuedWarnings.has(o))console.warn(r),q.litIssuedWarnings.add(r)},queueMicrotask(()=>{if(Y("dev-mode","Lit is in dev mode. Not recommended for production!"),q.ShadyDOM?.inUse&&pi===void 0)Y("polyfill-support-missing","Shadow DOM is being polyfilled via `ShadyDOM` but the `polyfill-support` module has not been loaded.")});var jt=K?(o)=>{if(!q.emitLitDebugLogEvents)return;q.dispatchEvent(new CustomEvent("lit-debug",{detail:o}))}:void 0,go=(o,r)=>o,yo={toAttribute(o,r){switch(r){case Boolean:o=o?zt:null;break;case Object:case Array:o=o==null?o:JSON.stringify(o);break}return o},fromAttribute(o,r){let i=o;switch(r){case Boolean:i=o!==null;break;case Number:i=o===null?null:Number(o);break;case Object:case Array:try{i=JSON.parse(o)}catch(t){i=null}break}return i}},Bo=(o,r)=>!yt(o,r),ci={attribute:!0,type:String,converter:yo,reflect:!1,useDefault:!1,hasChanged:Bo};Symbol.metadata??=Symbol("metadata");q.litPropertyMetadata??=new WeakMap;class J extends HTMLElement{static addInitializer(o){this.__prepare(),(this._initializers??=[]).push(o)}static get observedAttributes(){return this.finalize(),this.__attributeToPropertyMap&&[...this.__attributeToPropertyMap.keys()]}static createProperty(o,r=ci){if(r.state)r.attribute=!1;if(this.__prepare(),this.prototype.hasOwnProperty(o))r=Object.create(r),r.wrapped=!0;if(this.elementProperties.set(o,r),!r.noAccessor){let i=K?Symbol.for(`${String(o)} (@property() cache)`):Symbol(),t=this.getPropertyDescriptor(o,i,r);if(t!==void 0)ut(this.prototype,o,t)}}static getPropertyDescriptor(o,r,i){let{get:t,set:a}=si(this.prototype,o)??{get(){return this[r]},set(n){this[r]=n}};if(K&&t==null){if("value"in(si(this.prototype,o)??{}))throw Error(`Field ${JSON.stringify(String(o))} on ${this.name} was declared as a reactive property but it's actually declared as a value on the prototype. Usually this is due to using @property or @state on a method.`);Y("reactive-property-without-getter",`Field ${JSON.stringify(String(o))} on ${this.name} was declared as a reactive property but it does not have a getter. This will be an error in a future version of Lit.`)}return{get:t,set(n){let d=t?.call(this);a?.call(this,n),this.requestUpdate(o,d,i)},configurable:!0,enumerable:!0}}static getPropertyOptions(o){return this.elementProperties.get(o)??ci}static __prepare(){if(this.hasOwnProperty(go("elementProperties",this)))return;let o=ni(this);if(o.finalize(),o._initializers!==void 0)this._initializers=[...o._initializers];this.elementProperties=new Map(o.elementProperties)}static finalize(){if(this.hasOwnProperty(go("finalized",this)))return;if(this.finalized=!0,this.__prepare(),this.hasOwnProperty(go("properties",this))){let r=this.properties,i=[...wt(r),...mt(r)];for(let t of i)this.createProperty(t,r[t])}let o=this[Symbol.metadata];if(o!==null){let r=litPropertyMetadata.get(o);if(r!==void 0)for(let[i,t]of r)this.elementProperties.set(i,t)}this.__attributeToPropertyMap=new Map;for(let[r,i]of this.elementProperties){let t=this.__attributeNameForProperty(r,i);if(t!==void 0)this.__attributeToPropertyMap.set(t,r)}if(this.elementStyles=this.finalizeStyles(this.styles),K){if(this.hasOwnProperty("createProperty"))Y("no-override-create-property","Overriding ReactiveElement.createProperty() is deprecated. The override will not be called with standard decorators");if(this.hasOwnProperty("getPropertyDescriptor"))Y("no-override-get-property-descriptor","Overriding ReactiveElement.getPropertyDescriptor() is deprecated. The override will not be called with standard decorators")}}static finalizeStyles(o){let r=[];if(Array.isArray(o)){let i=new Set(o.flat(1/0).reverse());for(let t of i)r.unshift(So(t))}else if(o!==void 0)r.push(So(o));return r}static __attributeNameForProperty(o,r){let i=r.attribute;return i===!1?void 0:typeof i==="string"?i:typeof o==="string"?o.toLowerCase():void 0}constructor(){super();this.__instanceProperties=void 0,this.isUpdatePending=!1,this.hasUpdated=!1,this.__reflectingProperty=null,this.__initialize()}__initialize(){this.__updatePromise=new Promise((o)=>this.enableUpdating=o),this._$changedProperties=new Map,this.__saveInstanceProperties(),this.requestUpdate(),this.constructor._initializers?.forEach((o)=>o(this))}addController(o){if((this.__controllers??=new Set).add(o),this.renderRoot!==void 0&&this.isConnected)o.hostConnected?.()}removeController(o){this.__controllers?.delete(o)}__saveInstanceProperties(){let o=new Map,r=this.constructor.elementProperties;for(let i of r.keys())if(this.hasOwnProperty(i))o.set(i,this[i]),delete this[i];if(o.size>0)this.__instanceProperties=o}createRenderRoot(){let o=this.shadowRoot??this.attachShadow(this.constructor.shadowRootOptions);return ai(o,this.constructor.elementStyles),o}connectedCallback(){this.renderRoot??=this.createRenderRoot(),this.enableUpdating(!0),this.__controllers?.forEach((o)=>o.hostConnected?.())}enableUpdating(o){}disconnectedCallback(){this.__controllers?.forEach((o)=>o.hostDisconnected?.())}attributeChangedCallback(o,r,i){this._$attributeToProperty(o,i)}__propertyToAttribute(o,r){let t=this.constructor.elementProperties.get(o),a=this.constructor.__attributeNameForProperty(o,t);if(a!==void 0&&t.reflect===!0){let d=(t.converter?.toAttribute!==void 0?t.converter:yo).toAttribute(r,t.type);if(K&&this.constructor.enabledWarnings.includes("migration")&&d===void 0)Y("undefined-attribute-value",`The attribute value for the ${o} property is undefined on element ${this.localName}. The attribute will be removed, but in the previous version of \`ReactiveElement\`, the attribute would not have changed.`);if(this.__reflectingProperty=o,d==null)this.removeAttribute(a);else this.setAttribute(a,d);this.__reflectingProperty=null}}_$attributeToProperty(o,r){let i=this.constructor,t=i.__attributeToPropertyMap.get(o);if(t!==void 0&&this.__reflectingProperty!==t){let a=i.getPropertyOptions(t),n=typeof a.converter==="function"?{fromAttribute:a.converter}:a.converter?.fromAttribute!==void 0?a.converter:yo;this.__reflectingProperty=t;let d=n.fromAttribute(r,a.type);this[t]=d??this.__defaultValues?.get(t)??d,this.__reflectingProperty=null}}requestUpdate(o,r,i,t=!1,a){if(o!==void 0){if(K&&o instanceof Event)Y("","The requestUpdate() method was called with an Event as the property name. This is probably a mistake caused by binding this.requestUpdate as an event listener. Instead bind a function that will call it with no arguments: () => this.requestUpdate()");let n=this.constructor;if(t===!1)a=this[o];if(i??=n.getPropertyOptions(o),(i.hasChanged??Bo)(a,r)||i.useDefault&&i.reflect&&a===this.__defaultValues?.get(o)&&!this.hasAttribute(n.__attributeNameForProperty(o,i)))this._$changeProperty(o,r,i);else return}if(this.isUpdatePending===!1)this.__updatePromise=this.__enqueueUpdate()}_$changeProperty(o,r,{useDefault:i,reflect:t,wrapped:a},n){if(i&&!(this.__defaultValues??=new Map).has(o)){if(this.__defaultValues.set(o,n??r??this[o]),a!==!0||n!==void 0)return}if(!this._$changedProperties.has(o)){if(!this.hasUpdated&&!i)r=void 0;this._$changedProperties.set(o,r)}if(t===!0&&this.__reflectingProperty!==o)(this.__reflectingProperties??=new Set).add(o)}async __enqueueUpdate(){this.isUpdatePending=!0;try{await this.__updatePromise}catch(r){Promise.reject(r)}let o=this.scheduleUpdate();if(o!=null)await o;return!this.isUpdatePending}scheduleUpdate(){let o=this.performUpdate();if(K&&this.constructor.enabledWarnings.includes("async-perform-update")&&typeof o?.then==="function")Y("async-perform-update",`Element ${this.localName} returned a Promise from performUpdate(). This behavior is deprecated and will be removed in a future version of ReactiveElement.`);return o}performUpdate(){if(!this.isUpdatePending)return;if(jt?.({kind:"update"}),!this.hasUpdated){if(this.renderRoot??=this.createRenderRoot(),K){let a=[...this.constructor.elementProperties.keys()].filter((n)=>this.hasOwnProperty(n)&&(n in ni(this)));if(a.length)throw Error(`The following properties on element ${this.localName} will not trigger updates as expected because they are set using class fields: ${a.join(", ")}. Native class fields and some compiled output will overwrite accessors used for detecting changes. See https://lit.dev/msg/class-field-shadowing for more information.`)}if(this.__instanceProperties){for(let[t,a]of this.__instanceProperties)this[t]=a;this.__instanceProperties=void 0}let i=this.constructor.elementProperties;if(i.size>0)for(let[t,a]of i){let{wrapped:n}=a,d=this[t];if(n===!0&&!this._$changedProperties.has(t)&&d!==void 0)this._$changeProperty(t,void 0,a,d)}}let o=!1,r=this._$changedProperties;try{if(o=this.shouldUpdate(r),o)this.willUpdate(r),this.__controllers?.forEach((i)=>i.hostUpdate?.()),this.update(r);else this.__markUpdated()}catch(i){throw o=!1,this.__markUpdated(),i}if(o)this._$didUpdate(r)}willUpdate(o){}_$didUpdate(o){if(this.__controllers?.forEach((r)=>r.hostUpdated?.()),!this.hasUpdated)this.hasUpdated=!0,this.firstUpdated(o);if(this.updated(o),K&&this.isUpdatePending&&this.constructor.enabledWarnings.includes("change-in-update"))Y("change-in-update",`Element ${this.localName} scheduled an update (generally because a property was set) after an update completed, causing a new update to be scheduled. This is inefficient and should be avoided unless the next update can only be scheduled as a side effect of the previous update.`)}__markUpdated(){this._$changedProperties=new Map,this.isUpdatePending=!1}get updateComplete(){return this.getUpdateComplete()}getUpdateComplete(){return this.__updatePromise}shouldUpdate(o){return!0}update(o){this.__reflectingProperties&&=this.__reflectingProperties.forEach((r)=>this.__propertyToAttribute(r,this[r])),this.__markUpdated()}updated(o){}firstUpdated(o){}}J.elementStyles=[];J.shadowRootOptions={mode:"open"};J[go("elementProperties",J)]=new Map;J[go("finalized",J)]=new Map;pi?.({ReactiveElement:J});if(K){J.enabledWarnings=["change-in-update","async-perform-update"];let o=function(r){if(!r.hasOwnProperty(go("enabledWarnings",r)))r.enabledWarnings=r.enabledWarnings.slice()};J.enableWarning=function(r){if(o(this),!this.enabledWarnings.includes(r))this.enabledWarnings.push(r)},J.disableWarning=function(r){o(this);let i=this.enabledWarnings.indexOf(r);if(i>=0)this.enabledWarnings.splice(i,1)}}(q.reactiveElementVersions??=[]).push("2.1.2");if(K&&q.reactiveElementVersions.length>1)queueMicrotask(()=>{Y("multiple-versions","Multiple versions of Lit loaded. Loading multiple versions is not recommended.")});var Q=globalThis,v=(o)=>{if(!Q.emitLitDebugLogEvents)return;Q.dispatchEvent(new CustomEvent("lit-debug",{detail:o}))},Gt=0,mo;Q.litIssuedWarnings??=new Set,mo=(o,r)=>{if(r+=o?` See https://lit.dev/msg/${o} for more information.`:"",!Q.litIssuedWarnings.has(r)&&!Q.litIssuedWarnings.has(o))console.warn(r),Q.litIssuedWarnings.add(r)},queueMicrotask(()=>{mo("dev-mode","Lit is in dev mode. Not recommended for production!")});var A=Q.ShadyDOM?.inUse&&Q.ShadyDOM?.noPatch===!0?Q.ShadyDOM.wrap:(o)=>o,Ko=Q.trustedTypes,li=Ko?Ko.createPolicy("lit-html",{createHTML:(o)=>o}):void 0,Ht=(o)=>o,Yo=(o,r,i)=>Ht,Ft=(o)=>{if(to!==Yo)throw Error("Attempted to overwrite existing lit-html security policy. setSanitizeDOMValueFactory should be called at most once.");to=o},qt=()=>{to=Yo},ar=(o,r,i)=>{return to(o,r,i)},_i="$lit$",R=`lit$${Math.random().toFixed(9).slice(2)}$`,vi="?"+R,Bt=`<${vi}>`,ro=document,$o=()=>ro.createComment(""),zo=(o)=>o===null||typeof o!="object"&&typeof o!="function",sr=Array.isArray,Kt=(o)=>sr(o)||typeof o?.[Symbol.iterator]==="function",or=`[ 	
\f\r]`,Jt=`[^ 	
\f\r"'\`<>=]`,Qt=`[^\\s"'>=/]`,uo=/<(?:(!--|\/[^a-zA-Z])|(\/?[a-zA-Z][^>\s]*)|(\/?$))/g,gi=1,rr=2,Zt=3,xi=/-->/g,bi=/>/g,E=new RegExp(`>|${or}(?:(${Qt}+)(${or}*=${or}*(?:${Jt}|("|')|))|$)`,"g"),Yt=0,hi=1,At=2,fi=3,ir=/'/g,tr=/"/g,ki=/^(?:script|style|textarea|title)$/i,Mt=1,Jo=2,Qo=3,nr=1,Zo=2,Ut=3,Xt=4,Lt=5,dr=6,Rt=7,cr=(o)=>(r,...i)=>{if(r.some((t)=>t===void 0))console.warn(`Some template strings are undefined.
...
Please use the static 'html' tag function. See https://lit.dev/docs/templates/expressions/#static-expressions`);return{["_$litType$"]:o,strings:r,values:i}},f=cr(Mt),vs=cr(Jo),ks=cr(Qo),io=Symbol.for("lit-noChange"),j=Symbol.for("lit-nothing"),ei=new WeakMap,oo=ro.createTreeWalker(ro,129),to=Yo;function yi(o,r){if(!sr(o)||!o.hasOwnProperty("raw")){let i="invalid template strings array";throw i=`
        ...
        `.trim().replace(/\n */g,`
`),Error(i)}return li!==void 0?li.createHTML(r):r}var Nt=(o,r)=>{let i=o.length-1,t=[],a=r===Jo?"<svg>":r===Qo?"<math>":"",n,d=uo;for(let p=0;p<i;p++){let g=o[p],l=-1,b,_=0,h;while(_<g.length){if(d.lastIndex=_,h=d.exec(g),h===null)break;if(_=d.lastIndex,d===uo){if(h[gi]==="!--")d=xi;else if(h[gi]!==void 0)d=bi;else if(h[rr]!==void 0){if(ki.test(h[rr]))n=new RegExp(`</${h[rr]}`,"g");d=E}else if(h[Zt]!==void 0)throw Error("Bindings in tag names are not supported. Please use static templates instead. See https://lit.dev/docs/templates/expressions/#static-expressions")}else if(d===E)if(h[Yt]===">")d=n??uo,l=-1;else if(h[hi]===void 0)l=-2;else l=d.lastIndex-h[At].length,b=h[hi],d=h[fi]===void 0?E:h[fi]==='"'?tr:ir;else if(d===tr||d===ir)d=E;else if(d===xi||d===bi)d=uo;else d=E,n=void 0}console.assert(l===-1||d===E||d===ir||d===tr,"unexpected parse state B");let y=d===E&&o[p+1].startsWith("/>")?" ":"";a+=d===uo?g+Bt:l>=0?(t.push(b),g.slice(0,l)+_i+g.slice(l))+R+y:g+R+(l===-2?p:y)}let c=a+(o[i]||"<?>")+(r===Jo?"</svg>":r===Qo?"</math>":"");return[yi(o,c),t]};class jo{constructor({strings:o,["_$litType$"]:r},i){this.parts=[];let t,a=0,n=0,d=o.length-1,c=this.parts,[p,g]=Nt(o,r);if(this.el=jo.createElement(p,i),oo.currentNode=this.el.content,r===Jo||r===Qo){let l=this.el.content.firstChild;l.replaceWith(...l.childNodes)}while((t=oo.nextNode())!==null&&c.length<d){if(t.nodeType===1){{let l=t.localName;if(/^(?:textarea|template)$/i.test(l)&&t.innerHTML.includes(R)){let b=`Expressions are not supported inside \`${l}\` elements. See https://lit.dev/msg/expression-in-${l} for more information.`;if(l==="template")throw Error(b);else mo("",b)}}if(t.hasAttributes()){for(let l of t.getAttributeNames())if(l.endsWith(_i)){let b=g[n++],h=t.getAttribute(l).split(R),y=/([.?@])?(.*)/.exec(b);c.push({type:nr,index:a,name:y[2],strings:h,ctor:y[1]==="."?wi:y[1]==="?"?mi:y[1]==="@"?$i:Ho}),t.removeAttribute(l)}else if(l.startsWith(R))c.push({type:dr,index:a}),t.removeAttribute(l)}if(ki.test(t.tagName)){let l=t.textContent.split(R),b=l.length-1;if(b>0){t.textContent=Ko?Ko.emptyScript:"";for(let _=0;_<b;_++)t.append(l[_],$o()),oo.nextNode(),c.push({type:Zo,index:++a});t.append(l[b],$o())}}}else if(t.nodeType===8)if(t.data===vi)c.push({type:Zo,index:a});else{let b=-1;while((b=t.data.indexOf(R,b+1))!==-1)c.push({type:Rt,index:a}),b+=R.length-1}a++}if(g.length!==n)throw Error('Detected duplicate attribute bindings. This occurs if your template has duplicate attributes on an element tag. For example "<input ?disabled=${true} ?disabled=${false}>" contains a duplicate "disabled" attribute. The error was detected in the following template: \n`'+o.join("${...}")+"`");v&&v({kind:"template prep",template:this,clonableTemplate:this.el,parts:this.parts,strings:o})}static createElement(o,r){let i=ro.createElement("template");return i.innerHTML=o,i}}function xo(o,r,i=o,t){if(r===io)return r;let a=t!==void 0?i.__directives?.[t]:i.__directive,n=zo(r)?void 0:r._$litDirective$;if(a?.constructor!==n){if(a?._$notifyDirectiveConnectionChanged?.(!1),n===void 0)a=void 0;else a=new n(o),a._$initialize(o,i,t);if(t!==void 0)(i.__directives??=[])[t]=a;else i.__directive=a}if(a!==void 0)r=xo(o,a._$resolve(o,r.values),a,t);return r}class ui{constructor(o,r){this._$parts=[],this._$disconnectableChildren=void 0,this._$template=o,this._$parent=r}get parentNode(){return this._$parent.parentNode}get _$isConnected(){return this._$parent._$isConnected}_clone(o){let{el:{content:r},parts:i}=this._$template,t=(o?.creationScope??ro).importNode(r,!0);oo.currentNode=t;let a=oo.nextNode(),n=0,d=0,c=i[0];while(c!==void 0){if(n===c.index){let p;if(c.type===Zo)p=new Go(a,a.nextSibling,this,o);else if(c.type===nr)p=new c.ctor(a,c.name,c.strings,this,o);else if(c.type===dr)p=new zi(a,this,o);this._$parts.push(p),c=i[++d]}if(n!==c?.index)a=oo.nextNode(),n++}return oo.currentNode=ro,t}_update(o){let r=0;for(let i of this._$parts){if(i!==void 0)if(v&&v({kind:"set part",part:i,value:o[r],valueIndex:r,values:o,templateInstance:this}),i.strings!==void 0)i._$setValue(o,i,r),r+=i.strings.length-2;else i._$setValue(o[r]);r++}}}class Go{get _$isConnected(){return this._$parent?._$isConnected??this.__isConnected}constructor(o,r,i,t){this.type=Zo,this._$committedValue=j,this._$disconnectableChildren=void 0,this._$startNode=o,this._$endNode=r,this._$parent=i,this.options=t,this.__isConnected=t?.isConnected??!0,this._textSanitizer=void 0}get parentNode(){let o=A(this._$startNode).parentNode,r=this._$parent;if(r!==void 0&&o?.nodeType===11)o=r.parentNode;return o}get startNode(){return this._$startNode}get endNode(){return this._$endNode}_$setValue(o,r=this){if(this.parentNode===null)throw Error("This `ChildPart` has no `parentNode` and therefore cannot accept a value. This likely means the element containing the part was manipulated in an unsupported way outside of Lit's control such that the part's marker nodes were ejected from DOM. For example, setting the element's `innerHTML` or `textContent` can do this.");if(o=xo(this,o,r),zo(o)){if(o===j||o==null||o===""){if(this._$committedValue!==j)v&&v({kind:"commit nothing to child",start:this._$startNode,end:this._$endNode,parent:this._$parent,options:this.options}),this._$clear();this._$committedValue=j}else if(o!==this._$committedValue&&o!==io)this._commitText(o)}else if(o._$litType$!==void 0)this._commitTemplateResult(o);else if(o.nodeType!==void 0){if(this.options?.host===o){this._commitText("[probable mistake: rendered a template's host in itself (commonly caused by writing ${this} in a template]"),console.warn("Attempted to render the template host",o,"inside itself. This is almost always a mistake, and in dev mode ","we render some warning text. In production however, we'll ","render it, which will usually result in an error, and sometimes ","in the element disappearing from the DOM.");return}this._commitNode(o)}else if(Kt(o))this._commitIterable(o);else this._commitText(o)}_insert(o){return A(A(this._$startNode).parentNode).insertBefore(o,this._$endNode)}_commitNode(o){if(this._$committedValue!==o){if(this._$clear(),to!==Yo){let r=this._$startNode.parentNode?.nodeName;if(r==="STYLE"||r==="SCRIPT"){let i="Forbidden";if(r==="STYLE")i="Lit does not support binding inside style nodes. This is a security risk, as style injection attacks can exfiltrate data and spoof UIs. Consider instead using css`...` literals to compose styles, and do dynamic styling with css custom properties, ::parts, <slot>s, and by mutating the DOM rather than stylesheets.";else i="Lit does not support binding inside script nodes. This is a security risk, as it could allow arbitrary code execution.";throw Error(i)}}v&&v({kind:"commit node",start:this._$startNode,parent:this._$parent,value:o,options:this.options}),this._$committedValue=this._insert(o)}}_commitText(o){if(this._$committedValue!==j&&zo(this._$committedValue)){let r=A(this._$startNode).nextSibling;if(this._textSanitizer===void 0)this._textSanitizer=ar(r,"data","property");o=this._textSanitizer(o),v&&v({kind:"commit text",node:r,value:o,options:this.options}),r.data=o}else{let r=ro.createTextNode("");if(this._commitNode(r),this._textSanitizer===void 0)this._textSanitizer=ar(r,"data","property");o=this._textSanitizer(o),v&&v({kind:"commit text",node:r,value:o,options:this.options}),r.data=o}this._$committedValue=o}_commitTemplateResult(o){let{values:r,["_$litType$"]:i}=o,t=typeof i==="number"?this._$getTemplate(o):(i.el===void 0&&(i.el=jo.createElement(yi(i.h,i.h[0]),this.options)),i);if(this._$committedValue?._$template===t)v&&v({kind:"template updating",template:t,instance:this._$committedValue,parts:this._$committedValue._$parts,options:this.options,values:r}),this._$committedValue._update(r);else{let a=new ui(t,this),n=a._clone(this.options);v&&v({kind:"template instantiated",template:t,instance:a,parts:a._$parts,options:this.options,fragment:n,values:r}),a._update(r),v&&v({kind:"template instantiated and updated",template:t,instance:a,parts:a._$parts,options:this.options,fragment:n,values:r}),this._commitNode(n),this._$committedValue=a}}_$getTemplate(o){let r=ei.get(o.strings);if(r===void 0)ei.set(o.strings,r=new jo(o));return r}_commitIterable(o){if(!sr(this._$committedValue))this._$committedValue=[],this._$clear();let r=this._$committedValue,i=0,t;for(let a of o){if(i===r.length)r.push(t=new Go(this._insert($o()),this._insert($o()),this,this.options));else t=r[i];t._$setValue(a),i++}if(i<r.length)this._$clear(t&&A(t._$endNode).nextSibling,i),r.length=i}_$clear(o=A(this._$startNode).nextSibling,r){this._$notifyConnectionChanged?.(!1,!0,r);while(o!==this._$endNode){let i=A(o).nextSibling;A(o).remove(),o=i}}setConnected(o){if(this._$parent===void 0)this.__isConnected=o,this._$notifyConnectionChanged?.(o);else throw Error("part.setConnected() may only be called on a RootPart returned from render().")}}class Ho{get tagName(){return this.element.tagName}get _$isConnected(){return this._$parent._$isConnected}constructor(o,r,i,t,a){if(this.type=nr,this._$committedValue=j,this._$disconnectableChildren=void 0,this.element=o,this.name=r,this._$parent=t,this.options=a,i.length>2||i[0]!==""||i[1]!=="")this._$committedValue=Array(i.length-1).fill(new String),this.strings=i;else this._$committedValue=j;this._sanitizer=void 0}_$setValue(o,r=this,i,t){let a=this.strings,n=!1;if(a===void 0){if(o=xo(this,o,r,0),n=!zo(o)||o!==this._$committedValue&&o!==io,n)this._$committedValue=o}else{let d=o;o=a[0];let c,p;for(c=0;c<a.length-1;c++){if(p=xo(this,d[i+c],r,c),p===io)p=this._$committedValue[c];if(n||=!zo(p)||p!==this._$committedValue[c],p===j)o=j;else if(o!==j)o+=(p??"")+a[c+1];this._$committedValue[c]=p}}if(n&&!t)this._commitValue(o)}_commitValue(o){if(o===j)A(this.element).removeAttribute(this.name);else{if(this._sanitizer===void 0)this._sanitizer=to(this.element,this.name,"attribute");o=this._sanitizer(o??""),v&&v({kind:"commit attribute",element:this.element,name:this.name,value:o,options:this.options}),A(this.element).setAttribute(this.name,o??"")}}}class wi extends Ho{constructor(){super(...arguments);this.type=Ut}_commitValue(o){if(this._sanitizer===void 0)this._sanitizer=to(this.element,this.name,"property");o=this._sanitizer(o),v&&v({kind:"commit property",element:this.element,name:this.name,value:o,options:this.options}),this.element[this.name]=o===j?void 0:o}}class mi extends Ho{constructor(){super(...arguments);this.type=Xt}_commitValue(o){v&&v({kind:"commit boolean attribute",element:this.element,name:this.name,value:!!(o&&o!==j),options:this.options}),A(this.element).toggleAttribute(this.name,!!o&&o!==j)}}class $i extends Ho{constructor(o,r,i,t,a){super(o,r,i,t,a);if(this.type=Lt,this.strings!==void 0)throw Error(`A \`<${o.localName}>\` has a \`@${r}=...\` listener with invalid content. Event listeners in templates must have exactly one expression and no surrounding text.`)}_$setValue(o,r=this){if(o=xo(this,o,r,0)??j,o===io)return;let i=this._$committedValue,t=o===j&&i!==j||o.capture!==i.capture||o.once!==i.once||o.passive!==i.passive,a=o!==j&&(i===j||t);if(v&&v({kind:"commit event listener",element:this.element,name:this.name,value:o,options:this.options,removeListener:t,addListener:a,oldListener:i}),t)this.element.removeEventListener(this.name,this,i);if(a)this.element.addEventListener(this.name,this,o);this._$committedValue=o}handleEvent(o){if(typeof this._$committedValue==="function")this._$committedValue.call(this.options?.host??this.element,o);else this._$committedValue.handleEvent(o)}}class zi{constructor(o,r,i){this.element=o,this.type=dr,this._$disconnectableChildren=void 0,this._$parent=r,this.options=i}get _$isConnected(){return this._$parent._$isConnected}_$setValue(o){v&&v({kind:"commit to element binding",element:this.element,value:o,options:this.options}),xo(this,o)}}var Ct=Q.litHtmlPolyfillSupportDevMode;Ct?.(jo,Go);(Q.litHtmlVersions??=[]).push("3.3.2");if(Q.litHtmlVersions.length>1)queueMicrotask(()=>{mo("multiple-versions","Multiple versions of Lit loaded. Loading multiple versions is not recommended.")});var wo=(o,r,i)=>{if(r==null)throw TypeError(`The container to render into may not be ${r}`);let t=Gt++,a=i?.renderBefore??r,n=a._$litPart$;if(v&&v({kind:"begin render",id:t,value:o,container:r,options:i,part:n}),n===void 0){let d=i?.renderBefore??null;a._$litPart$=n=new Go(r.insertBefore($o(),d),d,void 0,i??{})}return n._$setValue(o),v&&v({kind:"end render",id:t,value:o,container:r,options:i,part:n}),n};wo.setSanitizer=Ft,wo.createSanitizer=ar,wo._testOnlyClearSanitizerFactoryDoNotCallOrElse=qt;var Wt=(o,r)=>o,pr=!0,N=globalThis,ji;if(pr)N.litIssuedWarnings??=new Set,ji=(o,r)=>{if(r+=` See https://lit.dev/msg/${o} for more information.`,!N.litIssuedWarnings.has(r)&&!N.litIssuedWarnings.has(o))console.warn(r),N.litIssuedWarnings.add(r)};class L extends J{constructor(){super(...arguments);this.renderOptions={host:this},this.__childPart=void 0}createRenderRoot(){let o=super.createRenderRoot();return this.renderOptions.renderBefore??=o.firstChild,o}update(o){let r=this.render();if(!this.hasUpdated)this.renderOptions.isConnected=this.isConnected;super.update(o),this.__childPart=wo(r,this.renderRoot,this.renderOptions)}connectedCallback(){super.connectedCallback(),this.__childPart?.setConnected(!0)}disconnectedCallback(){super.disconnectedCallback(),this.__childPart?.setConnected(!1)}render(){return io}}L._$litElement$=!0;L[Wt("finalized",L)]=!0;N.litElementHydrateSupport?.({LitElement:L});var Vt=pr?N.litElementPolyfillSupportDevMode:N.litElementPolyfillSupport;Vt?.({LitElement:L});(N.litElementVersions??=[]).push("4.2.2");if(pr&&N.litElementVersions.length>1)queueMicrotask(()=>{ji("multiple-versions","Multiple versions of Lit loaded. Loading multiple versions is not recommended.")});var Ao=(o)=>(r,i)=>{if(i!==void 0)i.addInitializer(()=>{customElements.define(o,r)});else customElements.define(o,r)};var Gi=!0,Hi;if(Gi)globalThis.litIssuedWarnings??=new Set,Hi=(o,r)=>{if(r+=` See https://lit.dev/msg/${o} for more information.`,!globalThis.litIssuedWarnings.has(r)&&!globalThis.litIssuedWarnings.has(o))console.warn(r),globalThis.litIssuedWarnings.add(r)};var Tt=(o,r,i)=>{let t=r.hasOwnProperty(i);return r.constructor.createProperty(i,o),t?Object.getOwnPropertyDescriptor(r,i):void 0},Ot={attribute:!0,type:String,converter:yo,reflect:!1,hasChanged:Bo},Dt=(o=Ot,r,i)=>{let{kind:t,metadata:a}=i;if(Gi&&a==null)Hi("missing-class-metadata",`The class ${r} is missing decorator metadata. This could mean that you're using a compiler that supports decorators but doesn't support decorator metadata, such as TypeScript 5.1. Please update your compiler.`);let n=globalThis.litPropertyMetadata.get(a);if(n===void 0)globalThis.litPropertyMetadata.set(a,n=new Map);if(t==="setter")o=Object.create(o),o.wrapped=!0;if(n.set(i.name,o),t==="accessor"){let{name:d}=i;return{set(c){let p=r.get.call(this);r.set.call(this,c),this.requestUpdate(d,p,o,!0,c)},init(c){if(c!==void 0)this._$changeProperty(d,void 0,o,c);return c}}}else if(t==="setter"){let{name:d}=i;return function(c){let p=this[d];r.call(this,c),this.requestUpdate(d,p,o,!0,c)}}throw Error(`Unsupported decorator location: ${t}`)};function C(o){return(r,i)=>{return typeof i==="object"?Dt(o,r,i):Tt(o,r,i)}}function W(o){return C({...o,state:!0,attribute:!1})}var ao=(o,r,i)=>{if(i.configurable=!0,i.enumerable=!0,Reflect.decorate&&typeof r!=="object")Object.defineProperty(o,r,i);return i};var lr=!0,Fi;if(lr)globalThis.litIssuedWarnings??=new Set,Fi=(o,r)=>{if(r+=o?` See https://lit.dev/msg/${o} for more information.`:"",!globalThis.litIssuedWarnings.has(r)&&!globalThis.litIssuedWarnings.has(o))console.warn(r),globalThis.litIssuedWarnings.add(r)};function qi(o,r){return(i,t,a)=>{let n=(d)=>{let c=d.renderRoot?.querySelector(o)??null;if(lr&&c===null&&r&&!d.hasUpdated){let p=typeof t==="object"?t.name:t;Fi("",`@query'd field ${JSON.stringify(String(p))} with the 'cache' flag set for selector '${o}' has been accessed before the first update and returned null. This is expected if the renderRoot tree has not been provided beforehand (e.g. via Declarative Shadow DOM). Therefore the value hasn't been cached.`)}return c};if(r){let{get:d,set:c}=typeof t==="object"?i:a??(()=>{let p=lr?Symbol(`${String(t)} (@query() cache)`):Symbol();return{get(){return this[p]},set(g){this[p]=g}}})();return ao(i,t,{get(){let p=d.call(this);if(p===void 0){if(p=n(this),p!==null||this.hasUpdated)c.call(this,p)}return p}})}else return ao(i,t,{get(){return n(this)}})}}Z();u();var Et={allowedTags:["h1","h2","h3","h4","h5","h6","p","br","div","span","strong","b","em","i","u","s","strike","a","img","ul","ol","li","pre","code","blockquote","label","input"],allowedAttrs:{"*":["style","class"],a:["href","title","target"],img:["src","alt","title","width","height"],input:["type","id","checked"],label:["for"]},allowedSchemes:["http","https","data","mailto"]};class Bi{config;constructor(o){this.config={...Et,...o}}sanitize(o){if(!o||typeof o!=="string")return"";let i=new DOMParser().parseFromString(o,"text/html");return this.cleanNode(i.body),i.body.innerHTML}cleanNode(o){let r=Array.from(o.childNodes);for(let i=r.length-1;i>=0;i--){let t=r[i];if(t.nodeType===Node.TEXT_NODE)continue;if(t.nodeType===Node.COMMENT_NODE){o.removeChild(t);continue}if(t.nodeType===Node.ELEMENT_NODE){let a=t,n=a.tagName.toLowerCase();if(!this.config.allowedTags.includes(n)){o.removeChild(t);continue}this.cleanAttributes(a),this.cleanNode(a)}else o.removeChild(t)}}cleanAttributes(o){let r=o.tagName.toLowerCase(),i=[...this.config.allowedAttrs["*"]||[],...this.config.allowedAttrs[r]||[]],t=Array.from(o.attributes);for(let a of t){let n=a.name.toLowerCase();if(!i.includes(n)){o.removeAttribute(a.name);continue}if(n==="href"||n==="src"){let d=a.value.trim();if(this.isDangerousUrl(d)){o.removeAttribute(a.name);continue}}if(n==="style"){let d=this.sanitizeStyle(a.value);if(d!==a.value)o.setAttribute("style",d)}if(r==="input"&&n==="type"){if(a.value.toLowerCase()!=="checkbox")o.setAttribute("type","checkbox")}}}isDangerousUrl(o){let r=o.toLowerCase().trim();if(r.startsWith("javascript:"))return!0;if(r.startsWith("data:")&&r.includes("script"))return!0;if(r.startsWith("vbscript:"))return!0;if(r.includes(":")){let i=r.split(":")[0];if(!this.config.allowedSchemes.includes(i))return!0}return!1}sanitizeStyle(o){let r=["behavior","expression","moz-binding","-o-link","-o-link-source","import","@import"],i=o;for(let t of r){let a=new RegExp(t,"gi");i=i.replace(a,"")}return i=i.replace(/url\s*\(\s*['"]?javascript:/gi,"url(about:blank"),i}}var so=new Bi;class Mo{static parse(o){try{if(typeof o==="object"&&o!==null)return o;if(typeof o==="string"){let r=JSON.parse(o);if(typeof r==="object"&&r!==null)return r}return{widgetId:"unknown"}}catch(r){return console.warn("[WidgetContentHelper] Failed to parse content:",r),{widgetId:"unknown"}}}static serialize(o){try{return JSON.stringify(o)}catch(r){return console.error("[WidgetContentHelper] Failed to serialize content:",r),"{}"}}static getNotepadText(o){return this.parse(o).text||""}static setNotepadText(o,r){let t={...this.parse(o),widgetId:"notepad",text:r};return this.serialize(t)}static merge(o,r){let t={...this.parse(o),...r};return this.serialize(t)}static validate(o,r){return this.parse(o).widgetId===r}}u();class gr extends L{constructor(){super(...arguments);this.blockId="";this.items=[];this.readonly=!1;this.focusedItemId=null}static styles=qo`
        :host {
            display: block;
            background: var(--surface);
            border-radius: 8px;
    ...
    `;connectedCallback(){super.connectedCallback(),this._unsubscribe=x.subscribe((r)=>{let i=this.isDashboardEditing;if(this.isDashboardEditing=r.isEditing||!1,i!==this.isDashboardEditing)if(this.isDashboardEditing)if(this.hasUnsavedChanges&&this.isInternalEditing)if(confirm(s.t("widget.notepad.confirm_discard")))this.isInternalEditing=!1,this.hasUnsavedChanges=!1,this.cancelAutosave(),this.requestUpdate();else setTimeout(()=>x.toggleEditMode(),0);else this.isInternalEditing=!1,this.cancelAutosave(),this.requestUpdate();else this.loadFromStore()});let o=x.getState();this.isDashboardEditing=o.isEditing||!1,this.loadFromStore()}disconnectedCallback(){if(super.disconnectedCallback(),this._unsubscribe)this._unsubscribe();this.cancelAutosave()}updated(o){if(super.updated(o),o.has("content")&&!this.isInternalEditing&&this.editorElement){let r=so.sanitize(this.content);if(this.editorElement.innerHTML!==r)this.editorElement.innerHTML=r}}loadFromStore(){if(!this.itemId)return;let r=x.getState().items.find((i)=>i.id===this.itemId);if(r){let i=Mo.getNotepadText(r.content);try{let t=JSON.parse(i);if(t&&typeof t==="object"&&"html"in t)this.content=t.html||"",this.checklistBlocks=t.checklists||[];else this.content=i,this.checklistBlocks=[]}catch{this.content=i,this.checklistBlocks=[]}this.characterCount=this.content.length,this._lastSavedContent=i}}exec(o,r){if(document.execCommand(o,!1,r),this.editorElement)this.editorElement.focus()}cancelAutosave(){if(this._autosaveTimeout)clearTimeout(this._autosaveTimeout),this._autosaveTimeout=null}async saveContent(o=!1){if(this._savePromise){await this._savePromise;return}try{let r=this.shadowRoot?.querySelector(".editor")||this.shadowRoot?.querySelector("[contenteditable]");if(!r)throw Error("Critical: Editor element missing");let i=r.innerHTML;if(i.length>Uo){alert(s.t("widget.notepad.error.too_large"));return}let t=so.sanitize(i),a=JSON.stringify({html:t,checklists:this.checklistBlocks});if(!o)this.content=t,this.characterCount=t.length,this.isInternalEditing=!1,this.hasUnsavedChanges=!1,this.requestUpdate();this._savePromise=(async()=>{if(!this.itemId)return;if(!o)this.isSaving=!0,this.requestUpdate();try{let n=x.getState().items.find((c)=>c.id===this.itemId);if(!n){console.warn("[NotepadWidget] Item no longer exists");return}let d=Mo.setNotepadText(n.content,a);if(await x.updateItem({id:this.itemId,content:d}),this._lastSavedContent=a,o)this.hasUnsavedChanges=!1}catch(n){if(console.error("[NotepadWidget] Save failed:",n),!o)alert(s.t("widget.notepad.error.save")+n)}finally{if(!o)this.isSaving=!1,this.requestUpdate()}})(),await this._savePromise}catch(r){if(console.error("[NotepadWidget] Save error:",r),!o)alert(s.t("widget.notepad.error.save")+r)}finally{this._savePromise=null}}startEditing(){if(this.isDashboardEditing)return;this.isInternalEditing=!0,setTimeout(()=>{if(this.editorElement){this.editorElement.focus();let o=so.sanitize(this.content);if(this.editorElement.innerHTML!==o)this.editorElement.innerHTML=o;this.focusEditor(!0)}},0)}focusEditor(o=!0){if(!this.editorElement)return;if(this.editorElement.focus(),o){let r=window.getSelection();if(r){let i=document.createRange();i.selectNodeContents(this.editorElement),i.collapse(!1),r.removeAllRanges(),r.addRange(i)}}}handleColor(o){let r=o.target;this.exec("foreColor",r.value)}handleToolbarWheel(o){o.preventDefault();let r=o.currentTarget;r.scrollLeft+=o.deltaY}insertChecklist(){let o="block-"+Math.random().toString(36).substring(2,11),r={id:o,items:[{id:"item-"+Math.random().toString(36).substring(2,11),checked:!1,text:""}]};this.checklistBlocks=[...this.checklistBlocks,r],this.hasUnsavedChanges=!0,this.scheduleAutosave(),this.updateComplete.then(()=>{this.shadowRoot?.querySelector(`checklist-block[block-id="${o}"]`)?.shadowRoot?.querySelector('input[type="text"]')?.focus()})}insertCode(){let r=`<pre><code>${s.t("widget.notepad.prompt.code_block")}</code></pre><p><br></p>`;this.exec("insertHTML",r),setTimeout(()=>{let i=window.getSelection();if(i&&this.editorElement){let t=this.editorElement.querySelectorAll("code"),a=t[t.length-1];if(a){let n=document.createRange();n.selectNodeContents(a),i.removeAllRanges(),i.addRange(n)}}},0)}handleChecklistChange(o){let{blockId:r,items:i}=o.detail;this.checklistBlocks=this.checklistBlocks.map((t)=>t.id===r?{...t,items:i}:t),this.hasUnsavedChanges=!0,this.scheduleAutosave()}deleteChecklistBlock(o){this.checklistBlocks=this.checklistBlocks.filter((r)=>r.id!==o),this.hasUnsavedChanges=!0,this.scheduleAutosave()}handleEditorInput(o){let r=o.target;if(this.characterCount=r.innerHTML.length,this.hasUnsavedChanges=r.innerHTML!==this._lastSavedContent,this.cancelAutosave(),Ki>0)this._autosaveTimeout=setTimeout(()=>{if(this.hasUnsavedChanges)this.saveContent(!0)},Ki)}handleEditorKeydown(o){if(o.ctrlKey||o.metaKey)switch(o.key.toLowerCase()){case"b":o.preventDefault(),this.exec("bold");return;case"i":o.preventDefault(),this.exec("italic");return;case"s":o.preventDefault(),this.saveContent(!1);return;case"z":if(o.shiftKey)o.preventDefault(),this.exec("redo");else o.preventDefault(),this.exec("undo");return;case"k":o.preventDefault();let r=prompt(s.t("widget.notepad.prompt.url"));if(r)this.exec("createLink",r);return}}handlePaste(o){let r=o.clipboardData?.items;if(!r)return;for(let i of Array.from(r))if(i.type.indexOf("image")!==-1){o.preventDefault();let t=i.getAsFile();if(!t)continue;let a=new FileReader;a.onload=(n)=>{let d=n.target?.result;if(d.length>1e6){alert(s.t("widget.notepad.error.image_too_large"));return}this.exec("insertImage",d)},a.readAsDataURL(t);return}}render(){try{if(!this.isInternalEditing){let t=so.sanitize(this.content)||`<span style='opacity:0.5; font-style:italic; color: var(--text-dim);'>${s.t("widget.notepad.placeholder")}</span>`;return f`
                ...
                `}let o=this.characterCount>Uo*0.9,r=this.characterCount>Uo;return f`
                <div class="container">
                    <div class="toolbar" @wheel="${this.handleToolbarWheel}" title="${s.t("widget.notepad.tool.scroll_hint")}">
                        <!-- History Group -->
                        <div class="group">
                             <button @click="${(i)=>{i.preventDefault(),this.exec("undo")}}" title="${s.t("widget.notepad.tool.undo")} (Ctrl+Z)">${m.undo}</button>
                             <button @click="${(i)=>{i.preventDefault(),this.exec("redo")}}" title="${s.t("widget.notepad.tool.redo")} (Ctrl+Shift+Z)">${m.redo}</button>
                        </div>

                        <!-- Text Group -->
                        <div class="group">
                            <button @click="${(i)=>{i.preventDefault(),this.exec("formatBlock","H1")}}" title="${s.t("widget.notepad.tool.h1")}">${m.h1}</button>
                            <button @click="${(i)=>{i.preventDefault(),this.exec("formatBlock","H2")}}" title="${s.t("widget.notepad.tool.h2")}">${m.h2}</button>
                            <button @click="${(i)=>{i.preventDefault(),this.exec("bold")}}" title="${s.t("widget.notepad.tool.bold")} (Ctrl+B)">${m.bold}</button>
                            <button @click="${(i)=>{i.preventDefault(),this.exec("italic")}}" title="${s.t("widget.notepad.tool.italic")} (Ctrl+I)">${m.italic}</button>

                            <!-- Color Picker -->
                            <div class="color-wrapper">
                                <button title="${s.t("widget.notepad.tool.color")}">${m.color}</button>
                        ...
                        <!-- Paragraph Group -->
                        <div class="group">
                             <button @click="${(i)=>{i.preventDefault(),this.exec("justifyLeft")}}" title="${s.t("widget.notepad.tool.align_left")}">${m.alignLeft}</button>
                             <button @click="${(i)=>{i.preventDefault(),this.exec("justifyCenter")}}" title="${s.t("widget.notepad.tool.align_center")}">${m.alignCenter}</button>
                             <button @click="${(i)=>{i.preventDefault(),this.exec("justifyRight")}}" title="${s.t("widget.notepad.tool.align_right")}">${m.alignRight}</button>
                             <button @click="${(i)=>{i.preventDefault(),this.exec("justifyFull")}}" title="${s.t("widget.notepad.tool.align_justify")}">${m.alignJustify}</button>
                        </div>

                        <div class="group">
                            <button @click="${(i)=>{i.preventDefault(),this.insertChecklist()}}" title="${s.t("widget.notepad.tool.checklist")}">${m.checklist}</button>
                            <button @click="${(i)=>{i.preventDefault(),this.exec("insertUnorderedList")}}" title="${s.t("widget.notepad.tool.list_bullet")}">${m.list}</button>
                            <button @click="${(i)=>{i.preventDefault(),this.exec("insertOrderedList")}}" title="${s.t("widget.notepad.tool.list_ordered")}">${m.listOrdered}</button>
                        </div>

                        <!-- Insert Group -->
                        <div class="group">
                            <button @click="${(i)=>{i.preventDefault(),this.insertCode()}}" title="${s.t("widget.notepad.tool.code")}">${m.code}</button>
                            <button @click="${(i)=>{i.preventDefault();let t=prompt(s.t("widget.notepad.prompt.url"));if(t)this.exec("createLink",t)}}" title="${s.t("widget.notepad.tool.link")} (Ctrl+K)">${m.link}</button>
                            <button @click="${(i)=>{i.preventDefault();let t=prompt(s.t("widget.notepad.prompt.image_url"));if(t)this.exec("insertImage",t)}}" title="${s.t("widget.notepad.tool.image")}">${m.image}</button>
                             <button @click="${(i)=>{i.preventDefault(),this.exec("removeFormat")}}" title="${s.t("widget.notepad.tool.clear_format")}">${m.clear}</button>
                        </div>
                    ...
                    <div class="content-area editor"
                         ...
                         .innerHTML="${so.sanitize(this.content)}">
                    </div>
        ...
        `,this.cpuBar=this.shadowRoot.querySelector(".cpu-bar"),this.ramBar=this.shadowRoot.querySelector(".ram-bar"),this.tempBar=this.shadowRoot.querySelector(".temp-bar"),this.cpuText=this.shadowRoot.querySelector(".cpu-text"),this.ramText=this.shadowRoot.querySelector(".ram-text"),this.tempText=this.shadowRoot.querySelector(".temp-text")}}customElements.define("widget-telemetry",Ji);var Qi=`:host {
    /* Fixed Grid System */
...
.bookmark-grid__nested-content {
    ...
    align-content: start;
        ...
        `;this.shadowRoot.innerHTML=`
            <style>${Qi}</style>
            ${a}
            ${oi({bookmarks:this.bookmarks,isEditing:this.isEditing,isSearching:!!this.searchQuery,isTouchDevice:this.isTouchDevice,maxCols:this.currentGridCols})}
        `,this.setupActionListeners(),this.ghostEl=this.shadowRoot.getElementById("ghost-element")}}if(!customElements.get("bookmark-grid"))customElements.define("bookmark-grid",Yi);var Ai=()=>`
    <div class="toast-container"></div>
`;var Mi=`.toast-container {
    position: fixed;
    bottom: 20px;
    right: 20px;
    z-index: 1000;
...
}`;class Wi extends HTMLElement{icons=[];filteredIcons=[];selectedIcon="";searchQuery="";isLoading=!0;debounceTimer=null;inputElement=null;id_debug=Math.random().toString(36).substr(2,5);constructor(){super();this.attachShadow({mode:"open"})}async connectedCallback(){if(this.shadowRoot.childElementCount>0)return;if(this.renderBase(),this.setupListeners(),this.icons.length===0)await this.loadIcons()}async loadIcons(){try{this.icons=await hr.loadIcons(),this.filteredIcons=this.icons.slice(0,50),this.isLoading=!1,this.updateGrid()}catch(o){console.error(`[IconPicker:${this.id_debug}] Failed to load icons`,o),this.isLoading=!1,this.updateGrid()}}setupListeners(){let o=this.shadowRoot;this.inputElement=o.getElementById("icon-search"),this.inputElement?.addEventListener("input",(i)=>{let t=i.target;if(this.searchQuery=t.value,this.debounceTimer)window.clearTimeout(this.debounceTimer);this.debounceTimer=window.setTimeout(()=>{this.performSearch()},100)}),o.getElementById("grid-container")?.addEventListener("click",(i)=>{let a=i.target.closest(".icon-picker__item");if(a&&a.dataset.icon)this.selectedIcon=a.dataset.icon,this.updateSelected(),this.updateGrid(),this.dispatchEvent(new CustomEvent("icon-selected",{detail:{iconName:this.selectedIcon},bubbles:!0,composed:!0}))})}performSearch(){this.filteredIcons=hr.searchIcons(this.searchQuery,50),this.updateGrid()}getSelectedIcon(){return this.selectedIcon}setSelectedIcon(o){this.selectedIcon=o,this.updateSelected(),this.updateGrid()}renderBase(){this.shadowRoot.innerHTML=`
            <style>${Ci}</style>
            ${Xi()}
        `}updateGrid(){let o=this.shadowRoot.getElementById("grid-container");if(o){if(!this.searchQuery.trim()&&!this.isLoading){o.style.display="none",o.innerHTML="";return}o.style.display="grid",o.innerHTML=Ri(this.filteredIcons,this.selectedIcon,this.isLoading)}}updateSelected(){let o=this.shadowRoot.getElementById("selected-container");if(o)o.innerHTML=Li(this.selectedIcon)}}if(!customElements.get("icon-picker"))customElements.define("icon-picker",Wi);u();var Vi=({isOpen:o,isEditMode:r})=>`
    <dialog id="modal">
        <div class="modal-header">
...
.visibility-row .checkbox-group {
    ...
    margin-bottom: 0;
        ...
        `,this.dialog=this.shadowRoot.querySelector("dialog"),this.shadowRoot.querySelectorAll(".card").forEach((o)=>{o.addEventListener("click",()=>{let r=o.getAttribute("data-id"),i=fr.find((t)=>t.id===r);if(i)this.selectWidget(i)})})}}customElements.define("add-widget-modal",Di);Z();u();var Ei=`:host {
    display: contents;
}
...
.actions app-button {
    min-width: 120px;
}`;var er=new Map;async function ia(o,r){try{let i=await fetch(`https://timeapi.io/api/timezone/coordinate?latitude=${o}&longitude=${r}`);if(!i.ok)throw Error("TimeAPI failed");return(await i.json()).timeZone||"local"}catch(i){return console.error("[Timezone] TimeAPI error:",i),"local"}}async function ta(o){if(!o||o.trim()==="")return"local";let r=o.toLowerCase().trim();if(er.has(r))return er.get(r);try{let i=`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(o)}&limit=1`,t=await fetch(i,{headers:{"User-Agent":"Lastboard/1.0"}});if(!t.ok)throw Error("Geocoding failed");let a=await t.json();if(a.length===0)return"local";let{lat:n,lon:d}=a[0],c=await ia(parseFloat(n),parseFloat(d));return er.set(r,c),c}catch(i){return console.error("[Timezone] Error resolving city:",o,i),"local"}}class Pi extends HTMLElement{dialog=null;currentItem=null;constructor(){super();this.attachShadow({mode:"open"})}connectedCallback(){this.render()}open(o){if(this.currentItem=o,this.render(),this.dialog)this.dialog.showModal()}close(){if(this.dialog)this.dialog.close();this.currentItem=null}async save(){if(!this.currentItem)return;let o=typeof this.currentItem.content==="string"?JSON.parse(this.currentItem.content):this.currentItem.content,r=o.widgetId,i={...o};if(r==="clock"){let t=this.shadowRoot?.getElementById("clock-city"),a=this.shadowRoot?.getElementById("clock-12h"),n=this.shadowRoot?.getElementById("clock-date"),d=this.shadowRoot?.getElementById("save-btn"),c=t?.value||"";if(d)d.disabled=!0,d.textContent=s.t("general.loading");let p=await ta(c);if(i.city=c,i.timezone=p,i.hour12=a?.checked||!1,i.showDate=n?.checked||!1,d)d.disabled=!1,d.textContent=s.t("general.save")}else if(r==="telemetry"){let t=this.shadowRoot?.getElementById("telemetry-interval");i.interval=t?parseInt(t.value):1000}if(this.currentItem.type==="section"){let t=this.shadowRoot?.getElementById("section-title");i.title=t?t.value:"",delete i.name}await x.updateItem({id:this.currentItem.id,content:JSON.stringify(i)}),this.close()}render(){if(!this.shadowRoot)return;let o={},r="";if(this.currentItem)o=typeof this.currentItem.content==="string"?JSON.parse(this.currentItem.content):this.currentItem.content,r=o.widgetId;let i=(d)=>String(d).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;"),t=()=>{if(!this.currentItem)return"";if(r==="clock"){let d=o.city||"",c=o.hour12||!1,p=o.showDate!==!1;return`
        ...
        `,this.dialog=this.shadowRoot.querySelector("dialog");let n=this.shadowRoot.getElementById("telemetry-interval");if(n)n.options=[{value:"1000",label:"1s"},{value:"2000",label:"2s"},{value:"5000",label:"5s"},{value:"10000",label:"10s"}];this.shadowRoot.getElementById("close-btn")?.addEventListener("click",()=>this.close()),this.shadowRoot.getElementById("save-btn")?.addEventListener("click",()=>this.save())}}customElements.define("widget-config-modal",Pi);u();var Ii=({isOpen:o,title:r,message:i})=>`
    <dialog id="modal">
        <div class="modal-header">
...
.modal-actions {
    ...
    justify-content: flex-end;
            ...
            <style>${Si}</style>
            ${Ii({isOpen:!0,title:this.titleText,message:this.messageText})}
        `,this.setupDynamicListeners()}}if(!customElements.get("confirmation-modal"))customElements.define("confirmation-modal",ot);u();async function rt(o){try{await s.ensureInitialized(),await o()}catch(r){console.error("[Bootstrap] Critical failure:",r),document.body.innerHTML=`
            <div style="
                display: flex; 
                flex-direction: column; 
                align-items: center; 
        ...
        `}}Xo();var B=document.getElementById("main-topbar"),it=document.getElementById("right-drawer"),Lo=document.getElementById("dashboard-root"),fo,Ro,No;rt(async()=>{X.init(),await $.fetchUser();let o=$.getUser();if(o){if(o.language)await s.setLanguage(o.language);if(o.preferences&&o.preferences.grid_columns)document.documentElement.style.setProperty("--user-preferred-columns",o.preferences.grid_columns.toString());if(o.theme==="dark")X.enableDark();else if(o.theme==="light")X.enableLight();else if(window.matchMedia&&window.matchMedia("(prefers-color-scheme: dark)").matches)X.enableDark();else X.enableLight();if(o.id)x.setUserId(o.id)}if(B){if(B.setState({user:o}),o&&o.project_name)B.setAttribute("title",o.project_name);$.subscribe((i)=>{if(i){if(B.setState({user:i}),i.project_name)B.setAttribute("title",i.project_name)}})}await x.fetchItems(),sa(),ko.start(),fo=document.createElement("add-bookmark-modal"),document.body.appendChild(fo),Ro=document.createElement("add-widget-modal"),document.body.appendChild(Ro),No=document.createElement("confirmation-modal"),document.body.appendChild(No);let r=document.createElement("widget-config-modal");document.body.appendChild(r)});ho.on(bo.SHOW_CONFIRMATION,async(o)=>{let{title:r,message:i,onConfirm:t}=o.detail;if(No){if(await No.confirm(r,i)&&t)t()}});ho.on(bo.SHOW_WIDGET_CONFIG,(o)=>{let{item:r,type:i}=o.detail;if(i==="bookmark"){if(fo)fo.openForEdit(r)}else{let t=document.querySelector("widget-config-modal");if(t)t.open(r)}});ho.on(bo.NOTIFY,(o)=>{let r=document.querySelector("app-notifier");if(r)r.show(o.detail.message,o.detail.type)});if(B)B.addEventListener("drawer-toggle",(o)=>{if(o.detail.action==="open")it.open(),B.setState({drawerOpen:!0});else it.close(),B.setState({drawerOpen:!1})}),B.addEventListener("edit-mode-change",(o)=>{let r=o.detail.active;if(Lo.classList.toggle("edit-mode",r),!r)window.location.reload()}),B.addEventListener("search-input",(o)=>{let r=o.detail.query;x.setSearchQuery(r)}),B.addEventListener("add-item",async(o)=>{let r=o.detail.action;if(r==="add-bookmark"){if(fo)fo.open()}else if(r==="add-widget"){if(Ro)Ro.open()}else if(r==="add-section"){let t=x.getState().items||[],{collisionService:a}=await Promise.resolve().then(() => (S(),po)),n=a.findFirstAvailableSlot(1,1,t),d={type:"section",x:n.x,y:n.y,w:1,h:1,content:JSON.stringify({title:""})},c=await x.addItem(d);if(c)ho.emit(bo.SHOW_WIDGET_CONFIG,{item:c,type:"section"})}});window.addEventListener("widget-selected",async(o)=>{let i=o.detail,a=x.getState().items||[],{collisionService:n}=await Promise.resolve().then(() => (S(),po)),d=n.findFirstAvailableSlot(i.defaultW,i.defaultH,a),c={type:"widget",x:d.x,y:d.y,w:i.defaultW,h:i.defaultH,content:JSON.stringify({widgetId:i.id,text:i.id==="notepad"?"":void 0})};await x.addItem(c)});window.addEventListener("drawer-close",()=>{if(B)B.setState({drawerOpen:!1})});window.addEventListener("click",(o)=>{let r=o.target,i=document.getElementById("add-menu"),t=document.getElementById("add-toggle");if(i&&i.classList.contains("active")&&!i.contains(r)&&t&&!t.contains(r))i.classList.remove("active")});function sa(){if(!Lo)return;Lo.innerHTML="";let o=document.createElement("bookmark-grid");Lo.appendChild(o)}
