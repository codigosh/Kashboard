...
.paper {
    ...
    height: 100%;
            ...
            <style>${xr}</style>
            ${gr()}
        `}}if(!customElements.get("app-paper"))customElements.define("app-paper",br);var hr=`:host {
...
.avatar__initials {
    user-select: none;
}`;class _r extends HTMLElement{static get observedAttributes(){return["src","alt","initials"]}constructor(){super();this.attachShadow({mode:"open"})}connectedCallback(){this.render()}attributeChangedCallback(o,r,a){if(r!==a)this.render()}render(){let o=this.getAttribute("src"),r=this.getAttribute("initials")||"??",a=this.getAttribute("alt")||"User Avatar";this.shadowRoot.innerHTML=`
            <style>${ur}</style>
            ${fr({src:o||void 0,initials:r,alt:a})}
        `}}if(!customElements.get("app-avatar"))customElements.define("app-avatar",_r);_();U();Ao();_();var mr=(o)=>`
...
`,zr=(o,r,a)=>{let t=a?.toLowerCase()==="admin"||a?.toLowerCase()==="administrator";return`
    <div class="bento-grid" style="grid-template-columns: 1fr;">
...
.select-option.selected:hover {
    background: rgba(var(--accent-rgb), 0.15);
        ...
        `,this.shadowRoot.getElementById("trigger")?.addEventListener("click",(a)=>{a.stopPropagation(),this.toggle()})}}if(!customElements.get("app-select"))customElements.define("app-select",jr);var Gr=`:host {
    display: block;
    color: var(--text-main);
    max-width: 900px;
    margin: 0 auto;
...
@keyframes blink {
    ...
    100% {
        ...
        `}}if(!customElements.get("app-topbar"))customElements.define("app-topbar",Qr);_();var Kr=({title:o,dropdownOpen:r})=>`
    <header class="header">
...
(max-width: 1024px) {
...
}`;class Mr extends HTMLElement{dropdownOpen;constructor(){super();this.attachShadow({mode:"open"}),this.dropdownOpen=!1}connectedCallback(){this.render(),this.setupListeners()}toggleDropdown(o){this.dropdownOpen=typeof o==="boolean"?o:!this.dropdownOpen,this.render()}setupListeners(){this.shadowRoot.addEventListener("click",(o)=>{let r=o.target,a=r.closest(".header__btn-changelog");if(a)a.style.display="none";if(r.closest("#profile-trigger"))o.stopPropagation(),this.toggleDropdown()}),window.addEventListener("click",()=>{if(this.dropdownOpen)this.toggleDropdown(!1)})}render(){let o=this.getAttribute("title")||"Task";this.shadowRoot.innerHTML=`
            <style>${Ar}</style>
            ${Kr({title:o,dropdownOpen:this.dropdownOpen})}
        `}}if(!customElements.get("app-header"))customElements.define("app-header",Mr);_();var to=(o)=>String(o).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;"),Da=(o)=>{if(!o)return"#";let r=o.trim().toLowerCase();if(r.startsWith("http://")||r.startsWith("https://")||r==="#")return to(o);return"#"},Rr=({bookmarks:o,isEditing:r,isSearching:a,isTouchDevice:t,maxCols:i=12})=>{let e=Array.isArray(o)?o:[],s=(l)=>e.filter((c)=>c.parent_id===l);return`
    ${(t?e:a?e:e.filter((l)=>!l.parent_id)).map((l)=>{let c={};try{c=typeof l.content==="string"?JSON.parse(l.content):l.content}catch(k){console.error("Failed to parse content for item",l.id,k)}let p=l.type==="section",x={x:l.x||1,y:l.y||1},h=Math.min(l.w||1,i),u=l.h||1;if(p){let k=s(l.id),G=(c.title||"").trim();return`
            <fieldset class="bookmark-grid__section"
               data-id="${l.id}"
               data-orig-x="${l.x}" data-orig-y="${l.y}"
               draggable="${r}"
               style="--x: ${x.x}; --y: ${x.y}; --w: ${h}; --h: ${u};">
               ...
               `:""}
            </fieldset>`}return Er(l,c,r,x,h,u,t)}).join("")}

    <div id="ghost-element" class="bookmark-grid__ghost" style="display: none;"></div>
    `};function Er(o,r,a,t,i,e=0,s=!1){let d=e||o.h||1;if(o.type==="widget"){let h=r.widgetId,k={clock:"widget-clock",notepad:"widget-notepad",telemetry:"widget-telemetry"}[h]||"div",m=(r.text||"").replace(/"/g,"&quot;");return`
        <div class="bookmark-grid__card"
            draggable="${a}"
            data-id="${o.id}"
            data-type="${o.type}"
            data-orig-x="${o.x}" data-orig-y="${o.y}"
            ${!s?`style="--x: ${t.x}; --y: ${t.y}; --w: ${i}; --h: ${d};"`:""}>
            ...
            ${a?`
            <div class="bookmark-actions">
                 ${["clock","telemetry"].includes(h)?`<button class="action-btn btn-edit" title="${n.t("general.edit")}">✎</button>`:""}
                 <button class="action-btn btn-delete" title="${n.t("general.delete")}">\uD83D\uDDD1</button>
            </div>
            ${["clock","telemetry"].includes(h)?"":'<div class="resize-handle"></div>'}
            `:""}
        </div>
    `}let l=r.icon||"",p=l.startsWith("http")||l.startsWith("/")?`<img src="${to(l)}" alt="${to(r.label)}" class="bookmark-grid__icon-img" draggable="false" />`:l?to(l):'<svg class="bookmark-grid__icon-svg" viewBox="0 0 24 24"><path d="M10 6L8.59 7.41 13.17 12l-4.58 4.59L10 18l6-6z"/></svg>';return`
        <a ${a?'role="button"':`href="${Da(r.url)}" target="_blank"`} class="bookmark-grid__card"
           draggable="${a}"
           data-id="${o.id}"
           data-type="${o.type}"
           data-orig-x="${o.x}" data-orig-y="${o.y}"
    ...
    `}function Xa(o,r,a=!1,t=12,i=!1){return o.map((e)=>{let s={};try{s=typeof e.content==="string"?JSON.parse(e.content):e.content}catch(p){console.error("Failed to parse content for item (nested)",e.id,p)}let d={x:e.x||1,y:e.y||1},l=e.h||1,c=Math.min(e.w||1,t);return Er(e,s,r,d,c,l,i)}).join("")}B();T();_();_();B();class Zr extends HTMLElement{timer;timeEl=null;dateEl=null;_unsubscribe=null;_unsubscribeI18n;constructor(){super();this.attachShadow({mode:"open"})}isEditing=!1;configMode=!1;_config={timezone:"local",hour12:!1,showDate:!0};_itemId=0;static get observedAttributes(){return["item-id","content","mode"]}attributeChangedCallback(o,r,a){if(o==="item-id")this._itemId=parseInt(a);if(o==="content")try{let t=typeof a==="string"?JSON.parse(a):a;if(t&&typeof t==="object")this._config={...this._config,...t},this.updateTime()}catch(t){}}connectedCallback(){this.render(),this.updateTime(),this.timer=setInterval(()=>{this.updateTime()},1000),this._unsubscribe=g.subscribe((o)=>{if(this.isEditing!==o.isEditing)this.isEditing=o.isEditing,this.render();if(this._itemId){let r=o.items.find((a)=>a.id===this._itemId);if(r&&r.content)try{let a=typeof r.content==="string"?JSON.parse(r.content):r.content;if(a.widgetId==="clock")this._config={...this._config,...a},this.updateTime()}catch(a){}}}),this._unsubscribeI18n=n.subscribe(()=>{this.updateTime()})}disconnectedCallback(){if(this._unsubscribe)this._unsubscribe();if(this._unsubscribeI18n)this._unsubscribeI18n();if(this.timer)clearInterval(this.timer)}updateTime(){if(!this.timeEl||!this.dateEl)return;let o=new Date,r={hour:"2-digit",minute:"2-digit",second:"2-digit",hour12:this._config.hour12},a=void 0;if(this._config.timezone&&this._config.timezone!=="local")a=this._config.timezone;let t=n.getLocale().code;try{let s=new Intl.DateTimeFormat(t,{...r,timeZone:a}).formatToParts(o).map((d)=>{if(d.type==="dayPeriod")return`<span class="ampm">${d.value.toUpperCase().replace(/\./g,"").trim()}</span>`;return d.value}).join("");this.timeEl.innerHTML=s}catch(i){this.timeEl.textContent=o.toLocaleTimeString()}if(this._config.showDate){let i={weekday:"long",day:"numeric",month:"long",timeZone:a};this.dateEl.textContent=o.toLocaleDateString(t,i),this.dateEl.style.display="block"}else this.dateEl.style.display="none"}render(){if(!this.shadowRoot)return;this.shadowRoot.innerHTML=`
            ...
            <div class="time">--:--:--</div>
            <div class="date">-- --</div>
        `,this.timeEl=this.shadowRoot.querySelector(".time"),this.dateEl=this.shadowRoot.querySelector(".date")}}customElements.define("widget-clock",Zr);var ko=globalThis,wo=ko.ShadowRoot&&(ko.ShadyCSS===void 0||ko.ShadyCSS.nativeShadow)&&"adoptedStyleSheets"in Document.prototype&&"replace"in CSSStyleSheet.prototype,Yo=Symbol(),Cr=new WeakMap;class $o{constructor(o,r,a){if(this._$cssResult$=!0,a!==Yo)throw Error("CSSResult is not constructable. Use `unsafeCSS` or `css` instead.");this.cssText=o,this.t=r}get styleSheet(){let o=this.o,r=this.t;if(wo&&o===void 0){let a=r!==void 0&&r.length===1;a&&(o=Cr.get(r)),o===void 0&&((this.o=o=new CSSStyleSheet).replaceSync(this.cssText),a&&Cr.set(r,o))}return o}toString(){return this.cssText}}var Yr=(o)=>new $o(typeof o=="string"?o:o+"",void 0,Yo),go=(o,...r)=>{let a=o.length===1?o[0]:r.reduce((t,i,e)=>t+((s)=>{if(s._$cssResult$===!0)return s.cssText;if(typeof s=="number")return s;throw Error("Value passed to 'css' function must be a 'css' function result: "+s+". Use 'unsafeCSS' to pass non-literal values, but take care to ensure page security.")})(i)+o[e+1],o[0]);return new $o(a,o,Yo)},Or=(o,r)=>{if(wo)o.adoptedStyleSheets=r.map((a)=>a instanceof CSSStyleSheet?a:a.styleSheet);else for(let a of r){let t=document.createElement("style"),i=ko.litNonce;i!==void 0&&t.setAttribute("nonce",i),t.textContent=a.cssText,o.appendChild(t)}},Oo=wo?(o)=>o:(o)=>o instanceof CSSStyleSheet?((r)=>{let a="";for(let t of r.cssRules)a+=t.cssText;return Yr(a)})(o):o;var{is:Pa,defineProperty:Wa,getOwnPropertyDescriptor:Ia,getOwnPropertyNames:Sa,getOwnPropertySymbols:ot,getPrototypeOf:rt}=Object,zo=globalThis,Nr=zo.trustedTypes,at=Nr?Nr.emptyScript:"",tt=zo.reactiveElementPolyfillSupport,xo=(o,r)=>o,bo={toAttribute(o,r){switch(r){case Boolean:o=o?at:null;break;case Object:case Array:o=o==null?o:JSON.stringify(o)}return o},fromAttribute(o,r){let a=o;switch(r){case Boolean:a=o!==null;break;case Number:a=o===null?null:Number(o);break;case Object:case Array:try{a=JSON.parse(o)}catch(t){a=null}}return a}},jo=(o,r)=>!Pa(o,r),Vr={attribute:!0,type:String,converter:bo,reflect:!1,useDefault:!1,hasChanged:jo};Symbol.metadata??=Symbol("metadata"),zo.litPropertyMetadata??=new WeakMap;class Y extends HTMLElement{static addInitializer(o){this._$Ei(),(this.l??=[]).push(o)}static get observedAttributes(){return this.finalize(),this._$Eh&&[...this._$Eh.keys()]}static createProperty(o,r=Vr){if(r.state&&(r.attribute=!1),this._$Ei(),this.prototype.hasOwnProperty(o)&&((r=Object.create(r)).wrapped=!0),this.elementProperties.set(o,r),!r.noAccessor){let a=Symbol(),t=this.getPropertyDescriptor(o,a,r);t!==void 0&&Wa(this.prototype,o,t)}}static getPropertyDescriptor(o,r,a){let{get:t,set:i}=Ia(this.prototype,o)??{get(){return this[r]},set(e){this[r]=e}};return{get:t,set(e){let s=t?.call(this);i?.call(this,e),this.requestUpdate(o,s,a)},configurable:!0,enumerable:!0}}static getPropertyOptions(o){return this.elementProperties.get(o)??Vr}static _$Ei(){if(this.hasOwnProperty(xo("elementProperties")))return;let o=rt(this);o.finalize(),o.l!==void 0&&(this.l=[...o.l]),this.elementProperties=new Map(o.elementProperties)}static finalize(){if(this.hasOwnProperty(xo("finalized")))return;if(this.finalized=!0,this._$Ei(),this.hasOwnProperty(xo("properties"))){let r=this.properties,a=[...Sa(r),...ot(r)];for(let t of a)this.createProperty(t,r[t])}let o=this[Symbol.metadata];if(o!==null){let r=litPropertyMetadata.get(o);if(r!==void 0)for(let[a,t]of r)this.elementProperties.set(a,t)}this._$Eh=new Map;for(let[r,a]of this.elementProperties){let t=this._$Eu(r,a);t!==void 0&&this._$Eh.set(t,r)}this.elementStyles=this.finalizeStyles(this.styles)}static finalizeStyles(o){let r=[];if(Array.isArray(o)){let a=new Set(o.flat(1/0).reverse());for(let t of a)r.unshift(Oo(t))}else o!==void 0&&r.push(Oo(o));return r}static _$Eu(o,r){let a=r.attribute;return a===!1?void 0:typeof a=="string"?a:typeof o=="string"?o.toLowerCase():void 0}constructor(){super(),this._$Ep=void 0,this.isUpdatePending=!1,this.hasUpdated=!1,this._$Em=null,this._$Ev()}_$Ev(){this._$ES=new Promise((o)=>this.enableUpdating=o),this._$AL=new Map,this._$E_(),this.requestUpdate(),this.constructor.l?.forEach((o)=>o(this))}addController(o){(this._$EO??=new Set).add(o),this.renderRoot!==void 0&&this.isConnected&&o.hostConnected?.()}removeController(o){this._$EO?.delete(o)}_$E_(){let o=new Map,r=this.constructor.elementProperties;for(let a of r.keys())this.hasOwnProperty(a)&&(o.set(a,this[a]),delete this[a]);o.size>0&&(this._$Ep=o)}createRenderRoot(){let o=this.shadowRoot??this.attachShadow(this.constructor.shadowRootOptions);return Or(o,this.constructor.elementStyles),o}connectedCallback(){this.renderRoot??=this.createRenderRoot(),this.enableUpdating(!0),this._$EO?.forEach((o)=>o.hostConnected?.())}enableUpdating(o){}disconnectedCallback(){this._$EO?.forEach((o)=>o.hostDisconnected?.())}attributeChangedCallback(o,r,a){this._$AK(o,a)}_$ET(o,r){let a=this.constructor.elementProperties.get(o),t=this.constructor._$Eu(o,a);if(t!==void 0&&a.reflect===!0){let i=(a.converter?.toAttribute!==void 0?a.converter:bo).toAttribute(r,a.type);this._$Em=o,i==null?this.removeAttribute(t):this.setAttribute(t,i),this._$Em=null}}_$AK(o,r){let a=this.constructor,t=a._$Eh.get(o);if(t!==void 0&&this._$Em!==t){let i=a.getPropertyOptions(t),e=typeof i.converter=="function"?{fromAttribute:i.converter}:i.converter?.fromAttribute!==void 0?i.converter:bo;this._$Em=t;let s=e.fromAttribute(r,i.type);this[t]=s??this._$Ej?.get(t)??s,this._$Em=null}}requestUpdate(o,r,a,t=!1,i){if(o!==void 0){let e=this.constructor;if(t===!1&&(i=this[o]),a??=e.getPropertyOptions(o),!((a.hasChanged??jo)(i,r)||a.useDefault&&a.reflect&&i===this._$Ej?.get(o)&&!this.hasAttribute(e._$Eu(o,a))))return;this.C(o,r,a)}this.isUpdatePending===!1&&(this._$ES=this._$EP())}C(o,r,{useDefault:a,reflect:t,wrapped:i},e){a&&!(this._$Ej??=new Map).has(o)&&(this._$Ej.set(o,e??r??this[o]),i!==!0||e!==void 0)||(this._$AL.has(o)||(this.hasUpdated||a||(r=void 0),this._$AL.set(o,r)),t===!0&&this._$Em!==o&&(this._$Eq??=new Set).add(o))}async _$EP(){this.isUpdatePending=!0;try{await this._$ES}catch(r){Promise.reject(r)}let o=this.scheduleUpdate();return o!=null&&await o,!this.isUpdatePending}scheduleUpdate(){return this.performUpdate()}performUpdate(){if(!this.isUpdatePending)return;if(!this.hasUpdated){if(this.renderRoot??=this.createRenderRoot(),this._$Ep){for(let[t,i]of this._$Ep)this[t]=i;this._$Ep=void 0}let a=this.constructor.elementProperties;if(a.size>0)for(let[t,i]of a){let{wrapped:e}=i,s=this[t];e!==!0||this._$AL.has(t)||s===void 0||this.C(t,void 0,i,s)}}let o=!1,r=this._$AL;try{o=this.shouldUpdate(r),o?(this.willUpdate(r),this._$EO?.forEach((a)=>a.hostUpdate?.()),this.update(r)):this._$EM()}catch(a){throw o=!1,this._$EM(),a}o&&this._$AE(r)}willUpdate(o){}_$AE(o){this._$EO?.forEach((r)=>r.hostUpdated?.()),this.hasUpdated||(this.hasUpdated=!0,this.firstUpdated(o)),this.updated(o)}_$EM(){this._$AL=new Map,this.isUpdatePending=!1}get updateComplete(){return this.getUpdateComplete()}getUpdateComplete(){return this._$ES}shouldUpdate(o){return!0}update(o){this._$Eq&&=this._$Eq.forEach((r)=>this._$ET(r,this[r])),this._$EM()}updated(o){}firstUpdated(o){}}Y.elementStyles=[],Y.shadowRootOptions={mode:"open"},Y[xo("elementProperties")]=new Map,Y[xo("finalized")]=new Map,tt?.({ReactiveElement:Y}),(zo.reactiveElementVersions??=[]).push("2.1.2");var No=globalThis,Ur=(o)=>o,Go=No.trustedTypes,Tr=Go?Go.createPolicy("lit-html",{createHTML:(o)=>o}):void 0;var O=`lit$${Math.random().toFixed(9).slice(2)}$`,Sr="?"+O,it=`<${Sr}>`,P=document,vo=()=>P.createComment(""),fo=(o)=>o===null||typeof o!="object"&&typeof o!="function",Vo=Array.isArray,nt=(o)=>Vo(o)||typeof o?.[Symbol.iterator]=="function";var ho=/<(?:(!--|\/[^a-zA-Z])|(\/?[a-zA-Z][^>\s]*)|(\/?$))/g,Dr=/-->/g,Xr=/>/g,D=RegExp(`>|[ 	
\f\r](?:([^\\s"'>=/]+)([ 	
\f\r]*=[ 	
\f\r]*(?:[^ 	
\f\r"'\`<>=]|("|')|))|$)`,"g"),Pr=/'/g,Wr=/"/g,oa=/^(?:script|style|textarea|title)$/i,Uo=(o)=>(r,...a)=>({_$litType$:o,strings:r,values:a}),b=Uo(1),Qi=Uo(2),Ki=Uo(3),W=Symbol.for("lit-noChange"),z=Symbol.for("lit-nothing"),Ir=new WeakMap,X=P.createTreeWalker(P,129);function ra(o,r){if(!Vo(o)||!o.hasOwnProperty("raw"))throw Error("invalid template strings array");return Tr!==void 0?Tr.createHTML(r):r}var et=(o,r)=>{let a=o.length-1,t=[],i,e=r===2?"<svg>":r===3?"<math>":"",s=ho;for(let d=0;d<a;d++){let l=o[d],c,p,x=-1,h=0;for(;h<l.length&&(s.lastIndex=h,p=s.exec(l),p!==null);)h=s.lastIndex,s===ho?p[1]==="!--"?s=Dr:p[1]!==void 0?s=Xr:p[2]!==void 0?(oa.test(p[2])&&(i=RegExp("</"+p[2],"g")),s=D):p[3]!==void 0&&(s=D):s===D?p[0]===">"?(s=i??ho,x=-1):p[1]===void 0?x=-2:(x=s.lastIndex-p[2].length,c=p[1],s=p[3]===void 0?D:p[3]==='"'?Wr:Pr):s===Wr||s===Pr?s=D:s===Dr||s===Xr?s=ho:(s=D,i=void 0);let u=s===D&&o[d+1].startsWith("/>")?" ":"";e+=s===ho?l+it:x>=0?(t.push(c),l.slice(0,x)+"$lit$"+l.slice(x)+O+u):l+O+(x===-2?d:u)}return[ra(o,e+(o[a]||"<?>")+(r===2?"</svg>":r===3?"</math>":"")),t]};class uo{constructor({strings:o,_$litType$:r},a){let t;this.parts=[];let i=0,e=0,s=o.length-1,d=this.parts,[l,c]=et(o,r);if(this.el=uo.createElement(l,a),X.currentNode=this.el.content,r===2||r===3){let p=this.el.content.firstChild;p.replaceWith(...p.childNodes)}for(;(t=X.nextNode())!==null&&d.length<s;){if(t.nodeType===1){if(t.hasAttributes())for(let p of t.getAttributeNames())if(p.endsWith("$lit$")){let x=c[e++],h=t.getAttribute(p).split(O),u=/([.?@])?(.*)/.exec(x);d.push({type:1,index:i,name:u[2],strings:h,ctor:u[1]==="."?ta:u[1]==="?"?ia:u[1]==="@"?na:mo}),t.removeAttribute(p)}else p.startsWith(O)&&(d.push({type:6,index:i}),t.removeAttribute(p));if(oa.test(t.tagName)){let p=t.textContent.split(O),x=p.length-1;if(x>0){t.textContent=Go?Go.emptyScript:"";for(let h=0;h<x;h++)t.append(p[h],vo()),X.nextNode(),d.push({type:2,index:++i});t.append(p[x],vo())}}}else if(t.nodeType===8)if(t.data===Sr)d.push({type:2,index:i});else{let p=-1;for(;(p=t.data.indexOf(O,p+1))!==-1;)d.push({type:7,index:i}),p+=O.length-1}i++}}static createElement(o,r){let a=P.createElement("template");return a.innerHTML=o,a}}function io(o,r,a=o,t){if(r===W)return r;let i=t!==void 0?a._$Co?.[t]:a._$Cl,e=fo(r)?void 0:r._$litDirective$;return i?.constructor!==e&&(i?._$AO?.(!1),e===void 0?i=void 0:(i=new e(o),i._$AT(o,a,t)),t!==void 0?(a._$Co??=[])[t]=i:a._$Cl=i),i!==void 0&&(r=io(o,i._$AS(o,r.values),i,t)),r}class aa{constructor(o,r){this._$AV=[],this._$AN=void 0,this._$AD=o,this._$AM=r}get parentNode(){return this._$AM.parentNode}get _$AU(){return this._$AM._$AU}u(o){let{el:{content:r},parts:a}=this._$AD,t=(o?.creationScope??P).importNode(r,!0);X.currentNode=t;let i=X.nextNode(),e=0,s=0,d=a[0];for(;d!==void 0;){if(e===d.index){let l;d.type===2?l=new _o(i,i.nextSibling,this,o):d.type===1?l=new d.ctor(i,d.name,d.strings,this,o):d.type===6&&(l=new ea(i,this,o)),this._$AV.push(l),d=a[++s]}e!==d?.index&&(i=X.nextNode(),e++)}return X.currentNode=P,t}p(o){let r=0;for(let a of this._$AV)a!==void 0&&(a.strings!==void 0?(a._$AI(o,a,r),r+=a.strings.length-2):a._$AI(o[r])),r++}}class _o{get _$AU(){return this._$AM?._$AU??this._$Cv}constructor(o,r,a,t){this.type=2,this._$AH=z,this._$AN=void 0,this._$AA=o,this._$AB=r,this._$AM=a,this.options=t,this._$Cv=t?.isConnected??!0}get parentNode(){let o=this._$AA.parentNode,r=this._$AM;return r!==void 0&&o?.nodeType===11&&(o=r.parentNode),o}get startNode(){return this._$AA}get endNode(){return this._$AB}_$AI(o,r=this){o=io(this,o,r),fo(o)?o===z||o==null||o===""?(this._$AH!==z&&this._$AR(),this._$AH=z):o!==this._$AH&&o!==W&&this._(o):o._$litType$!==void 0?this.$(o):o.nodeType!==void 0?this.T(o):nt(o)?this.k(o):this._(o)}O(o){return this._$AA.parentNode.insertBefore(o,this._$AB)}T(o){this._$AH!==o&&(this._$AR(),this._$AH=this.O(o))}_(o){this._$AH!==z&&fo(this._$AH)?this._$AA.nextSibling.data=o:this.T(P.createTextNode(o)),this._$AH=o}$(o){let{values:r,_$litType$:a}=o,t=typeof a=="number"?this._$AC(o):(a.el===void 0&&(a.el=uo.createElement(ra(a.h,a.h[0]),this.options)),a);if(this._$AH?._$AD===t)this._$AH.p(r);else{let i=new aa(t,this),e=i.u(this.options);i.p(r),this.T(e),this._$AH=i}}_$AC(o){let r=Ir.get(o.strings);return r===void 0&&Ir.set(o.strings,r=new uo(o)),r}k(o){Vo(this._$AH)||(this._$AH=[],this._$AR());let r=this._$AH,a,t=0;for(let i of o)t===r.length?r.push(a=new _o(this.O(vo()),this.O(vo()),this,this.options)):a=r[t],a._$AI(i),t++;t<r.length&&(this._$AR(a&&a._$AB.nextSibling,t),r.length=t)}_$AR(o=this._$AA.nextSibling,r){for(this._$AP?.(!1,!0,r);o!==this._$AB;){let a=Ur(o).nextSibling;Ur(o).remove(),o=a}}setConnected(o){this._$AM===void 0&&(this._$Cv=o,this._$AP?.(o))}}class mo{get tagName(){return this.element.tagName}get _$AU(){return this._$AM._$AU}constructor(o,r,a,t,i){this.type=1,this._$AH=z,this._$AN=void 0,this.element=o,this.name=r,this._$AM=t,this.options=i,a.length>2||a[0]!==""||a[1]!==""?(this._$AH=Array(a.length-1).fill(new String),this.strings=a):this._$AH=z}_$AI(o,r=this,a,t){let i=this.strings,e=!1;if(i===void 0)o=io(this,o,r,0),e=!fo(o)||o!==this._$AH&&o!==W,e&&(this._$AH=o);else{let s=o,d,l;for(o=i[0],d=0;d<i.length-1;d++)l=io(this,s[a+d],r,d),l===W&&(l=this._$AH[d]),e||=!fo(l)||l!==this._$AH[d],l===z?o=z:o!==z&&(o+=(l??"")+i[d+1]),this._$AH[d]=l}e&&!t&&this.j(o)}j(o){o===z?this.element.removeAttribute(this.name):this.element.setAttribute(this.name,o??"")}}class ta extends mo{constructor(){super(...arguments),this.type=3}j(o){this.element[this.name]=o===z?void 0:o}}class ia extends mo{constructor(){super(...arguments),this.type=4}j(o){this.element.toggleAttribute(this.name,!!o&&o!==z)}}class na extends mo{constructor(o,r,a,t,i){super(o,r,a,t,i),this.type=5}_$AI(o,r=this){if((o=io(this,o,r,0)??z)===W)return;let a=this._$AH,t=o===z&&a!==z||o.capture!==a.capture||o.once!==a.once||o.passive!==a.passive,i=o!==z&&(a===z||t);t&&this.element.removeEventListener(this.name,this,a),i&&this.element.addEventListener(this.name,this,o),this._$AH=o}handleEvent(o){typeof this._$AH=="function"?this._$AH.call(this.options?.host??this.element,o):this._$AH.handleEvent(o)}}class ea{constructor(o,r,a){this.element=o,this.type=6,this._$AN=void 0,this._$AM=r,this.options=a}get _$AU(){return this._$AM._$AU}_$AI(o){io(this,o)}}var st=No.litHtmlPolyfillSupport;st?.(uo,_o),(No.litHtmlVersions??=[]).push("3.3.2");var sa=(o,r,a)=>{let t=a?.renderBefore??r,i=t._$litPart$;if(i===void 0){let e=a?.renderBefore??null;t._$litPart$=i=new _o(r.insertBefore(vo(),e),e,void 0,a??{})}return i._$AI(o),i};var To=globalThis;class K extends Y{constructor(){super(...arguments),this.renderOptions={host:this},this._$Do=void 0}createRenderRoot(){let o=super.createRenderRoot();return this.renderOptions.renderBefore??=o.firstChild,o}update(o){let r=this.render();this.hasUpdated||(this.renderOptions.isConnected=this.isConnected),super.update(o),this._$Do=sa(r,this.renderRoot,this.renderOptions)}connectedCallback(){super.connectedCallback(),this._$Do?.setConnected(!0)}disconnectedCallback(){super.disconnectedCallback(),this._$Do?.setConnected(!1)}render(){return W}}K._$litElement$=!0,K.finalized=!0,To.litElementHydrateSupport?.({LitElement:K});var dt=To.litElementPolyfillSupport;dt?.({LitElement:K});(To.litElementVersions??=[]).push("4.2.2");var Fo=(o)=>(r,a)=>{a!==void 0?a.addInitializer(()=>{customElements.define(o,r)}):customElements.define(o,r)};var lt={attribute:!0,type:String,converter:bo,reflect:!1,hasChanged:jo},pt=(o=lt,r,a)=>{let{kind:t,metadata:i}=a,e=globalThis.litPropertyMetadata.get(i);if(e===void 0&&globalThis.litPropertyMetadata.set(i,e=new Map),t==="setter"&&((o=Object.create(o)).wrapped=!0),e.set(a.name,o),t==="accessor"){let{name:s}=a;return{set(d){let l=r.get.call(this);r.set.call(this,d),this.requestUpdate(s,l,o,!0,d)},init(d){return d!==void 0&&this.C(s,void 0,o,d),d}}}if(t==="setter"){let{name:s}=a;return function(d){let l=this[s];r.call(this,d),this.requestUpdate(s,l,o,!0,d)}}throw Error("Unsupported decorator location: "+t)};function A(o){return(r,a)=>typeof a=="object"?pt(o,r,a):((t,i,e)=>{let s=i.hasOwnProperty(e);return i.constructor.createProperty(e,t),s?Object.getOwnPropertyDescriptor(i,e):void 0})(o,r,a)}function M(o){return A({...o,state:!0,attribute:!1})}var I=(o,r,a)=>(a.configurable=!0,a.enumerable=!0,Reflect.decorate&&typeof r!="object"&&Object.defineProperty(o,r,a),a);function da(o,r){return(a,t,i)=>{let e=(s)=>s.renderRoot?.querySelector(o)??null;if(r){let{get:s,set:d}=typeof t=="object"?a:i??(()=>{let l=Symbol();return{get(){return this[l]},set(c){this[l]=c}}})();return I(a,t,{get(){let l=s.call(this);return l===void 0&&(l=e(this),(l!==null||this.hasUpdated)&&d.call(this,l)),l}})}return I(a,t,{get(){return e(this)}})}}B();_();var ct={allowedTags:["h1","h2","h3","h4","h5","h6","p","br","div","span","strong","b","em","i","u","s","strike","a","img","ul","ol","li","pre","code","blockquote","label","input"],allowedAttrs:{"*":["style","class"],a:["href","title","target"],img:["src","alt","title","width","height"],input:["type","id","checked"],label:["for"]},allowedSchemes:["http","https","data","mailto"]};class la{config;constructor(o){this.config={...ct,...o}}sanitize(o){if(!o||typeof o!=="string")return"";let a=new DOMParser().parseFromString(o,"text/html");return this.cleanNode(a.body),a.body.innerHTML}cleanNode(o){let r=Array.from(o.childNodes);for(let a=r.length-1;a>=0;a--){let t=r[a];if(t.nodeType===Node.TEXT_NODE)continue;if(t.nodeType===Node.COMMENT_NODE){o.removeChild(t);continue}if(t.nodeType===Node.ELEMENT_NODE){let i=t,e=i.tagName.toLowerCase();if(!this.config.allowedTags.includes(e)){o.removeChild(t);continue}this.cleanAttributes(i),this.cleanNode(i)}else o.removeChild(t)}}cleanAttributes(o){let r=o.tagName.toLowerCase(),a=[...this.config.allowedAttrs["*"]||[],...this.config.allowedAttrs[r]||[]],t=Array.from(o.attributes);for(let i of t){let e=i.name.toLowerCase();if(!a.includes(e)){o.removeAttribute(i.name);continue}if(e==="href"||e==="src"){let s=i.value.trim();if(this.isDangerousUrl(s)){o.removeAttribute(i.name);continue}}if(e==="style"){let s=this.sanitizeStyle(i.value);if(s!==i.value)o.setAttribute("style",s)}if(r==="input"&&e==="type"){if(i.value.toLowerCase()!=="checkbox")o.setAttribute("type","checkbox")}}}isDangerousUrl(o){let r=o.toLowerCase().trim();if(r.startsWith("javascript:"))return!0;if(r.startsWith("data:")&&r.includes("script"))return!0;if(r.startsWith("vbscript:"))return!0;if(r.includes(":")){let a=r.split(":")[0];if(!this.config.allowedSchemes.includes(a))return!0}return!1}sanitizeStyle(o){let r=["behavior","expression","moz-binding","-o-link","-o-link-source","import","@import"],a=o;for(let t of r){let i=new RegExp(t,"gi");a=a.replace(i,"")}return a=a.replace(/url\s*\(\s*['"]?javascript:/gi,"url(about:blank"),a}}var S=new la;class Lo{static parse(o){try{if(typeof o==="object"&&o!==null)return o;if(typeof o==="string"){let r=JSON.parse(o);if(typeof r==="object"&&r!==null)return r}return{widgetId:"unknown"}}catch(r){return console.warn("[WidgetContentHelper] Failed to parse content:",r),{widgetId:"unknown"}}}static serialize(o){try{return JSON.stringify(o)}catch(r){return console.error("[WidgetContentHelper] Failed to serialize content:",r),"{}"}}static getNotepadText(o){return this.parse(o).text||""}static setNotepadText(o,r){let t={...this.parse(o),widgetId:"notepad",text:r};return this.serialize(t)}static merge(o,r){let t={...this.parse(o),...r};return this.serialize(t)}static validate(o,r){return this.parse(o).widgetId===r}}_();class Do extends K{constructor(){super(...arguments);this.blockId="";this.items=[];this.readonly=!1;this.focusedItemId=null}static styles=go`
        :host {
            display: block;
            background: var(--surface);
            border-radius: 8px;
    ...
    `;connectedCallback(){super.connectedCallback(),this._unsubscribe=g.subscribe((r)=>{let a=this.isDashboardEditing;if(this.isDashboardEditing=r.isEditing||!1,a!==this.isDashboardEditing)if(this.isDashboardEditing)if(this.hasUnsavedChanges&&this.isInternalEditing)if(confirm(n.t("widget.notepad.confirm_discard")))this.isInternalEditing=!1,this.hasUnsavedChanges=!1,this.cancelAutosave(),this.requestUpdate();else setTimeout(()=>g.toggleEditMode(),0);else this.isInternalEditing=!1,this.cancelAutosave(),this.requestUpdate();else this.loadFromStore()});let o=g.getState();this.isDashboardEditing=o.isEditing||!1,this.loadFromStore()}disconnectedCallback(){if(super.disconnectedCallback(),this._unsubscribe)this._unsubscribe();this.cancelAutosave()}updated(o){if(super.updated(o),o.has("content")&&!this.isInternalEditing&&this.editorElement){let r=S.sanitize(this.content);if(this.editorElement.innerHTML!==r)this.editorElement.innerHTML=r}}loadFromStore(){if(!this.itemId)return;let r=g.getState().items.find((a)=>a.id===this.itemId);if(r){let a=Lo.getNotepadText(r.content);try{let t=JSON.parse(a);if(t&&typeof t==="object"&&"html"in t)this.content=t.html||"",this.checklistBlocks=t.checklists||[];else this.content=a,this.checklistBlocks=[]}catch{this.content=a,this.checklistBlocks=[]}this.characterCount=this.content.length,this._lastSavedContent=a}}exec(o,r){if(document.execCommand(o,!1,r),this.editorElement)this.editorElement.focus()}cancelAutosave(){if(this._autosaveTimeout)clearTimeout(this._autosaveTimeout),this._autosaveTimeout=null}async saveContent(o=!1){if(this._savePromise){await this._savePromise;return}try{let r=this.shadowRoot?.querySelector(".editor")||this.shadowRoot?.querySelector("[contenteditable]");if(!r)throw Error("Critical: Editor element missing");let a=r.innerHTML;if(a.length>qo){alert(n.t("widget.notepad.error.too_large"));return}let t=S.sanitize(a),i=JSON.stringify({html:t,checklists:this.checklistBlocks});if(!o)this.content=t,this.characterCount=t.length,this.isInternalEditing=!1,this.hasUnsavedChanges=!1,this.requestUpdate();this._savePromise=(async()=>{if(!this.itemId)return;if(!o)this.isSaving=!0,this.requestUpdate();try{let e=g.getState().items.find((d)=>d.id===this.itemId);if(!e){console.warn("[NotepadWidget] Item no longer exists");return}let s=Lo.setNotepadText(e.content,i);if(await g.updateItem({id:this.itemId,content:s}),this._lastSavedContent=i,o)this.hasUnsavedChanges=!1}catch(e){if(console.error("[NotepadWidget] Save failed:",e),!o)alert(n.t("widget.notepad.error.save")+e)}finally{if(!o)this.isSaving=!1,this.requestUpdate()}})(),await this._savePromise}catch(r){if(console.error("[NotepadWidget] Save error:",r),!o)alert(n.t("widget.notepad.error.save")+r)}finally{this._savePromise=null}}startEditing(){if(this.isDashboardEditing)return;this.isInternalEditing=!0,setTimeout(()=>{if(this.editorElement){this.editorElement.focus();let o=S.sanitize(this.content);if(this.editorElement.innerHTML!==o)this.editorElement.innerHTML=o;this.focusEditor(!0)}},0)}focusEditor(o=!0){if(!this.editorElement)return;if(this.editorElement.focus(),o){let r=window.getSelection();if(r){let a=document.createRange();a.selectNodeContents(this.editorElement),a.collapse(!1),r.removeAllRanges(),r.addRange(a)}}}handleColor(o){let r=o.target;this.exec("foreColor",r.value)}handleToolbarWheel(o){o.preventDefault();let r=o.currentTarget;r.scrollLeft+=o.deltaY}insertChecklist(){let o="block-"+Math.random().toString(36).substring(2,11),r={id:o,items:[{id:"item-"+Math.random().toString(36).substring(2,11),checked:!1,text:""}]};this.checklistBlocks=[...this.checklistBlocks,r],this.hasUnsavedChanges=!0,this.scheduleAutosave(),this.updateComplete.then(()=>{this.shadowRoot?.querySelector(`checklist-block[block-id="${o}"]`)?.shadowRoot?.querySelector('input[type="text"]')?.focus()})}insertCode(){let r=`<pre><code>${n.t("widget.notepad.prompt.code_block")}</code></pre><p><br></p>`;this.exec("insertHTML",r),setTimeout(()=>{let a=window.getSelection();if(a&&this.editorElement){let t=this.editorElement.querySelectorAll("code"),i=t[t.length-1];if(i){let e=document.createRange();e.selectNodeContents(i),a.removeAllRanges(),a.addRange(e)}}},0)}handleChecklistChange(o){let{blockId:r,items:a}=o.detail;this.checklistBlocks=this.checklistBlocks.map((t)=>t.id===r?{...t,items:a}:t),this.hasUnsavedChanges=!0,this.scheduleAutosave()}deleteChecklistBlock(o){this.checklistBlocks=this.checklistBlocks.filter((r)=>r.id!==o),this.hasUnsavedChanges=!0,this.scheduleAutosave()}handleEditorInput(o){let r=o.target;if(this.characterCount=r.innerHTML.length,this.hasUnsavedChanges=r.innerHTML!==this._lastSavedContent,this.cancelAutosave(),pa>0)this._autosaveTimeout=setTimeout(()=>{if(this.hasUnsavedChanges)this.saveContent(!0)},pa)}handleEditorKeydown(o){if(o.ctrlKey||o.metaKey)switch(o.key.toLowerCase()){case"b":o.preventDefault(),this.exec("bold");return;case"i":o.preventDefault(),this.exec("italic");return;case"s":o.preventDefault(),this.saveContent(!1);return;case"z":if(o.shiftKey)o.preventDefault(),this.exec("redo");else o.preventDefault(),this.exec("undo");return;case"k":o.preventDefault();let r=prompt(n.t("widget.notepad.prompt.url"));if(r)this.exec("createLink",r);return}}handlePaste(o){let r=o.clipboardData?.items;if(!r)return;for(let a of Array.from(r))if(a.type.indexOf("image")!==-1){o.preventDefault();let t=a.getAsFile();if(!t)continue;let i=new FileReader;i.onload=(e)=>{let s=e.target?.result;if(s.length>1e6){alert(n.t("widget.notepad.error.image_too_large"));return}this.exec("insertImage",s)},i.readAsDataURL(t);return}}render(){try{if(!this.isInternalEditing){let t=S.sanitize(this.content)||`<span style='opacity:0.5; font-style:italic; color: var(--text-dim);'>${n.t("widget.notepad.placeholder")}</span>`;return b`
                ...
                `}let o=this.characterCount>qo*0.9,r=this.characterCount>qo;return b`
                <div class="container">
                    ...
                    <div class="content-area editor"
                         ...
                         .innerHTML="${S.sanitize(this.content)}">
                    </div>
        ...
        `,this.cpuBar=this.shadowRoot.querySelector(".cpu-bar"),this.ramBar=this.shadowRoot.querySelector(".ram-bar"),this.tempBar=this.shadowRoot.querySelector(".temp-bar"),this.cpuText=this.shadowRoot.querySelector(".cpu-text"),this.ramText=this.shadowRoot.querySelector(".ram-text"),this.tempText=this.shadowRoot.querySelector(".temp-text")}}customElements.define("widget-telemetry",ca);var ga=`:host {
    /* Fixed Grid System */
...
}`;class wa extends HTMLElement{icons=[];filteredIcons=[];selectedIcon="";searchQuery="";isLoading=!0;debounceTimer=null;inputElement=null;constructor(){super();this.attachShadow({mode:"open"})}async connectedCallback(){if(this.shadowRoot.childElementCount>0)return;if(this.renderBase(),this.setupListeners(),this.icons.length===0)await this.loadIcons()}async loadIcons(){try{this.icons=await Wo.loadIcons(),this.filteredIcons=this.icons.slice(0,50),this.isLoading=!1,this.updateGrid()}catch(o){console.error("[IconPicker] Failed to load icons",o),this.isLoading=!1,this.updateGrid()}}setupListeners(){let o=this.shadowRoot;this.inputElement=o.getElementById("icon-search"),this.inputElement?.addEventListener("input",(a)=>{let t=a.target;if(this.searchQuery=t.value,this.debounceTimer)window.clearTimeout(this.debounceTimer);this.debounceTimer=window.setTimeout(()=>{this.performSearch()},100)}),o.getElementById("grid-container")?.addEventListener("click",(a)=>{let i=a.target.closest(".icon-picker__item");if(i&&i.dataset.icon)this.selectedIcon=i.dataset.icon,this.updateSelected(),this.updateGrid(),this.dispatchEvent(new CustomEvent("icon-selected",{detail:{iconName:this.selectedIcon},bubbles:!0,composed:!0}))})}performSearch(){this.filteredIcons=Wo.searchIcons(this.searchQuery,50),this.updateGrid()}getSelectedIcon(){return this.selectedIcon}setSelectedIcon(o){this.selectedIcon=o,this.updateSelected(),this.updateGrid()}renderBase(){this.shadowRoot.innerHTML=`
            <style>${ka}</style>
            ${ua()}
        `}updateGrid(){let o=this.shadowRoot.getElementById("grid-container");if(o){if(!this.searchQuery.trim()&&!this.isLoading){o.style.display="none",o.innerHTML="";return}o.style.display="grid",o.innerHTML=ma(this.filteredIcons,this.selectedIcon,this.isLoading)}}updateSelected(){let o=this.shadowRoot.getElementById("selected-container");if(o)o.innerHTML=_a(this.selectedIcon)}}if(!customElements.get("icon-picker"))customElements.define("icon-picker",wa);_();var $a=({isOpen:o,isEditMode:r})=>`
    <dialog id="modal">
        <div class="modal-header">
...
.visibility-row .checkbox-group {
    ...
    margin-bottom: 0;
        ...
        `,this.dialog=this.shadowRoot.querySelector("dialog"),this.shadowRoot.querySelectorAll(".card").forEach((o)=>{o.addEventListener("click",()=>{let r=o.getAttribute("data-id"),a=Io.find((t)=>t.id===r);if(a)this.selectWidget(a)})})}}customElements.define("add-widget-modal",Ga);B();_();var Fa=`:host {
    display: contents;
}
...
.actions app-button {
    min-width: 120px;
}`;var So=new Map;async function ft(o,r){try{let a=await fetch(`https://timeapi.io/api/timezone/coordinate?latitude=${o}&longitude=${r}`);if(!a.ok)throw Error("TimeAPI failed");return(await a.json()).timeZone||"local"}catch(a){return console.error("[Timezone] TimeAPI error:",a),"local"}}async function ut(o){if(!o||o.trim()==="")return"local";let r=o.toLowerCase().trim();if(So.has(r))return So.get(r);try{let a=`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(o)}&limit=1`,t=await fetch(a,{headers:{"User-Agent":"Lastboard/1.0"}});if(!t.ok)throw Error("Geocoding failed");let i=await t.json();if(i.length===0)return"local";let{lat:e,lon:s}=i[0],d=await ft(parseFloat(e),parseFloat(s));return So.set(r,d),d}catch(a){return console.error("[Timezone] Error resolving city:",o,a),"local"}}class La extends HTMLElement{dialog=null;currentItem=null;constructor(){super();this.attachShadow({mode:"open"})}connectedCallback(){this.render()}open(o){if(this.currentItem=o,this.render(),this.dialog)this.dialog.showModal()}close(){if(this.dialog)this.dialog.close();this.currentItem=null}async save(){if(!this.currentItem)return;let o=typeof this.currentItem.content==="string"?JSON.parse(this.currentItem.content):this.currentItem.content,r=o.widgetId,a={...o};if(r==="clock"){let t=this.shadowRoot?.getElementById("clock-city"),i=this.shadowRoot?.getElementById("clock-12h"),e=this.shadowRoot?.getElementById("clock-date"),s=this.shadowRoot?.getElementById("save-btn"),d=t?.value||"";if(s)s.disabled=!0,s.textContent=n.t("general.loading");let l=await ut(d);if(a.city=d,a.timezone=l,a.hour12=i?.checked||!1,a.showDate=e?.checked||!1,s)s.disabled=!1,s.textContent=n.t("general.save")}else if(r==="telemetry"){let t=this.shadowRoot?.getElementById("telemetry-interval");a.interval=t?parseInt(t.value):1000}if(this.currentItem.type==="section"){let t=this.shadowRoot?.getElementById("section-title");a.title=t?t.value:"",delete a.name}await g.updateItem({id:this.currentItem.id,content:JSON.stringify(a)}),this.close()}render(){if(!this.shadowRoot)return;let o={},r="";if(this.currentItem)o=typeof this.currentItem.content==="string"?JSON.parse(this.currentItem.content):this.currentItem.content,r=o.widgetId;let a=(s)=>String(s).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;"),t=()=>{if(!this.currentItem)return"";if(r==="clock"){let s=o.city||"",d=o.hour12||!1,l=o.showDate!==!1;return`
        ...
        `,this.dialog=this.shadowRoot.querySelector("dialog");let e=this.shadowRoot.getElementById("telemetry-interval");if(e)e.options=[{value:"1000",label:"1s"},{value:"2000",label:"2s"},{value:"5000",label:"5s"},{value:"10000",label:"10s"}];this.shadowRoot.getElementById("close-btn")?.addEventListener("click",()=>this.close()),this.shadowRoot.getElementById("save-btn")?.addEventListener("click",()=>this.save())}}customElements.define("widget-config-modal",La);_();var qa=({isOpen:o,title:r,message:a})=>`
    <dialog id="modal">
        <div class="modal-header">
...
.modal-actions {
    ...
    justify-content: flex-end;
        ...
        `}}Ho();var q=document.getElementById("main-topbar"),Qa=document.getElementById("right-drawer"),Bo=document.getElementById("dashboard-root"),so,Jo,Qo;Ja(async()=>{Q.init(),await w.fetchUser();let o=w.getUser();if(o){if(o.language)await n.setLanguage(o.language);if(o.preferences&&o.preferences.grid_columns)document.documentElement.style.setProperty("--user-preferred-columns",o.preferences.grid_columns.toString());if(o.theme==="dark")Q.enableDark();else if(o.theme==="light")Q.enableLight();else if(window.matchMedia&&window.matchMedia("(prefers-color-scheme: dark)").matches)Q.enableDark();else Q.enableLight();if(o.id)g.setUserId(o.id)}if(q){if(q.setState({user:o}),o&&o.project_name)q.setAttribute("title",o.project_name);w.subscribe((a)=>{if(a){if(q.setState({user:a}),a.project_name)q.setAttribute("title",a.project_name)}})}await g.fetchItems(),mt(),co.start(),so=document.createElement("add-bookmark-modal"),document.body.appendChild(so),Jo=document.createElement("add-widget-modal"),document.body.appendChild(Jo),Qo=document.createElement("confirmation-modal"),document.body.appendChild(Qo);let r=document.createElement("widget-config-modal");document.body.appendChild(r)});eo.on(no.SHOW_CONFIRMATION,async(o)=>{let{title:r,message:a,onConfirm:t}=o.detail;if(Qo){if(await Qo.confirm(r,a)&&t)t()}});eo.on(no.SHOW_WIDGET_CONFIG,(o)=>{let{item:r,type:a}=o.detail;if(a==="bookmark"){if(so)so.openForEdit(r)}else{let t=document.querySelector("widget-config-modal");if(t)t.open(r)}});eo.on(no.NOTIFY,(o)=>{let r=document.querySelector("app-notifier");if(r)r.show(o.detail.message,o.detail.type)});if(q)q.addEventListener("drawer-toggle",(o)=>{if(o.detail.action==="open")Qa.open(),q.setState({drawerOpen:!0});else Qa.close(),q.setState({drawerOpen:!1})}),q.addEventListener("edit-mode-change",(o)=>{let r=o.detail.active;if(Bo.classList.toggle("edit-mode",r),!r)window.location.reload()}),q.addEventListener("search-input",(o)=>{let r=o.detail.query;g.setSearchQuery(r)}),q.addEventListener("add-item",async(o)=>{let r=o.detail.action;if(r==="add-bookmark"){if(so)so.open()}else if(r==="add-widget"){if(Jo)Jo.open()}else if(r==="add-section"){let t=g.getState().items||[],{collisionService:i}=await Promise.resolve().then(() => (T(),ao)),e=i.findFirstAvailableSlot(1,1,t),s={type:"section",x:e.x,y:e.y,w:1,h:1,content:JSON.stringify({title:""})},d=await g.addItem(s);if(d)eo.emit(no.SHOW_WIDGET_CONFIG,{item:d,type:"section"})}});window.addEventListener("widget-selected",async(o)=>{let a=o.detail,i=g.getState().items||[],{collisionService:e}=await Promise.resolve().then(() => (T(),ao)),s=e.findFirstAvailableSlot(a.defaultW,a.defaultH,i),d={type:"widget",x:s.x,y:s.y,w:a.defaultW,h:a.defaultH,content:JSON.stringify({widgetId:a.id,text:a.id==="notepad"?"":void 0})};await g.addItem(d)});window.addEventListener("drawer-close",()=>{if(q)q.setState({drawerOpen:!1})});window.addEventListener("click",(o)=>{let r=o.target,a=document.getElementById("add-menu"),t=document.getElementById("add-toggle");if(a&&a.classList.contains("active")&&!a.contains(r)&&t&&!t.contains(r))a.classList.remove("active")});function mt(){if(!Bo)return;Bo.innerHTML="";let o=document.createElement("bookmark-grid");Bo.appendChild(o)}
