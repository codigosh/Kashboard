...
.toast--error {
    border-left-color: var(--danger-color);
            ...
            <style>${Hr}</style>
            ${Ar()}
        `}}if(!customElements.get("app-notifier"))customElements.define("app-notifier",Mr);v();var Fr=()=>`
...
}`;class Sr extends HTMLElement{dialog=null;titleText=i.t("general.confirm");messageText="Please enter your password to continue";resolvePromise=null;_unsubscribeI18n;constructor(){super();this.attachShadow({mode:"open"})}connectedCallback(){this.render(),this._unsubscribeI18n=i.subscribe(()=>{if(this.dialog?.open)this.render()})}disconnectedCallback(){if(this._unsubscribeI18n)this._unsubscribeI18n()}setupDynamicListeners(){let o=this.shadowRoot.getElementById("modal-close"),r=this.shadowRoot.getElementById("btn-confirm"),t=this.shadowRoot.getElementById("password-confirm"),a=this.shadowRoot.querySelector(".password-input-group");this.dialog=this.shadowRoot.getElementById("modal");let n=(s)=>{if(this.resolvePromise)this.resolvePromise(s),this.resolvePromise=null;if(this.dialog?.close(),t)t.value=""},e=()=>{let s=t?.value||"";if(s.trim()===""){if(window.notifier)window.notifier.show(i.t("notifier.password_required"),"error");if(a)a.classList.remove("shake"),a.offsetWidth,a.classList.add("shake");t?.focus();return}n(s)};if(o)o.onclick=()=>n(null);if(r)r.onclick=()=>e();if(t)t.onkeydown=(s)=>{if(s.key==="Enter")s.preventDefault(),e()},t.oninput=()=>{if(a)a.classList.remove("shake")};if(this.dialog)this.dialog.onclick=(s)=>{if(s.target===this.dialog)n(null)}}async prompt(o,r){this.titleText=o,this.messageText=r,this.render(),this.dialog?.showModal();let t=this.shadowRoot.getElementById("password-confirm");if(t)t.value="",setTimeout(()=>t.focus(),50);return new Promise((a)=>{this.resolvePromise=a})}render(){this.shadowRoot.innerHTML=`
            <style>${Ir}</style>
