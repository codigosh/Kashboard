...
.paper {
    ...
    height: 100%;
            ...
            <style>${Ho}</style>
            ${Go()}
        `}}if(!customElements.get("app-paper"))customElements.define("app-paper",Ro);var Fo=`:host {
...
.avatar__initials {
    user-select: none;
}`;class Zo extends HTMLElement{static get observedAttributes(){return["src","alt","initials"]}constructor(){super();this.attachShadow({mode:"open"})}connectedCallback(){this.render()}attributeChangedCallback(o,r,t){if(r!==t)this.render()}render(){let o=this.getAttribute("src"),r=this.getAttribute("initials")||"??",t=this.getAttribute("alt")||"User Avatar";this.shadowRoot.innerHTML=`
            <style>${Eo}</style>
            ${Jo({src:o||void 0,initials:r,alt:t})}
        `}}if(!customElements.get("app-avatar"))customElements.define("app-avatar",Zo);y();B();no();y();var Ko=(o)=>`
...
`,Pr=(o,r)=>{if(!o)return"";if(!r)return"";if(r.is_docker)return`
    ...
    `},co=(o,r,t)=>{return`
        ...
        `:""}
    `},Xo=(o,r,t,i,n)=>{let e=t?.toLowerCase()==="admin"||t?.toLowerCase()==="administrator";return`
...
.overlay.visible {
    display: block;
}`;class No extends HTMLElement{static get observedAttributes(){return["value","trigger-opacity"]}_value="#0078D4";_h=210;_s=100;_v=83;_popover=null;_satArea=null;_satCursor=null;_hueSlider=null;_hueCursor=null;_hexInput=null;_preview=null;_overlay=null;_portal=null;_isDraggingSat=!1;_isDraggingHue=!1;constructor(){super();this.attachShadow({mode:"open"})}connectedCallback(){this.render(),this.bindEvents(),this.hexToHsv(this._value),this.updateUI()}get value(){return this._value}set value(o){if(this._value!==o){if(this._value=o,this.hexToHsv(o),this.updateUI(),this._hexInput)this._hexInput.value=o}}attributeChangedCallback(o,r,t){if(o==="value"&&r!==t)this.value=t;else if(o==="trigger-opacity"&&r!==t)this.updateTriggerOpacity(t)}updateTriggerOpacity(o){if(this.shadowRoot){let r=this.shadowRoot.getElementById("trigger");if(r)r.style.opacity=o}}render(){if(!this.shadowRoot)return;this.shadowRoot.innerHTML=`
            <style>${xo}</style>
        ...
        `,this._popover=this.shadowRoot.querySelector("#popover"),this._satArea=this.shadowRoot.querySelector("#sat-area"),this._satCursor=this.shadowRoot.querySelector("#sat-cursor"),this._hueSlider=this.shadowRoot.querySelector("#hue-slider"),this._hueCursor=this.shadowRoot.querySelector("#hue-cursor"),this._hexInput=this.shadowRoot.querySelector("#hex-input"),this._preview=this.shadowRoot.querySelector("#preview"),this._overlay=this.shadowRoot.querySelector("#overlay")}bindEvents(){this.shadowRoot?.querySelector("#trigger")?.addEventListener("click",(r)=>{r.stopPropagation(),this.togglePicker()}),this._overlay?.addEventListener("click",()=>this.closePopover()),this._satArea?.addEventListener("mousedown",(r)=>{this._isDraggingSat=!0,this.handleSatDrag(r),window.addEventListener("mousemove",this.onMouseMoveSat),window.addEventListener("mouseup",this.onMouseUpSat)}),this._hueSlider?.addEventListener("mousedown",(r)=>{this._isDraggingHue=!0,this.handleHueDrag(r),window.addEventListener("mousemove",this.onMouseMoveHue),window.addEventListener("mouseup",this.onMouseUpHue)}),this._hexInput?.addEventListener("input",(r)=>{let t=r.target.value;if(/^#[0-9A-Fa-f]{6}$/.test(t))this.value=t,this.emitChange()}),this._popover?.addEventListener("click",(r)=>r.stopPropagation())}onMouseMoveSat=(o)=>{if(this._isDraggingSat)this.handleSatDrag(o)};onMouseUpSat=()=>{this._isDraggingSat=!1,window.removeEventListener("mousemove",this.onMouseMoveSat),window.removeEventListener("mouseup",this.onMouseUpSat),this.emitChange()};onMouseMoveHue=(o)=>{if(this._isDraggingHue)this.handleHueDrag(o)};onMouseUpHue=()=>{this._isDraggingHue=!1,window.removeEventListener("mousemove",this.onMouseMoveHue),window.removeEventListener("mouseup",this.onMouseUpHue),this.emitChange()};handleSatDrag(o){if(!this._satArea)return;let r=this._satArea.getBoundingClientRect(),t=o.clientX-r.left,i=o.clientY-r.top;t=Math.max(0,Math.min(t,r.width)),i=Math.max(0,Math.min(i,r.height)),this._s=t/r.width*100,this._v=100-i/r.height*100,this.updateColorFromHsv(),this.updateUI()}handleHueDrag(o){if(!this._hueSlider)return;let r=this._hueSlider.getBoundingClientRect(),t=o.clientX-r.left;t=Math.max(0,Math.min(t,r.width)),this._h=t/r.width*360,this.updateColorFromHsv(),this.updateUI()}togglePicker(){if(this._popover?.classList.contains("visible"))this.closePopover();else this.openPopover()}openPopover(){if(!this._popover||!this.shadowRoot)return;if(!this._portal)this._portal=document.createElement("div"),this._portal.style.display="contents",document.body.appendChild(this._portal);if(this._portal.appendChild(this._popover),this._popover.style.display="flex",this._popover.offsetHeight,this._popover.classList.add("visible"),this._overlay)this._overlay.classList.add("visible");requestAnimationFrame(()=>{this.updatePosition()}),this.hexToHsv(this._value),this.updateUI(),window.addEventListener("resize",this.updatePositionRef),window.addEventListener("scroll",this.updatePositionRef,!0),setTimeout(()=>document.addEventListener("click",this.onGlobalClick),0)}onGlobalClick=(o)=>{let r=o.target;if(this._popover&&this._popover.contains(r))return;if(this.contains(r))return;let t=o.composedPath();if(t.includes(this._popover)||t.includes(this))return;this.closePopover()};updatePositionRef=()=>{if(this._popover?.classList.contains("visible"))this.updatePosition()};updatePosition(){if(!this._popover)return;let o=this.getBoundingClientRect(),r=this._popover.getBoundingClientRect(),t=o.bottom+8;if(t+300>window.innerHeight)t=o.top-8-(r.height||310);let i=o.left+o.width/2;this._popover.style.position="fixed",this._popover.style.zIndex="10002",this._popover.style.margin="0",this._popover.style.top=`${t}px`,this._popover.style.left=`${i}px`,this._popover.style.transform="translateX(-50%)",this._popover.style.width="240px"}closePopover(){if(!this._popover)return;if(this._popover.classList.remove("visible"),this._overlay)this._overlay.classList.remove("visible");let o=()=>{if(!this._popover)return;if(this._popover.style.display="none",this.shadowRoot)this.shadowRoot.appendChild(this._popover);if(this._portal)this._portal.remove(),this._portal=null};this._popover.addEventListener("transitionend",o,{once:!0}),window.removeEventListener("resize",this.updatePositionRef),window.removeEventListener("scroll",this.updatePositionRef,!0),document.removeEventListener("click",this.onGlobalClick)}emitChange(){this.dispatchEvent(new CustomEvent("input",{detail:{value:this._value},bubbles:!0,composed:!0})),this.dispatchEvent(new Event("change",{bubbles:!0,composed:!0}))}updateColorFromHsv(){this._value=this.hsvToHex(this._h,this._s,this._v)}updateUI(){if(!this._satCursor||!this._hueCursor||!this._hexInput||!this._preview||!this._satArea)return;this._satCursor.style.left=`${this._s}%`,this._satCursor.style.top=`${100-this._v}%`,this._hueCursor.style.left=`${this._h/360*100}%`;let o=this.hsvToHex(this._h,100,100);this._satArea.style.backgroundColor=o,this._hexInput.value=this._value,this._preview.style.backgroundColor=this._value}hexToHsv(o){let r=/^#?([a-f\d])([a-f\d])([a-f\d])$/i;o=o.replace(r,(u,f,_,v)=>f+f+_+_+v+v);let t=/^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(o);if(!t)return;let i=parseInt(t[1],16)/255,n=parseInt(t[2],16)/255,e=parseInt(t[3],16)/255,s=Math.max(i,n,e),p=Math.min(i,n,e),d=0,c,l=s,g=s-p;if(c=s===0?0:g/s,s===p)d=0;else{switch(s){case i:d=(n-e)/g+(n<e?6:0);break;case n:d=(e-i)/g+2;break;case e:d=(i-n)/g+4;break}d/=6}this._h=d*360,this._s=c*100,this._v=l*100}hsvToHex(o,r,t){r/=100,t/=100;let i=Math.floor(o/60),n=o/60-i,e=t*(1-r),s=t*(1-n*r),p=t*(1-(1-n)*r),d=0,c=0,l=0;switch(i%6){case 0:d=t,c=p,l=e;break;case 1:d=s,c=t,l=e;break;case 2:d=e,c=t,l=p;break;case 3:d=e,c=s,l=t;break;case 4:d=p,c=e,l=t;break;case 5:d=t,c=e,l=s;break}let g=(u)=>{let f=Math.round(u*255).toString(16);return f.length===1?"0"+f:f};return`#${g(d)}${g(c)}${g(l)}`}}customElements.define("app-color-picker",No);var Vo=`:host {
    display: block;
    color: var(--text-main);
    max-width: 900px;
    margin: 0 auto;
...
}`;class Do extends HTMLElement{prefs;users=[];getCsrfToken(){let o=document.cookie.split("; ").find((r)=>r.startsWith("csrf_token="));return o?decodeURIComponent(o.split("=")[1]):""}static get observedAttributes(){return["section"]}constructor(){super();this.attachShadow({mode:"open"}),this.prefs={accent_color:"#0078D4",language:"en",widget_min_width:140,beta_updates:!1}}unsubscribe=null;_renderPending=!1;connectedCallback(){this.unsubscribe=k.subscribe((r)=>{if(r&&r.preferences)this.prefs={...r.preferences,project_name:r.project_name||r.preferences.project_name||"Lastboard"},this.render()}),this.fetchPrefs();let o=this.getAttribute("section");if(o==="users")this.fetchUsers();this.render()}disconnectedCallback(){if(this.unsubscribe)this.unsubscribe(),this.unsubscribe=null}async fetchPrefs(){let o=k.getUser();if(o&&o.preferences)this.prefs={...o.preferences,project_name:o.project_name||o.preferences.project_name||"Lastboard"},this.render()}async fetchUsers(){try{this.users=await G.getUsers(),this.render()}catch(o){console.error("Failed to fetch users",o)}}attributeChangedCallback(o,r,t){if(o==="section"&&r!==t){if(t==="users")this.fetchUsers();this.render()}}async savePrefs(o){let r={...this.prefs};if(this.prefs={...this.prefs,...o},o.accent_color)this.applyAccent(o.accent_color);if(o.language)a.setLanguage(o.language);if(o.theme)if(o.theme==="dark")H.enableDark();else H.enableLight();try{await k.updatePreferences(o)}catch{if(this.prefs=r,o.language&&r.language)a.setLanguage(r.language)}this.render()}applyAccent(o){let r=o,t={blue:"#228be6",indigo:"#4c6ef5",cyan:"#15aabf",teal:"#12b886",orange:"#fd7e14",red:"#fa5252",grape:"#be4bdb"};if(!o.startsWith("#"))r=t[o]||"#0078D4";document.documentElement.style.setProperty("--accent",r);let i=parseInt(r.slice(1,3),16),n=parseInt(r.slice(3,5),16),e=parseInt(r.slice(5,7),16);if(!isNaN(i)&&!isNaN(n)&&!isNaN(e))document.documentElement.style.setProperty("--accent-rgb",`${i}, ${n}, ${e}`)}async handleBetaToggle(o){this.savePrefs({beta_updates:o}),this.render()}updateDensity(o){let r=this.shadowRoot.getElementById("val-widget_min_width");if(r)r.textContent=o+"px";document.documentElement.style.setProperty("--widget-min-size",o+"px")}commitDensity(o){let r=parseInt(o);k.updatePreferences({widget_min_width:r})}updateColumns(o){let r=this.shadowRoot.getElementById("val-grid_columns");if(r)r.textContent=o;document.documentElement.style.setProperty("--user-preferred-columns",o)}commitColumns(o){let r=parseInt(o);k.updatePreferences({grid_columns:r})}async updateUsername(o){let r=k.getUser();if(!r)return;if(await k.updateProfile({username:o,avatar_url:this.prefs.avatar_url||r.avatar_url}),window.notifier)window.notifier.show(a.t("notifier.username_updated"))}async updatePassword(){let o=this.shadowRoot.getElementById("current-password")?.value,r=this.shadowRoot.getElementById("new-password")?.value,t=this.shadowRoot.getElementById("confirm-password")?.value;if(!o){if(window.notifier)window.notifier.show(a.t("notifier.password_required"),"error");return}if(r!==t){if(window.notifier)window.notifier.show(a.t("notifier.password_mismatch"),"error");return}if(!r){if(window.notifier)window.notifier.show(a.t("notifier.password_empty"),"error");return}try{let i=await k.changePassword({current_password:o,new_password:r});if(this.shadowRoot.getElementById("current-password").value="",this.shadowRoot.getElementById("new-password").value="",this.shadowRoot.getElementById("confirm-password").value="",window.notifier)window.notifier.show(a.t("notifier.password_changed_relogin"));setTimeout(()=>{localStorage.removeItem("lastboard_user_cache"),document.body.style.opacity="0",window.location.href="/login"},2000)}catch(i){if(window.notifier)window.notifier.show(a.t("notifier.password_incorrect"),"error")}}async handleAvatarChange(o){let r=o.target.files?.[0];if(!r)return;if(!r.type.startsWith("image/")){if(window.notifier)window.notifier.show(a.t("notifier.invalid_image_type"),"error");return}try{let t=await this.resizeImage(r,256,256),i=k.getUser();if(!i)return;if(this.prefs.avatar_url=t,await k.updateProfile({username:i.username,avatar_url:t}),this.render(),window.notifier)window.notifier.show(a.t("notifier.avatar_updated"))}catch(t){if(console.error("Resize failed",t),window.notifier)window.notifier.show(a.t("notifier.avatar_error"),"error")}}resizeImage(o,r,t){return new Promise((i,n)=>{let e=new FileReader;e.onload=(s)=>{let p=new Image;p.onload=()=>{let d=document.createElement("canvas"),c=p.width,l=p.height;if(c>l){if(c>r)l=Math.round(l*r/c),c=r}else if(l>t)c=Math.round(c*t/l),l=t;d.width=c,d.height=l;let g=d.getContext("2d");if(!g){n(Error("Canvas context not available"));return}g.drawImage(p,0,0,c,l),i(d.toDataURL("image/jpeg",0.8))},p.onerror=(d)=>n(d),p.src=s.target?.result},e.onerror=(s)=>n(s),e.readAsDataURL(o)})}openAddUserModal(){let o=this.shadowRoot.getElementById("add-user-modal");if(o){o.showModal();let r=this.shadowRoot.getElementById("new-user-role");if(r)r.options=[{value:"user",label:a.t("settings.role_user")},{value:"admin",label:a.t("settings.role_admin")}]}}async createUser(){let o=this.shadowRoot.getElementById("new-user-username").value,r=this.shadowRoot.getElementById("new-user-password").value,t=this.shadowRoot.getElementById("new-user-role").value;if(!o||!r){if(window.notifier)window.notifier.show(a.t("notifier.fields_required"),"error");return}try{if(await G.createUser({username:o,password:r,role:t}),window.notifier)window.notifier.show(a.t("notifier.user_created"));let i=this.shadowRoot.getElementById("add-user-modal");if(i)i.close();this.fetchUsers()}catch(i){if(window.notifier)window.notifier.show(a.t("notifier.user_create_error"),"error")}}openEditUserModal(o,r,t){let i=this.shadowRoot.getElementById("edit-user-modal");if(i){this.shadowRoot.getElementById("edit-user-id").value=o.toString(),this.shadowRoot.getElementById("edit-user-username").value=r,this.shadowRoot.getElementById("edit-user-password").value="",i.showModal();let n=this.shadowRoot.getElementById("edit-user-role");if(n)n.options=[{value:"user",label:a.t("settings.role_user")},{value:"admin",label:a.t("settings.role_admin")}],n.value=t}}async updateAdminUser(){let o=parseInt(this.shadowRoot.getElementById("edit-user-id").value),r=this.shadowRoot.getElementById("edit-user-username").value,t=this.shadowRoot.getElementById("edit-user-password").value,i=this.shadowRoot.getElementById("edit-user-role").value;if(!r){if(window.notifier)window.notifier.show(a.t("notifier.username_required"),"error");return}try{if(await G.adminUpdateUser({id:o,username:r,role:i,password:t||void 0}),window.notifier)window.notifier.show(a.t("notifier.user_updated"));let n=this.shadowRoot.getElementById("edit-user-modal");if(n)n.close();this.fetchUsers()}catch(n){if(window.notifier)window.notifier.show(a.t("notifier.user_update_error"),"error")}}async deleteUser(o){let r=document.querySelector("confirmation-modal");if(r){if(!await r.confirm(a.t("general.delete"),a.t("notifier.user_delete_confirm")))return}try{let t=[...this.users];if(this.users=this.users.filter((i)=>i.id!==o),this.render(),await G.deleteUser(o),window.notifier)window.notifier.show(a.t("notifier.user_deleted"));this.fetchUsers()}catch(t){this.fetchUsers();let i=a.t("notifier.user_delete_error");if(t.message&&t.message.includes("error.cannot_delete_superadmin"))i=a.t("notifier.user_delete_superadmin");if(window.notifier)window.notifier.show(i,"error")}}getContent(o){let r=k.getUser()||{username:"Guest",initials:"??",role:"View Only",avatar_url:"",accent_color:"#0078D4",language:"en",preferences:{}};switch(o){case"account":return Ko(r);case"theme":{let t={blue:"#228be6",indigo:"#4c6ef5",cyan:"#15aabf",teal:"#12b886",orange:"#fd7e14",red:"#fa5252",grape:"#be4bdb"},i=Object.keys(t);return Bo(this.prefs,t,i)}case"personalization":return Yo(this.prefs);case"advanced":return To();case"users":return Uo(this.users);case"about":return Xo(this.version,this.updateInfo,r.role||"user",r.preferences?.beta_updates||!1,this.isCheckingUpdates);default:return`<div class="bento-card"><h3>${o}</h3><p class="settings-content__text-dim">${a.t("settings.default_module_desc")}</p></div>`}}version="v1.2.5";updateInfo=null;checkUpdatesPromise=null;isCheckingUpdates=!1;async handleManualCheckUpdates(){if(this.isCheckingUpdates)return;this.isCheckingUpdates=!0,this.updateUpdateUI();try{if(await this.checkForUpdates(),this.updateInfo&&!this.updateInfo.available){if(window.notifier)window.notifier.show(a.t("settings.up_to_date"))}}finally{this.isCheckingUpdates=!1,this.updateUpdateUI()}}async checkForUpdates(){if(this.checkUpdatesPromise)return this.checkUpdatesPromise;return this.checkUpdatesPromise=(async()=>{try{let{updateService:o}=await Promise.resolve().then(() => (lo(),po)),r=this.prefs.beta_updates||!1;if(this.updateInfo=await o.check(r),this.updateInfo)this.version=this.updateInfo.current_version,this.updateUpdateUI()}catch(o){console.error("Check update failed",o)}finally{this.checkUpdatesPromise=null}})(),this.checkUpdatesPromise}async performUpdate(o){let r=document.querySelector("confirmation-modal");if(r){if(!await r.confirm(a.t("settings.update_available"),a.t("notifier.update_start_confirm")))return}let t=this.shadowRoot.getElementById("btn-update-now"),i=this.shadowRoot.getElementById("update-status");if(t)t.loading=!0;if(i){i.style.display="block";let n=i.querySelector("p");if(n)n.textContent=a.t("notifier.update_downloading")}try{let n=await fetch("/api/system/update/perform",{method:"POST",headers:{"Content-Type":"application/json","X-CSRF-Token":this.getCsrfToken()},body:JSON.stringify({asset_url:o})});if(n.ok){if(i){let e=i.querySelector("p");if(e)e.textContent=a.t("notifier.update_verified")}setTimeout(()=>{window.location.reload()},5000)}else{let e=await n.text();if(i){i.style.background="rgba(var(--danger-rgb), 0.1)",i.style.borderColor="rgba(var(--danger-rgb), 0.3)";let s=i.querySelector("p");if(s)s.style.color="var(--danger-color)",s.textContent=a.t("notifier.update_failed")+": "+e}if(t)t.loading=!1}}catch(n){if(console.error("Update failed",n),i){i.style.background="rgba(var(--danger-rgb), 0.1)",i.style.borderColor="rgba(var(--danger-rgb), 0.3)";let e=i.querySelector("p");if(e)e.style.color="var(--danger-color)",e.textContent=a.t("notifier.update_error")}if(t)t.loading=!1}}downloadBackup(){window.location.href="/api/system/backup"}async restoreBackup(o){if(!o)return;let r=document.querySelector("confirmation-modal");if(r){if(!await r.confirm(a.t("general.restore"),a.t("notifier.restore_confirm")))return}let t=new FormData;t.append("backup_file",o);try{if((await fetch("/api/system/restore",{method:"POST",headers:{"X-CSRF-Token":this.getCsrfToken()},body:t})).ok){if(window.notifier)window.notifier.show(a.t("notifier.restore_success"));setTimeout(()=>window.location.reload(),2000)}else if(window.notifier)window.notifier.show(a.t("notifier.restore_failed"),"error")}catch(i){if(console.error("Restore error",i),window.notifier)window.notifier.show(a.t("notifier.restore_failed"),"error")}}openResetModal(){let o=this.shadowRoot.getElementById("reset-confirm-modal"),r=this.shadowRoot.getElementById("reset-confirm-input");if(o){if(r)r.value="";o.showModal()}}async executeFactoryReset(){let o=this.shadowRoot.getElementById("reset-confirm-input");if(!o||o.value.trim()!=="delete"){if(window.notifier)window.notifier.show(a.t("notifier.reset_confirm_text"),"error");o.focus();return}let r=this.shadowRoot.getElementById("btn-reset-confirm");try{if(r)r.disabled=!0,r.textContent=a.t("settings.restoring")||"Restoring...";if((await fetch("/api/system/reset",{method:"POST",headers:{"X-CSRF-Token":this.getCsrfToken()}})).ok){let i=document.createElement("div");Object.assign(i.style,{position:"fixed",top:"0",left:"0",width:"100%",height:"100%",backgroundColor:"rgba(0,0,0,0.9)",zIndex:"9999",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",color:"white",fontFamily:"var(--font-main, sans-serif)"}),i.innerHTML=`
...
(max-width: 1024px) {
...
}`;class ir extends HTMLElement{dropdownOpen;constructor(){super();this.attachShadow({mode:"open"}),this.dropdownOpen=!1}connectedCallback(){this.render(),this.setupListeners()}toggleDropdown(o){this.dropdownOpen=typeof o==="boolean"?o:!this.dropdownOpen,this.render()}setupListeners(){this.shadowRoot.addEventListener("click",(o)=>{let r=o.target,t=r.closest(".header__btn-changelog");if(t)t.style.display="none";if(r.closest("#profile-trigger"))o.stopPropagation(),this.toggleDropdown()}),window.addEventListener("click",()=>{if(this.dropdownOpen)this.toggleDropdown(!1)})}render(){let o=this.getAttribute("title")||"Task";this.shadowRoot.innerHTML=`
            <style>${tr}</style>
            ${rr({title:o,dropdownOpen:this.dropdownOpen})}
        `}}if(!customElements.get("app-header"))customElements.define("app-header",ir);y();var O=(o)=>String(o).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;"),it=(o)=>{if(!o)return"#";let r=o.trim().toLowerCase();if(r.startsWith("http://")||r.startsWith("https://")||r==="#")return O(o);return"#"},ar=({bookmarks:o,isEditing:r,isLoading:t,isSearching:i,isTouchDevice:n,maxCols:e=12})=>{let s=Array.isArray(o)?o:[];if(t&&s.length===0)return nt(e);let p=(c)=>s.filter((l)=>l.parent_id===c);return`
    ${(n?s:i?s:s.filter((c)=>!c.parent_id)).map((c)=>{let l={};try{l=typeof c.content==="string"?JSON.parse(c.content):c.content}catch(v){console.error("Failed to parse content for item",c.id,v)}let g=c.type==="section",u={x:c.x||1,y:c.y||1},f=Math.min(c.w||1,e),_=c.h||1;if(g){let v=p(c.id),w=(l.title||"").trim(),b=l.borderColor&&!r?`border-color: ${l.borderColor} !important;`:"",A=!r?`border-width: ${l.borderWidth||1}px !important;`:"";return`
            <fieldset class="bookmark-grid__section"
               data-id="${c.id}"
               data-orig-x="${c.x}" data-orig-y="${c.y}"
               draggable="${r}"
               style="--x: ${u.x}; --y: ${u.y}; --w: ${f}; --h: ${_}; ${b} ${A}">
               ${w?`<legend class="section-title">${O(w)}</legend>`:""}
               ...
               `:""}
            </fieldset>`}return nr(c,l,r,u,f,_,n)}).join("")}

    <div id="ghost-element" class="bookmark-grid__ghost" style="display: none;"></div>
    `};function nr(o,r,t,i,n,e=0,s=!1){let p=e||o.h||1;if(o.type==="widget"){let v=r.widgetId,b={clock:"widget-clock",notepad:"widget-notepad",telemetry:"widget-telemetry",weather:"widget-weather",markdown:"widget-markdown"}[v]||"div",q=(r.text||"").replace(/"/g,"&quot;"),h=r.borderColor&&!t?`border-color: ${r.borderColor} !important;`:"",E=!t?`border-width: ${r.borderWidth||1}px !important;`:"";return`
        <div class="bookmark-grid__card"
            draggable="${t}"
            data-id="${o.id}"
            data-type="${o.type}"
            data-orig-x="${o.x}" data-orig-y="${o.y}"
            ${!s?`style="--x: ${i.x}; --y: ${i.y}; --w: ${n}; --h: ${p}; ${h} ${E}"`:""}>
            ...
            ${t?`
            <div class="bookmark-actions">
                 ${["clock","telemetry","weather","notepad","markdown"].includes(v)?`<button class="action-btn btn-edit" title="${a.t("general.edit")}">✎</button>`:""}
                 <button class="action-btn btn-delete" title="${a.t("general.delete")}">\uD83D\uDDD1</button>
            </div>
            ${["clock","telemetry"].includes(v)?"":'<div class="resize-handle"></div>'}
            `:""}
        </div>
    `}let d=r.icon||"",l=d.startsWith("http")||d.startsWith("/")?`<img src="${O(d)}" alt="${O(r.label)}" class="bookmark-grid__icon-img" draggable="false" />`:d?O(d):'<svg class="bookmark-grid__icon-svg" viewBox="0 0 24 24"><path d="M10 6L8.59 7.41 13.17 12l-4.58 4.59L10 18l6-6z"/></svg>',g=t?'role="button"':`href="${it(r.url)}" target="_blank"`,u=!s?`--x: ${i.x}; --y: ${i.y}; --w: ${n}; --h: ${p};`:"",f=r.borderColor&&!t?`border-color: ${r.borderColor} !important;`:"",_=r.labelPos==="top"?"label-top":r.labelPos==="section"?"label-section":r.labelPos==="off"?"label-off":"";return`
        <a ${g} class="bookmark-grid__card ${_}"
           draggable="${t}"
           data-id="${o.id}"
           data-type="${o.type}"
           data-orig-x="${o.x}" data-orig-y="${o.y}"
           style="${u} ${f} ${!t?`border-width: ${r.borderWidth||1}px !important;`:""}">
            ...
            <span class="bookmark-grid__label">${O(r.label)||"Bookmark"}</span>
    ...
    `}function at(o,r,t=!1,i=12,n=!1){return o.map((e)=>{let s={};try{s=typeof e.content==="string"?JSON.parse(e.content):e.content}catch(l){console.error("Failed to parse content for item (nested)",e.id,l)}let p={x:e.x||1,y:e.y||1},d=e.h||1,c=Math.min(e.w||1,i);return nr(e,s,r,p,c,d,n)}).join("")}function nt(o){return`
        <style>
            .skeleton {
                background: var(--surface);
                pointer-events: none;
...
`;class sr extends HTMLElement{_itemId=0;_content="";_isDirty=!1;_isLocked=!0;_isSaving=!1;_lastLockTime=0;_editor=null;_syncStatus=null;_dock=null;_editorContainer=null;_modal=null;_imgResizer=null;_activeImage=null;_pendingAction=null;static get observedAttributes(){return["item-id"]}constructor(){super();this.attachShadow({mode:"open"})}connectedCallback(){this.render(),this.loadContent(),window.addEventListener("beforeunload",this.handleBeforeUnload.bind(this)),a.subscribe(()=>{if(this._editor)this._editor.dataset.placeholder=a.t("widget.notepad.placeholder")})}disconnectedCallback(){window.removeEventListener("beforeunload",this.handleBeforeUnload.bind(this))}attributeChangedCallback(o,r,t){if(o==="item-id")this._itemId=parseInt(t),this.loadContent()}handleBeforeUnload(o){if(this._isDirty)o.preventDefault(),o.returnValue=""}render(){if(!this.shadowRoot)return;let o=this._isLocked?"locked":"unlocked",r=this._isLocked?"locked":"",t=this._isLocked?"hidden":"";this.shadowRoot.innerHTML=`
            <style>
                ${et}
        ...
        `,this.cpuBar=this.shadowRoot.querySelector(".cpu-bar"),this.ramBar=this.shadowRoot.querySelector(".ram-bar"),this.tempBar=this.shadowRoot.querySelector(".temp-bar"),this.cpuText=this.shadowRoot.querySelector(".cpu-text"),this.ramText=this.shadowRoot.querySelector(".ram-text"),this.tempText=this.shadowRoot.querySelector(".temp-text")}}customElements.define("widget-telemetry",pr);y();L();var dr=new Map,cr=900000,lr={widgetId:"weather",latitude:0,longitude:0,city:"",unit:"celsius",showForecast:!1,forecastDays:5};function N(o){if(o===0)return`<svg viewBox="0 0 64 64" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round">
    ...
    </svg>`}function bo(o){let r={0:"clear_sky",1:"mainly_clear",2:"partly_cloudy",3:"overcast",45:"fog",48:"fog",51:"light_drizzle",53:"drizzle",55:"heavy_drizzle",56:"freezing_drizzle",57:"freezing_drizzle",61:"light_rain",63:"rain",65:"heavy_rain",66:"freezing_rain",67:"freezing_rain",71:"light_snow",73:"snow",75:"heavy_snow",77:"snow_grains",80:"light_showers",81:"showers",82:"heavy_showers",85:"snow_showers",86:"heavy_snow_showers",95:"thunderstorm",96:"thunderstorm_hail",99:"thunderstorm_hail"};return a.t(`widget.weather.${r[o]||"unknown"}`)}function J(o,r){if(r==="fahrenheit")return`${Math.round(o*9/5+32)}°F`;return`${Math.round(o)}°C`}async function st(o,r){let t=`${o.toFixed(2)},${r.toFixed(2)}`,i=dr.get(t);if(i&&Date.now()-i.timestamp<cr)return i.data;let n=`https://api.open-meteo.com/v1/forecast?latitude=${o}&longitude=${r}&current_weather=true&daily=weathercode,temperature_2m_max,temperature_2m_min,sunrise,sunset,wind_speed_10m_max&timezone=auto&forecast_days=7`,e=await fetch(n);if(!e.ok)throw Error("Weather API error");let s=await e.json();return dr.set(t,{data:s,timestamp:Date.now()}),s}class gr extends HTMLElement{_config={...lr};_itemId=0;_unsubscribe=null;_unsubscribeI18n;_weatherData=null;_refreshTimer=null;_loading=!1;_error=!1;constructor(){super();this.attachShadow({mode:"open"})}static get observedAttributes(){return["item-id"]}attributeChangedCallback(o,r,t){if(o==="item-id")this._itemId=parseInt(t)}connectedCallback(){this.render(),this.loadWeather(),this._refreshTimer=setInterval(()=>this.loadWeather(),cr),this._unsubscribe=x.subscribe((o)=>{if(!this._itemId)return;let r=o.items.find((t)=>t.id===this._itemId);if(!r?.content)return;try{let t=typeof r.content==="string"?JSON.parse(r.content):r.content;if(t.widgetId!=="weather")return;let i=this._config.latitude,n=this._config.longitude;if(this._config={...lr,...t},i!==this._config.latitude||n!==this._config.longitude)this.loadWeather();else this.updateDisplay()}catch(t){}}),this._unsubscribeI18n=a.subscribe(()=>this.updateDisplay())}disconnectedCallback(){if(this._unsubscribe)this._unsubscribe();if(this._unsubscribeI18n)this._unsubscribeI18n();if(this._refreshTimer)clearInterval(this._refreshTimer)}async loadWeather(){if(!this._config.latitude&&!this._config.longitude){this._loading=!1,this._error=!1,this.updateDisplay();return}this._loading=!0,this._error=!1,this.updateDisplay();try{this._weatherData=await st(this._config.latitude,this._config.longitude),this._loading=!1}catch(o){this._loading=!1,this._error=!0}this.updateDisplay()}updateDisplay(){let o=this.shadowRoot?.querySelector(".weather-body");if(!o){this.render();return}if(!this._config.latitude&&!this._config.longitude){o.innerHTML=this.emptyState(N(0),a.t("widget.weather.no_location"));return}if(this._loading){o.innerHTML=this.emptyState(N(3),a.t("general.loading"),!0);return}if(this._error||!this._weatherData){o.innerHTML=this.emptyState(N(-1),a.t("general.error"));return}let r=this._weatherData.current_weather,t=this._weatherData.daily,i=this._config.unit||"celsius";if(this._config.showForecast&&t)o.innerHTML=this.forecastView(r,t,i);else o.innerHTML=this.simpleView(r,t,i)}emptyState(o,r,t=!1){return`<div class="state"><div class="state-icon${t?" pulse":""}">${o}</div><span class="state-text">${r}</span></div>`}simpleView(o,r,t){return`
            <div class="icon-main">${N(o.weathercode)}</div>
            <div class="temp-main">${J(o.temperature,t)}</div>
            <div class="desc">${bo(o.weathercode)}</div>
        ...
        `}forecastView(o,r,t){let i=a.getLocale().code,n=Math.min(this._config.forecastDays||5,r.time.length-1),e="";for(let s=1;s<=n;s++){let p=new Date(r.time[s]+"T00:00:00"),d=p.toLocaleDateString(i,{weekday:"short"}),c=p.toLocaleDateString(i,{weekday:"long",day:"numeric",month:"long"}),l=bo(r.weathercode[s]),g=J(r.temperature_2m_max[s],t),u=J(r.temperature_2m_min[s],t),f=r.sunrise?.[s]?.split("T")[1]||"",_=r.sunset?.[s]?.split("T")[1]||"",v=r.wind_speed_10m_max?.[s]!=null?`${Math.round(r.wind_speed_10m_max[s])} km/h`:"";e+=`<div class="fc-day">
                <span class="fc-name">${d}</span>
                <span class="fc-icon">${N(r.weathercode[s])}</span>
                <span class="fc-temp">${g}</span>
                <div class="fc-tooltip">
                    <div class="tt-date">${c}</div>
                    <div class="tt-desc">${l}</div>
                    <div class="tt-row"><span class="tt-label">${a.t("widget.weather.max_min")}</span><span>${g} / ${u}</span></div>
                    ${f?`<div class="tt-row"><span class="tt-label">${a.t("widget.weather.sunrise")}</span><span>${f}</span></div>`:""}
                    ${_?`<div class="tt-row"><span class="tt-label">${a.t("widget.weather.sunset")}</span><span>${_}</span></div>`:""}
                    ${v?`<div class="tt-row"><span class="tt-label">${a.t("widget.weather.wind")}</span><span>${v}</span></div>`:""}
                </div>
            </div>`}return`
            <div class="fc-header">
                <div class="fc-h-icon">${N(o.weathercode)}</div>
                <div class="fc-h-info">
                    <div class="fc-h-temp">${J(o.temperature,t)}</div>
                    <div class="fc-h-desc">${bo(o.weathercode)}</div>
                </div>
        ...
        `}render(){if(!this.shadowRoot)return;this.shadowRoot.innerHTML=`<style>${pt}</style><div class="weather-body"></div>`,this.updateDisplay()}}var pt=`
:host {
    display: flex;
    flex-direction: column;
    align-items: center;
...
`;customElements.define("widget-weather",gr);var xr=`:host {
    /* Fixed Grid System */
...
`,_r=(o,r,t)=>{if(t)return`<div class="icon-picker__loading">${a.t("general.loading")}</div>`;if(o.length===0)return`<div class="icon-picker__empty">${a.t("general.no_icons")}</div>`;return o.map((i)=>`
        <div class="icon-picker__item ${r===i.name?"icon-picker__item--selected":""}"
             data-icon="${i.name}"
             title="${i.name}">
            <img src="${i.url}" alt="${i.name}" loading="lazy" />
        </div>
    `).join("")};class yr{icons=[];loaded=!1;loading=!1;BASE_URL="https://cdn.jsdelivr.net/gh/homarr-labs/dashboard-icons";FALLBACK_URL="https://raw.githubusercontent.com/homarr-labs/dashboard-icons/main";async loadIcons(){if(this.loaded)return this.icons;if(this.loading)return await new Promise((o)=>setTimeout(o,100)),this.loadIcons();this.loading=!0;try{let o=await fetch(`${this.BASE_URL}/tree.json`);if(!o.ok)console.log("[IconService] CDN failed, trying GitHub raw"),o=await fetch(`${this.FALLBACK_URL}/tree.json`);if(!o.ok)throw Error("Failed to fetch icon list");let r=await o.json(),t=Array.isArray(r)?r:r.png||[];if(Array.isArray(t)&&t.length>0)this.icons=t.filter((i)=>i.endsWith(".png")||i.endsWith(".svg")).map((i)=>{let n=i.replace(/\.(png|svg)$/,"");return{name:n,url:`${this.BASE_URL}/png/${n}.png`}}).sort((i,n)=>i.name.localeCompare(n.name));else console.warn("[IconService] Unexpected tree.json structure, using fallback",r),this.icons=this.getFallbackIcons();return this.loaded=!0,this.loading=!1,console.log(`[IconService] Loaded ${this.icons.length} icons`),this.icons}catch(o){return console.error("[IconService] Failed to load icons:",o),this.loading=!1,this.icons=this.getFallbackIcons(),this.loaded=!0,this.icons}}getFallbackIcons(){return["github","gitlab","docker","proxmox","truenas","plex","jellyfin","nextcloud","cloudflare","nginx","traefik","portainer","grafana","prometheus","influxdb","pihole","adguard","homeassistant","esphome","frigate","unraid","synology","opnsense","pfsense","wireguard","openvpn","bitwarden","vaultwarden","sonarr","radarr","lidarr","bazarr","prowlarr","overseerr","tautulli","transmission","qbittorrent","deluge","sabnzbd","nzbget","calibre","paperless","photoprism","immich","mealie","freshrss","miniflux","wallabag","linkding","shiori","firefox","chrome","vscode","code-server","jupyter","portainer"].map((r)=>({name:r,url:`${this.BASE_URL}/png/${r}.png`}))}searchIcons(o,r=50){if(!o.trim())return this.icons.slice(0,r);let t=o.toLowerCase().trim();return this.icons.filter((i)=>i.name.toLowerCase().includes(t)).slice(0,r)}getIconUrl(o){return`${this.BASE_URL}/png/${o}.png`}}var fo=new yr;var wr=`:host {
    display: block;
}
...
@keyframes pulse {
...
}`;class kr extends HTMLElement{icons=[];filteredIcons=[];selectedIcon="";searchQuery="";isLoading=!0;debounceTimer=null;inputElement=null;constructor(){super();this.attachShadow({mode:"open"})}async connectedCallback(){if(this.shadowRoot.childElementCount>0)return;if(this.renderBase(),this.setupListeners(),this.icons.length===0)await this.loadIcons()}async loadIcons(){try{this.icons=await fo.loadIcons(),this.filteredIcons=this.icons.slice(0,50),this.isLoading=!1,this.updateGrid()}catch(o){console.error("[IconPicker] Failed to load icons",o),this.isLoading=!1,this.updateGrid()}}setupListeners(){let o=this.shadowRoot;this.inputElement=o.getElementById("icon-search"),this.inputElement?.addEventListener("input",(t)=>{let i=t.target;if(this.searchQuery=i.value,this.debounceTimer)window.clearTimeout(this.debounceTimer);this.debounceTimer=window.setTimeout(()=>{this.performSearch()},100)}),o.getElementById("grid-container")?.addEventListener("click",(t)=>{let n=t.target.closest(".icon-picker__item");if(n&&n.dataset.icon)this.selectedIcon=n.dataset.icon,this.updateGrid(),this.dispatchEvent(new CustomEvent("icon-selected",{detail:{iconName:this.selectedIcon},bubbles:!0,composed:!0}))})}performSearch(){this.filteredIcons=fo.searchIcons(this.searchQuery,50),this.updateGrid()}getSelectedIcon(){return this.selectedIcon}setSelectedIcon(o){this.selectedIcon=o,this.updateGrid()}renderBase(){this.shadowRoot.innerHTML=`
            <style>${wr}</style>
            ${mr()}
        `}updateGrid(){let o=this.shadowRoot.getElementById("grid-container");if(o){if(o.style.display="grid",!this.searchQuery.trim()&&!this.isLoading){o.innerHTML='<div class="icon-picker__empty">Start typing to search icons...</div>';return}o.innerHTML=_r(this.filteredIcons,this.selectedIcon,this.isLoading)}}}if(!customElements.get("icon-picker"))customElements.define("icon-picker",kr);y();var zr=["#3b82f6","#10b981","#f59e0b","#ef4444","#8b5cf6","#ec4899","#64748b","#f8fafc","#1e293b","#333333"],gt=(o,r)=>{let t=!zr.includes(o)&&o!=="";return`
    ...
    `},$r=({isOpen:o,isEditMode:r,title:t,url:i,protocol:n,selectedIcon:e,color:s,statusPosition:p,checkStatus:d,labelPosition:c,visibleTouch:l,borderWidth:g})=>`
    <dialog id="modal">
        <div class="modal-header">
...
.premium-swatch-picker {
    ...
    /* Ensure it's on top */
        ...
        `,this.dialog=this.shadowRoot.querySelector("dialog"),this.shadowRoot.querySelectorAll(".card").forEach((o)=>{o.addEventListener("click",()=>{let r=o.getAttribute("data-id"),t=vo.find((i)=>i.id===r);if(t)this.selectWidget(t)})})}}customElements.define("add-widget-modal",Gr);L();y();var Hr=`:host {
    display: contents;
}
...
.border-width-btn span {
    ...
    font-weight: 500;
                ...
                `};if(!this.currentItem)return{general:"",customization:""};if(r==="clock"){let d=o.city||"",c=o.hour12||!1,l=o.showDate!==!1,g=`
                ...
                `,u="";return{general:g,customization:""+p()}}else if(r==="weather"){let d=o.city||"",c=o.latitude||"",l=o.longitude||"",g=o.unit==="fahrenheit",u=o.showForecast||!1,f=o.forecastDays||5,_=`
                    <div class="field-group">
                ...
                `,v=`
                    <div class="field-group check-row">
                        <input type="checkbox" id="weather-fahrenheit" ${g?"checked":""} />
                ...
                `;return{general:_,customization:v+p()}}else if(r==="telemetry"){let d=o.interval||1000,c=`
                ...
                `+p()}}else if(r==="notepad"){let d=o.text||"",c=`
                    <div style="text-align: center; padding: 32px 20px; color: var(--text-dim);">
                        <p>${a.t("widget.config.check_personalize")||"Please check the <b>Personalize</b> tab for content and style options."}</p>
                    </div>
...
.modal-actions {
    ...
    justify-content: flex-end;
        ...
        `}}oo();var j=document.getElementById("main-topbar"),Br=document.getElementById("right-drawer"),ro=document.getElementById("dashboard-root"),C,to,io;Kr(async()=>{H.init();let[o,r]=await Promise.all([k.fetchUser(),x.fetchItems()]),t=k.getUser();if(t){if(t.language)await a.setLanguage(t.language);if(t.preferences&&t.preferences.grid_columns)document.documentElement.style.setProperty("--user-preferred-columns",t.preferences.grid_columns.toString());if(t.theme==="dark")H.enableDark();else if(t.theme==="light")H.enableLight();else if(window.matchMedia&&window.matchMedia("(prefers-color-scheme: dark)").matches)H.enableDark();else H.enableLight();if(t.id)x.setUserId(t.id)}if(j){if(j.setState({user:t}),t&&t.project_name)j.setAttribute("title",t.project_name);k.subscribe((n)=>{if(n){if(j.setState({user:n}),n.project_name)j.setAttribute("title",n.project_name)}})}mt(),W.start(),C=document.createElement("add-bookmark-modal"),document.body.appendChild(C),to=document.createElement("add-widget-modal"),document.body.appendChild(to),io=document.createElement("confirmation-modal"),document.body.appendChild(io);let i=document.createElement("widget-config-modal");document.body.appendChild(i)});D.on(V.SHOW_CONFIRMATION,async(o)=>{let{title:r,message:t,onConfirm:i}=o.detail;if(io){if(await io.confirm(r,t)&&i)i()}});D.on(V.SHOW_WIDGET_CONFIG,(o)=>{let{item:r,type:t}=o.detail;if(t==="bookmark"){if(C)C.openForEdit(r)}else{let i=document.querySelector("widget-config-modal");if(i)i.open(r)}});D.on(V.NOTIFY,(o)=>{let r=document.querySelector("app-notifier");if(r)r.show(o.detail.message,o.detail.type)});if(j)j.addEventListener("drawer-toggle",(o)=>{if(o.detail.action==="open")Br.open(),j.setState({drawerOpen:!0});else Br.close(),j.setState({drawerOpen:!1})}),j.addEventListener("edit-mode-change",(o)=>{let r=o.detail.active;if(ro.classList.toggle("edit-mode",r),!r)window.location.reload()}),j.addEventListener("search-input",(o)=>{let r=o.detail.query;x.setSearchQuery(r)}),j.addEventListener("add-item",async(o)=>{let r=o.detail.action;if(r==="add-bookmark"){if(C)C.open()}else if(r==="add-widget"){if(to)to.open()}else if(r==="add-section"){let i=x.getState().items||[],{collisionService:n}=await Promise.resolve().then(() => (Y(),X)),e=n.findFirstAvailableSlot(1,1,i),s={type:"section",x:e.x,y:e.y,w:1,h:1,content:JSON.stringify({title:""})},p=await x.addItem(s);if(p)D.emit(V.SHOW_WIDGET_CONFIG,{item:p,type:"section"})}});window.addEventListener("widget-selected",async(o)=>{let t=o.detail,n=x.getState().items||[],{collisionService:e}=await Promise.resolve().then(() => (Y(),X)),s=e.findFirstAvailableSlot(t.defaultW,t.defaultH,n),p={type:"widget",x:s.x,y:s.y,w:t.defaultW,h:t.defaultH,content:JSON.stringify({widgetId:t.id,text:t.id==="notepad"||t.id==="markdown"?"":void 0})};await x.addItem(p)});window.addEventListener("drawer-close",()=>{if(j)j.setState({drawerOpen:!1})});window.addEventListener("click",(o)=>{let r=o.target,t=document.getElementById("add-menu"),i=document.getElementById("add-toggle");if(t&&t.classList.contains("active")&&!t.contains(r)&&i&&!i.contains(r))t.classList.remove("active")});function mt(){if(!ro)return;ro.innerHTML="";let o=document.createElement("bookmark-grid");ro.appendChild(o)}
