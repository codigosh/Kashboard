...
.avatar__initials {
    user-select: none;
        ...
        `}}if(!customElements.get("app-avatar"))customElements.define("app-avatar",hr);_();oo();Mo();_();var fr=(o)=>`
...
(max-width: 1024px) {
    ...
    .header__profile-container {
        ...
        `}}if(!customElements.get("app-header"))customElements.define("app-header",Rr);_();var ao=(o)=>String(o).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;"),Ya=(o)=>{if(!o)return"#";let r=o.trim().toLowerCase();if(r.startsWith("http://")||r.startsWith("https://")||r==="#")return ao(o);return"#"},Mr=({bookmarks:o,isEditing:r,isSearching:a,isTouchDevice:t,maxCols:i=12})=>{let e=Array.isArray(o)?o:[],s=(p)=>e.filter((c)=>c.parent_id===p);return`
    ${(t?e.filter((p)=>p.type!=="section"):a?e:e.filter((p)=>!p.parent_id)).map((p)=>{let c={};try{c=typeof p.content==="string"?JSON.parse(p.content):p.content}catch($){console.error("Failed to parse content for item",p.id,$)}let l=p.type==="section",x={x:p.x||1,y:p.y||1},u=Math.min(p.w||1,i),h=p.h||1;if(l){let $=s(p.id),G=(c.title||"").trim();return`
