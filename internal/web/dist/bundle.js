...
(max-width: 1024px) {
    ...
    ${(t?n:i?n:n.filter((p)=>!p.parent_id)).map((p)=>{let g={};try{g=typeof p.content==="string"?JSON.parse(p.content):p.content}catch(y){console.error("Failed to parse content for item",p.id,y)}let l=p.type==="section",b={x:p.x||1,y:p.y||1},_=Math.min(p.w||1,a),h=p.h||1;if(l){let y=d(p.id),H=(g.title||"").trim();return`
            <fieldset class="bookmark-grid__section"
               ...
               `:""}
            </fieldset>`}return ii(p,g,r,b,_,h,t)}).join("")}

    <div id="ghost-element" class="bookmark-grid__ghost" style="display: none;"></div>
    `};function ii(o,r,i,t,a,n=0,d=!1){let c=n||o.h||1;if(o.type==="widget"){let _=r.widgetId,y={clock:"widget-clock",notepad:"widget-notepad",telemetry:"widget-telemetry"}[_]||"div",w=(r.text||"").replace(/"/g,"&quot;");return`
    ...
    `}let p=r.icon||"",l=p.startsWith("http")||p.startsWith("/")?`<img src="${go(p)}" alt="${go(r.label)}" class="bookmark-grid__icon-img" draggable="false" />`:p?go(p):'<svg class="bookmark-grid__icon-svg" viewBox="0 0 24 24"><path d="M10 6L8.59 7.41 13.17 12l-4.58 4.59L10 18l6-6z"/></svg>';return`
        <a ${i?'role="button"':`href="${et(r.url)}" target="_blank"`} class="bookmark-grid__card"
           draggable="${i}"
           data-id="${o.id}"
           data-type="${o.type}"
           data-orig-x="${o.x}" data-orig-y="${o.y}"
...
.visibility-row .checkbox-group {
    ...
    margin-bottom: 0;
        ...
        `,this.dialog=this.shadowRoot.querySelector("dialog"),this.shadowRoot.querySelectorAll(".card").forEach((o)=>{o.addEventListener("click",()=>{let r=o.getAttribute("data-id"),i=er.find((t)=>t.id===r);if(i)this.selectWidget(i)})})}}customElements.define("add-widget-modal",Ei);Z();u();var Pi=`:host {
    display: contents;
}
...
.modal-actions {
    ...
    justify-content: flex-end;
        ...
        `}}Lo();var B=document.getElementById("main-topbar"),tt=document.getElementById("right-drawer"),Ro=document.getElementById("dashboard-root"),eo,No,Co;function P(o){let r=document.getElementById("__dbg");if(!r)r=document.createElement("div"),r.id="__dbg",r.style.cssText="background:#0a0a1a;color:#0f0;font:11px/1.5 monospace;padding:8px 12px;margin:8px;border:1px solid #333;border-radius:6px;word-break:break-all;",(document.getElementById("dashboard-root")||document.body).prepend(r);r.innerHTML+=o+"<br>"}it(async()=>{P("▶ bootstrap START");try{X.init(),P("✓ ThemeService.init"),await m.fetchUser(),P("✓ userStore.fetchUser");let o=m.getUser();if(o){if(o.language)await s.setLanguage(o.language);if(o.preferences&&o.preferences.grid_columns)document.documentElement.style.setProperty("--user-preferred-columns",o.preferences.grid_columns.toString());if(o.theme==="dark")X.enableDark();else if(o.theme==="light")X.enableLight();else if(window.matchMedia&&window.matchMedia("(prefers-color-scheme: dark)").matches)X.enableDark();else X.enableLight();if(o.id)x.setUserId(o.id)}if(B){if(B.setState({user:o}),o&&o.project_name)B.setAttribute("title",o.project_name);m.subscribe((i)=>{if(i){if(B.setState({user:i}),i.project_name)B.setAttribute("title",i.project_name)}})}P("✓ per-user sync done"),await x.fetchItems(),P("✓ fetchItems ("+x.getState().items.length+" items)"),na(),P("✓ renderDashboard"),yo.start(),eo=document.createElement("add-bookmark-modal"),document.body.appendChild(eo),No=document.createElement("add-widget-modal"),document.body.appendChild(No),Co=document.createElement("confirmation-modal"),document.body.appendChild(Co);let r=document.createElement("widget-config-modal");document.body.appendChild(r),P("✓ bootstrap COMPLETE — UA: "+navigator.userAgent.substring(0,80))}catch(o){P("✗ CRASH: "+(o.stack||o.message||o))}});fo.on(ho.SHOW_CONFIRMATION,async(o)=>{let{title:r,message:i,onConfirm:t}=o.detail;if(Co){if(await Co.confirm(r,i)&&t)t()}});fo.on(ho.SHOW_WIDGET_CONFIG,(o)=>{let{item:r,type:i}=o.detail;if(i==="bookmark"){if(eo)eo.openForEdit(r)}else{let t=document.querySelector("widget-config-modal");if(t)t.open(r)}});fo.on(ho.NOTIFY,(o)=>{let r=document.querySelector("app-notifier");if(r)r.show(o.detail.message,o.detail.type)});if(B)B.addEventListener("drawer-toggle",(o)=>{if(o.detail.action==="open")tt.open(),B.setState({drawerOpen:!0});else tt.close(),B.setState({drawerOpen:!1})}),B.addEventListener("edit-mode-change",(o)=>{let r=o.detail.active;if(Ro.classList.toggle("edit-mode",r),!r)window.location.reload()}),B.addEventListener("search-input",(o)=>{let r=o.detail.query;x.setSearchQuery(r)}),B.addEventListener("add-item",async(o)=>{let r=o.detail.action;if(r==="add-bookmark"){if(eo)eo.open()}else if(r==="add-widget"){if(No)No.open()}else if(r==="add-section"){let t=x.getState().items||[],{collisionService:a}=await Promise.resolve().then(() => (oo(),lo)),n=a.findFirstAvailableSlot(1,1,t),d={type:"section",x:n.x,y:n.y,w:1,h:1,content:JSON.stringify({title:""})},c=await x.addItem(d);if(c)fo.emit(ho.SHOW_WIDGET_CONFIG,{item:c,type:"section"})}});window.addEventListener("widget-selected",async(o)=>{let i=o.detail,a=x.getState().items||[],{collisionService:n}=await Promise.resolve().then(() => (oo(),lo)),d=n.findFirstAvailableSlot(i.defaultW,i.defaultH,a),c={type:"widget",x:d.x,y:d.y,w:i.defaultW,h:i.defaultH,content:JSON.stringify({widgetId:i.id,text:i.id==="notepad"?"":void 0})};await x.addItem(c)});window.addEventListener("drawer-close",()=>{if(B)B.setState({drawerOpen:!1})});window.addEventListener("click",(o)=>{let r=o.target,i=document.getElementById("add-menu"),t=document.getElementById("add-toggle");if(i&&i.classList.contains("active")&&!i.contains(r)&&t&&!t.contains(r))i.classList.remove("active")});function na(){if(!Ro)return;Ro.innerHTML="";let o=document.createElement("bookmark-grid");Ro.appendChild(o)}
