...
(max-width: 1024px) {
    ...
    ${(t?e.filter((p)=>p.type!=="section"):a?e:e.filter((p)=>!p.parent_id)).map((p)=>{let c={};try{c=typeof p.content==="string"?JSON.parse(p.content):p.content}catch(m){console.error("Failed to parse content for item",p.id,m)}let l=p.type==="section",g={x:p.x||1,y:p.y||1},u=Math.min(p.w||1,i),h=p.h||1;if(l){let m=s(p.id),G=(c.title||"").trim();return`
            <fieldset class="bookmark-grid__section"
               ...
               `:""}
            </fieldset>`}return Mr(p,c,r,g,u,h)}).join("")}

    <div id="ghost-element" class="bookmark-grid__ghost" style="display: none;"></div>
    `};function Mr(o,r,a,t,i,e=0){let s=e||o.h||1;if(o.type==="widget"){let g=r.widgetId,h={clock:"widget-clock",notepad:"widget-notepad",telemetry:"widget-telemetry"}[g]||"div",G=(r.text||"").replace(/"/g,"&quot;");return`
