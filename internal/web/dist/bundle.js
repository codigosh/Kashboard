...
`,wr=(o,r,a)=>{let t=a?.toLowerCase()==="admin"||a?.toLowerCase()==="administrator";return`
    <div class="bento-grid" style="grid-template-columns: 1fr;">
...
.select-option.selected:hover {
    background: rgba(var(--accent-rgb), 0.15);
        ...
        `,this.shadowRoot.getElementById("trigger")?.addEventListener("click",(a)=>{a.stopPropagation(),this.toggle()})}}if(!customElements.get("app-select"))customElements.define("app-select",kr);var $r=`:host {
    display: block;
    color: var(--text-main);
    max-width: 900px;
    margin: 0 auto;
...
(max-width: 1024px) {
    ...
    .header__profile-container {
        ...
        `}}if(!customElements.get("app-header"))customElements.define("app-header",Qr);_();var ao=(o)=>String(o).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;"),Ta=(o)=>{if(!o)return"#";let r=o.trim().toLowerCase();if(r.startsWith("http://")||r.startsWith("https://")||r==="#")return ao(o);return"#"},Kr=({bookmarks:o,isEditing:r,isSearching:a,isTouchDevice:t,maxCols:i=12})=>{let e=Array.isArray(o)?o:[],s=(p)=>e.filter((c)=>c.parent_id===p);return`
    ${(t?e:a?e:e.filter((p)=>!p.parent_id)).map((p)=>{let c={};try{c=typeof p.content==="string"?JSON.parse(p.content):p.content}catch(w){console.error("Failed to parse content for item",p.id,w)}let l=p.type==="section",x={x:p.x||1,y:p.y||1},v=Math.min(p.w||1,i),u=p.h||1;if(l){let w=s(p.id),G=(c.title||"").trim();return`
            <fieldset class="bookmark-grid__section"
               ...
               `:""}
            </fieldset>`}return Rr(p,c,r,x,v,u,t)}).join("")}

    <div id="ghost-element" class="bookmark-grid__ghost" style="display: none;"></div>
    `};function Rr(o,r,a,t,i,e=0,s=!1){let d=e||o.h||1;if(o.type==="widget"){let v=r.widgetId,w={clock:"widget-clock",notepad:"widget-notepad",telemetry:"widget-telemetry"}[v]||"div",m=(r.text||"").replace(/"/g,"&quot;");return`
    ...
    `}let p=r.icon||"",l=p.startsWith("http")||p.startsWith("/")?`<img src="${ao(p)}" alt="${ao(r.label)}" class="bookmark-grid__icon-img" draggable="false" />`:p?ao(p):'<svg class="bookmark-grid__icon-svg" viewBox="0 0 24 24"><path d="M10 6L8.59 7.41 13.17 12l-4.58 4.59L10 18l6-6z"/></svg>';return`
        <a ${a?'role="button"':`href="${Ta(r.url)}" target="_blank"`} class="bookmark-grid__card"
           draggable="${a}"
           data-id="${o.id}"
           data-type="${o.type}"
           data-orig-x="${o.x}" data-orig-y="${o.y}"
