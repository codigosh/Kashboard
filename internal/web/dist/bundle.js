...
.avatar__initials {
    user-select: none;
        ...
        `}}if(!customElements.get("app-avatar"))customElements.define("app-avatar",hr);_();oo();Ro();_();var fr=(o)=>`
...
@keyframes blink {
    ...
    100% {
        ...
        `}}if(!customElements.get("app-topbar"))customElements.define("app-topbar",Br);_();var Hr=({title:o,dropdownOpen:r})=>`
    <header class="header">
...
(max-width: 1024px) {
    ...
    .header__profile-container {
        ...
        `}}if(!customElements.get("app-header"))customElements.define("app-header",Qr);_();var ao=(o)=>String(o).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;"),Oa=(o)=>{if(!o)return"#";let r=o.trim().toLowerCase();if(r.startsWith("http://")||r.startsWith("https://")||r==="#")return ao(o);return"#"},Rr=({bookmarks:o,isEditing:r,isSearching:a,isTouchDevice:t,maxCols:i=12})=>{let e=Array.isArray(o)?o:[],s=(p)=>e.filter((c)=>c.parent_id===p);return`
    ${(t?e:a?e:e.filter((p)=>!p.parent_id)).map((p)=>{let c={};try{c=typeof p.content==="string"?JSON.parse(p.content):p.content}catch($){console.error("Failed to parse content for item",p.id,$)}let l=p.type==="section",x={x:p.x||1,y:p.y||1},u=Math.min(p.w||1,i),h=p.h||1;if(l){let $=s(p.id),G=(c.title||"").trim();return`
            <fieldset class="bookmark-grid__section"
               data-id="${p.id}"
               data-orig-x="${p.x}" data-orig-y="${p.y}"
               draggable="${r}"
               style="--x: ${x.x}; --y: ${x.y}; --w: ${u}; --h: ${h};">
               ...
               `:""}
            </fieldset>`}return Kr(p,c,r,x,u,h)}).join("")}

    <div id="ghost-element" class="bookmark-grid__ghost" style="display: none;"></div>
    `};function Kr(o,r,a,t,i,e=0){let s=e||o.h||1;if(o.type==="widget"){let x=r.widgetId,h={clock:"widget-clock",notepad:"widget-notepad",telemetry:"widget-telemetry"}[x]||"div",G=(r.text||"").replace(/"/g,"&quot;");return`
        <div class="bookmark-grid__card"
            ...
            ${a?`
            <div class="bookmark-actions">
                 ${["clock","telemetry"].includes(x)?`<button class="action-btn btn-edit" title="${n.t("general.edit")}">✎</button>`:""}
                 <button class="action-btn btn-delete" title="${n.t("general.delete")}">\uD83D\uDDD1</button>
            </div>
            ${["clock","telemetry"].includes(x)?"":'<div class="resize-handle"></div>'}
            `:""}
        </div>
    ...
    `;connectedCallback(){super.connectedCallback(),this._unsubscribe=g.subscribe((r)=>{let a=this.isDashboardEditing;if(this.isDashboardEditing=r.isEditing||!1,a!==this.isDashboardEditing)if(this.isDashboardEditing)if(this.hasUnsavedChanges&&this.isInternalEditing)if(confirm(n.t("widget.notepad.confirm_discard")))this.isInternalEditing=!1,this.hasUnsavedChanges=!1,this.cancelAutosave(),this.requestUpdate();else setTimeout(()=>g.toggleEditMode(),0);else this.isInternalEditing=!1,this.cancelAutosave(),this.requestUpdate();else this.loadFromStore()});let o=g.getState();this.isDashboardEditing=o.isEditing||!1,this.loadFromStore()}disconnectedCallback(){if(super.disconnectedCallback(),this._unsubscribe)this._unsubscribe();this.cancelAutosave()}updated(o){if(super.updated(o),o.has("content")&&!this.isInternalEditing&&this.editorElement){let r=I.sanitize(this.content);if(this.editorElement.innerHTML!==r)this.editorElement.innerHTML=r}}loadFromStore(){if(!this.itemId)return;let r=g.getState().items.find((a)=>a.id===this.itemId);if(r){let a=Fo.getNotepadText(r.content);try{let t=JSON.parse(a);if(t&&typeof t==="object"&&"html"in t)this.content=t.html||"",this.checklistBlocks=t.checklists||[];else this.content=a,this.checklistBlocks=[]}catch{this.content=a,this.checklistBlocks=[]}this.characterCount=this.content.length,this._lastSavedContent=a}}exec(o,r){if(document.execCommand(o,!1,r),this.editorElement)this.editorElement.focus()}cancelAutosave(){if(this._autosaveTimeout)clearTimeout(this._autosaveTimeout),this._autosaveTimeout=null}async saveContent(o=!1){if(this._savePromise){await this._savePromise;return}try{let r=this.shadowRoot?.querySelector(".editor")||this.shadowRoot?.querySelector("[contenteditable]");if(!r)throw Error("Critical: Editor element missing");let a=r.innerHTML;if(a.length>Lo){alert(n.t("widget.notepad.error.too_large"));return}let t=I.sanitize(a),i=JSON.stringify({html:t,checklists:this.checklistBlocks});if(!o)this.content=t,this.characterCount=t.length,this.isInternalEditing=!1,this.hasUnsavedChanges=!1,this.requestUpdate();this._savePromise=(async()=>{if(!this.itemId)return;if(!o)this.isSaving=!0,this.requestUpdate();try{let e=g.getState().items.find((d)=>d.id===this.itemId);if(!e){console.warn("[NotepadWidget] Item no longer exists");return}let s=Fo.setNotepadText(e.content,i);if(await g.updateItem({id:this.itemId,content:s}),this._lastSavedContent=i,o)this.hasUnsavedChanges=!1}catch(e){if(console.error("[NotepadWidget] Save failed:",e),!o)alert(n.t("widget.notepad.error.save")+e)}finally{if(!o)this.isSaving=!1,this.requestUpdate()}})(),await this._savePromise}catch(r){if(console.error("[NotepadWidget] Save error:",r),!o)alert(n.t("widget.notepad.error.save")+r)}finally{this._savePromise=null}}startEditing(){if(this.isDashboardEditing)return;this.isInternalEditing=!0,setTimeout(()=>{if(this.editorElement){this.editorElement.focus();let o=I.sanitize(this.content);if(this.editorElement.innerHTML!==o)this.editorElement.innerHTML=o;this.focusEditor(!0)}},0)}focusEditor(o=!0){if(!this.editorElement)return;if(this.editorElement.focus(),o){let r=window.getSelection();if(r){let a=document.createRange();a.selectNodeContents(this.editorElement),a.collapse(!1),r.removeAllRanges(),r.addRange(a)}}}handleColor(o){let r=o.target;this.exec("foreColor",r.value)}handleToolbarWheel(o){o.preventDefault();let r=o.currentTarget;r.scrollLeft+=o.deltaY}insertChecklist(){let o="block-"+Math.random().toString(36).substring(2,11),r={id:o,items:[{id:"item-"+Math.random().toString(36).substring(2,11),checked:!1,text:""}]};this.checklistBlocks=[...this.checklistBlocks,r],this.hasUnsavedChanges=!0,this.scheduleAutosave(),this.updateComplete.then(()=>{this.shadowRoot?.querySelector(`checklist-block[block-id="${o}"]`)?.shadowRoot?.querySelector('input[type="text"]')?.focus()})}insertCode(){let r=`<pre><code>${n.t("widget.notepad.prompt.code_block")}</code></pre><p><br></p>`;this.exec("insertHTML",r),setTimeout(()=>{let a=window.getSelection();if(a&&this.editorElement){let t=this.editorElement.querySelectorAll("code"),i=t[t.length-1];if(i){let e=document.createRange();e.selectNodeContents(i),a.removeAllRanges(),a.addRange(e)}}},0)}handleChecklistChange(o){let{blockId:r,items:a}=o.detail;this.checklistBlocks=this.checklistBlocks.map((t)=>t.id===r?{...t,items:a}:t),this.hasUnsavedChanges=!0,this.scheduleAutosave()}deleteChecklistBlock(o){this.checklistBlocks=this.checklistBlocks.filter((r)=>r.id!==o),this.hasUnsavedChanges=!0,this.scheduleAutosave()}handleEditorInput(o){let r=o.target;if(this.characterCount=r.innerHTML.length,this.hasUnsavedChanges=r.innerHTML!==this._lastSavedContent,this.cancelAutosave(),sa>0)this._autosaveTimeout=setTimeout(()=>{if(this.hasUnsavedChanges)this.saveContent(!0)},sa)}handleEditorKeydown(o){if(o.ctrlKey||o.metaKey)switch(o.key.toLowerCase()){case"b":o.preventDefault(),this.exec("bold");return;case"i":o.preventDefault(),this.exec("italic");return;case"s":o.preventDefault(),this.saveContent(!1);return;case"z":if(o.shiftKey)o.preventDefault(),this.exec("redo");else o.preventDefault(),this.exec("undo");return;case"k":o.preventDefault();let r=prompt(n.t("widget.notepad.prompt.url"));if(r)this.exec("createLink",r);return}}handlePaste(o){let r=o.clipboardData?.items;if(!r)return;for(let a of Array.from(r))if(a.type.indexOf("image")!==-1){o.preventDefault();let t=a.getAsFile();if(!t)continue;let i=new FileReader;i.onload=(e)=>{let s=e.target?.result;if(s.length>1e6){alert(n.t("widget.notepad.error.image_too_large"));return}this.exec("insertImage",s)},i.readAsDataURL(t);return}}render(){try{if(!this.isInternalEditing){let t=I.sanitize(this.content)||`<span style='opacity:0.5; font-style:italic; color: var(--text-dim);'>${n.t("widget.notepad.placeholder")}</span>`;return b`
                ...
                <div class="container">
                    <div class="toolbar" @wheel="${this.handleToolbarWheel}" title="${n.t("widget.notepad.tool.scroll_hint")}">
                        <!-- History Group -->
                        <div class="group">
                             <button @click="${(a)=>{a.preventDefault(),this.exec("undo")}}" title="${n.t("widget.notepad.tool.undo")} (Ctrl+Z)">${m.undo}</button>
                             <button @click="${(a)=>{a.preventDefault(),this.exec("redo")}}" title="${n.t("widget.notepad.tool.redo")} (Ctrl+Shift+Z)">${m.redo}</button>
                        </div>

                        <!-- Text Group -->
                        <div class="group">
                            <button @click="${(a)=>{a.preventDefault(),this.exec("formatBlock","H1")}}" title="${n.t("widget.notepad.tool.h1")}">${m.h1}</button>
                            <button @click="${(a)=>{a.preventDefault(),this.exec("formatBlock","H2")}}" title="${n.t("widget.notepad.tool.h2")}">${m.h2}</button>
                            <button @click="${(a)=>{a.preventDefault(),this.exec("bold")}}" title="${n.t("widget.notepad.tool.bold")} (Ctrl+B)">${m.bold}</button>
                            <button @click="${(a)=>{a.preventDefault(),this.exec("italic")}}" title="${n.t("widget.notepad.tool.italic")} (Ctrl+I)">${m.italic}</button>
                        ...
                        <!-- Paragraph Group -->
                        <div class="group">
                             <button @click="${(a)=>{a.preventDefault(),this.exec("justifyLeft")}}" title="${n.t("widget.notepad.tool.align_left")}">${m.alignLeft}</button>
                             <button @click="${(a)=>{a.preventDefault(),this.exec("justifyCenter")}}" title="${n.t("widget.notepad.tool.align_center")}">${m.alignCenter}</button>
                             <button @click="${(a)=>{a.preventDefault(),this.exec("justifyRight")}}" title="${n.t("widget.notepad.tool.align_right")}">${m.alignRight}</button>
                             <button @click="${(a)=>{a.preventDefault(),this.exec("justifyFull")}}" title="${n.t("widget.notepad.tool.align_justify")}">${m.alignJustify}</button>
                        </div>

                        <div class="group">
                            <button @click="${(a)=>{a.preventDefault(),this.insertChecklist()}}" title="${n.t("widget.notepad.tool.checklist")}">${m.checklist}</button>
                            <button @click="${(a)=>{a.preventDefault(),this.exec("insertUnorderedList")}}" title="${n.t("widget.notepad.tool.list_bullet")}">${m.list}</button>
                            <button @click="${(a)=>{a.preventDefault(),this.exec("insertOrderedList")}}" title="${n.t("widget.notepad.tool.list_ordered")}">${m.listOrdered}</button>
                        </div>

                        <!-- Insert Group -->
                        <div class="group">
                            <button @click="${(a)=>{a.preventDefault(),this.insertCode()}}" title="${n.t("widget.notepad.tool.code")}">${m.code}</button>
                            <button @click="${(a)=>{a.preventDefault();let t=prompt(n.t("widget.notepad.prompt.url"));if(t)this.exec("createLink",t)}}" title="${n.t("widget.notepad.tool.link")} (Ctrl+K)">${m.link}</button>
                            <button @click="${(a)=>{a.preventDefault();let t=prompt(n.t("widget.notepad.prompt.image_url"));if(t)this.exec("insertImage",t)}}" title="${n.t("widget.notepad.tool.image")}">${m.image}</button>
                             <button @click="${(a)=>{a.preventDefault(),this.exec("removeFormat")}}" title="${n.t("widget.notepad.tool.clear_format")}">${m.clear}</button>
                        </div>
        ...
        `,this.cpuBar=this.shadowRoot.querySelector(".cpu-bar"),this.ramBar=this.shadowRoot.querySelector(".ram-bar"),this.tempBar=this.shadowRoot.querySelector(".temp-bar"),this.cpuText=this.shadowRoot.querySelector(".cpu-text"),this.ramText=this.shadowRoot.querySelector(".ram-text"),this.tempText=this.shadowRoot.querySelector(".temp-text")}}customElements.define("widget-telemetry",da);var pa=`:host {
    /* Fixed Grid System */
...
.bookmark-grid__nested-content {
    ...
    height: 100%;
    align-content: start;
}`;class ca extends HTMLElement{bookmarks=[];allItems=[];isEditing=!1;searchQuery="";_unsubscribe;_unsubscribeI18n;_resizeObserver;_resizeDebounce=null;dragTargetId=null;ghostEl=null;dragOffsetX=0;dragOffsetY=0;isResizing=!1;resizeTargetId=null;initialResizeX=0;initialResizeY=0;initialResizeW=0;initialResizeH=0;currentColWidth=0;currentGridCols=12;isTouchDevice=!1;constructor(){super();this.attachShadow({mode:"open"}),this.checkMobileMode(),window.matchMedia("(pointer: coarse)").addEventListener("change",()=>{this.checkMobileMode()}),window.addEventListener("resize",()=>{this.checkMobileMode()})}checkMobileMode(){let o=window.matchMedia("(pointer: coarse)").matches,r=window.innerWidth<768,a=o||r;if(this.isTouchDevice!==a)console.log("[BookmarkGrid] Mobile Mode Changed:",a,{isTouch:o,isSmall:r,width:window.innerWidth}),this.isTouchDevice=a,this.updateTouchMode(),this.applyFilters(),this.render()}updateTouchMode(){if(this.isTouchDevice)this.classList.add("touch-mode");else this.classList.remove("touch-mode")}_widgetModal;_boundActionClick=this.handleActionClick.bind(this);_boundMouseMove=this.handleWindowMouseMove.bind(this);_boundMouseUp=this.handleWindowMouseUp.bind(this);applyFilters(){let o=this.isTouchDevice;if(console.log("[BookmarkGrid] applyFilters() called",{isTouch:o,searchQuery:this.searchQuery,totalItems:this.allItems.length,bookmarkCount:this.allItems.filter((r)=>r.type==="bookmark").length}),this.searchQuery||o)this.classList.add("search-active"),this.bookmarks=this.allItems.filter((r)=>{if(r.type!=="bookmark")return!1;let a=r.content;if(typeof r.content==="string")try{a=JSON.parse(r.content)}catch{return!1}if(o){let t=a.hasOwnProperty("visibleTouch"),i=a.visibleTouch,e=a.visibleMobile,s=a.visibleTablet,d=window.innerWidth,p=d<768;if(console.log(`[BookmarkGrid] Bookmark "${a.label}"`,{hasVisibleTouch:t,visibleTouch:i,visibleMobile:e,visibleTablet:s,width:d,isMobileWidth:p}),t){if(i===!1)return console.log(`[BookmarkGrid] FILTERED OUT "${a.label}" - visibleTouch === false`),!1}else{if(p&&e===!1)return console.log(`[BookmarkGrid] FILTERED OUT "${a.label}" - legacy visibleMobile === false`),!1;if(!p&&s===!1)return console.log(`[BookmarkGrid] FILTERED OUT "${a.label}" - legacy visibleTablet === false`),!1}}if(this.searchQuery)return(a.label||"").toLowerCase().includes(this.searchQuery);return!0}),console.log("[BookmarkGrid] After filtering:",this.bookmarks.length,"bookmarks visible");else this.classList.remove("search-active"),this.bookmarks=this.allItems}connectedCallback(){this.applyFilters(),this.render(),this.updateGridMetrics(),this._resizeObserver=new ResizeObserver(()=>{if(this._resizeDebounce)return;this._resizeDebounce=setTimeout(()=>{this._resizeDebounce=null,this.updateGridMetrics();let o=this.getBoundingClientRect();g.setGridMetrics(o.width,this.currentGridCols),this.applyFilters(),this.render()},16)}),this._resizeObserver.observe(this),this._unsubscribe=g.subscribe((o)=>{let r=!1;if(this.isEditing!==o.isEditing)this.isEditing=o.isEditing,r=!0;if(this.searchQuery!==o.searchQuery)this.searchQuery=o.searchQuery,r=!0;let a=Array.isArray(o.items)?o.items:[];if(JSON.stringify(this.allItems)!==JSON.stringify(a)||r)this.allItems=a,this.applyFilters(),r=!0;if(r)this.render()}),this.setupDragListeners(),this.setupResizeListeners(),this.setupActionListeners(),co.start(),this._unsubscribeI18n=n.subscribe(()=>this.render()),Promise.resolve().then(() => (oo(),Ko)).then(({userStore:o})=>{this._unsubscribeUser=o.subscribe(()=>{requestAnimationFrame(()=>{this.updateGridMetrics(),this.render()})})})}_unsubscribeUser=null;setupActionListeners(){let o=this.shadowRoot;o.removeEventListener("click",this._boundActionClick),o.addEventListener("click",this._boundActionClick)}async handleActionClick(o){if(!this.isEditing)return;let r=o.target;if(!r)return;let a=r.closest(".btn-delete"),t=r.closest(".btn-edit");if(a||t)o.preventDefault(),o.stopPropagation();else if(r.closest("a"))o.preventDefault();if(a){let i=a.closest(".bookmark-grid__card, .bookmark-grid__group, .bookmark-grid__section");if(!i)return;let e=parseInt(i.dataset.id||"0"),s=this.allItems.find((l)=>l.id===e);if(!s)return;let d=s.type==="section"?n.t("type.section"):s.type==="widget"?n.t("type.widget"):n.t("type.bookmark"),{eventBus:p,EVENTS:c}=await Promise.resolve().then(() => (qo(),Vo));p.emit(c.SHOW_CONFIRMATION,{title:`${n.t("general.delete")} ${d}`,message:n.t("bookmark.delete_confirm_message"),onConfirm:()=>{g.deleteItem(e)}});return}if(t){let i=t.closest(".bookmark-grid__card, .bookmark-grid__group, .bookmark-grid__section");if(!i)return;let e=parseInt(i.dataset.id||"0"),s=this.allItems.find((d)=>d.id===e);if(s){let{eventBus:d,EVENTS:p}=await Promise.resolve().then(() => (qo(),Vo));if(s.type==="widget")d.emit(p.SHOW_WIDGET_CONFIG,{item:s,type:"widget"});else if(s.type==="section")d.emit(p.SHOW_WIDGET_CONFIG,{item:s,type:"section"});else d.emit(p.SHOW_WIDGET_CONFIG,{item:s,type:"bookmark"})}return}}disconnectedCallback(){if(this._unsubscribe)this._unsubscribe();if(this._unsubscribeI18n)this._unsubscribeI18n();if(this._unsubscribeUser)this._unsubscribeUser();if(this._resizeObserver)this._resizeObserver.disconnect();if(this._resizeDebounce)clearTimeout(this._resizeDebounce);if(this._updateGhostTimeout)clearTimeout(this._updateGhostTimeout);window.removeEventListener("mousemove",this._boundMouseMove),window.removeEventListener("mouseup",this._boundMouseUp),co.stop()}updateGridMetrics(){let o=this.getBoundingClientRect(),r=getComputedStyle(this),a=r.getPropertyValue("--grid-cols").trim(),t=parseInt(a)||12,i=r.columnGap||r.gap||"16px",e=parseFloat(i)||16;if(o.width<=0){this.currentGridCols=t,this.currentColWidth=100,this.style.setProperty("--current-grid-cols",t.toString()),this.style.setProperty("--row-height","100px");return}let s=(t-1)*e,d=(o.width-s)/t;if(d<10)d=10;this.currentGridCols=t,this.currentColWidth=d,this.style.setProperty("--current-grid-cols",String(t)),this.style.setProperty("--row-height",`${d}px`),this.ensureGridBuffer()}ensureGridBuffer(){let o=0;this.bookmarks.forEach((s)=>{let d=(s.y||1)+(s.h||1);if(d>o)o=d});let a=o+1,t=this.currentColWidth||100,i=16,e=a*t+(a-1)*i;this.style.minHeight=`${e}px`}setupResizeListeners(){window.addEventListener("mousemove",this._boundMouseMove),window.addEventListener("mouseup",this._boundMouseUp),this.shadowRoot.addEventListener("mousedown",(o)=>{let r=o;if(!this.isEditing)return;let a=r.target;if(a.classList.contains("resize-handle")){r.preventDefault(),r.stopPropagation();let t=a.closest(".bookmark-grid__card, .bookmark-grid__section");if(!t||!t.dataset.id)return;let i=parseInt(t.dataset.id),e=this.bookmarks.find((h)=>h.id===i);if(!e)return;this.isResizing=!0,this.resizeTargetId=i,this.initialResizeX=r.clientX,this.initialResizeY=r.clientY,this.initialResizeW=e.w,this.initialResizeH=e.h;let s=this,d=s.getBoundingClientRect(),c=getComputedStyle(s).getPropertyValue("--current-grid-cols").trim(),l=c?parseInt(c,10):9,x=isNaN(l)?9:l,u=16;this.currentColWidth=(d.width-(x-1)*u)/x,this.updateGhost({x:e.x,y:e.y,w:e.w,h:e.h},!0)}})}_updateGhostTimeout=null;handleWindowMouseMove(o){if(!this.isResizing||!this.resizeTargetId)return;if(this._updateGhostTimeout)return;this._updateGhostTimeout=setTimeout(()=>{this._updateGhostTimeout=null,this.processResizeMove(o)},16)}processResizeMove(o){if(!this.isResizing||!this.resizeTargetId)return;let r=o.clientX-this.initialResizeX,a=o.clientY-this.initialResizeY,t=Math.round(r/(this.currentColWidth+16)),i=Math.round(a/(this.currentColWidth+16)),e=this.initialResizeW+t,s=this.initialResizeH+i,d=this.bookmarks.find((p)=>p.id===this.resizeTargetId);if(d){let p=this.applyResizeConstraints(e,s,d),c={x:d.x,y:d.y,w:p.w,h:p.h,id:d.id,parent_id:d.parent_id},l=C.calculateDropValidity(c,this.bookmarks,this.currentGridCols);this.updateGhost(c,l.valid)}}applyResizeConstraints(o,r,a){let t=1,i=1;if(a.type==="widget"){let e=a.content;if(typeof a.content==="string")try{e=JSON.parse(a.content)}catch{return{w:Math.max(1,Math.min(12,o)),h:Math.max(1,Math.min(12,r))}}let s=(e.widgetId||"").toLowerCase();if(s==="notepad")t=2,i=2;else if(s==="clock")return{w:2,h:1};else if(s==="telemetry")return{w:2,h:1};let d=Math.max(t,Math.min(12,o)),p=Math.max(i,Math.min(12,r));return{w:d,h:p}}else if(a.type==="section"){let e=Math.max(1,Math.min(12,o)),s=Math.max(1,Math.min(12,r));return{w:e,h:s}}else{let e=Math.max(1,Math.min(2,o)),s=Math.max(1,Math.min(2,r));return{w:e,h:s}}}async handleWindowMouseUp(o){if(!this.isResizing||!this.resizeTargetId)return;if(this._updateGhostTimeout)clearTimeout(this._updateGhostTimeout),this._updateGhostTimeout=null;let r=o.clientX-this.initialResizeX,a=o.clientY-this.initialResizeY,t=Math.round(r/(this.currentColWidth+16)),i=Math.round(a/(this.currentColWidth+16)),e=this.initialResizeW+t,s=this.initialResizeH+i,d=this.bookmarks.find((x)=>x.id===this.resizeTargetId);if(!d)return;let p=this.applyResizeConstraints(e,s,d);e=p.w,s=p.h;let c={x:d.x,y:d.y,w:e,h:s,id:d.id,parent_id:d.parent_id};if(C.calculateDropValidity(c,this.bookmarks,this.currentGridCols).valid&&(d.w!==e||d.h!==s)){if(d.type==="section"){let x=e*s,u=this.bookmarks.filter((h)=>h.parent_id===d.id);if(u.sort((h,$)=>h.y-$.y||h.x-$.x),u.length>x){let h=u.slice(x),$=[...this.bookmarks],G=h.map((y)=>{let f=C.findFirstAvailableSlot(y.w,y.h,$,this.currentGridCols);return $.push({...y,x:f.x,y:f.y,parent_id:void 0}),{id:y.id,x:f.x,y:f.y,parent_id:void 0}});await Promise.all(G.map((y)=>g.updateItem(y)))}}await g.resizeItem(d.id,e,s)}if(this.isResizing=!1,this.resizeTargetId=null,this.ghostEl)this.ghostEl.style.display="none"}setupDragListeners(){let o=this.shadowRoot,r=this;o.addEventListener("dragstart",(t)=>{let i=t;if(!this.isEditing){i.preventDefault();return}let e=i.target.closest('[draggable="true"]');if(e&&e.dataset.id){this.dragTargetId=parseInt(e.dataset.id),i.dataTransfer.effectAllowed="move",e.style.opacity="0.5";let s=e.getBoundingClientRect();if(this.dragOffsetX=i.clientX-s.left,this.dragOffsetY=i.clientY-s.top,i.dataTransfer)i.dataTransfer.setDragImage(e,this.dragOffsetX,this.dragOffsetY)}}),o.addEventListener("dragend",(t)=>{let i=t.target.closest('[draggable="true"]');if(i)i.style.opacity="1";if(this.dragTargetId=null,this.ghostEl)this.ghostEl.style.display="none";this.shadowRoot.querySelectorAll(".bookmark-grid__section").forEach((s)=>s.classList.remove("drop-target"))});let a=null;r.addEventListener("dragover",(t)=>{let i=t;if(!this.isEditing||!this.dragTargetId)return;if(i.preventDefault(),i.dataTransfer.dropEffect="move",a)return;a=setTimeout(()=>{a=null,this.processDragOver(i,r)},16)}),r.addEventListener("drop",async(t)=>{let i=t;if(a)clearTimeout(a),a=null;await this.handleDrop(i,r)})}processDragOver(o,r){let i=document.querySelector(".main-content");if(i){let w=i.getBoundingClientRect(),po=o.clientY;if(po>w.bottom-100)i.scrollBy(0,15);else if(po<w.top+100)i.scrollBy(0,-15)}let e=r.getBoundingClientRect(),s=this.currentGridCols||12,d=e.width,p=16,c=(d-(s-1)*p)/s,l=this.bookmarks.find((w)=>w.id===this.dragTargetId);if(!l)return;let x=o.clientX-this.dragOffsetX,u=o.clientY-this.dragOffsetY,h=x-e.left,$=u-e.top,G=Math.floor(h/(c+p)),y=Math.floor($/(c+p)),f=Math.max(1,Math.min(s-l.w+1,G+1)),M=Math.max(1,y+1),Z={x:f,y:M,w:l.w,h:l.h,id:l.id,parent_id:l.parent_id},v=C.calculateDropValidity(Z,this.bookmarks,s),O=!this.bookmarks.some((w)=>{if(w.id===l.id)return!1;if(w.parent_id!==l.parent_id&&!v.targetGroup)return!1;let po=w.x||1,So=w.y||1,Ha=w.w||1,Ja=w.h||1;if(v.targetGroup&&w.id===v.targetGroup.id)return!1;return f<po+Ha&&f+l.w>po&&M<So+Ja&&M+l.h>So})&&v.valid;if(this.updateGhost(Z,O),this.shadowRoot.querySelectorAll(".bookmark-grid__section").forEach((w)=>w.classList.remove("drop-target")),v.targetGroup&&O){let w=this.shadowRoot.querySelector(`.bookmark-grid__section[data-id="${v.targetGroup.id}"]`);if(w)w.classList.add("drop-target")}}async handleDrop(o,r){if(!this.isEditing||!this.dragTargetId)return;o.preventDefault();let a=this.bookmarks.find((v)=>v.id===this.dragTargetId);if(!a)return;let t=r.getBoundingClientRect(),i=this.currentGridCols||12,e=t.width,s=16,d=(e-(i-1)*s)/i,p=o.clientX-this.dragOffsetX,c=o.clientY-this.dragOffsetY,l=p-t.left,x=c-t.top,u=Math.floor(l/(d+s)),h=Math.floor(x/(d+s)),$=Math.max(1,Math.min(i-a.w+1,u+1)),G=Math.max(1,h+1),y={x:$,y:G,w:a.w,h:a.h,id:a.id,parent_id:a.parent_id},f=C.calculateDropValidity(y,this.bookmarks,i);if(this.bookmarks.some((v)=>{if(v.id===a.id)return!1;if(v.parent_id!==a.parent_id&&!f.targetGroup)return!1;let so=v.x||1,O=v.y||1,Io=v.w||1,w=v.h||1;if(f.targetGroup&&v.id===f.targetGroup.id)return!1;return $<so+Io&&$+a.w>so&&G<O+w&&G+a.h>O})&&!f.targetGroup){if(this.ghostEl)this.ghostEl.style.display="none";this.shadowRoot.querySelectorAll(".drop-target").forEach((v)=>v.classList.remove("drop-target"));return}if(f.valid){let v={id:a.id,x:f.x,y:f.y};if(f.targetGroup)v.parent_id=f.targetGroup.id,v.x=f.x-f.targetGroup.x+1,v.y=f.y-f.targetGroup.y+1;else v.parent_id=void 0;await g.updateItem(v)}if(this.ghostEl)this.ghostEl.style.display="none";this.shadowRoot.querySelectorAll(".bookmark-grid__section").forEach((v)=>v.classList.remove("drop-target"))}updateGhost(o,r){if(!this.ghostEl)this.ghostEl=this.shadowRoot.getElementById("ghost-element");if(!this.ghostEl)return;if(this.ghostEl.style.display="block",this.ghostEl.style.setProperty("--ghost-x",String(o.x)),this.ghostEl.style.setProperty("--ghost-y",String(o.y)),this.ghostEl.style.setProperty("--ghost-w",String(o.w)),this.ghostEl.style.setProperty("--ghost-h",String(o.h)),r)this.ghostEl.classList.remove("invalid");else this.ghostEl.classList.add("invalid")}render(){if(this.isEditing)this.classList.add("edit-mode");else this.classList.remove("edit-mode");this.shadowRoot.innerHTML=`
            <style>${pa}</style>
