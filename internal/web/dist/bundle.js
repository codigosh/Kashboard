...
.paper {
    ...
    height: 100%;
            ...
            <style>${Ho}</style>
            ${Ao()}
        `}}if(!customElements.get("app-paper"))customElements.define("app-paper",Go);var qo=`:host {
...
.avatar__initials {
    user-select: none;
}`;class Eo extends HTMLElement{static get observedAttributes(){return["src","alt","initials"]}constructor(){super();this.attachShadow({mode:"open"})}connectedCallback(){this.render()}attributeChangedCallback(o,t,r){if(t!==r)this.render()}render(){let o=this.getAttribute("src"),t=this.getAttribute("initials")||"??",r=this.getAttribute("alt")||"User Avatar";this.shadowRoot.innerHTML=`
            <style>${Qo}</style>
            ${Fo({src:o||void 0,initials:t,alt:r})}
        `}}if(!customElements.get("app-avatar"))customElements.define("app-avatar",Eo);k();K();no();k();var Jo=(o)=>`
            ...
            <div class="settings-content__form-container" style="margin-top: 32px;">
                <div class="settings-content__form-group">
                    ...
                    <div style="display: flex; gap: 8px;">
                        ...
                        <app-button variant="primary" onclick="this.getRootNode().host.updateUsername(document.getElementById('username-input').value)">${i.t("action.update")}</app-button>
                    </div>
                </div>
            </div>
        </div>
...
`,Yo=(o,t,r)=>{let a=r?.toLowerCase()==="admin"||r?.toLowerCase()==="administrator";return`
    <div class="bento-grid" style="grid-template-columns: 1fr;">
                        ...
                        ${t.available?`
                             <div class="update-modal">
                                ...
                                <div class="update-modal__content">
                                    ...
                                    <p id="update-status" style="margin: 0; font-size: 12px; color: var(--text-dim); display: none; text-align: center;">${i.t("notifier.downloading_secure")}</p>
                                </div>
                            </div>
                        `:`
...
.select-option.selected:hover {
    background: rgba(var(--accent-rgb), 0.15);
        ...
        `,this.shadowRoot.getElementById("trigger")?.addEventListener("click",(r)=>{r.stopPropagation(),this.toggle()})}}if(!customElements.get("app-select"))customElements.define("app-select",No);var To=`:host {
    display: block;
    color: var(--text-main);
    max-width: 900px;
    margin: 0 auto;
...
input:checked+.slider:before {
    ...
    background-color: white;
}`;class Vo extends HTMLElement{prefs;users=[];getCsrfToken(){let o=document.cookie.split("; ").find((t)=>t.startsWith("csrf_token="));return o?decodeURIComponent(o.split("=")[1]):""}static get observedAttributes(){return["section"]}constructor(){super();this.attachShadow({mode:"open"}),this.prefs={accent_color:"#0078D4",language:"en",widget_min_width:140,beta_updates:!1}}unsubscribe=null;_renderPending=!1;connectedCallback(){this.unsubscribe=w.subscribe((t)=>{if(t&&t.preferences)this.prefs={...t.preferences,project_name:t.project_name||t.preferences.project_name||"Lastboard"},this.render()}),this.fetchPrefs();let o=this.getAttribute("section");if(o==="users")this.fetchUsers();if(o==="about")this.checkForUpdates();this.render()}disconnectedCallback(){if(this.unsubscribe)this.unsubscribe(),this.unsubscribe=null}async fetchPrefs(){let o=w.getUser();if(o&&o.preferences)this.prefs={...o.preferences,project_name:o.project_name||o.preferences.project_name||"Lastboard"},this.render()}async fetchUsers(){try{this.users=await H.getUsers(),this.render()}catch(o){console.error("Failed to fetch users",o)}}attributeChangedCallback(o,t,r){if(o==="section"&&t!==r){if(r==="users")this.fetchUsers();if(r==="about")this.checkForUpdates();this.render()}}async savePrefs(o){let t={...this.prefs};if(this.prefs={...this.prefs,...o},o.accent_color)this.applyAccent(o.accent_color);if(o.language)i.setLanguage(o.language);if(o.theme)if(o.theme==="dark")G.enableDark();else G.enableLight();try{await w.updatePreferences(o)}catch{if(this.prefs=t,o.language&&t.language)i.setLanguage(t.language)}this.render()}applyAccent(o){let t=o,r={blue:"#228be6",indigo:"#4c6ef5",cyan:"#15aabf",teal:"#12b886",orange:"#fd7e14",red:"#fa5252",grape:"#be4bdb"};if(!o.startsWith("#"))t=r[o]||"#0078D4";document.documentElement.style.setProperty("--accent",t);let a=parseInt(t.slice(1,3),16),n=parseInt(t.slice(3,5),16),e=parseInt(t.slice(5,7),16);if(!isNaN(a)&&!isNaN(n)&&!isNaN(e))document.documentElement.style.setProperty("--accent-rgb",`${a}, ${n}, ${e}`)}async handleBetaToggle(o){this.savePrefs({beta_updates:o});let t=this.shadowRoot.getElementById("beta-text");if(t)if(o)t.style.color="#2fb344",t.style.borderColor="#2fb344",t.style.boxShadow="0 0 10px rgba(47, 179, 68, 0.2)";else t.style.color="var(--text-dim)",t.style.borderColor="var(--border)",t.style.boxShadow="none";await this.checkForUpdates()}updateDensity(o){let t=this.shadowRoot.getElementById("val-widget_min_width");if(t)t.textContent=o+"px";document.documentElement.style.setProperty("--widget-min-size",o+"px")}commitDensity(o){let t=parseInt(o);w.updatePreferences({widget_min_width:t})}updateColumns(o){let t=this.shadowRoot.getElementById("val-grid_columns");if(t)t.textContent=o;document.documentElement.style.setProperty("--user-preferred-columns",o)}commitColumns(o){let t=parseInt(o);w.updatePreferences({grid_columns:t})}async updateUsername(o){let t=w.getUser();if(!t)return;if(await w.updateProfile({username:o,avatar_url:this.prefs.avatar_url||t.avatar_url}),window.notifier)window.notifier.show(i.t("notifier.username_updated"))}async updatePassword(){let o=this.shadowRoot.getElementById("current-password")?.value,t=this.shadowRoot.getElementById("new-password")?.value,r=this.shadowRoot.getElementById("confirm-password")?.value;if(!o){if(window.notifier)window.notifier.show(i.t("notifier.password_required"),"error");return}if(t!==r){if(window.notifier)window.notifier.show(i.t("notifier.password_mismatch"),"error");return}if(!t){if(window.notifier)window.notifier.show(i.t("notifier.password_empty"),"error");return}try{if(await w.changePassword({current_password:o,new_password:t}),window.notifier)window.notifier.show(i.t("notifier.password_changed"));this.shadowRoot.getElementById("current-password").value="",this.shadowRoot.getElementById("new-password").value="",this.shadowRoot.getElementById("confirm-password").value="",setTimeout(async()=>{try{await fetch("/logout",{method:"POST",headers:{"X-CSRF-Token":this.getCsrfToken()}})}catch(a){}document.body.style.opacity="0",window.location.href="/login"},1500)}catch(a){if(window.notifier)window.notifier.show(i.t("notifier.password_incorrect"),"error")}}async handleAvatarChange(o){let t=o.target.files?.[0];if(!t)return;let r=new FileReader;r.onload=async(a)=>{let n=a.target?.result,e=w.getUser();if(!e)return;if(this.prefs.avatar_url=n,await w.updateProfile({username:e.username,avatar_url:n}),this.render(),window.notifier)window.notifier.show(i.t("notifier.avatar_updated"))},r.readAsDataURL(t)}openAddUserModal(){let o=this.shadowRoot.getElementById("add-user-modal");if(o){o.showModal();let t=this.shadowRoot.getElementById("new-user-role");if(t)t.options=[{value:"user",label:i.t("settings.role_user")},{value:"admin",label:i.t("settings.role_admin")}]}}async createUser(){let o=this.shadowRoot.getElementById("new-user-username").value,t=this.shadowRoot.getElementById("new-user-password").value,r=this.shadowRoot.getElementById("new-user-role").value;if(!o||!t){if(window.notifier)window.notifier.show(i.t("notifier.fields_required"),"error");return}try{if(await H.createUser({username:o,password:t,role:r}),window.notifier)window.notifier.show(i.t("notifier.user_created"));let a=this.shadowRoot.getElementById("add-user-modal");if(a)a.close();this.fetchUsers()}catch(a){if(window.notifier)window.notifier.show(i.t("notifier.user_create_error"),"error")}}openEditUserModal(o,t,r){let a=this.shadowRoot.getElementById("edit-user-modal");if(a){this.shadowRoot.getElementById("edit-user-id").value=o.toString(),this.shadowRoot.getElementById("edit-user-username").value=t,this.shadowRoot.getElementById("edit-user-password").value="",a.showModal();let n=this.shadowRoot.getElementById("edit-user-role");if(n)n.options=[{value:"user",label:i.t("settings.role_user")},{value:"admin",label:i.t("settings.role_admin")}],n.value=r}}async updateAdminUser(){let o=parseInt(this.shadowRoot.getElementById("edit-user-id").value),t=this.shadowRoot.getElementById("edit-user-username").value,r=this.shadowRoot.getElementById("edit-user-password").value,a=this.shadowRoot.getElementById("edit-user-role").value;if(!t){if(window.notifier)window.notifier.show(i.t("notifier.username_required"),"error");return}try{if(await H.adminUpdateUser({id:o,username:t,role:a,password:r||void 0}),window.notifier)window.notifier.show(i.t("notifier.user_updated"));let n=this.shadowRoot.getElementById("edit-user-modal");if(n)n.close();this.fetchUsers()}catch(n){if(window.notifier)window.notifier.show(i.t("notifier.user_update_error"),"error")}}async deleteUser(o){let t=document.querySelector("confirmation-modal");if(t){if(!await t.confirm(i.t("general.delete"),i.t("notifier.user_delete_confirm")))return}try{let r=[...this.users];if(this.users=this.users.filter((a)=>a.id!==o),this.render(),await H.deleteUser(o),window.notifier)window.notifier.show(i.t("notifier.user_deleted"));this.fetchUsers()}catch(r){this.fetchUsers();let a=i.t("notifier.user_delete_error");if(r.message&&r.message.includes("error.cannot_delete_superadmin"))a=i.t("notifier.user_delete_superadmin");if(window.notifier)window.notifier.show(a,"error")}}getContent(o){let t=w.getUser()||{username:"Guest",initials:"??",role:"View Only",avatar_url:"",accent_color:"#0078D4",language:"en",preferences:{}};switch(o){case"account":return Jo(t);case"theme":{let r={blue:"#228be6",indigo:"#4c6ef5",cyan:"#15aabf",teal:"#12b886",orange:"#fd7e14",red:"#fa5252",grape:"#be4bdb"},a=Object.keys(r);return Zo(this.prefs,r,a)}case"personalization":return Ko(this.prefs);case"advanced":return Bo();case"users":return Uo(this.users);case"about":return Yo(this.version,this.updateInfo,t.role||"");default:return`<div class="bento-card"><h3>${o}</h3><p class="settings-content__text-dim">${i.t("settings.default_module_desc")}</p></div>`}}version="v1.2.0-Beta.07";updateInfo=null;checkUpdatesPromise=null;async checkForUpdates(){if(this.checkUpdatesPromise)return this.checkUpdatesPromise;return this.checkUpdatesPromise=(async()=>{try{let{updateService:o}=await Promise.resolve().then(() => (po(),lo)),t=this.prefs.beta_updates||!1;this.updateInfo=await o.check(t),this.version=this.updateInfo.current_version}catch(o){console.error("Check update failed",o)}finally{this.checkUpdatesPromise=null,this.render()}})(),this.checkUpdatesPromise}async performUpdate(o){let t=document.querySelector("confirmation-modal");if(t){if(!await t.confirm(i.t("settings.update_available"),i.t("notifier.update_start_confirm")))return}let r=this.shadowRoot.getElementById("btn-update-now"),a=this.shadowRoot.getElementById("update-status");if(r)r.loading=!0;if(a)a.style.display="block",a.textContent=i.t("notifier.update_downloading");try{let n=await fetch("/api/system/update/perform",{method:"POST",headers:{"Content-Type":"application/json","X-CSRF-Token":this.getCsrfToken()},body:JSON.stringify({asset_url:o})});if(n.ok){if(a)a.textContent=i.t("notifier.update_verified");setTimeout(()=>{window.location.reload()},5000)}else{let e=await n.text();if(a)a.style.color="var(--danger-color)",a.textContent=i.t("notifier.update_failed")+e;if(r)r.loading=!1}}catch(n){if(console.error("Update failed",n),a)a.style.color="var(--danger-color)",a.textContent=i.t("notifier.update_error");if(r)r.loading=!1}}downloadBackup(){window.location.href="/api/system/backup"}async restoreBackup(o){if(!o)return;let t=document.querySelector("confirmation-modal");if(t){if(!await t.confirm(i.t("general.restore"),i.t("notifier.restore_confirm")))return}let r=new FormData;r.append("backup_file",o);try{if((await fetch("/api/system/restore",{method:"POST",headers:{"X-CSRF-Token":this.getCsrfToken()},body:r})).ok){if(window.notifier)window.notifier.show(i.t("notifier.restore_success"));setTimeout(()=>window.location.reload(),2000)}else if(window.notifier)window.notifier.show(i.t("notifier.restore_failed"),"error")}catch(a){if(console.error("Restore error",a),window.notifier)window.notifier.show(i.t("notifier.restore_failed"),"error")}}openResetModal(){let o=this.shadowRoot.getElementById("reset-confirm-modal"),t=this.shadowRoot.getElementById("reset-confirm-input");if(o){if(t)t.value="";o.showModal()}}async executeFactoryReset(){let o=this.shadowRoot.getElementById("reset-confirm-input");if(!o||o.value.trim()!=="delete"){if(window.notifier)window.notifier.show(i.t("notifier.reset_confirm_text"),"error");o.focus();return}let t=this.shadowRoot.getElementById("btn-reset-confirm");try{if(t)t.disabled=!0,t.textContent=i.t("settings.restoring")||"Restoring...";if((await fetch("/api/system/reset",{method:"POST",headers:{"X-CSRF-Token":this.getCsrfToken()}})).ok){let a=document.createElement("div");Object.assign(a.style,{position:"fixed",top:"0",left:"0",width:"100%",height:"100%",backgroundColor:"rgba(0,0,0,0.9)",zIndex:"9999",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",color:"white",fontFamily:"var(--font-main, sans-serif)"}),a.innerHTML=`
                ...
                `,document.body.appendChild(a);let n=0,e=async()=>{n++;try{if((await fetch("/",{method:"HEAD",cache:"no-store"})).ok){window.location.href="/setup";return}}catch(s){}if(n<60)setTimeout(e,1000);else window.location.href="/setup"};setTimeout(e,2000)}else if(window.notifier)window.notifier.show(i.t("notifier.reset_failed"),"error")}catch(r){if(console.error("Reset error",r),window.notifier)window.notifier.show(i.t("notifier.reset_error"),"error")}}render(){if(this._renderPending)return;this._renderPending=!0,requestAnimationFrame(()=>{this._renderPending=!1,this._doRender()})}_doRender(){if(this.shadowRoot.innerHTML=`
            <style>${To}</style>
            <div class="fade-in">
                ${this.getContent(this.getAttribute("section")||"account")}
            </div>
        `,this.shadowRoot.querySelectorAll(".settings-content__checkbox").forEach((o)=>{o.addEventListener("click",()=>o.classList.toggle("settings-content__checkbox--checked"))}),this.initSelects(),this.getAttribute("section")==="about")this.updateBetaBadgeVisuals()}initSelects(){let o=this.shadowRoot.getElementById("language-select");if(o){let n=i.getAvailableLocales();o.options=n.map((e)=>({value:e.code,label:`${e.flag} ${e.name}`})),o.addEventListener("change",(e)=>{this.savePrefs({language:e.detail})})}let t=[{value:"user",label:i.t("settings.role_user")},{value:"admin",label:i.t("settings.role_admin")}],r=this.shadowRoot.getElementById("new-user-role");if(r)r.options=t;let a=this.shadowRoot.getElementById("edit-user-role");if(a)a.options=t}updateBetaBadgeVisuals(){setTimeout(()=>{let o=this.shadowRoot.getElementById("beta-updates-toggle-badge"),t=this.shadowRoot.getElementById("beta-text"),r=this.prefs.beta_updates||!1;if(o&&t)if(o.checked=r,r)t.style.color="#2fb344",t.style.borderColor="#2fb344",t.style.boxShadow="0 0 10px rgba(47, 179, 68, 0.2)";else t.style.color="var(--text-dim)",t.style.borderColor="var(--border)",t.style.boxShadow="none"},0)}}if(!customElements.get("settings-content"))customElements.define("settings-content",Vo);K();k();var Oo=({user:o,isOpen:t,selectedSection:r,updateAvailable:a})=>`
...
}`;class Wo extends HTMLElement{state;_unsubscribeDashboard;_unsubscribeI18n;_windowClickHandler;static get observedAttributes(){return["title"]}constructor(){super();this.attachShadow({mode:"open"}),this.state={editMode:!1,searchActive:!1,addMenuActive:!1,drawerOpen:!1,searchQuery:""}}attributeChangedCallback(o,t,r){if(o==="title"&&t!==r)this.render()}connectedCallback(){this.render(),this.setupListeners(),this._unsubscribeDashboard=g.subscribe((o)=>{if(this.state.editMode!==o.isEditing||this.state.updateAvailable!==o.updateAvailable)this.setState({editMode:o.isEditing,updateAvailable:o.updateAvailable})}),this._unsubscribeI18n=i.subscribe(()=>{this.render()})}disconnectedCallback(){if(this._unsubscribeDashboard)this._unsubscribeDashboard();if(this._unsubscribeI18n)this._unsubscribeI18n();if(this._windowClickHandler)window.removeEventListener("click",this._windowClickHandler)}setState(o){let t=this.state.editMode;if(this.state={...this.state,...o},this.render(),o.editMode!==void 0&&o.editMode!==t)this.dispatchEvent(new CustomEvent("edit-mode-change",{detail:{active:this.state.editMode},bubbles:!0,composed:!0}))}setupListeners(){this.shadowRoot.addEventListener("click",(o)=>{let t=o.target;if(t.closest("#search-clear")){o.stopPropagation(),this.state.searchQuery="";let l=this.shadowRoot.getElementById("search-input");if(l)l.value="",l.focus();this.render(),this.dispatchEvent(new CustomEvent("search-input",{detail:{query:""},bubbles:!0,composed:!0}));return}if(t.closest("#search-wrapper")&&!this.state.searchActive)o.stopPropagation(),this.setState({searchActive:!0}),this.shadowRoot.getElementById("search-input")?.focus();if(t.closest("#edit-toggle"))g.toggleEditMode();if(t.closest("#add-toggle"))o.stopPropagation(),this.setState({addMenuActive:!this.state.addMenuActive});let s=t.closest(".add-menu-item");if(s){let l=s.dataset.action;if(l)this.setState({addMenuActive:!1}),this.dispatchEvent(new CustomEvent("add-item",{detail:{action:l},bubbles:!0,composed:!0}));return}if(t.closest("#drawer-toggle")){let l=this.state.drawerOpen?"close":"open";this.dispatchEvent(new CustomEvent("drawer-toggle",{detail:{action:l},bubbles:!0,composed:!0}));return}}),this.shadowRoot.addEventListener("input",(o)=>{let t=o.target;if(t.id==="search-input"){let r=t.value;this.state.searchQuery=r;let a=this.shadowRoot.getElementById("search-clear");if(a)a.style.display=r?"flex":"none";this.dispatchEvent(new CustomEvent("search-input",{detail:{query:r},bubbles:!0,composed:!0}))}}),this.shadowRoot.addEventListener("keydown",(o)=>{let t=o.target,r=o;if(t.id==="search-input"&&r.key==="Escape")t.value="",this.setState({searchActive:!1}),this.dispatchEvent(new CustomEvent("search-input",{detail:{query:""},bubbles:!0,composed:!0})),t.blur()}),this._windowClickHandler=(o)=>{if(this.state.addMenuActive)this.setState({addMenuActive:!1});let t=o.composedPath(),r=this.shadowRoot.getElementById("search-wrapper");if(this.state.searchActive&&r&&!t.includes(r)){let a=this.shadowRoot.getElementById("search-input");if(a&&a.value==="")this.setState({searchActive:!1})}},window.addEventListener("click",this._windowClickHandler)}render(){let o=this.getAttribute("title")||"Lastboard";this.shadowRoot.innerHTML=`
            <style>${Do}</style>
            ${Co({title:o,editMode:this.state.editMode,searchActive:this.state.searchActive,addMenuActive:this.state.addMenuActive,drawerOpen:this.state.drawerOpen,searchQuery:this.state.searchQuery,user:this.state.user,updateAvailable:this.state.updateAvailable})}
        `}}if(!customElements.get("app-topbar"))customElements.define("app-topbar",Wo);k();var Io=({title:o,dropdownOpen:t})=>`
    <header class="header">
...
(max-width: 1024px) {
...
}`;class ot extends HTMLElement{dropdownOpen;constructor(){super();this.attachShadow({mode:"open"}),this.dropdownOpen=!1}connectedCallback(){this.render(),this.setupListeners()}toggleDropdown(o){this.dropdownOpen=typeof o==="boolean"?o:!this.dropdownOpen,this.render()}setupListeners(){this.shadowRoot.addEventListener("click",(o)=>{let t=o.target,r=t.closest(".header__btn-changelog");if(r)r.style.display="none";if(t.closest("#profile-trigger"))o.stopPropagation(),this.toggleDropdown()}),window.addEventListener("click",()=>{if(this.dropdownOpen)this.toggleDropdown(!1)})}render(){let o=this.getAttribute("title")||"Task";this.shadowRoot.innerHTML=`
            <style>${So}</style>
            ${Io({title:o,dropdownOpen:this.dropdownOpen})}
        `}}if(!customElements.get("app-header"))customElements.define("app-header",ot);k();var T=(o)=>String(o).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;"),Ct=(o)=>{if(!o)return"#";let t=o.trim().toLowerCase();if(t.startsWith("http://")||t.startsWith("https://")||t==="#")return T(o);return"#"},tt=({bookmarks:o,isEditing:t,isLoading:r,isSearching:a,isTouchDevice:n,maxCols:e=12})=>{let s=Array.isArray(o)?o:[];if(r&&s.length===0)return Wt(e);let d=(c)=>s.filter((p)=>p.parent_id===c);return`
    ${(n?s:a?s:s.filter((c)=>!c.parent_id)).map((c)=>{let p={};try{p=typeof c.content==="string"?JSON.parse(c.content):c.content}catch(y){console.error("Failed to parse content for item",c.id,y)}let f=c.type==="section",v={x:c.x||1,y:c.y||1},x=Math.min(c.w||1,e),m=c.h||1;if(f){let y=d(c.id),u=(p.title||"").trim();return`
            <fieldset class="bookmark-grid__section"
               ...
               `:""}
            </fieldset>`}return rt(c,p,t,v,x,m,n)}).join("")}

    <div id="ghost-element" class="bookmark-grid__ghost" style="display: none;"></div>
    `};function rt(o,t,r,a,n,e=0,s=!1){let d=e||o.h||1;if(o.type==="widget"){let v=t.widgetId,m={clock:"widget-clock",notepad:"widget-notepad",telemetry:"widget-telemetry",weather:"widget-weather",markdown:"widget-markdown"}[v]||"div",u=(t.text||"").replace(/"/g,"&quot;");return`
        <div class="bookmark-grid__card"
            draggable="${r}"
            data-id="${o.id}"
            data-type="${o.type}"
            data-orig-x="${o.x}" data-orig-y="${o.y}"
            ${!s?`style="--x: ${a.x}; --y: ${a.y}; --w: ${n}; --h: ${d};"`:""}>
            ...
            ${r?`
            <div class="bookmark-actions">
                 ${["clock","telemetry","weather"].includes(v)?`<button class="action-btn btn-edit" title="${i.t("general.edit")}">✎</button>`:""}
                 <button class="action-btn btn-delete" title="${i.t("general.delete")}">\uD83D\uDDD1</button>
            </div>
            ${["clock","telemetry"].includes(v)?"":'<div class="resize-handle"></div>'}
            `:""}
        </div>
    `}let l=t.icon||"",p=l.startsWith("http")||l.startsWith("/")?`<img src="${T(l)}" alt="${T(t.label)}" class="bookmark-grid__icon-img" draggable="false" />`:l?T(l):'<svg class="bookmark-grid__icon-svg" viewBox="0 0 24 24"><path d="M10 6L8.59 7.41 13.17 12l-4.58 4.59L10 18l6-6z"/></svg>';return`
        <a ${r?'role="button"':`href="${Ct(t.url)}" target="_blank"`} class="bookmark-grid__card"
           draggable="${r}"
           data-id="${o.id}"
           data-type="${o.type}"
           data-orig-x="${o.x}" data-orig-y="${o.y}"
    ...
    `}function Dt(o,t,r=!1,a=12,n=!1){return o.map((e)=>{let s={};try{s=typeof e.content==="string"?JSON.parse(e.content):e.content}catch(p){console.error("Failed to parse content for item (nested)",e.id,p)}let d={x:e.x||1,y:e.y||1},l=e.h||1,c=Math.min(e.w||1,a);return rt(e,s,t,d,c,l,n)}).join("")}function Wt(o){return`
        <style>
            .skeleton {
                background: var(--surface);
                pointer-events: none;
...
`;class it extends HTMLElement{_itemId=0;_content="";_isDirty=!1;_isLocked=!0;_isSaving=!1;_lastLockTime=0;_editor=null;_syncStatus=null;_dock=null;_editorContainer=null;_modal=null;_imgResizer=null;_activeImage=null;_pendingAction=null;static get observedAttributes(){return["item-id"]}constructor(){super();this.attachShadow({mode:"open"})}connectedCallback(){this.render(),this.loadContent(),window.addEventListener("beforeunload",this.handleBeforeUnload.bind(this)),i.subscribe(()=>{if(this._editor)this._editor.dataset.placeholder=i.t("widget.notepad.placeholder")})}disconnectedCallback(){window.removeEventListener("beforeunload",this.handleBeforeUnload.bind(this))}attributeChangedCallback(o,t,r){if(o==="item-id")this._itemId=parseInt(r),this.loadContent()}handleBeforeUnload(o){if(this._isDirty)o.preventDefault(),o.returnValue=""}render(){if(!this.shadowRoot)return;let o=this._isLocked?"locked":"unlocked",t=this._isLocked?"locked":"",r=this._isLocked?"hidden":"";this.shadowRoot.innerHTML=`
        ...
        `,this.cpuBar=this.shadowRoot.querySelector(".cpu-bar"),this.ramBar=this.shadowRoot.querySelector(".ram-bar"),this.tempBar=this.shadowRoot.querySelector(".temp-bar"),this.cpuText=this.shadowRoot.querySelector(".cpu-text"),this.ramText=this.shadowRoot.querySelector(".ram-text"),this.tempText=this.shadowRoot.querySelector(".temp-text")}}customElements.define("widget-telemetry",nt);k();L();var et=new Map,dt=900000,st={widgetId:"weather",latitude:0,longitude:0,city:"",unit:"celsius",showForecast:!1,forecastDays:5};function V(o){if(o===0)return`<svg viewBox="0 0 64 64" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round">
    ...
    </svg>`}function go(o){let t={0:"clear_sky",1:"mainly_clear",2:"partly_cloudy",3:"overcast",45:"fog",48:"fog",51:"light_drizzle",53:"drizzle",55:"heavy_drizzle",56:"freezing_drizzle",57:"freezing_drizzle",61:"light_rain",63:"rain",65:"heavy_rain",66:"freezing_rain",67:"freezing_rain",71:"light_snow",73:"snow",75:"heavy_snow",77:"snow_grains",80:"light_showers",81:"showers",82:"heavy_showers",85:"snow_showers",86:"heavy_snow_showers",95:"thunderstorm",96:"thunderstorm_hail",99:"thunderstorm_hail"};return i.t(`widget.weather.${t[o]||"unknown"}`)}function E(o,t){if(t==="fahrenheit")return`${Math.round(o*9/5+32)}°F`;return`${Math.round(o)}°C`}async function St(o,t){let r=`${o.toFixed(2)},${t.toFixed(2)}`,a=et.get(r);if(a&&Date.now()-a.timestamp<dt)return a.data;let n=`https://api.open-meteo.com/v1/forecast?latitude=${o}&longitude=${t}&current_weather=true&daily=weathercode,temperature_2m_max,temperature_2m_min,sunrise,sunset,wind_speed_10m_max&timezone=auto&forecast_days=7`,e=await fetch(n);if(!e.ok)throw Error("Weather API error");let s=await e.json();return et.set(r,{data:s,timestamp:Date.now()}),s}class lt extends HTMLElement{_config={...st};_itemId=0;_unsubscribe=null;_unsubscribeI18n;_weatherData=null;_refreshTimer=null;_loading=!1;_error=!1;constructor(){super();this.attachShadow({mode:"open"})}static get observedAttributes(){return["item-id"]}attributeChangedCallback(o,t,r){if(o==="item-id")this._itemId=parseInt(r)}connectedCallback(){this.render(),this.loadWeather(),this._refreshTimer=setInterval(()=>this.loadWeather(),dt),this._unsubscribe=g.subscribe((o)=>{if(!this._itemId)return;let t=o.items.find((r)=>r.id===this._itemId);if(!t?.content)return;try{let r=typeof t.content==="string"?JSON.parse(t.content):t.content;if(r.widgetId!=="weather")return;let a=this._config.latitude,n=this._config.longitude;if(this._config={...st,...r},a!==this._config.latitude||n!==this._config.longitude)this.loadWeather();else this.updateDisplay()}catch(r){}}),this._unsubscribeI18n=i.subscribe(()=>this.updateDisplay())}disconnectedCallback(){if(this._unsubscribe)this._unsubscribe();if(this._unsubscribeI18n)this._unsubscribeI18n();if(this._refreshTimer)clearInterval(this._refreshTimer)}async loadWeather(){if(!this._config.latitude&&!this._config.longitude){this._loading=!1,this._error=!1,this.updateDisplay();return}this._loading=!0,this._error=!1,this.updateDisplay();try{this._weatherData=await St(this._config.latitude,this._config.longitude),this._loading=!1}catch(o){this._loading=!1,this._error=!0}this.updateDisplay()}updateDisplay(){let o=this.shadowRoot?.querySelector(".weather-body");if(!o){this.render();return}if(!this._config.latitude&&!this._config.longitude){o.innerHTML=this.emptyState(V(0),i.t("widget.weather.no_location"));return}if(this._loading){o.innerHTML=this.emptyState(V(3),i.t("general.loading"),!0);return}if(this._error||!this._weatherData){o.innerHTML=this.emptyState(V(-1),i.t("general.error"));return}let t=this._weatherData.current_weather,r=this._weatherData.daily,a=this._config.unit||"celsius";if(this._config.showForecast&&r)o.innerHTML=this.forecastView(t,r,a);else o.innerHTML=this.simpleView(t,r,a)}emptyState(o,t,r=!1){return`<div class="state"><div class="state-icon${r?" pulse":""}">${o}</div><span class="state-text">${t}</span></div>`}simpleView(o,t,r){return`
            <div class="icon-main">${V(o.weathercode)}</div>
            <div class="temp-main">${E(o.temperature,r)}</div>
            <div class="desc">${go(o.weathercode)}</div>
        ...
        `}forecastView(o,t,r){let a=i.getLocale().code,n=Math.min(this._config.forecastDays||5,t.time.length-1),e="";for(let s=1;s<=n;s++){let d=new Date(t.time[s]+"T00:00:00"),l=d.toLocaleDateString(a,{weekday:"short"}),c=d.toLocaleDateString(a,{weekday:"long",day:"numeric",month:"long"}),p=go(t.weathercode[s]),f=E(t.temperature_2m_max[s],r),v=E(t.temperature_2m_min[s],r),x=t.sunrise?.[s]?.split("T")[1]||"",m=t.sunset?.[s]?.split("T")[1]||"",y=t.wind_speed_10m_max?.[s]!=null?`${Math.round(t.wind_speed_10m_max[s])} km/h`:"";e+=`<div class="fc-day">
                <span class="fc-name">${l}</span>
                <span class="fc-icon">${V(t.weathercode[s])}</span>
                <span class="fc-temp">${f}</span>
                <div class="fc-tooltip">
                    <div class="tt-date">${c}</div>
                    <div class="tt-desc">${p}</div>
                    <div class="tt-row"><span class="tt-label">${i.t("widget.weather.max_min")}</span><span>${f} / ${v}</span></div>
                    ${x?`<div class="tt-row"><span class="tt-label">${i.t("widget.weather.sunrise")}</span><span>${x}</span></div>`:""}
            ...
            <div class="fc-header">
                ...
                <div class="fc-h-info">
                    ...
                    <div class="fc-h-desc">${go(o.weathercode)}</div>
                </div>
        ...
        `}render(){if(!this.shadowRoot)return;this.shadowRoot.innerHTML=`<style>${or}</style><div class="weather-body"></div>`,this.updateDisplay()}}var or=`
:host {
    display: flex;
    flex-direction: column;
    align-items: center;
...
`;customElements.define("widget-weather",lt);var pt=`:host {
    /* Fixed Grid System */
...
@keyframes skeleton-shimmer {
    0% { background-position: 200% 0; }
    100% { background-position: -200% 0; }
}`;class gt extends HTMLElement{bookmarks=[];allItems=[];isEditing=!1;isLoading=!0;searchQuery="";_unsubscribe;_unsubscribeI18n;_resizeObserver;_resizeDebounce=null;dragTargetId=null;ghostEl=null;dragOffsetX=0;dragOffsetY=0;isResizing=!1;resizeTargetId=null;initialResizeX=0;initialResizeY=0;initialResizeW=0;initialResizeH=0;currentColWidth=0;currentGridCols=12;isTouchDevice=!1;constructor(){super();this.attachShadow({mode:"open"})}_widgetModal;_boundActionClick=this.handleActionClick.bind(this);_boundMouseMove=this.handleWindowMouseMove.bind(this);_boundMouseUp=this.handleWindowMouseUp.bind(this);applyFilters(){let o=this.isTouchDevice,t=!!this.searchQuery;if(t&&!o)this.classList.add("search-active");else this.classList.remove("search-active");if(o||t)this.bookmarks=this.allItems.filter((r)=>{let a=r.content;if(typeof r.content==="string")try{a=JSON.parse(r.content)}catch{return!1}if(o&&r.type!=="bookmark")return!1;if(o&&a.visibleTouch===!1)return!1;if(t)return(a.label||"").toLowerCase().includes(this.searchQuery);return!0});else this.bookmarks=this.allItems}connectedCallback(){let o=()=>{let r=window.innerWidth<768,a=window.matchMedia("(pointer: coarse)").matches||"ontouchstart"in window||navigator.maxTouchPoints>0;return r&&a},t=()=>{let r=o();if(this.isTouchDevice!==r){if(this.isTouchDevice=r,r)this.classList.add("touch-mode");else this.classList.remove("touch-mode");this.applyFilters(),this.render()}};if(window.addEventListener("resize",t),window.addEventListener("orientationchange",t),this.isTouchDevice=o(),this.isTouchDevice)this.classList.add("touch-mode");this.applyFilters(),this.render(),this.updateGridMetrics(),this._resizeObserver=new ResizeObserver(()=>{if(this._resizeDebounce)return;this._resizeDebounce=setTimeout(()=>{this._resizeDebounce=null,this.updateGridMetrics();let r=this.getBoundingClientRect();g.setGridMetrics(r.width,this.currentGridCols),this.applyFilters(),this.render()},16)}),this._resizeObserver.observe(this),this._unsubscribe=g.subscribe((r)=>{let a=!1;if(this.isEditing!==r.isEditing)this.isEditing=r.isEditing,a=!0;if(this.isLoading!==r.loading)this.isLoading=r.loading,a=!0;if(this.searchQuery!==r.searchQuery)this.searchQuery=r.searchQuery,a=!0;let n=Array.isArray(r.items)?r.items:[];if(JSON.stringify(this.allItems)!==JSON.stringify(n)||a)this.allItems=n,this.applyFilters(),a=!0;if(a)this.render()}),this.setupDragListeners(),this.setupResizeListeners(),this.setupActionListeners(),W.start(),this._unsubscribeI18n=i.subscribe(()=>this.render()),Promise.resolve().then(() => (K(),S)).then(({userStore:r})=>{this._unsubscribeUser=r.subscribe(()=>{requestAnimationFrame(()=>{this.updateGridMetrics(),this.render()})})})}_unsubscribeUser=null;setupActionListeners(){let o=this.shadowRoot;o.removeEventListener("click",this._boundActionClick),o.addEventListener("click",this._boundActionClick)}async handleActionClick(o){if(!this.isEditing)return;let t=o.target;if(!t)return;let r=t.closest(".btn-delete"),a=t.closest(".btn-edit");if(r||a)o.preventDefault(),o.stopPropagation();else if(t.closest("a"))o.preventDefault();if(r){let n=r.closest(".bookmark-grid__card, .bookmark-grid__group, .bookmark-grid__section");if(!n)return;let e=parseInt(n.dataset.id||"0"),s=this.allItems.find((p)=>p.id===e);if(!s)return;let d=s.type==="section"?i.t("type.section"):s.type==="widget"?i.t("type.widget"):i.t("type.bookmark"),{eventBus:l,EVENTS:c}=await Promise.resolve().then(() => (oo(),xo));l.emit(c.SHOW_CONFIRMATION,{title:`${i.t("general.delete")} ${d}`,message:i.t("bookmark.delete_confirm_message"),onConfirm:()=>{g.deleteItem(e)}});return}if(a){let n=a.closest(".bookmark-grid__card, .bookmark-grid__group, .bookmark-grid__section");if(!n)return;let e=parseInt(n.dataset.id||"0"),s=this.allItems.find((d)=>d.id===e);if(s){let{eventBus:d,EVENTS:l}=await Promise.resolve().then(() => (oo(),xo));if(s.type==="widget")d.emit(l.SHOW_WIDGET_CONFIG,{item:s,type:"widget"});else if(s.type==="section")d.emit(l.SHOW_WIDGET_CONFIG,{item:s,type:"section"});else d.emit(l.SHOW_WIDGET_CONFIG,{item:s,type:"bookmark"})}return}}disconnectedCallback(){if(this._unsubscribe)this._unsubscribe();if(this._unsubscribeI18n)this._unsubscribeI18n();if(this._unsubscribeUser)this._unsubscribeUser();if(this._resizeObserver)this._resizeObserver.disconnect();if(this._resizeDebounce)clearTimeout(this._resizeDebounce);if(this._updateGhostTimeout)clearTimeout(this._updateGhostTimeout);window.removeEventListener("mousemove",this._boundMouseMove),window.removeEventListener("mouseup",this._boundMouseUp),W.stop()}updateGridMetrics(){let o=this.getBoundingClientRect(),t=getComputedStyle(this),r=t.getPropertyValue("--grid-cols").trim(),a=parseInt(r)||12,n=t.columnGap||t.gap||"16px",e=parseFloat(n)||16;if(o.width<=0){this.currentGridCols=a,this.currentColWidth=100,this.style.setProperty("--current-grid-cols",a.toString()),this.style.setProperty("--row-height","100px");return}let s=(a-1)*e,d=(o.width-s)/a;if(d<10)d=10;this.currentGridCols=a,this.currentColWidth=d,this.style.setProperty("--current-grid-cols",String(a)),this.style.setProperty("--row-height",`${d}px`),this.ensureGridBuffer()}ensureGridBuffer(){if(this.isTouchDevice){this.style.minHeight="";return}let o=0;this.bookmarks.forEach((s)=>{let d=(s.y||1)+(s.h||1);if(d>o)o=d});let r=o+1,a=this.currentColWidth||100,n=16,e=r*a+(r-1)*n;this.style.minHeight=`${e}px`}setupResizeListeners(){window.addEventListener("mousemove",this._boundMouseMove),window.addEventListener("mouseup",this._boundMouseUp),this.shadowRoot.addEventListener("mousedown",(o)=>{let t=o;if(!this.isEditing)return;let r=t.target;if(r.classList.contains("resize-handle")){t.preventDefault(),t.stopPropagation();let a=r.closest(".bookmark-grid__card, .bookmark-grid__section");if(!a||!a.dataset.id)return;let n=parseInt(a.dataset.id),e=this.bookmarks.find((x)=>x.id===n);if(!e)return;this.isResizing=!0,this.resizeTargetId=n,this.initialResizeX=t.clientX,this.initialResizeY=t.clientY,this.initialResizeW=e.w,this.initialResizeH=e.h;let s=this,d=s.getBoundingClientRect(),c=getComputedStyle(s).getPropertyValue("--current-grid-cols").trim(),p=c?parseInt(c,10):9,f=isNaN(p)?9:p,v=16;this.currentColWidth=(d.width-(f-1)*v)/f,this.updateGhost({x:e.x,y:e.y,w:e.w,h:e.h},!0)}})}_updateGhostTimeout=null;handleWindowMouseMove(o){if(!this.isResizing||!this.resizeTargetId)return;if(this._updateGhostTimeout)return;this._updateGhostTimeout=setTimeout(()=>{this._updateGhostTimeout=null,this.processResizeMove(o)},16)}processResizeMove(o){if(!this.isResizing||!this.resizeTargetId)return;let t=o.clientX-this.initialResizeX,r=o.clientY-this.initialResizeY,a=Math.round(t/(this.currentColWidth+16)),n=Math.round(r/(this.currentColWidth+16)),e=this.initialResizeW+a,s=this.initialResizeH+n,d=this.bookmarks.find((l)=>l.id===this.resizeTargetId);if(d){let l=this.applyResizeConstraints(e,s,d),c={x:d.x,y:d.y,w:l.w,h:l.h,id:d.id,parent_id:d.parent_id},p=F.calculateDropValidity(c,this.bookmarks,this.currentGridCols);this.updateGhost(c,p.valid)}}applyResizeConstraints(o,t,r){let a=1,n=1;if(r.type==="widget"){let e=r.content;if(typeof r.content==="string")try{e=JSON.parse(r.content)}catch{return{w:Math.max(1,Math.min(12,o)),h:Math.max(1,Math.min(12,t))}}let s=(e.widgetId||"").toLowerCase();if(s==="notepad")a=2,n=2;else if(s==="clock")return{w:2,h:1};else if(s==="telemetry")return{w:2,h:1};let d=Math.max(a,Math.min(12,o)),l=Math.max(n,Math.min(12,t));return{w:d,h:l}}else if(r.type==="section"){let e=Math.max(1,Math.min(12,o)),s=Math.max(1,Math.min(12,t));return{w:e,h:s}}else{let e=Math.max(1,Math.min(2,o)),s=Math.max(1,Math.min(2,t));return{w:e,h:s}}}async handleWindowMouseUp(o){if(!this.isResizing||!this.resizeTargetId)return;if(this._updateGhostTimeout)clearTimeout(this._updateGhostTimeout),this._updateGhostTimeout=null;let t=o.clientX-this.initialResizeX,r=o.clientY-this.initialResizeY,a=Math.round(t/(this.currentColWidth+16)),n=Math.round(r/(this.currentColWidth+16)),e=this.initialResizeW+a,s=this.initialResizeH+n,d=this.bookmarks.find((f)=>f.id===this.resizeTargetId);if(!d)return;let l=this.applyResizeConstraints(e,s,d);e=l.w,s=l.h;let c={x:d.x,y:d.y,w:e,h:s,id:d.id,parent_id:d.parent_id};if(F.calculateDropValidity(c,this.bookmarks,this.currentGridCols).valid&&(d.w!==e||d.h!==s)){if(d.type==="section"){let f=e*s,v=this.bookmarks.filter((x)=>x.parent_id===d.id);if(v.sort((x,m)=>x.y-m.y||x.x-m.x),v.length>f){let x=v.slice(f),m=[...this.bookmarks],y=x.map((u)=>{let b=F.findFirstAvailableSlot(u.w,u.h,m,this.currentGridCols);return m.push({...u,x:b.x,y:b.y,parent_id:void 0}),{id:u.id,x:b.x,y:b.y,parent_id:void 0}});await Promise.all(y.map((u)=>g.updateItem(u)))}}await g.resizeItem(d.id,e,s)}if(this.isResizing=!1,this.resizeTargetId=null,this.ghostEl)this.ghostEl.style.display="none"}setupDragListeners(){let o=this.shadowRoot,t=this;o.addEventListener("dragstart",(a)=>{let n=a;if(!this.isEditing){n.preventDefault();return}let e=n.target.closest('[draggable="true"]');if(e&&e.dataset.id){this.dragTargetId=parseInt(e.dataset.id),n.dataTransfer.effectAllowed="move",e.style.opacity="0.5";let s=e.getBoundingClientRect();if(this.dragOffsetX=n.clientX-s.left,this.dragOffsetY=n.clientY-s.top,n.dataTransfer)n.dataTransfer.setDragImage(e,this.dragOffsetX,this.dragOffsetY)}}),o.addEventListener("dragend",(a)=>{let n=a.target.closest('[draggable="true"]');if(n)n.style.opacity="1";if(this.dragTargetId=null,this.ghostEl)this.ghostEl.style.display="none";this.shadowRoot.querySelectorAll(".bookmark-grid__section").forEach((s)=>s.classList.remove("drop-target"))});let r=null;t.addEventListener("dragover",(a)=>{let n=a;if(!this.isEditing||!this.dragTargetId)return;if(n.preventDefault(),n.dataTransfer.dropEffect="move",r)return;r=setTimeout(()=>{r=null,this.processDragOver(n,t)},16)}),t.addEventListener("drop",async(a)=>{let n=a;if(r)clearTimeout(r),r=null;await this.handleDrop(n,t)})}processDragOver(o,t){let n=document.querySelector(".main-content");if(n){let z=n.getBoundingClientRect(),D=o.clientY;if(D>z.bottom-100)n.scrollBy(0,15);else if(D<z.top+100)n.scrollBy(0,-15)}let e=t.getBoundingClientRect(),s=this.currentGridCols||12,d=e.width,l=16,c=(d-(s-1)*l)/s,p=this.bookmarks.find((z)=>z.id===this.dragTargetId);if(!p)return;let f=o.clientX-this.dragOffsetX,v=o.clientY-this.dragOffsetY,x=f-e.left,m=v-e.top,y=Math.floor(x/(c+l)),u=Math.floor(m/(c+l)),b=Math.max(1,Math.min(s-p.w+1,y+1)),A=Math.max(1,u+1),q={x:b,y:A,w:p.w,h:p.h,id:p.id,parent_id:p.parent_id},h=F.calculateDropValidity(q,this.bookmarks,s),J=!this.bookmarks.some((z)=>{if(z.id===p.id)return!1;if(z.parent_id!==p.parent_id&&!h.targetGroup)return!1;let D=z.x||1,_o=z.y||1,Qt=z.w||1,Et=z.h||1;if(h.targetGroup&&z.id===h.targetGroup.id)return!1;return b<D+Qt&&b+p.w>D&&A<_o+Et&&A+p.h>_o})&&h.valid;if(this.updateGhost(q,J),this.shadowRoot.querySelectorAll(".bookmark-grid__section").forEach((z)=>z.classList.remove("drop-target")),h.targetGroup&&J){let z=this.shadowRoot.querySelector(`.bookmark-grid__section[data-id="${h.targetGroup.id}"]`);if(z)z.classList.add("drop-target")}}async handleDrop(o,t){if(!this.isEditing||!this.dragTargetId)return;o.preventDefault();let r=this.bookmarks.find((h)=>h.id===this.dragTargetId);if(!r)return;let a=t.getBoundingClientRect(),n=this.currentGridCols||12,e=a.width,s=16,d=(e-(n-1)*s)/n,l=o.clientX-this.dragOffsetX,c=o.clientY-this.dragOffsetY,p=l-a.left,f=c-a.top,v=Math.floor(p/(d+s)),x=Math.floor(f/(d+s)),m=Math.max(1,Math.min(n-r.w+1,v+1)),y=Math.max(1,x+1),u={x:m,y,w:r.w,h:r.h,id:r.id,parent_id:r.parent_id},b=F.calculateDropValidity(u,this.bookmarks,n);if(this.bookmarks.some((h)=>{if(h.id===r.id)return!1;if(h.parent_id!==r.parent_id&&!b.targetGroup)return!1;let C=h.x||1,J=h.y||1,uo=h.w||1,z=h.h||1;if(b.targetGroup&&h.id===b.targetGroup.id)return!1;return m<C+uo&&m+r.w>C&&y<J+z&&y+r.h>J})&&!b.targetGroup){if(this.ghostEl)this.ghostEl.style.display="none";this.shadowRoot.querySelectorAll(".drop-target").forEach((h)=>h.classList.remove("drop-target"));return}if(b.valid){let h={id:r.id,x:b.x,y:b.y};if(b.targetGroup)h.parent_id=b.targetGroup.id,h.x=b.x-b.targetGroup.x+1,h.y=b.y-b.targetGroup.y+1;else h.parent_id=void 0;await g.updateItem(h)}if(this.ghostEl)this.ghostEl.style.display="none";this.shadowRoot.querySelectorAll(".bookmark-grid__section").forEach((h)=>h.classList.remove("drop-target"))}updateGhost(o,t){if(!this.ghostEl)this.ghostEl=this.shadowRoot.getElementById("ghost-element");if(!this.ghostEl)return;if(this.ghostEl.style.display="block",this.ghostEl.style.setProperty("--ghost-x",String(o.x)),this.ghostEl.style.setProperty("--ghost-y",String(o.y)),this.ghostEl.style.setProperty("--ghost-w",String(o.w)),this.ghostEl.style.setProperty("--ghost-h",String(o.h)),t)this.ghostEl.classList.remove("invalid");else this.ghostEl.classList.add("invalid")}render(){if(this.isEditing)this.classList.add("edit-mode");else this.classList.remove("edit-mode");this.shadowRoot.innerHTML=`
            <style>${pt}</style>
            ${tt({bookmarks:this.bookmarks,isEditing:this.isEditing,isLoading:this.isLoading,isSearching:!!this.searchQuery,isTouchDevice:this.isTouchDevice,maxCols:this.currentGridCols})}
        `,this.setupActionListeners(),this.ghostEl=this.shadowRoot.getElementById("ghost-element")}}if(!customElements.get("bookmark-grid"))customElements.define("bookmark-grid",gt);var xt=()=>`
    <div class="toast-container"></div>
`;var bt=`.toast-container {
    position: fixed;
    bottom: 20px;
    right: 20px;
    z-index: 1000;
    ...
    <dialog id="modal">
        ...
        <form class="modal-form" id="bookmark-form">
            <div class="form-group">
                ...
                <input type="text" id="bookmark-label" name="label" placeholder="${i.t("bookmark.placeholder_label")}" required />
            </div>
            <div class="form-group">
                <label for="bookmark-url">${i.t("bookmark.url")}</label>
                <input type="url" id="bookmark-url" name="url" placeholder="${i.t("bookmark.placeholder_url")}" required />
            </div>
            <div class="form-group">
                <label>${i.t("bookmark.icon")}</label>
            ...
            <div class="visibility-row">
                <div class="checkbox-group">
                    <label for="bookmark-status">${i.t("bookmark.monitor_status")}</label>
                    <label class="toggle-switch">
                ...
                <div class="checkbox-group">
                    <label for="bookmark-touch">${i.t("bookmark.visible_touch")}</label>
                    <label class="toggle-switch">
            ...
            <div class="form-actions">
                <app-button type="submit" variant="primary" class="btn-submit">${i.t("general.save")}</app-button>
            </div>
        </form>
    </dialog>
`;L();k();var wt=`:host {
...
.visibility-row .checkbox-group {
    flex: 1;
    margin-bottom: 0;
}`;class zt extends HTMLElement{dialog=null;iconPicker=null;selectedIconName="";clickHandler=null;submitHandler=null;escapeHandler=null;_unsubscribeI18n;isEditMode=!1;currentItemId=null;constructor(){super();this.attachShadow({mode:"open"}),this.setupHandlers()}setupHandlers(){this.clickHandler=(o)=>{if(o.target.closest("#modal-close")){o.preventDefault(),o.stopPropagation(),this.close();return}if(o.target===this.dialog){this.close();return}},this.submitHandler=async(o)=>{o.preventDefault(),o.stopPropagation();let t=o.target,r=new FormData(t),a=r.get("label"),n=r.get("url"),e=r.get("statusCheck")==="on",s=r.get("visibleTouch")==="on",d=this.iconPicker?this.iconPicker.getSelectedIcon():"",l=d?`https://cdn.jsdelivr.net/gh/homarr-labs/dashboard-icons/png/${d}.png`:"";try{let c=JSON.stringify({label:a,url:n,icon:l,iconName:d,statusCheck:e,visibleTouch:s});if(this.isEditMode&&this.currentItemId){if(await g.updateItem({id:this.currentItemId,content:c}),window.notifier)window.notifier.show(i.t("notifier.bookmark_updated"))}else{let p=g.getState(),f=Array.isArray(p.items)?p.items:[],{collisionService:v}=await Promise.resolve().then(() => (U(),N)),x=v.findFirstAvailableSlot(1,1,f);if(await g.addItem({type:"bookmark",x:x.x,y:x.y,w:1,h:1,content:c}),window.notifier)window.notifier.show(i.t("notifier.bookmark_added"))}this.close()}catch(c){if(console.error("[Modal] Error:",c),window.notifier)window.notifier.show(i.t("notifier.bookmark_error"),"error")}},this.escapeHandler=(o)=>{if(o.key==="Escape"&&this.dialog?.open)this.close()}}connectedCallback(){this.render(),this.setupListeners(),this._unsubscribeI18n=i.subscribe(()=>{if(this.dialog?.open)this.render()})}disconnectedCallback(){if(this._unsubscribeI18n)this._unsubscribeI18n();let o=this.shadowRoot;if(this.clickHandler)o.removeEventListener("click",this.clickHandler);if(this.submitHandler)o.removeEventListener("submit",this.submitHandler);if(this.escapeHandler)document.removeEventListener("keydown",this.escapeHandler)}setupListeners(){let o=this.shadowRoot;o.addEventListener("click",this.clickHandler),o.addEventListener("submit",this.submitHandler),document.addEventListener("keydown",this.escapeHandler)}open(){this.isEditMode=!1,this.currentItemId=null,this.selectedIconName="",this.render(),this.dialog?.showModal(),requestAnimationFrame(()=>{this.resetForm(),this.initializeIconPicker()})}openForEdit(o){this.isEditMode=!0,this.currentItemId=o.id;let t=o.content;if(typeof t==="string")try{t=JSON.parse(t)}catch(r){console.error("Failed to parse item content",r)}this.selectedIconName=t.iconName||"",this.render(),this.dialog?.showModal(),this.initializeIconPicker(),setTimeout(()=>{let r=this.shadowRoot.getElementById("bookmark-form");if(r){let a=r.elements.namedItem("label"),n=r.elements.namedItem("url"),e=r.elements.namedItem("statusCheck"),s=r.elements.namedItem("visibleTouch");if(a)a.value=t.label||"";if(n)n.value=t.url||"";if(e)e.checked=!!t.statusCheck;if(s)s.checked=t.visibleTouch!==!1}if(this.iconPicker)this.iconPicker.setSelectedIcon(this.selectedIconName)},100)}close(){this.dialog?.close(),this.selectedIconName="",this.resetForm()}resetForm(){setTimeout(()=>{let o=this.shadowRoot.getElementById("bookmark-form");if(o)o.reset();if(this.iconPicker)this.iconPicker.setSelectedIcon("")},100)}initializeIconPicker(){requestAnimationFrame(()=>{let o=this.shadowRoot.getElementById("icon-picker-container");if(!o){console.error("[Modal] Icon picker container not found");return}if(!this.iconPicker)this.iconPicker=document.createElement("icon-picker"),this.iconPicker.addEventListener("icon-selected",(t)=>{this.selectedIconName=t.detail.iconName});o.innerHTML="",o.appendChild(this.iconPicker)})}render(){this.shadowRoot.innerHTML=`
            <style>${wt}</style>
            ${yt({isOpen:!0,isEditMode:this.isEditMode})}
        `,this.dialog=this.shadowRoot.getElementById("modal"),this.setupListeners()}}if(!customElements.get("add-bookmark-modal"))customElements.define("add-bookmark-modal",zt);k();var ho=[{id:"clock",name:"Clock",icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>',description:"Digital clock with date",defaultW:2,defaultH:1,componentTag:"widget-clock"},{id:"notepad",name:"Notepad",icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><line x1="10" y1="9" x2="8" y2="9"/></svg>',description:"Markdown notepad",defaultW:2,defaultH:2,componentTag:"widget-notepad"},{id:"telemetry",name:"System Status",icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="3" width="20" height="7" rx="2"/><rect x="2" y="14" width="20" height="7" rx="2"/><circle cx="6" cy="6.5" r="1" fill="currentColor" stroke="none"/><circle cx="6" cy="17.5" r="1" fill="currentColor" stroke="none"/><line x1="10" y1="6.5" x2="18" y2="6.5"/><line x1="10" y1="17.5" x2="18" y2="17.5"/></svg>',description:"CPU, RAM and Temp",defaultW:2,defaultH:1,componentTag:"widget-telemetry"},{id:"weather",name:"Weather",icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><circle cx="9" cy="9" r="4"/><line x1="9" y1="1" x2="9" y2="3"/><line x1="1" y1="9" x2="3" y2="9"/><line x1="3.3" y1="3.3" x2="4.7" y2="4.7"/><line x1="14.7" y1="3.3" x2="13.3" y2="4.7"/><line x1="3.3" y1="14.7" x2="4.7" y2="13.3"/><path d="M10 19 Q10 15 14 15 Q14 11 18 11 Q22 11 22 15 Q24 15 24 18 Q24 21 21 21 L12 21 Q10 21 10 19Z"/></svg>',description:"Current weather & forecast",defaultW:2,defaultH:2,componentTag:"widget-weather"},{id:"markdown",name:"Markdown",icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/><line x1="7" y1="8" x2="17" y2="8"/><line x1="7" y1="12" x2="17" y2="12"/><line x1="7" y1="16" x2="17" y2="16"/></svg>',description:"Native Markdown Editor",defaultW:2,defaultH:2,componentTag:"widget-markdown"}];class $t extends HTMLElement{dialog=null;constructor(){super();this.attachShadow({mode:"open"})}connectedCallback(){this.render()}open(){if(this.dialog)this.dialog.showModal()}close(){if(this.dialog)this.dialog.close()}selectWidget(o){this.dispatchEvent(new CustomEvent("widget-selected",{detail:o,bubbles:!0,composed:!0})),this.close()}render(){if(!this.shadowRoot)return;this.shadowRoot.innerHTML=`
            <style>
                :host {
                    display: contents;
                }
        ...
        `,this.dialog=this.shadowRoot.querySelector("dialog"),this.shadowRoot.querySelectorAll(".card").forEach((o)=>{o.addEventListener("click",()=>{let t=o.getAttribute("data-id"),r=ho.find((a)=>a.id===t);if(r)this.selectWidget(r)})})}}customElements.define("add-widget-modal",$t);L();k();var jt=`:host {
    display: contents;
}
...
.search-empty {
    ...
    text-align: center;
    padding: 12px;
}`;var vo=new Map;async function Lt(o,t){try{let r=await fetch(`https://timeapi.io/api/timezone/coordinate?latitude=${o}&longitude=${t}`);if(!r.ok)throw Error("TimeAPI failed");return(await r.json()).timeZone||"local"}catch(r){return console.error("[Timezone] TimeAPI error:",r),"local"}}async function er(o){if(!o||o.trim()==="")return"local";let t=o.toLowerCase().trim();if(vo.has(t))return vo.get(t);try{let r=`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(o)}&limit=1`,a=await fetch(r,{headers:{"User-Agent":"Lastboard/1.0"}});if(!a.ok)throw Error("Geocoding failed");let n=await a.json();if(n.length===0)return"local";let{lat:e,lon:s}=n[0],d=await Lt(parseFloat(e),parseFloat(s));return vo.set(t,d),d}catch(r){return console.error("[Timezone] Error resolving city:",o,r),"local"}}class Mt extends HTMLElement{dialog=null;currentItem=null;_clockSelectedLat=null;_clockSelectedLon=null;constructor(){super();this.attachShadow({mode:"open"})}connectedCallback(){this.render()}open(o){if(this.currentItem=o,this.render(),this.dialog)this.dialog.showModal()}close(){if(this.dialog)this.dialog.close();this.currentItem=null}async save(){if(!this.currentItem)return;let o=typeof this.currentItem.content==="string"?JSON.parse(this.currentItem.content):this.currentItem.content,t=o.widgetId,r={...o};if(t==="clock"){let a=this.shadowRoot?.getElementById("clock-city"),n=this.shadowRoot?.getElementById("clock-12h"),e=this.shadowRoot?.getElementById("clock-date"),s=this.shadowRoot?.getElementById("save-btn"),d=a?.value||"";if(s)s.disabled=!0,s.textContent=i.t("general.loading");let l="local";if(this._clockSelectedLat!=null&&this._clockSelectedLon!=null)l=await Lt(this._clockSelectedLat,this._clockSelectedLon);else if(d)l=await er(d);if(r.city=d,r.timezone=l,r.hour12=n?.checked||!1,r.showDate=e?.checked||!1,this._clockSelectedLat=null,this._clockSelectedLon=null,s)s.disabled=!1,s.textContent=i.t("general.save")}else if(t==="weather"){let a=this.shadowRoot?.getElementById("weather-city"),n=this.shadowRoot?.getElementById("weather-lat"),e=this.shadowRoot?.getElementById("weather-lon"),s=this.shadowRoot?.getElementById("weather-fahrenheit"),d=this.shadowRoot?.getElementById("weather-forecast"),l=this.shadowRoot?.getElementById("weather-days");r.city=a?.value||"",r.latitude=parseFloat(n?.value)||0,r.longitude=parseFloat(e?.value)||0,r.unit=s?.checked?"fahrenheit":"celsius",r.showForecast=d?.checked||!1,r.forecastDays=parseInt(l?.value)||5}else if(t==="telemetry"){let a=this.shadowRoot?.getElementById("telemetry-interval");r.interval=a?parseInt(a.value):1000}if(this.currentItem.type==="section"){let a=this.shadowRoot?.getElementById("section-title");r.title=a?a.value:"",delete r.name}await g.updateItem({id:this.currentItem.id,content:JSON.stringify(r)}),this.close()}render(){if(!this.shadowRoot)return;let o={},t="";if(this.currentItem)o=typeof this.currentItem.content==="string"?JSON.parse(this.currentItem.content):this.currentItem.content,t=o.widgetId;let r=(x)=>String(x).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;"),a=()=>{if(!this.currentItem)return"";if(t==="clock"){let x=o.city||"",m=o.hour12||!1,y=o.showDate!==!1;return`
                    <div class="field-group">
                        <label>${i.t("widget.clock.city")}</label>
                        <div class="input-row">
                            <input type="text" id="clock-city" value="${r(x)}" placeholder="${i.t("widget.clock.city_placeholder")}"/>
                    ...
                    <div class="field-group check-row">
                        <input type="checkbox" id="clock-date" ${y?"checked":""} />
                        <label for="clock-date">${i.t("widget.clock.show_date")}</label>
                    </div>
                `}else if(t==="weather"){let x=o.city||"",m=o.latitude||"",y=o.longitude||"",u=o.unit==="fahrenheit",b=o.showForecast||!1,A=o.forecastDays||5;return`
                    <div class="field-group">
                        <label>${i.t("widget.weather.city")}</label>
                        <div class="input-row">
                            <input type="text" id="weather-city" value="${r(x)}" placeholder="${i.t("widget.weather.city_placeholder")}"/>
                    ...
                    <div class="field-group">
                        ...
                        <div class="input-row">
                            ...
                            <input type="number" id="weather-lon" value="${y}" placeholder="Lon" step="0.0001" />
                        </div>
                    </div>
                ...
                `}else if(this.currentItem.type==="section"){let x=o.title||"";return`
                    <div class="field-group">
                        ...
                        <div class="input-row">
                            <input type="text" id="section-title" value="${r(x)}" placeholder="${i.t("section.placeholder_title")}" />
                        </div>
                        <small>${i.t("section.leave_empty")}</small>
                    </div>
                `}return`<p>${i.t("widget.config.no_config")}</p>`},n=()=>{if(this.currentItem?.type==="section")return i.t("section.edit_title");return i.t("widget.config.title")};this.shadowRoot.innerHTML=`
            <style>${jt}</style>
            <dialog id="modal">
                <div class="modal-header">
        ...
        `,this.dialog=this.shadowRoot.querySelector("dialog");let e=this.shadowRoot.getElementById("telemetry-interval");if(e)e.options=[{value:"1000",label:"1s"},{value:"2000",label:"2s"},{value:"5000",label:"5s"},{value:"10000",label:"10s"}];this.shadowRoot.getElementById("close-btn")?.addEventListener("click",()=>this.close()),this.shadowRoot.getElementById("save-btn")?.addEventListener("click",()=>this.save());let s=this.shadowRoot.getElementById("clock-search-btn");if(s)s.addEventListener("click",()=>this.searchClockCity());let d=this.shadowRoot.getElementById("weather-search-btn");if(d)d.addEventListener("click",()=>this.searchWeatherCity());let l=this.shadowRoot.getElementById("weather-forecast"),c=this.shadowRoot.getElementById("weather-days-row");if(l&&c)l.addEventListener("change",()=>{c.style.display=l.checked?"":"none"});let p=this.shadowRoot.getElementById("weather-days"),f=this.shadowRoot.getElementById("weather-days-dec"),v=this.shadowRoot.getElementById("weather-days-inc");if(p&&f&&v)f.addEventListener("click",()=>{let x=parseInt(p.value)||1;if(x>1)p.value=String(x-1)}),v.addEventListener("click",()=>{let x=parseInt(p.value)||1;if(x<6)p.value=String(x+1)})}async searchClockCity(){let o=this.shadowRoot?.getElementById("clock-city"),t=this.shadowRoot?.getElementById("clock-results-container"),r=this.shadowRoot?.getElementById("clock-results");if(!o||!t||!r)return;let a=o.value.trim();if(!a)return;r.innerHTML=`<div class="search-loading">${i.t("general.loading")}</div>`,t.style.display="";try{let e=await(await fetch(`https://geocoding-api.open-meteo.com/v1/search?name=${encodeURIComponent(a)}&count=5&language=${i.getLocale().code}`)).json();if(!e.results||e.results.length===0){r.innerHTML=`<div class="search-empty">${i.t("general.no_icons")}</div>`;return}r.innerHTML=e.results.map((s)=>`
            ...
            `).join(""),r.querySelectorAll(".city-result").forEach((s)=>{s.addEventListener("click",()=>{let d=parseFloat(s.getAttribute("data-lat")||"0"),l=parseFloat(s.getAttribute("data-lon")||"0"),c=s.getAttribute("data-name")||"";(this.shadowRoot?.getElementById("clock-city")).value=c,this._clockSelectedLat=d,this._clockSelectedLon=l,t.style.display="none"})})}catch(n){r.innerHTML=`<div class="search-empty">${i.t("general.error")}</div>`}}async searchWeatherCity(){let o=this.shadowRoot?.getElementById("weather-city"),t=this.shadowRoot?.getElementById("weather-results-container"),r=this.shadowRoot?.getElementById("weather-results");if(!o||!t||!r)return;let a=o.value.trim();if(!a)return;r.innerHTML=`<div class="search-loading">${i.t("general.loading")}</div>`,t.style.display="";try{let e=await(await fetch(`https://geocoding-api.open-meteo.com/v1/search?name=${encodeURIComponent(a)}&count=5&language=${i.getLocale().code}`)).json();if(!e.results||e.results.length===0){r.innerHTML=`<div class="search-empty">${i.t("general.no_icons")}</div>`;return}r.innerHTML=e.results.map((s)=>`
                <button class="city-result" data-lat="${s.latitude}" data-lon="${s.longitude}" data-name="${s.name}">
                    <span class="city-name">${s.name}</span>
                    <span class="city-detail">${[s.admin1,s.country].filter(Boolean).join(", ")}</span>
                </button>
            `).join(""),r.querySelectorAll(".city-result").forEach((s)=>{s.addEventListener("click",()=>{let d=parseFloat(s.getAttribute("data-lat")||"0"),l=parseFloat(s.getAttribute("data-lon")||"0"),c=s.getAttribute("data-name")||"";(this.shadowRoot?.getElementById("weather-lat")).value=d.toString(),(this.shadowRoot?.getElementById("weather-lon")).value=l.toString(),(this.shadowRoot?.getElementById("weather-city")).value=c,t.style.display="none"})})}catch(n){r.innerHTML=`<div class="search-empty">${i.t("general.error")}</div>`}}}customElements.define("widget-config-modal",Mt);k();var At=({isOpen:o,title:t,message:r})=>`
    <dialog id="modal">
        <div class="modal-header">
...
.modal-actions {
    ...
    justify-content: flex-end;
        ...
        `}}oo();var j=document.getElementById("main-topbar"),Ft=document.getElementById("right-drawer"),to=document.getElementById("dashboard-root"),P,ro,ao;Rt(async()=>{G.init();let[o,t]=await Promise.all([w.fetchUser(),g.fetchItems()]),r=w.getUser();if(r){if(r.language)await i.setLanguage(r.language);if(r.preferences&&r.preferences.grid_columns)document.documentElement.style.setProperty("--user-preferred-columns",r.preferences.grid_columns.toString());if(r.theme==="dark")G.enableDark();else if(r.theme==="light")G.enableLight();else if(window.matchMedia&&window.matchMedia("(prefers-color-scheme: dark)").matches)G.enableDark();else G.enableLight();if(r.id)g.setUserId(r.id)}if(j){if(j.setState({user:r}),r&&r.project_name)j.setAttribute("title",r.project_name);w.subscribe((n)=>{if(n){if(j.setState({user:n}),n.project_name)j.setAttribute("title",n.project_name)}})}dr(),W.start(),P=document.createElement("add-bookmark-modal"),document.body.appendChild(P),ro=document.createElement("add-widget-modal"),document.body.appendChild(ro),ao=document.createElement("confirmation-modal"),document.body.appendChild(ao);let a=document.createElement("widget-config-modal");document.body.appendChild(a)});X.on(O.SHOW_CONFIRMATION,async(o)=>{let{title:t,message:r,onConfirm:a}=o.detail;if(ao){if(await ao.confirm(t,r)&&a)a()}});X.on(O.SHOW_WIDGET_CONFIG,(o)=>{let{item:t,type:r}=o.detail;if(r==="bookmark"){if(P)P.openForEdit(t)}else{let a=document.querySelector("widget-config-modal");if(a)a.open(t)}});X.on(O.NOTIFY,(o)=>{let t=document.querySelector("app-notifier");if(t)t.show(o.detail.message,o.detail.type)});if(j)j.addEventListener("drawer-toggle",(o)=>{if(o.detail.action==="open")Ft.open(),j.setState({drawerOpen:!0});else Ft.close(),j.setState({drawerOpen:!1})}),j.addEventListener("edit-mode-change",(o)=>{let t=o.detail.active;if(to.classList.toggle("edit-mode",t),!t)window.location.reload()}),j.addEventListener("search-input",(o)=>{let t=o.detail.query;g.setSearchQuery(t)}),j.addEventListener("add-item",async(o)=>{let t=o.detail.action;if(t==="add-bookmark"){if(P)P.open()}else if(t==="add-widget"){if(ro)ro.open()}else if(t==="add-section"){let a=g.getState().items||[],{collisionService:n}=await Promise.resolve().then(() => (U(),N)),e=n.findFirstAvailableSlot(1,1,a),s={type:"section",x:e.x,y:e.y,w:1,h:1,content:JSON.stringify({title:""})},d=await g.addItem(s);if(d)X.emit(O.SHOW_WIDGET_CONFIG,{item:d,type:"section"})}});window.addEventListener("widget-selected",async(o)=>{let r=o.detail,n=g.getState().items||[],{collisionService:e}=await Promise.resolve().then(() => (U(),N)),s=e.findFirstAvailableSlot(r.defaultW,r.defaultH,n),d={type:"widget",x:s.x,y:s.y,w:r.defaultW,h:r.defaultH,content:JSON.stringify({widgetId:r.id,text:r.id==="notepad"||r.id==="markdown"?"":void 0})};await g.addItem(d)});window.addEventListener("drawer-close",()=>{if(j)j.setState({drawerOpen:!1})});window.addEventListener("click",(o)=>{let t=o.target,r=document.getElementById("add-menu"),a=document.getElementById("add-toggle");if(r&&r.classList.contains("active")&&!r.contains(t)&&a&&!a.contains(t))r.classList.remove("active")});function dr(){if(!to)return;to.innerHTML="";let o=document.createElement("bookmark-grid");to.appendChild(o)}
