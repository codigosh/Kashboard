...
(max-width: 1024px) {
    ...
    ${(n?s:i?s:s.filter((c)=>!c.parent_id)).map((c)=>{let l={};try{l=typeof c.content==="string"?JSON.parse(c.content):c.content}catch(_){console.error("Failed to parse content for item",c.id,_)}let x=c.type==="section",u={x:c.x||1,y:c.y||1},v=Math.min(c.w||1,e),y=c.h||1;if(x){let _=p(c.id),w=(l.title||"").trim(),b=l.borderColor&&!r?`border-color: ${l.borderColor} !important;`:"",A=!r?`border-width: ${l.borderWidth||1}px !important;`:"";return`
    ...
    `};function _r(o,r,t,i,n,e=0,s=!1){let p=e||o.h||1;if(o.type==="widget"){let _=r.widgetId,b={clock:"widget-clock",notepad:"widget-notepad",telemetry:"widget-telemetry",weather:"widget-weather",markdown:"widget-markdown"}[_]||"div",H=(r.text||"").replace(/"/g,"&quot;"),h=r.borderColor&&!t?`border-color: ${r.borderColor} !important;`:"",B=!t?`border-width: ${r.borderWidth||1}px !important;`:"";return`
    ...
    `}function kt(o,r,t=!1,i=12,n=!1){return o.map((e)=>{let s={};try{s=typeof e.content==="string"?JSON.parse(e.content):e.content}catch(l){console.error("Failed to parse content for item (nested)",e.id,l)}let p={x:e.x||1,y:e.y||1},d=e.h||1,c=Math.min(e.w||1,i);return _r(e,s,r,p,c,d,n)}).join("")}function zt(o){return`
        <style>
            .skeleton {
                background: var(--surface);
                pointer-events: none;
...
`,Mr=(o,r,t)=>{if(t)return`<div class="icon-picker__loading">${a.t("general.loading")}</div>`;if(o.length===0)return`<div class="icon-picker__empty">${a.t("general.no_icons")}</div>`;let i=o.reduce((e,s)=>{if(!e[s.provider])e[s.provider]=[];return e[s.provider].push(s),e},{}),n="";for(let[e,s]of Object.entries(i))n+=`
